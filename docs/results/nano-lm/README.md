# Nano-LM results

| Doc | Role |
|-----|------|
| [RECIPES.md](RECIPES.md) | **Deploy one-pager** |
| [champion-card.md](champion-card.md) | Tips + recipe priority |
| [wave-w-summary.md](wave-w-summary.md) | Wave W **COMPLETE** |
| [wave-x-summary.md](wave-x-summary.md) | Wave X+ **COMPLETE** (PFB **PROMOTE**; QI/ABS **KILL**) |
| [wave-y-summary.md](wave-y-summary.md) | Wave Y **COMPLETE** (PFB256/ROLL/SUMCACHE/GPFB4-LONG **PROMOTE**; STREAM/KVCACHE-Q/GENCACHE **KILL**) |
| [wave-z-summary.md](wave-z-summary.md) | Wave Z **COMPLETE** |
| [wave-aa-summary.md](wave-aa-summary.md) | Wave AA **COMPLETE** (+ AA-REPORT) |
| [aa-freeze.md](aa-freeze.md) | §8.2 Wave AA NO-REOPEN freeze (AB via §8.3) |
| [wave-ab-session.md](wave-ab-session.md) | Wave AB0 SESSION **PROMOTE** (10 real HITL Qs) |
| [formal-hsemwrap-semwrap.md](formal-hsemwrap-semwrap.md) | Wave AB1 H-SEMWRAP **PROMOTE** (fuzzy near-known) |
| [formal-haskfast-askfast.md](formal-haskfast-askfast.md) | Wave AB2 H-ASKFAST **PROMOTE** (wall↓ compose) |
| [formal-hlongapp-longapp.md](formal-hlongapp-longapp.md) | Wave AB3 H-LONGAPP **PROMOTE** (L_eff≫W curated) |
| [paper-lab-wave-aa.md](paper-lab-wave-aa.md) | Paper-lab one-pager for Wave AA |
| [wave-z-hitl.md](wave-z-hitl.md) | **Z6 public HITL report** (PFB ≠ interactive LM; H-ZWRAP) |
| [lab-freeze.md](lab-freeze.md) | §8 #6 NO-REOPEN lab freeze |
| [paper-lab-wave-z.md](paper-lab-wave-z.md) | Paper-lab one-pager for Wave Z |
| [wave-z-export.md](wave-z-export.md) | Champion freeze `champion-qpfb2-v0` |
| [wave-z-hitl-z1.md](wave-z-hitl-z1.md) | Z1 HITL-10 baseline (mean 1.0; period collapse) |
| [wave-z-hitl-z2.md](wave-z-hitl-z2.md) | Z2 wrap PASS (mean 9.0; WRAP_LOOKUP) |
| [wave-z-hitl-z4.md](wave-z-hitl-z4.md) | Z4 verify PASS (claim H-ZWRAP) |
| [wave-z-zerr.md](wave-z-zerr.md) | Z3 H-ZERR PROMOTE (bank CE; parent−ε) |
| [wave-z-depl-y.md](wave-z-depl-y.md) | DEPL-Y freeze (128 vs long) |
| [archive/](archive/) | All KILL evidence (tooling purged) |

## Survivors (formal)

| ID | Doc |
|----|-----|
| PACK / PROG / BTC / EFF | [hpack](formal-hpack-vs-hearly.md) · [prog](formal-hprog-programming.md) · [btc](formal-hbtc-bitcoin.md) · [eff](formal-heff-efficiency.md) |
| TCHR / QT / GENC | [tchr](formal-htchr-code-teacher.md) · [qt](formal-hqt-quantize.md) · [genc](formal-hgenc-genome.md) |
| PFB family | [pfb](formal-hpfb-pfb.md) · [pfb2](formal-hpfb2-pfb2.md) · [qpfb2](formal-hqpfb2-qpfb2.md) · [bpfb](formal-hbpfb-bpfb.md) · [gpfb4](formal-hgpfb4-gpfb4.md) |
| Wave Y cache / long | [beamkv](formal-hbeamkv-beamkv.md) · [tcache](formal-htcache-tcache.md) · [scoreram](formal-hscoreram-scoreram.md) · [pfb256](formal-hpfb256-pfb256.md) · [roll](formal-hroll-roll.md) · [sumcache](formal-hsumcache-sumcache.md) · [gpfb4long](formal-hgpfb4long-gpfb4long.md) |
| Train | [TIPD](formal-htipd-vs-hstag.md) · [tpack](formal-htpack-vs-hstag.md) · [amort](formal-hamort-vs-hstag.md) |

**Wave Y COMPLETE** — [wave-y-summary.md](wave-y-summary.md). **Wave Z COMPLETE** — [wave-z-hitl.md](wave-z-hitl.md). **Wave AA COMPLETE + FROZEN** — [wave-aa-summary.md](wave-aa-summary.md) · [aa-freeze.md](aa-freeze.md). **Wave AB OPEN** — AB0–AB3 [formal-hlongapp-longapp.md](formal-hlongapp-longapp.md). **Lab freeze** — [lab-freeze.md](lab-freeze.md).
