"""Formal equal-budget: B4 vs H-DECK (proxy+top-k decode search), 3 seeds."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hdeck-b4",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 4,
...
def _eval_gene(c: dict[str, Any], ckpt: Path, gene: dict, seed: int) -> dict:
    ...
    texts = [p["text"] for p in yaml.safe_load(c["prompts"].read_text(encoding="utf-8"))["prompts"]]
