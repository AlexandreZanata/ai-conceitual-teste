"""Batched multi-prompt BoN decode (left-pad; shared forward per step)."""
...
def decode_bon_batch(
    ...
    *,
    ...
    """
    GIVEN many prompts and BoN width n
    WHEN decoding in one rectangular batch
    ...
    for i in range(b):
        if stop_at[i] == 0:
            stop_at[i] = gen
    results = _results(
        tokenizer, prompts, owners, ids, lps, prompt_lens, stop_at, gen, wall_ms,
        token_evals,
    )
