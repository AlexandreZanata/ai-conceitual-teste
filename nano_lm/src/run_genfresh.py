"""Wave AL1 H-GENFRESH runner: dual-arm + peak ablation true-gen gate."""
...
_SUMMARY = REPO / "results/nano-lm/wave-al/genfresh_summary.json"
...
def _seed_pack(bank_path: Path, al_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(GENFRESH_PACK, start=1):
        q = str(item["question"]).strip()
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AL-GENFRESH-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = GENFRESH_ID
        row["judge_notes"] = [
            "GENFRESH seed for LOOKUP arm",
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AL-GENFRESH-FIX-{i:02d}",
...
def run_genfresh(
    *,
    ...
    """
    if len(GENFRESH_PACK) != GENFRESH_N:
        raise ValueError("GENFRESH pack must be 10")

    trials_dir.mkdir(parents=True, exist_ok=True)
    t0 = time.perf_counter()
    seeded = _seed_pack(bank_path, al_bank)
    bank = load_bank_rows(bank_path)
    questions = [p["question"] for p in GENFRESH_PACK]
    ...
    gen_off, gen_on = _run_gen_ablation(
        champ=root,
        items=[dict(p) for p in GENFRESH_PACK],
    ...
    for i, (item, lp) in enumerate(
        zip(GENFRESH_PACK, lookup_payloads, strict=True),
        start=1,
    ):
        ...
        score_l, err_l, notes_l = score_genfresh_lookup(
        ...
        lookup_trials.append(
            {
                "trial_id": f"AL-GENFRESH-LOOKUP-HITL-{i:02d}",
                "stage": "AL1",
                "hyp_id": GENFRESH_ID,
                "arm": "LOOKUP",
                "app_id": item["app_id"],
    ...
    for i, (item, off, on) in enumerate(
        zip(GENFRESH_PACK, gen_off, gen_on, strict=True)
    ):
        score_g, err_g, notes_g = score_genfresh_gen(
            completion=str(off.get("completion", "")),
            expected_gold=item["gold"],
            payload=off,
            peak_ablated=True,
        )
        score_p, _err_p, notes_p = score_genfresh_gen(
    ...
    fix_attempts = 0
    if weak_idx:
        re_items = [dict(GENFRESH_PACK[i]) for i in weak_idx]
        ...
        for j, idx in enumerate(weak_idx):
            item = GENFRESH_PACK[idx]
            score2, err2, notes2 = score_genfresh_gen(
                completion=str(re_off[j].get("completion", "")),
                expected_gold=item["gold"],
                payload=re_off[j],
                peak_ablated=True,
            )
            score_p2, _, _ = score_genfresh_gen(
    ...
    for i, (item, off, on) in enumerate(
        ...
        if bool(on.get("peak_used")):
            n_peak += 1
        gt = {
            "trial_id": f"AL-GENFRESH-GEN-HITL-{i:02d}",
            "stage": "AL1",
            "hyp_id": GENFRESH_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
    ...
    decision = decide_genfresh(stats)
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{GENFRESH_ID}: L_lookup={stats['lookup_mean']:.1f} "
        ...
        "public_note": "docs/results/nano-lm/formal-hgenfresh-genfresh.md",
