"""Render H-SHORTB smoke vs H-FUSEB (SHORT under FUSEB batch)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Dual-budget mean: FUSEB path with batched SHORT on the non-KV arm "
        f"(CHBAT B=`{data.get('chunk_size')}` when `max_new > kv_threshold`).",
        "Frozen SHORT tip `draft_max` / `stop_conf`. Kill if |Δlp| > ε vs H-FUSEB "
        "or no tok/s/wall win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/shortb_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hshortb-vs-hfuseb.md"),
    )
    args = p.parse_args()
