"""Wave BD2 H-FASTGAIN runner (nano:bd:fastgain) — prod p50/p99 hold + anti-FP."""
...
from bd_session_ops import BD0_FOREVER_ROWS, BD0_MODES, map_bd_product_mode
...
_SUMMARY = REPO / "results/nano-lm/wave-bd/bd_fastgain_summary.json"
_TRIALS = REPO / "results/nano-lm/wave-bd/trials"
_Z_BANK = REPO / "results/nano-lm/wave-z/error_bank.jsonl"
_CHAMPION = REPO / "results/nano-lm/wave-z/models/champion"
_CURATED = REPO / "nano_lm/data/curated"
_PUBLIC = REPO / "docs/results/nano-lm/formal-hfastgain-fastgain.md"
...
def _pack_latency(
    *,
    ...
    pmodes = [
        str(p.get("product_mode") or map_bd_product_mode(m))
...
def _write_public(
    *,
    ...
    body = "\n".join(
        [
            f"# H-FASTGAIN (BD2) — prod p50/p99 + anti-FP hold (**DONE** — {status})",
            "",
            "> Lab: `.local/pesquisa.md` §3 · §9 BD2 · Session: "
            "`.local/wave-bd/SESSION.md`  ",
            "> Parent: [formal-hsemint-semint.md](formal-hsemint-semint.md) · "
            "BD0 speed baseline (= H-FASTLIFT)  ",
            "> Module: `nano_lm/src/bd_fastgain_ops.py` · "
            "Runner: `npm run nano:bd:fastgain`  ",
            ...
            f"(≤{board.get('p99_regress_max_ratio')}× H-FASTLIFT) |",
            ...
            *lat_rows,
            ...
            "4. Live product p99 (DECODE·ABSTAIN) checked vs BD0/H-FASTLIFT "
            ...
            "| Allowed | Forbidden |",
            "|---------|-----------|",
            f"| {FASTGAIN_CLAIM} | Open chat / mini-AGI |",
            ...
            "",
            f"SAFE note: {FASTGAIN_SAFE_NOTE}  ",
            f"Anti-FP: {FASTGAIN_ANTI_FP}",
            "",
...
def _update_local_session(decision: str, board: dict[str, Any]) -> None:
    ...
    body = "\n".join(
        [
            f"# Wave BD session checklist (**OPEN** · BD2 {status})",
            "",
            "> Private under `.local/`. Lab: `.local/pesquisa.md` "
            "(Wave BD **OPEN** · semantic/wrong-bank anti-FP + ctx/speed + honest gen).  ",
            f"> Ship lock: **{FASTGAIN_CLAIM}** · ≤5M.",
            "",
            "## Current stage",
            "",
            f"**BD2 — H-FASTGAIN ({status})** · Next: **BD3 H-CTXGAIN**",
            "",
            "| Field | Value |",
            "|-------|--------|",
            f"| bd_forever_false_hit | **{board.get('bd_forever_false_hit')}** |",
...
def _patch_pesquisa(decision: str) -> None:
    ...
    if bd2_next in text:
        text = text.replace(bd2_next, bd2_done, 1)
    bd3_pending = (
    ...
    if bd3_pending in text:
        text = text.replace(bd3_pending, bd3_next, 1)
    text = text.replace(
        "3. **BD2 H-FASTGAIN** — **NEXT** — prod p50/p99 hold or improve "
        "**with** anti-FP hold.  ",
        "3. **BD2 H-FASTGAIN** — **DONE PROMOTE** "
        "(`npm run nano:bd:fastgain`) — prod p50/p99 + anti-FP hold.  ",
        1,
    )
    text = text.replace(
...
**BD0–BD2 DONE — PROMOTE** · **BD2 H-FASTGAIN** (`npm run nano:bd:fastgain`).

## Next

1. BD0–BD2 done.  
2. **BD3 H-CTXGAIN** — **NEXT** — context content bars without FP regress.  
...
**Wave BD ACTIVE** — BD0 SESSION · BD1 H-SEMINT · BD2 **H-FASTGAIN PROMOTE**.

Next: **BD3 H-CTXGAIN**. Parent: Wave BC **COMPLETE + FROZEN**.
...
def _bd_active_line() -> str:
    return (
        ...
        "(`npm run nano:bd:fastgain`) — prod p50/p99 + anti-FP hold; "
        "next BD3 H-CTXGAIN; ship remains **AF + AQ + AS trust + STRICT "
...
def _patch_recipes(board: dict[str, Any], line: str) -> None:
    ...
    insert = (
        "| Wave BD2 H-FASTGAIN | [formal-hfastgain-fastgain.md]"
        "(formal-hfastgain-fastgain.md) **PROMOTE** (`npm run nano:bd:fastgain`) "
        f"— p99_regress={board.get('p99_regress')} · BD FH "
        f"{board.get('bd_forever_false_hit')} · BC FH "
        f"{board.get('bc_forever_false_hit')} · live FP {board.get('live_fp')} "
        "(≠ AH `nano:fastlift` · ≠ BC `nano:bc:fastlift`) |"
    )
    if "Wave BD2 H-FASTGAIN" not in text2:
        marker = "| Wave BD1 H-SEMINT |"
        if marker in text2:
            text2 = text2.replace(marker, insert + "\n" + marker, 1)
    if n or "Wave BD2 H-FASTGAIN" in text2:
...
def _patch_card_agents_agenda(board: dict[str, Any]) -> None:
    if _CARD.is_file():
        ...
        card = (
            ...
            "BD1 [H-SEMINT PROMOTE](formal-hsemint-semint.md) · "
            "BD2 [H-FASTGAIN PROMOTE](formal-hfastgain-fastgain.md) "
            f"(`npm run nano:bd:fastgain`) — p99_regress={board.get('p99_regress')} "
            f"· BD FH {board.get('bd_forever_false_hit')}; next BD3 H-CTXGAIN; "
    ...
    if _AGENDA.is_file():
        ...
        row = (
            ...
            "(results/nano-lm/formal-hsemint-semint.md) · BD2 "
            "[H-FASTGAIN PROMOTE](results/nano-lm/formal-hfastgain-fastgain.md) "
            "(`npm run nano:bd:fastgain`); next BD3 H-CTXGAIN; ≤5M |"
        )
        text2, n = re.subn(
            r"\| \*\*BD\*\* \| \*\*ACTIVE\*\* \|[^\n]+", row, text, count=1
        )
...
def run_bd_fastgain(
    *,
    ...
    """
    GIVEN BD0/H-FASTLIFT speed baseline + SEMINT anti-FP
    WHEN measuring prod tetrad + BD/BA/BB/BC/AZ hold packs
    THEN PROMOTE/KILL per pesquisa §9 BD2.
    """
    ...
    decision = decide_fastgain(board=board, anti_fp_signed=True)
    ...
    write_json(
        trials_dir / "BD-FASTGAIN-BOARD.json",
        {
            "board": board,
            "bd_rows": bd_rows,
            "bc_rows": bc_rows,
    ...
    payload = {
        ...
        "wall_s": wall_s,
        "workers": workers,
        "claim": FASTGAIN_CLAIM,
        "public_note": "docs/results/nano-lm/formal-hfastgain-fastgain.md",
        "bc_archive": "docs/results/nano-lm/formal-hfastlift-bc2.md",
