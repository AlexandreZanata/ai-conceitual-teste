"""BoN decode with no-repeat n-gram ban (DECM knobs × NGRAM)."""
...
def decode_bon_ngram(
    ...
    *,
    ...
    """
    GIVEN BoN knobs and ngram_size ≥ 0
    WHEN sampling n beams
    THEN ban repeating n-grams on each beam; pick best self-lp.
    """
    if n < 1:
        raise ValueError("decode_bon_ngram: n must be >= 1")
    if int(ngram_size) < 0:
        raise ValueError("decode_bon_ngram: ngram_size must be >= 0")
