"""Formal H-ABS-BPFB: PFB K=2 on BTC; wall↓ vs k=4 (fit≠eval)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hbpfb"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-ABS-BPFB formal requires CUDA")
    out: Path = c["out"]
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "max_new": _MAX_NEW,
            "cpu_threads": threads,
            "mode": "BPFB formal: PFB K=2 on BTC; wall↓ vs k=4 (fit≠eval)",
            "mechanism": "PFB commit K=2 on bitcoin pack; domain-transfer gate",
            "parent": "H-EARLY n=1 greedy on B2 (BTC pack, formal genes)",
        },
    )
