"""Render formal H-FLASH vs H-EARLY (SDPA attention backend)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        f"Backend: `{data.get('backend')}`; n_prompts={data.get('n_prompts')}.",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "Commands: `npm run nano:formal:hflash` → "
            "`npm run nano:formal:hflash:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hflash/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hflash-vs-hearly.md"),
    )
    args = p.parse_args()
