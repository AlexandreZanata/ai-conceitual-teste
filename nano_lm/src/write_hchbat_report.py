"""Render H-CHBAT smoke vs H-CBAT (CHB B under CBAT)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Left-pad multi-prompt EARLY batch; prefill with CHB tip chunk_size "
        "(B=256) under FLASH SDPA vs CBAT tip B "
        f"(`{data.get('tip_chunk_size')}`).",
        "Long prompts (same pack for both arms). Kill if |Δlp| > ε vs H-CBAT or "
        "no tok/s win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/chbat_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hchbat-vs-hcbat.md"),
    )
    args = p.parse_args()
