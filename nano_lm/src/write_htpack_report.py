"""Render H-TPACK smoke — PRE3 ms/step vs live STAG."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Train I/O card hygiene: gate on **ms/step only** (not e2e). "
        "Live STAG = teacher-in-loop train; TPACK = top-k cache + PRE3 train. "
        "Cache build is reported but not gated (ETRAIN e2e claim stays KILL). "
        "Kill if lp < STAG−ε or no ms/step win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/tpack_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/htpack-vs-hstag.md"),
    )
    args = p.parse_args()
