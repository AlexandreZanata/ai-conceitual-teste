"""Wave AD5 AD-HITL-10: final verify on declared AD packaged stack."""

from __future__ import annotations

from typing import Any, Mapping, Sequence

from routeplus_ops import select_app as select_app_meta
...
STACK_CLAIM = (
    "scoped AD packaged stack "
    "(HARDPARA+COMPOSE+ROUTEPLUS+DEPLPLUS on APPPLUS) — not open chat LM"
)
...
def select_app(item_app_id: str) -> str:
    ...
    WHEN routing on the declared AD stack
    THEN return ROUTEPLUS canonical APPPLUS app_id.
    """
...
def score_ad5_trial(
    *,
    ...
    """
    GIVEN final AD-stack ask result
    WHEN scoring HITL (§13.1 AD5)
    THEN FALSE_HIT→0; TRUE_HIT→9; CTXPLUS ctx fail marks error.
    """
    ...
    notes = list(notes) + ["AD5 final stack — not open chat LM"]
    if ctx_ok is False:
        err = True
...
def ad5_stats(
    ...
    *,
    ...
    if len(scores) != AD5_N or len(errors) != AD5_N:
        raise ValueError(f"AD5 requires exactly {AD5_N} scores/errors")
    mean = float(sum(float(s) for s in scores) / float(AD5_N))
...
def decide_ad5(stats: Mapping[str, Any]) -> str:
    """
    GIVEN AD5 final HITL stats
    WHEN applying §8.6 / §13.1 gate
