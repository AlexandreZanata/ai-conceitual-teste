"""Render H-PRE3 smoke vs H-PRE2 (3-deep prefetch under PRE2)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Same ADAMF I/O stack; only prefetch_depth differs (3 vs 2). "
        "Kill if |Δlp| > ε or no ms/step win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/pre3_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hpre3-vs-hpre2.md"),
    )
    args = p.parse_args()
