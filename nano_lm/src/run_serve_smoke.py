"""Smoke H-SERVE: best frozen stack (GALL|GRAPHF) vs tip EARLY alone."""
...
def main() -> int:
    ...
    if device.type != "cuda":
        print("ERROR: H-SERVE requires CUDA", file=sys.stderr)
        return 2
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        ...
        for row in pair:
            if row["family"] == "H-SERVE":
                write_json(out / f"HSERVE_seed{seed}_eval.json", row)
        rows.extend(pair)
    write_json(
        out / "serve_smoke.json",
        {
            "rows": rows,
    ...
    print(json.dumps({"n_rows": len(rows), "out": str(out / "serve_smoke.json")}))
    return 0
