"""Render formal H-EARLY vs B4 markdown."""
...
def render(path: Path) -> str:
    ...
    decision = decide_hearly(stats.get("H-EARLY", {}), stats)
    b4 = stats.get("B4", {})
    hyp = stats.get("H-EARLY", {})
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:hearly` → "
            "`npm run nano:formal:hearly:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hearly/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hearly-vs-b4.md"),
    )
    args = p.parse_args()
