"""Smoke H-NGE on B2 ckpts; dual gate vs H-NGRAM."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_nge(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=out / f"HNGE_seed{seed}_train.json",
        )
        row = {
            "family": "H-NGE",
        ...
        write_json(out / f"HNGE_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "nge_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "nge_smoke.json")}))
    return 0
