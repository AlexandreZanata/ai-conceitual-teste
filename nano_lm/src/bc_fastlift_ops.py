"""Wave BC2 H-FASTLIFT: prod-path p50/p99 hold/lift · no anti-FP regress."""
...
FASTLIFT_THESIS = (
    ...
    "PROMOTE only if §1 anti-FP holds (BC-FOREVER FH 0 · BA/BB forever "
    "hold · AZ hold · over-refuse 0 · live FP 0) and live p99 does not "
    "regress vs BC0/BB-FASTHOLD baseline — never warm-cache vanity; "
    "LOOKUP wall=0 ≠ speed IQ; sub-ms PEAK walls ≠ speed IQ; "
    "≠ AH H-FASTLIFT archive · ≠ BB H-FASTHOLD rename · "
    "≠ BA H-FASTREAL archive · ≠ FP-for-ms"
)
FASTLIFT_CLAIM = BC0_SHIP_LOCK
FASTLIFT_SAFE_NOTE = BC0_SAFE_NOTE
FASTLIFT_ANTI_FP = BC0_ANTI_FP
FASTLIFT_BASELINE = BC0_SPEED_BASELINE
...
def extract_fastlift_board(
    *,
    ...
    """
    GIVEN latency tetrad + BC/BA/BB/AZ anti-FP hold packs
    WHEN building BC2 FASTLIFT board
    THEN publish p50/p99 · BC/BA/BB/AZ/orf · regress flags.
    """
    n_bc = len(bc_rows)
    ...
    return {
        ...
        "baseline_source": str(FASTLIFT_BASELINE.get("source", "")),
...
def _gate_latency_flags(board: Mapping[str, Any]) -> str | None:
    ...
    if bool(board.get("p99_regress")):
        ...
        return f"KILL (live p99 regress vs BB-FASTHOLD baseline: {paths})"
    ...
    if not bool(board.get("fp_for_ms_forbidden")):
        return "KILL (FP-for-ms must stay forbidden)"
    if not bool(board.get("bb_fasthold_archive_untouched")):
        return "KILL (BB H-FASTHOLD archive must stay untouched)"
    if not bool(board.get("ba_fastreal_archive_untouched")):
        return "KILL (BA H-FASTREAL archive must stay untouched)"
    if not bool(board.get("ah_fastlift_archive_untouched")):
        return "KILL (AH H-FASTLIFT archive must stay untouched)"
    return None
...
def decide_fastlift(
    *,
    ...
    """
    GIVEN BC2 FASTLIFT board
    WHEN applying pesquisa §4 + §9 BC2
    ...
    if err:
        return err
    return (
        f"PROMOTE ({FASTLIFT_ID}: prod p50/p99 hold/lift; "
        "anti-FP hold; no live p99 regress vs BB-FASTHOLD baseline)"
    )
