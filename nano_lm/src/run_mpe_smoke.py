"""Smoke H-MPE on B2 ckpts; dual gate vs H-MINP."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_mpe(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=out / f"HMPE_seed{seed}_train.json",
        )
        row = {
            "family": "H-MPE",
        ...
        write_json(out / f"HMPE_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "mpe_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "mpe_smoke.json")}))
    return 0
