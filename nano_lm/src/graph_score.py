"""Score H-GRAPH: dual-budget LAYB path with CUDA-graph non-KV arm."""
...
from decode_lay_graph import decode_lay_graph
...
def score_batch_graph_arm(
    *,
    ...
    """Batched CUDA-graph LAY arm under caller SDPA."""
    e = _gene(gene)
    ...
    results, wall_ms = decode_lay_graph(
        student,
        tok,
        prompts,
        n=int(e["n"]),
...
def score_batch_graph(
    *,
    ...
    seed: int,
    kv_threshold: int,
    chunk_size: int = GRAPH_CHUNK,
    budgets: tuple[int, ...] = SMOKE_BUDGETS,
) -> dict[str, float]:
    """
    GIVEN EARLY + LAY + KVSEL under SDPA
    WHEN dual budgets (CHBAT iff KV on; else CUDA-graph LAY)
