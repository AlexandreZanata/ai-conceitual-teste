"""Formal-budget H-TRIM vs H-POOL tip (frozen-n FLOP search)."""
...
from trim_ops import clamp_trim_gene
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-htrim"
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        def clamp_fn(g: dict, n: int = frozen_n) -> dict:
            return clamp_trim_gene(g, n)
        ...
        meta = run_h_trim(
            ...
            tip_gene=tip,
            lam=LAM,
            out_meta=c["out"] / f"HTRIM_seed{seed}_train.json",
        )
        rows.append(
            _eval(
                "H-TRIM",
