"""H-BEAM: evolve beam_width/length_penalty; dual gate vs B4."""
...
from beam_fit import fitness_beam_detail
from beam_ops import clamp_beam_gene, mutate_beam_gene, random_beam_gene
...
def run_h_beam(
    *,
    ...
    pop = [random_beam_gene(rng) for _ in range(pop_size)]
        ...
        history.append(
            {
                ...
                "best_beam_width": pop[ranked[0]]["beam_width"],
            }
        )
        if scores[ranked[0]] > best_score:
            best_score = scores[ranked[0]]
            best_lp = details[ranked[0]][0]
            best_gene = clamp_beam_gene(pop[ranked[0]])
        parents = [pop[i] for i in ranked[: max(1, pop_size // 2)]]
        pop = [
            mutate_beam_gene(parents[i % len(parents)], rng) for i in range(pop_size)
        ]
    eval_lp, eval_wall = fitness_beam_detail(
        best_gene,
        teacher=teacher,
        student=student,
