"""Wave AM6 AM-HITL-10 runner: final dual-arm verify (nano:am:hitl)."""
...
from am_session_ops import AM0_PACK, overlaps_prior_questions, pack_app_counts
...
_SUMMARY = REPO / "results/nano-lm/wave-am/am_hitl_summary.json"
...
def run_am_hitl(
    *,
    ...
    """
    GIVEN frozen AM0 held-out asks
    WHEN final LOOKUP + GENTRUTH peak GENERATE ASK→EVAL→FIX×10
    THEN lookup≥7; gen≥5 → PROMOTE else documented HOLD.
    """
    if len(AM0_PACK) != AM6_N:
    ...
    del _ablated
    if len(lookup_payloads) != AM6_N or len(gen_payloads) != AM6_N:
    ...
    for i, (item, lp, gp) in enumerate(
        zip(AM0_PACK, lookup_payloads, gen_payloads, strict=True),
        start=1,
    ):
        ...
        l_score, l_err, l_notes = score_am6_lookup(
        ...
        lt = {
            "trial_id": f"AM-FINAL-LOOKUP-HITL-{i:02d}",
            "stage": "AM6",
            "hyp_id": AM6_ID,
            "arm": "LOOKUP",
            "app_id": item["app_id"],
        ...
        if g_tel["wall_ms"] > 0.0 and g_tel["n_new"] > 0:
            n_gen_wall_ok += 1
        g_score, g_err, g_notes = score_am6_gen(
            completion=str(gp.get("completion", "")),
            expected_gold=str(item["gold"]),
            payload=gp,
        )
        gt = {
            "trial_id": f"AM-FINAL-GEN-HITL-{i:02d}",
            "stage": "AM6",
            "hyp_id": AM6_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
