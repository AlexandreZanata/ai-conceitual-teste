"""Render H-THIN smoke vs H-CURL (same EARLY decode)."""
...
def render(smoke_path: Path) -> str:
    ...
    s = stats.get("H-THIN", {})
    decision = decide_hthin(s, stats) if s else "needs H-THIN rows"
...
def main() -> int:
    ...
    p.add_argument("--smoke", type=Path, default=root / "thin_smoke.json")
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hthin-vs-hcurl.md"),
    )
    args = p.parse_args()
