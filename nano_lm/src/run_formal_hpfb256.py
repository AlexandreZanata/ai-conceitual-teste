"""Formal H-PFB256: PFB2 on prog@256 vs EARLY@256 (fit≠eval)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hpfb256"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-PFB256 formal requires CUDA")
    out: Path = c["out"]
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "max_new": _MAX_NEW,
            "cpu_threads": threads,
            "mode": "PFB256 formal: PFB2@256 vs EARLY@256 (fit≠eval)",
            "mechanism": "elongate like DOM to L=256; BEAMKV shared KV; not CTX chunk",
            "parent": "H-EARLY n=1 @256 on B2 (formal genes)",
        },
    )
