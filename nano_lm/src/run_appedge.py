"""Wave AN5 H-APPEDGE runner: dual-arm apps + DEPL-AN (nano:appedge)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-an/appedge_summary.json"
...
def _seed_pack(bank_path: Path, an_bank: Path) -> int:
    ...
    for i, item in enumerate(AN0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AN-APPEDGE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = APPEDGE_ID
        row["judge_notes"] = [
            "APPEDGE seed for AM held-out ask",
...
def _write_pages(docs: Path) -> list[dict[str, Any]]:
    ...
    depl_path = docs / Path(DEPL_AN_PAGE).name
    depl_body = depl_an_body()
    depl_path.write_text(depl_body, encoding="utf-8")
    reports.append(page_sync_report(DEPL_AN_PAGE, depl_body))
    return reports
...
def _run_one_app(
    *,
    ...
    if len(lookup_payloads) != APPEDGE_N or len(shared_gen) != APPEDGE_N:
    ...
    for i, item in enumerate(AN0_PACK, start=1):
        ...
        if not bool(route["in_scope"]):
            ...
            row = alias_bank_row(
                trial_id=f"AN-APPEDGE-FIX-{tag}-{i:02d}",
        ...
        l_score, l_err, l_notes = score_appedge_lookup(
            mode=l_mode,
            completion=l_comp,
        ...
        l_trial = {
            "trial_id": f"AN-APPEDGE-{tag}-LOOKUP-HITL-{i:02d}",
            "stage": "AN5",
            "hyp_id": APPEDGE_ID,
            "arm": "LOOKUP",
            "realapp_id": app_id,
        ...
        g_score, g_err, g_notes = score_appedge_gen(
        ...
        g_trial = {
            "trial_id": f"AN-APPEDGE-{tag}-GEN-HITL-{i:02d}",
            "stage": "AN5",
            "hyp_id": APPEDGE_ID,
            "arm": "GENERATE",
            "realapp_id": app_id,
...
def run_appedge(
    *,
    ...
    """
    GIVEN AN0 pack + APPEDGE apps
    WHEN ASK→EVAL→FIX×10 dual-arm per surface + DEPL-AN
    THEN expose LOOKUP|GENERATE ∧ DEPL → PROMOTE|HOLD|KILL.
    """
    if len(AN0_PACK) != APPEDGE_N:
    ...
    bank = load_bank_rows(bank_path)
    apps = [dict(a) for a in APPEDGE_APPS]
    if only_app:
        apps = [app_by_id(only_app)]

    # Shared GENEDGE peak GENERATE once — max HW without 3× CUDA reload.
    ...
    summary: dict[str, Any] = {
        ...
        "public_note": "docs/results/nano-lm/formal-happedge-appedge.md",
        "ship_claim": "AF packaged stack until AN6 HITL bar",
        "claim": (
