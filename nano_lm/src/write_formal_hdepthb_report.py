"""Render formal H-DEPTHB vs H-LAYB (DEPTH_prun under LAYB decode)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        "# Formal H-DEPTHB vs H-LAYB (DEPTH_prun under LAYB decode)",
        "",
        f"Source: `{path}`",
        f"Wall clock: {data.get('wall_s', float('nan')):.1f}s",
        "",
        "Shared formal B2 + formal HDEPTH_prun + EARLY + LAY + KVSEL. Fit≠eval.",
        "Dual-budget LAYB on DEPTH_prun vs tip LAYB on B2; GFLOPs density-scaled.",
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Thin+prune util under LAYB — does not replace H-EARLY / H-LAYB / H-DEPTH.",
            "",
            "Commands: `npm run nano:formal:hdepthb` → "
            "`npm run nano:formal:hdepthb:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hdepthb/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hdepthb-vs-hlayb.md"),
    )
    args = p.parse_args()
