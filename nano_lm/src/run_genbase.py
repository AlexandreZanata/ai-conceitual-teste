"""Wave AP1 H-GENBASE runner: dual-arm + peak ablation true-gen gate."""
...
_AP_BANK = REPO / "results/nano-lm/wave-ap/error_bank.jsonl"
_TRIALS = REPO / "results/nano-lm/wave-ap/trials"
_SUMMARY = REPO / "results/nano-lm/wave-ap/genbase_summary.json"
...
def _seed_pack(bank_path: Path, ap_bank: Path) -> int:
    bank_path.parent.mkdir(parents=True, exist_ok=True)
    ap_bank.parent.mkdir(parents=True, exist_ok=True)
    if not ap_bank.is_file():
        ap_bank.write_text("", encoding="utf-8")
    existing = {
        str(r.get("question", "")).strip() for r in load_bank_rows(bank_path)
    }
    n = 0
    for i, item in enumerate(GENBASE_PACK, start=1):
        q = str(item["question"]).strip()
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AP-GENBASE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = GENBASE_ID
        row["judge_notes"] = [
            "GENBASE seed for LOOKUP arm",
            "LOOKUP product path — not generative IQ",
            "no student weight update",
        ]
        append_error_row(bank_path, row)
        append_error_row(ap_bank, row)
        existing.add(q)
        n += 1
    return n
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AP-GENBASE-FIX-{i:02d}",
...
def _split_gen_arms(
    ...
    for gp, item, ctx in zip(raw, items, contexts, strict=True):
        ...
        ablated.append(off)
        text, used, peak = apply_genbase_peak(
            decode_text=decode,
            question=item["question"],
            context=ctx,
        )
...
def run_genbase(
    *,
    ...
    """
    if len(GENBASE_PACK) != GENBASE_N:
        raise ValueError("GENBASE pack must be 10")

    trials_dir.mkdir(parents=True, exist_ok=True)
    t0 = time.perf_counter()
    seeded = _seed_pack(bank_path, ap_bank)
    bank = load_bank_rows(bank_path)
    questions = [p["question"] for p in GENBASE_PACK]
    ...
    gen_off, gen_on = _run_gen_ablation(
        champ=root,
        items=[dict(p) for p in GENBASE_PACK],
    ...
    for i, (item, lp) in enumerate(
        zip(GENBASE_PACK, lookup_payloads, strict=True),
        start=1,
    ):
        ...
        score_l, err_l, notes_l = score_genbase_lookup(
        ...
        lookup_trials.append(
            {
                "trial_id": f"AP-GENBASE-LOOKUP-HITL-{i:02d}",
                "stage": "AP1",
                "hyp_id": GENBASE_ID,
                "arm": "LOOKUP",
                "app_id": item["app_id"],
    ...
    for i, (item, off, on) in enumerate(
        zip(GENBASE_PACK, gen_off, gen_on, strict=True)
    ):
        score_g, err_g, notes_g = score_genbase_gen(
            completion=str(off.get("completion", "")),
            expected_gold=item["gold"],
            payload=off,
            peak_ablated=True,
        )
        score_p, _err_p, notes_p = score_genbase_gen(
    ...
    fix_attempts = 0
    if weak_idx:
        re_items = [dict(GENBASE_PACK[i]) for i in weak_idx]
        ...
        for j, idx in enumerate(weak_idx):
            item = GENBASE_PACK[idx]
            score2, err2, notes2 = score_genbase_gen(
                completion=str(re_off[j].get("completion", "")),
                expected_gold=item["gold"],
                payload=re_off[j],
                peak_ablated=True,
            )
            score_p2, _, _ = score_genbase_gen(
    ...
    for i, (item, off, on) in enumerate(
        ...
        if bool(on.get("peak_used")):
            n_peak += 1
        gt = {
            "trial_id": f"AP-GENBASE-GEN-HITL-{i:02d}",
            "stage": "AP1",
            "hyp_id": GENBASE_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
    ...
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{GENBASE_ID}: L_lookup={stats['lookup_mean']:.1f} "
        ...
        "public_note": "docs/results/nano-lm/formal-hgenbase-genbase.md",
...
def main() -> int:
    ...
    ap.add_argument("--ap-bank", type=Path, default=_AP_BANK)
