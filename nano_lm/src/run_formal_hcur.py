"""Formal-budget H-CUR vs B2 (length-curriculum KD), equal steps."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hcur",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 4,
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        ckpt = c["out"] / f"HCUR_seed{seed}.pt"
        cur_meta = run_h_cur(
            teacher_id=c["teacher_id"],
            tokenizer_id=c["tokenizer_id"],
            cache_dir=c["cache"],
        ...
        write_json(c["out"] / f"HCUR_seed{seed}_train.json", cur_meta)
        rows.append(eval_ckpt(c, ckpt, seed, "H-CUR"))
        if device.type == "cuda":
            torch.cuda.empty_cache()
    payload = {
        "rows": rows,
