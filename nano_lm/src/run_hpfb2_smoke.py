"""Smoke H-ABS-PFB2: PFB K=2 vs EARLY; wall↓ vs same-run PFB k=4."""
...
def main() -> int:
    ...
    if device.type != "cuda":
        print("ERROR: H-ABS-PFB2 requires CUDA", file=sys.stderr)
        return 2
    ...
    payload = {
        ...
        "max_new": _MAX_NEW,
        "cpu_threads": threads,
        "mode": "PFB2: K=2 PFB vs EARLY; wall↓ vs same-run PFB k=4",
        "mechanism": "PFB commit K=2; efficiency gate vs PFB k=4",
        "parent": "H-EARLY n=1 greedy on B2",
    }
    write_json(out / "hpfb2_smoke.json", payload)
    print(json.dumps({"decision": decision, "out": str(out / "hpfb2_smoke.json")}))
    return 0
