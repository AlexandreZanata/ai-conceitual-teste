"""Wave AZ3 H-NANOGEN10: gen-defer gate — not NANOGEN9+rename."""
...
NANOGEN10_ID = "H-NANOGEN10"
NANOGEN10_THESIS = (
    "North-star generative gate under AZ0 gen stance: PROMOTE only with a "
    "real new train/data/arch method AND true_continue; else DEFER/HOLD. "
    "CAPCHECK closed; never NANOGEN10 = NANOGEN9+rename; "
    "NANOGEN6·7 HOLD · NANOGEN8·9 DEFER stand; span-fallback ≠ gen IQ; "
    "mini-AGI locked while deferred"
)
NANOGEN10_CLAIM = AZ0_SHIP_LOCK
NANOGEN10_SAFE_NOTE = AZ0_SAFE_NOTE
NANOGEN10_ANTI_FP = AZ0_ANTI_FP
NANOGEN10_STANCE = dict(AZ0_GEN_STANCE)
NANOGEN10_METHOD: Mapping[str, object] = {
    "id": "gen-defer",
    "kind": "defer",
    ...
    "rationale": (
        "AZ0 froze stance=defer: no real new train/data/arch ready; "
        "NANOGEN6·7 HOLD · NANOGEN8·9 DEFER stand; prefer product ship "
        "(PRODGEN·SHIPAZ) + honest paper over vanity NANOGEN10 clone"
    ),
}
...
def method_is_rename(method: Mapping[str, object]) -> bool:
    ...
    THEN True iff it is NANOGEN6/7/8/9 rename theater.
    """
    if bool(method.get("renames_nanogen9")):
        return True
    if bool(method.get("renames_nanogen8")):
        return True
    ...
    if "nanogen9" in mid and "rename" in kind:
        return True
    if "nanogen8" in mid and "rename" in kind:
        return True
...
def extract_nanogen10_board(
    *,
    ...
    """
    GIVEN AZ0 stance + claimed method + optional live true_continue probe
    WHEN building NANOGEN10 board
    THEN publish defer/method/true_continue honesty fields.
    """
    st = dict(stance) if stance is not None else dict(NANOGEN10_STANCE)
    meth = dict(method) if method is not None else dict(NANOGEN10_METHOD)
    return {
        ...
        "renames_forbidden": bool(st.get("nanogen10_rename_forbidden", True)),
        ...
        "ship_lock": NANOGEN10_CLAIM,
    }


def decide_nanogen10(
    *,
    board: Mapping[str, Any],
    anti_fp_signed: bool = True,
) -> str:
    """
    GIVEN NANOGEN10 board (stance · method · true_continue)
    WHEN applying pesquisa §5 AZ3
    ...
    if bool(board.get("is_rename")):
        return "KILL (NANOGEN10 = NANOGEN9/8/7/6 rename forbidden)"
    if not bool(board.get("renames_forbidden", True)):
        return "KILL (nanogen10_rename_forbidden must stay True)"
