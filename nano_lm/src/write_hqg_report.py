"""Render H-QG smoke vs H-EARLY tip (quality-gated FLOP min)."""
...
def render(path: Path) -> str:
    ...
    s = stats.get("H-QG", {})
    decision = decide_hqg(s, stats) if s else "needs H-QG rows"
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/qg_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hqg-vs-hearly.md"),
    )
    args = p.parse_args()
