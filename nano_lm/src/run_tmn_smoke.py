"""Smoke H-TMN: stack tip metas; dual gate vs H-TYP + H-MINP."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        typ_train = out / f"HTYP_seed{seed}_train.json"
        minp_train = out / f"HMINP_seed{seed}_train.json"
        for p in (ckpt, typ_train, minp_train):
            if not p.is_file():
        ...
        meta = run_h_tmn(
            ...
            out_meta=out / f"HTMN_seed{seed}_train.json",
        )
        row = {
            "family": "H-TMN",
        ...
        write_json(out / f"HTMN_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "tmn_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "tmn_smoke.json")}))
    return 0
