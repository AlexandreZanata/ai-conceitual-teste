"""Wave AM2 H-CTXNEXT runner: deca-doc dual-arm ASK→EVAL→FIX×10."""
...
_SUMMARY = REPO / "results/nano-lm/wave-am/ctxnext_summary.json"
...
def _seed_pack_golds(bank_path: Path, am_bank: Path) -> int:
    ...
    for i, item in enumerate(AM0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AM-CTXNEXT-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = CTXNEXT_ID
        row["judge_notes"] = [
            "CTXNEXT seed alias for held-out AM ask",
...
def _build_ctxs(
    *, curated_root: Path, workers: int, tok: Any
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        rows = list(pool.map(_read, [dict(p) for p in AM0_PACK]))
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AM-CTXNEXT-FIX-{i:02d}",
...
def _lookup_trial(
    *,
    ...
    tid = f"AM-CTXNEXT-LOOKUP-HITL-{i:02d}"
    mode = str(payload.get("mode", ""))
...
def _gen_trial(
    *,
    ...
    tid = f"AM-CTXNEXT-GEN-HITL-{i:02d}"
    score, err, notes, usable = score_ctxnext_gen(
        completion=str(payload.get("completion", "")),
        expected_gold=str(item["gold"]),
        meta=ctx,
        payload=payload,
...
def run_ctxnext(
    *,
    ...
    """
    GIVEN AM0 held-out asks + deca curated docs
    WHEN K=21 ROLL/SUMCACHE + LOOKUP askfast + GENERATE raw
    THEN L_eff↑ vs CTXFRESH · gen usable≥5 → PROMOTE|HOLD|KILL.
    """
    if len(AM0_PACK) != CTXNEXT_N:
        raise ValueError("AM0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack_golds(bank_path, am_bank)
    cfg = matrix_cfg()
    tok = load_tokenizer(str(cfg["tokenizer_id"]), cfg["cache"])
    bank = load_bank_rows(bank_path)
    ctxs = _build_ctxs(curated_root=curated_root, workers=workers, tok=tok)
    questions = [p["question"] for p in AM0_PACK]
    ...
    for i, (item, lp, gp, ctx) in enumerate(
        zip(AM0_PACK, lookup_payloads, gen_payloads, ctxs, strict=True),
        start=1,
    ):
    ...
    mean_l = float(sum(float(c["l_eff"]) for c in ctxs) / CTXNEXT_N)
    mean_a = float(
        sum(float(c["sumcache_active"]) for c in ctxs) / CTXNEXT_N
    )
    mean_s = float(sum(float(c["n_slices"]) for c in ctxs) / CTXNEXT_N)
    mean_src = float(sum(float(c["n_sources"]) for c in ctxs) / CTXNEXT_N)
    ...
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{CTXNEXT_ID}: L_lookup={stats['lookup_mean']:.1f} "
            f"L_gen={stats['gen_mean']:.1f} "
            f"usable_L={stats['n_lookup_usable']}/10 "
            f"usable_G={stats['n_gen_usable']}/10 "
            f"L_eff={mean_l:.0f} (>CTXFRESH {stats['ctxfresh_mean_leff']:.0f}) "
            f"sources={mean_src:.1f} slices={mean_s:.1f} "
            f"false_hit={n_false} fix={fix_count} decision={decision}."
        ),
        "public_note": "docs/results/nano-lm/formal-hctxnext-ctxnext.md",
        "ship_claim": "AF packaged stack (deca long-ctx; LOOKUP≠gen IQ)",
        "claim": (
            "deca-doc longer usable ctx beyond CTXFRESH parent — "
