"""Shared H-EARLY vs H-SERVE seed pair (tip alone vs best frozen stack)."""
...
from serve_score import SMOKE_BUDGETS, score_serve_candidates, tip_row

__all__ = ["run_seed_pair", "SERVE_CHUNK", "SMOKE_BUDGETS"]
...
def run_seed_pair(
    ...
    prompts: list[str],
    *,
    claim_offset: int = 8080,
    budgets: tuple[int, ...] = SMOKE_BUDGETS,
) -> list[dict[str, Any]]:
    """
    GIVEN shared B2 + tip genes (dirs on c)
    WHEN scoring EARLY alone vs best of GALL-speed / GRAPHF-quality
    THEN return tip_row for H-EARLY and H-SERVE.
    """
    ...
    recipe, chosen, all_m = score_serve_candidates(
        teacher=teacher,
        student=student,
        prompts=prompts,
        early_gene=early,
    ...
    early_m = all_m["early"]
    g_early = {**early, "backend": "serial-early", "budgets": list(budgets)}
    g_serve = {
        **lay,
        "kv_threshold": thr,
        "chunk_size": SERVE_CHUNK,
        "recipe": recipe,
        "backend": (
            "sdpa+lay+cudagraph-all"
            if recipe == "speed"
            else "sdpa+pool+lay+graphf-dual"
        ),
        "budgets": list(budgets),
    }
    return [
        tip_row("H-EARLY", f"HEARLY_serve_seed{seed}", early_m, seed, g_early),
        tip_row("H-SERVE", f"HSERVE_seed{seed}", chosen, seed, g_serve),
    ]
