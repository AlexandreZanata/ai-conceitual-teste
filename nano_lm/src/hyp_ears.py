"""H-EARS: evolve scheduled early-exit thr vs H-EARLY tip."""
...
from ears_fit import fitness_ears_detail
from ears_ops import clamp_ears_gene, mutate_ears_gene, random_ears_gene
...
def run_h_ears(
    *,
    ...
    pop = [random_ears_gene(rng) for _ in range(pop_size)]
        ...
        history.append(
            {
                ...
                "best_budget_coef": pop[ranked[0]]["budget_coef"],
                "best_len_coef": pop[ranked[0]]["len_coef"],
            }
        )
        if scores[ranked[0]] > best_score:
            best_score = scores[ranked[0]]
            best_lp = details[ranked[0]][0]
            best_gene = clamp_ears_gene(pop[ranked[0]])
        parents = [pop[i] for i in ranked[: max(1, pop_size // 2)]]
        pop = [
            mutate_ears_gene(parents[i % len(parents)], rng) for i in range(pop_size)
        ]
    eval_lp, eval_wall = fitness_ears_detail(
        best_gene,
        teacher=teacher,
        student=student,
