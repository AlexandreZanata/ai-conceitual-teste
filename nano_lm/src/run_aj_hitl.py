"""Wave AJ6 AJ-HITL-10 runner: final dual-arm verify (nano:aj:hitl)."""
...
from aj_session_ops import AJ0_PACK, overlaps_prior_questions, pack_app_counts
...
_SUMMARY = REPO / "results/nano-lm/wave-aj/aj_hitl_summary.json"
...
def run_aj_hitl(
    *,
    ...
    """
    GIVEN frozen AJ0 held-out asks
    WHEN final LOOKUP + GENPEAK GENERATE ASK→EVAL→FIX×10
    THEN lookup≥7; gen≥5 → PROMOTE else documented HOLD.
    """
    if len(AJ0_PACK) != AJ6_N:
    ...
    if len(lookup_payloads) != AJ6_N or len(gen_payloads) != AJ6_N:
    ...
    for i, (item, lp, gp) in enumerate(
        zip(AJ0_PACK, lookup_payloads, gen_payloads, strict=True),
        start=1,
    ):
        ...
        l_score, l_err, l_notes = score_aj6_lookup(
        ...
        lt = {
            "trial_id": f"AJ-FINAL-LOOKUP-HITL-{i:02d}",
            "stage": "AJ6",
            "hyp_id": AJ6_ID,
            "arm": "LOOKUP",
            "app_id": item["app_id"],
        ...
        if g_tel["wall_ms"] > 0.0 and g_tel["n_new"] > 0:
            n_gen_wall_ok += 1
        g_score, g_err, g_notes = score_aj6_gen(
            completion=str(gp.get("completion", "")),
            expected_gold=str(item["gold"]),
            payload=gp,
        )
        gt = {
            "trial_id": f"AJ-FINAL-GEN-HITL-{i:02d}",
            "stage": "AJ6",
            "hyp_id": AJ6_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
