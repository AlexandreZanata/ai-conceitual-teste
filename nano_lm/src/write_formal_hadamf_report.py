"""Render formal H-ADAMF vs H-HALF (fused AdamW under HALF)."""
...
def render(path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "Commands: `npm run nano:formal:hadamf` → "
            "`npm run nano:formal:hadamf:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hadamf/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hadamf-vs-hhalf.md"),
    )
    args = p.parse_args()
