"""Render H-CURL smoke seq_lo ablation markdown."""
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/curl_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hcurl-vs-hcur.md"),
    )
    args = p.parse_args()
