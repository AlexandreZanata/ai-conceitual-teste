"""Render formal H-POOL3 vs H-POOL (FLOP dual gate)."""
...
def render(path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:hpool3` → "
            "`npm run nano:formal:hpool3:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hpool3/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hpool3-vs-hpool.md"),
    )
    args = p.parse_args()
