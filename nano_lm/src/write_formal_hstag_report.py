"""Render formal H-STAG n_stages grid vs tip stages=3."""
...
def render(path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "Commands: `npm run nano:formal:hstag` → "
            "`npm run nano:formal:hstag:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hstag/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hstag-vs-hcurl2.md"),
    )
    args = p.parse_args()
