"""H-DECM: LAT2 search → elite gene mixture; claim multi-gene pick."""
...
from lat2_ops import MIN_LAM, mutate_gene_lat2, random_gene_lat2
...
def run_h_decm(
    *,
    ...
    if lam < MIN_LAM:
        raise ValueError(f"run_h_decm: lam must be >= {MIN_LAM}")
    ...
    last_scores = [float("-inf")] * pop_size
    ...
    for gen in range(generations):
        ...
        last_scores = [latency_aware_score(lp, w, lam) for lp, w in details]
        ranked = sorted(range(pop_size), key=lambda i: last_scores[i], reverse=True)
        history.append({"gen": gen, "best_score": last_scores[ranked[0]]})
        parents = [pop[i] for i in ranked[: max(1, pop_size // 2)]]
        pop = [
            mutate_gene_lat2(parents[i % len(parents)], rng) for i in range(pop_size)
        ]
    # Rescore final pop once for mixture + single-gene control.
    details = [
        fitness_gene_detail(
            g,
            teacher=teacher,
    ...
    mixture = elite_mixture(pop, scores, m=mix_m)
    best_gene = mixture[0]
    ...
    lat2_lp, lat2_wall = fitness_gene_detail(
        best_gene,
        teacher=teacher,
        student=student,
        prompts=claim_prompts,
    ...
    meta = {
        ...
        "lat2_eval_fit": lat2_lp,
        "lat2_eval_wall_ms": lat2_wall,
        "picks": picks,
        "eval_max_new": hold,
        "history": history,
