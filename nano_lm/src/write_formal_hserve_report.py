"""Render formal H-SERVE vs H-EARLY (full serving stack)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Fit≠eval. Control: serial EARLY alone. Candidate: best of "
        f"GALL-speed / GRAPHF-quality (recipes: `{_recipes(data['rows'])}`). "
        "Gate: |Δlp| ≤ ε **and** (wall < EARLY or tok/s > EARLY).",
    ...
    lines.extend(
        [
            ...
            "",
            "Commands: `npm run nano:formal:hserve` → "
            "`npm run nano:formal:hserve:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hserve/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hserve-vs-hearly.md"),
    )
    args = p.parse_args()
