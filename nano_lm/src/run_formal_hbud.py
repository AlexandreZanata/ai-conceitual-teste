"""Formal-budget H-BUD vs H-EARLY (max_new + exit gene)."""
...
def _eval_bud(c: dict[str, Any], ckpt: Path, gene: dict, seed: int) -> dict:
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        tip_eval = early_dir / f"HEARLY_seed{seed}_train.json"
        meta = run_h_bud(
            student_ckpt=b2,
            ...
            seed=seed,
            lam=MIN_LAM,
            early_tip_path=tip_eval,
            out_meta=c["out"] / f"HBUD_seed{seed}_train.json",
        )
        row = _eval_bud(c, b2, meta["best_gene"], seed)
        row["lam"] = MIN_LAM
