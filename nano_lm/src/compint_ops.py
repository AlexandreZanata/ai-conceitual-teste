"""Wave BE1 H-COMPINT: BE-FOREVER FH 0 · BA…BD/AZ hold · live board."""
...
COMPINT_ID = "H-COMPINT"
COMPINT_THESIS = (
    "Drive BE-FOREVER held-out FH → 0 (type/coercion str→int≠add · "
    "paraphrases · type-schema neighbors) via compositional SEMWRAP "
    "type/schema gate — not bank stuffing; hold BA…BD-FOREVER + AZ "
    "div·sub·BIP FH 0 + a.clear() LOOKUP; live ask scoreboard "
    "OK|FP|MISS|ABSTAIN-OK (≥10 novel); modes · p50/p99 · DECODE law"
)
COMPINT_CLAIM = BE0_SHIP_LOCK
COMPINT_SAFE_NOTE = BE0_SAFE_NOTE
COMPINT_ANTI_FP = BE0_ANTI_FP
...
NOVEL_PROBES: tuple[dict[str, str], ...] = (
    {
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Python: coerce text s into int without writing add(a,b)."
        ),
    },
    {
        "id": "BE-NOVEL-02",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "What’s the idiomatic way to parse digits in string s as int?"
        ),
    },
    {
        "id": "BE-NOVEL-03",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Cast the contents of s from str to integer type in Python."
        ),
    },
    {
        "id": "BE-NOVEL-04",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Turn floating point f into a truncated int in one call."
        ),
    },
    {
        "id": "BE-NOVEL-05",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Make an immutable tuple from list xs in Python."
        ),
    },
    {
        "id": "BE-NOVEL-06",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Decode bytes payload b into a Unicode string."
        ),
    },
    {
        "id": "BE-NOVEL-07",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "How can I test whether string s contains only digits?"
        ),
    },
    {
        "id": "BE-NOVEL-08",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Represent True/False as integers 1 and 0 in Python."
        ),
    },
    {
        "id": "BE-NOVEL-09",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "I want integer parsing of s — definitely not a+b helper."
        ),
    },
    {
        "id": "BE-NOVEL-10",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Convert number n into its decimal string form."
        ),
    },
)
...
def bars_from_scoreboard(
    ...
    """
    src = suite if suite is not None else BE0_SCOREBOARD
    bars = src.get("bars")
...
def score_live_row(row: Mapping[str, Any], *, expect_mode: str) -> str:
    ...
    """
    mode = str(
        row.get("product_mode") or map_be_product_mode(str(row.get("mode", "")))
    )
...
def extract_compint_board(
    *,
    ...
    """
    GIVEN BE-FOREVER + BA…BD hold + AZ hold + over-refuse + live scores
    WHEN building COMPINT board
    THEN publish BE FH · BA/BB/BC/BD hold · AZ hold · over-refuse · live.
    """
...
def _gate_core_flags(board: Mapping[str, Any], bar: Mapping[str, Any]) -> str | None:
    ...
    if not bool(bar.get("ba_bb_bc_bd_pass_neq_be_forever", True)):
        return "KILL (BA…BD PASS ≠ BE forever bar missing)"
...
def decide_compint(
    *,
    ...
    WHEN applying pesquisa §9 BE1
    THEN PROMOTE iff BE FH 0 + BA…BD hold 0 + AZ hold 0 + over-refuse 0
         + live FP 0.
    """
    ...
    err = (
        _gate_forever_pack(
            ...
            bar=bar,
            key="be_forever",
            min_n=int(bar.get("be_forever_min_n", 12)),
    ...
    if err:
        return err
    return (
        f"PROMOTE ({COMPINT_ID}: BE-FOREVER FH 0; BA…BD hold 0; "
