"""Render H-Q4 smoke vs H-DEPTH (CUDA int4 weight-only)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Same `HDEPTH_prun` ckpt + frozen EARLY genes; Linear layers use "
        "`aten::_weight_int4pack_mm` (gpt-fast affine group quant). "
        "`lm_head` stays fp (out_features % 8 ≠ 0).",
        "Kill if lp < DEPTH−ε or no wall win. Memory bytes reported.",
        f"Backend: `{data.get('backend')}`; "
        f"`groupsize={data.get('groupsize')}`; `tiles={data.get('tiles')}`.",
        "",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/q4_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/archive/hq4-vs-hdepth.md"),
    )
    args = p.parse_args()
