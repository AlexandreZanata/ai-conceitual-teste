"""Render H-PRE smoke vs H-PIN (prefetch H2D under PIN)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Same top-k soft cache, STAG curriculum, and pinned tensors; "
        "H-PRE overlaps H2D+expand of step i+1 with compute of step i. "
        "Not H-ASYNC (cache build overlap). Kill if |Δlp| > ε or no ms/step win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/pre_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hpre-vs-hpin.md"),
    )
    args = p.parse_args()
