"""Smoke H-EXIT: earlier min_new + n=1 vs frozen H-EARLY tip."""
...
from hyp_exit import run_h_exit
...
def _score_tip(
    ...
    return _row("H-EARLY", f"HEARLY_exit_seed{seed}", lp, wall, gf, seed, gene)
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_exit(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=out / f"HEXIT_seed{seed}_train.json",
        )
        row = _row(
            "H-EXIT",
            f"HEXIT_seed{seed}",
        ...
        write_json(out / f"HEXIT_seed{seed}_eval.json", row)
        rows.append(row)
    write_json(
        out / "exit_smoke.json",
        {"rows": rows, "wall_s": time.perf_counter() - t0},
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "exit_smoke.json")}))
    return 0
