"""Shared H-ADAMF vs H-PRE2 seed pair (1-deep vs 2-deep prefetch)."""
...
def run_seed_pair(
    ...
    steps: int,
    *,
    label_prefix: str = "HPRE2",
) -> list[dict[str, Any]]:
    """
    GIVEN shared STAG batches and one top-k cache
    WHEN training ADAMF (depth=1) vs PRE2 (depth=2) under ADAMF I/O
    ...
    write_json(out / f"{label_prefix}_seed{seed}_train.json", pre2)
    ev_a = eval_ckpt(c, Path(adamf["out_path"]), seed, "H-ADAMF")
    ev_p = eval_ckpt(c, Path(pre2["out_path"]), seed, "H-PRE2")
    if device.type == "cuda":
        torch.cuda.empty_cache()
    return [
        row_htop(
