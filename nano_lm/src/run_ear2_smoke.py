"""Smoke H-EAR2: widened early-exit gene vs H-EARLY tip."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_ear2(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=out / f"HEAR2_seed{seed}_train.json",
        )
        row = {
            "family": "H-EAR2",
        ...
        write_json(out / f"HEAR2_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "ear2_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "ear2_smoke.json")}))
    return 0
