"""Wave AN2 H-CTXEDGE runner: undeca-doc dual-arm ASK→EVAL→FIX×10."""
...
_SUMMARY = REPO / "results/nano-lm/wave-an/ctxedge_summary.json"
...
def _seed_pack_golds(bank_path: Path, an_bank: Path) -> int:
    ...
    for i, item in enumerate(AN0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AN-CTXEDGE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = CTXEDGE_ID
        row["judge_notes"] = [
            "CTXEDGE seed alias for held-out AN ask",
...
def _build_ctxs(
    *, curated_root: Path, workers: int, tok: Any
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        rows = list(pool.map(_read, [dict(p) for p in AN0_PACK]))
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AN-CTXEDGE-FIX-{i:02d}",
...
def _lookup_trial(
    *,
    ...
    tid = f"AN-CTXEDGE-LOOKUP-HITL-{i:02d}"
    mode = str(payload.get("mode", ""))
    score, err, notes, usable = score_ctxedge_lookup(
        mode=mode,
...
def _gen_trial(
    *,
    ...
    tid = f"AN-CTXEDGE-GEN-HITL-{i:02d}"
    score, err, notes, usable = score_ctxedge_gen(
        completion=str(payload.get("completion", "")),
        expected_gold=str(item["gold"]),
        meta=ctx,
        payload=payload,
...
def run_ctxedge(
    *,
    ...
    """
    GIVEN AN0 held-out asks + undeca curated docs
    WHEN K=23 ROLL/SUMCACHE + LOOKUP askfast + GENERATE raw
    THEN L_eff↑ vs CTXNEXT · gen usable≥5 → PROMOTE|HOLD|KILL.
    """
    if len(AN0_PACK) != CTXEDGE_N:
        raise ValueError("AN0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack_golds(bank_path, an_bank)
    cfg = matrix_cfg()
    ...
    for i, (item, lp, gp, ctx) in enumerate(
        zip(AN0_PACK, lookup_payloads, gen_payloads, ctxs, strict=True),
        start=1,
    ):
    ...
    mean_l = float(sum(float(c["l_eff"]) for c in ctxs) / CTXEDGE_N)
    mean_a = float(
        sum(float(c["sumcache_active"]) for c in ctxs) / CTXEDGE_N
    )
    mean_s = float(sum(float(c["n_slices"]) for c in ctxs) / CTXEDGE_N)
    mean_src = float(sum(float(c["n_sources"]) for c in ctxs) / CTXEDGE_N)
    ...
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{CTXEDGE_ID}: L_lookup={stats['lookup_mean']:.1f} "
            f"L_gen={stats['gen_mean']:.1f} "
            f"usable_L={stats['n_lookup_usable']}/10 "
            f"usable_G={stats['n_gen_usable']}/10 "
            f"L_eff={mean_l:.0f} (>CTXNEXT {stats['ctxnext_mean_leff']:.0f}) "
            f"sources={mean_src:.1f} slices={mean_s:.1f} "
            f"false_hit={n_false} fix={fix_count} decision={decision}."
        ),
        "public_note": "docs/results/nano-lm/formal-hctxedge-ctxedge.md",
        "ship_claim": "AF packaged stack (undeca long-ctx; LOOKUP≠gen IQ)",
        "claim": (
            "undeca-doc longer usable ctx beyond CTXNEXT parent — "
