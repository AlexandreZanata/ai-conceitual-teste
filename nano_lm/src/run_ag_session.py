"""Wave AG0 SESSION runner (nano:ag:session) — freeze held-out HITL×10."""
...
_OUT = REPO / "results/nano-lm/wave-ag/ag0_session.json"
...
def main() -> int:
    ...
    ids = [str(p["source_id"]) for p in AG0_PACK]
    ...
    trials_ready = Path(args.trials_dir).is_dir() and len(written) == AG0_N

    decision = _override_decision(
        decide_ag0_session(known_sources=known, trials_dir_ready=trials_ready),
        miss=miss,
        clash=clash,
    ...
    payload = {
        ...
        "ask_smoke": ask,
        "public_note": "docs/results/nano-lm/wave-ag-session.md",
        "rule": "pesquisa §5 AG0 · Cursor ASK→EVAL→FIX dual-arm law",
        "next": "AG1 H-ANTIFP (harness + ASK smoke)",
        "anti_fp": (
