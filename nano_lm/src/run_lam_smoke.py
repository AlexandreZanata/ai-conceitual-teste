"""Smoke H-LAM on matrix out dir; merge rows into matrix.json."""
...
def _merge(rows: list[dict[str, Any]], wall_s: float, out: Path) -> None:
    ...
    kept = [r for r in data.get("rows", []) if r.get("family") != "H-LAM"]
    data["rows"] = kept + rows
    data["lam_wall_s"] = wall_s
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        write_json(out / f"HLAM_seed{seed}_eval.json", ev)
        rows.append(ev)
    wall_s = time.perf_counter() - t0
    write_json(out / "lam_smoke.json", {"rows": rows, "wall_s": wall_s})
    _merge(rows, wall_s, out)
    print(json.dumps({"n_rows": len(rows), "out": str(out / "lam_smoke.json")}))
    return 0
