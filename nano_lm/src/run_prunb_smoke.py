"""Smoke H-PRUNB: PRUN ckpt under LAYB vs tip LAYB on B2."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        rows.append(tip_row("H-LAYB", f"HLAYB_prunb_seed{seed}", layb, seed, g_tip))
        ...
        row = tip_row("H-PRUNB", f"HPRUNB_seed{seed}", prunb, seed, g)
        row["density"] = dens
        write_json(out / f"HPRUNB_seed{seed}_eval.json", row)
        rows.append(row)
    write_json(
        out / "prunb_smoke.json",
        {
            "rows": rows,
            ...
            "mode": "PRUN under LAYB vs B2+LAYB; dual-budget; long prompts",
        },
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "prunb_smoke.json")}))
    return 0
