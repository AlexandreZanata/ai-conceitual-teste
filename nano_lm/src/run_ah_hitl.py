"""Wave AH6 AH-HITL-10 runner: final dual-arm verify (nano:ah:hitl)."""
...
from ah_session_ops import AH0_PACK, overlaps_prior_questions
...
_SUMMARY = REPO / "results/nano-lm/wave-ah/ah_hitl_summary.json"
...
def run_ah_hitl(
    *,
    ...
    if len(lookup_payloads) != AH6_N or len(gen_payloads) != AH6_N:
    ...
    for i, (item, lp, gp) in enumerate(
        zip(AH0_PACK, lookup_payloads, gen_payloads, strict=True),
        start=1,
    ):
        ...
        l_score, l_err, l_notes = score_ah6_lookup(
        ...
        lt = {
            "trial_id": f"AH-FINAL-LOOKUP-HITL-{i:02d}",
            "stage": "AH6",
            "hyp_id": AH6_ID,
            "arm": "LOOKUP",
            "app_id": item["app_id"],
        ...
        if g_tel["wall_ms"] > 0.0 and g_tel["n_new"] > 0:
            n_gen_wall_ok += 1
        g_score, g_err, g_notes = score_ah6_gen(
            completion=str(gp.get("completion", "")),
            expected_gold=str(item["gold"]),
            payload=gp,
        )
        gt = {
            "trial_id": f"AH-FINAL-GEN-HITL-{i:02d}",
            "stage": "AH6",
            "hyp_id": AH6_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
    ...
    n_known = sum(1 for p in AH0_PACK if p["app_id"] == "known-ask")
    n_howto = sum(1 for p in AH0_PACK if p["app_id"] == "howto")
    n_long = sum(1 for p in AH0_PACK if p["app_id"] == "long-doc")
    stats = ah6_stats(
