"""Render H-TRIM smoke vs H-POOL tip (frozen-n FLOP search)."""
...
def render(path: Path) -> str:
    ...
    s = stats.get("H-TRIM", {})
    decision = decide_htrim(s, stats) if s else "needs H-TRIM rows"
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/trim_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/htrim-vs-hpool.md"),
    )
    args = p.parse_args()
