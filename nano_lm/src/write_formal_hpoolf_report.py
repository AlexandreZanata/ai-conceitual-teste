"""Render formal H-POOLF vs H-POOL (FLOP dual gate)."""
...
def render(path: Path) -> str:
    ...
    decision = decide_hpoolf(stats.get("H-POOLF", {}), stats)
    tip = stats.get("H-POOL", {})
    hyp = stats.get("H-POOLF", {})
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:hpoolf` → "
            "`npm run nano:formal:hpoolf:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hpoolf/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hpoolf-vs-hpool.md"),
    )
    args = p.parse_args()
