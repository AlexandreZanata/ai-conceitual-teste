"""Wave BA REPORT runner: public closeout + forever/modes live smoke."""

from __future__ import annotations

import argparse
import json
import os
import re
import sys
from concurrent.futures import ThreadPoolExecutor
from pathlib import Path
from typing import Any

from ba_real_eval_ops import battery_row_ok
from ba_report_ops import (
    BA_EVIDENCE,
    BA_ID,
    BA_SCOREBOARD,
    BA_THESIS,
    SHIP_CLAIM,
    antifp_section_ok,
    decide_ba_report,
    realeval_section_ok,
    render_paper_lab_wave_ba,
    render_wave_ba_summary,
    report_markers_ok,
    scoreboard_ok,
)
from ba_session_ops import BA0_ASK_BATTERY
from matrix_common import REPO, write_json
from tipd_pair import tune_cpu_threads

_OUT = REPO / "results/nano-lm/wave-ba/ba_report_summary.json"
_SUMMARY = REPO / "docs/results/nano-lm/wave-ba-summary.md"
_PAPER = REPO / "docs/results/nano-lm/paper-lab-wave-ba.md"
_FREEZE_STUB = REPO / "docs/results/nano-lm/ba-freeze.md"
_FORMAL_FREEZE_STUB = REPO / "docs/results/nano-lm/formal-habfreeze-ba-freeze.md"
_LOCAL_SESSION = REPO / ".local/wave-ba/SESSION.md"
_LOCAL_PESQUISA = REPO / ".local/pesquisa.md"
_LOCAL_IMPL = REPO / ".local/IMPLEMENTATION-PLAN.md"
_LOCAL_README = REPO / ".local/README-pesquisa.md"
_RECIPES = REPO / "docs/results/nano-lm/RECIPES.md"
_CARD = REPO / "docs/results/nano-lm/champion-card.md"
_AGENTS = REPO / "AGENTS.md"
_AGENDA = REPO / "docs/NANO-STUDENT-AGENDA.md"
_EVOGEN = REPO / ".cursor/rules/evogen-project.mdc"
_Z_BANK = REPO / "results/nano-lm/wave-z/error_bank.jsonl"
_CHAMPION = REPO / "results/nano-lm/wave-z/models/champion"
_CURATED = REPO / "nano_lm/data/curated"

# Compact live smoke: LOOKUP · forever · over-refuse · OOD (max HW, low RAM).
_SMOKE_IDS = frozenset(
    {"BA-ASK-01", "BA-ASK-02", "BA-ASK-07", "BA-ASK-08"}
)


def _clear_proxy() -> None:
    for key in (
        "http_proxy",
        "https_proxy",
        "HTTP_PROXY",
        "HTTPS_PROXY",
        "ALL_PROXY",
        "all_proxy",
    ):
        os.environ.pop(key, None)


def _hardware() -> tuple[int, int]:
    # 16c / ~10Gi avail: leave ≥4 cores free; cap workers for model RAM.
    cpus = int(os.cpu_count() or 4)
    threads = tune_cpu_threads(max(4, cpus - 4))
    workers = min(10, max(4, cpus - 4))
    return threads, workers


def _evidence_map() -> dict[str, bool]:
    return {p: (REPO / p).is_file() for p in BA_EVIDENCE}


def _load_json(rel: str) -> dict[str, Any] | None:
    path = REPO / rel
    if not path.is_file():
        return None
    return json.loads(path.read_text(encoding="utf-8"))


def _stage_facts() -> dict[str, Any]:
    keys = {
        "session": "results/nano-lm/wave-ba/ba0_session.json",
        "realgain": "results/nano-lm/wave-ba/realgain_summary.json",
        "fastreal": "results/nano-lm/wave-ba/ba_fastreal_summary.json",
        "ctxreal2": "results/nano-lm/wave-ba/ba_ctxreal2_summary.json",
        "nanogen11": "results/nano-lm/wave-ba/nanogen11_summary.json",
        "real_eval": "results/nano-lm/wave-ba/ba_real_eval_summary.json",
    }
    out: dict[str, Any] = {}
    for name, rel in keys.items():
        data = _load_json(rel) or {}
        out[name] = data.get("decision")
    return out


def _smoke_ba_modes(*, workers: int) -> dict[str, Any]:
    """LOOKUP · OOD · forever FP · over-refuse via BA ask path."""
    from run_ba_real_eval import _ask_row

    items = [dict(p) for p in BA0_ASK_BATTERY if p["id"] in _SMOKE_IDS]

    def _one(item: dict[str, str]) -> dict[str, Any]:
        return _ask_row(
            item=item, root=_CHAMPION, bank=_Z_BANK, curated=_CURATED
        )

    with ThreadPoolExecutor(max_workers=min(workers, len(items) or 1)) as pool:
        rows = list(pool.map(_one, items))
    by_id = {str(r["id"]): r for r in rows}
    ordered = [by_id[i["id"]] for i in items]
    ok = all(battery_row_ok(r) for r in ordered)
    return {
        "ok": ok,
        "decision": "PROMOTE" if ok else "KILL (BA forever/modes smoke)",
        "n_pass": sum(1 for r in ordered if battery_row_ok(r)),
        "n_total": len(ordered),
        "arms": [
            {
                "id": r.get("id"),
                "kind": r.get("kind"),
                "product_mode": r.get("product_mode"),
                "expect_mode": r.get("expect_mode"),
                "wall_ms": r.get("wall_ms"),
                "n_new": r.get("n_new"),
                "content_ok": r.get("content_ok"),
                "row_ok": battery_row_ok(r),
            }
            for r in ordered
        ],
    }


def _write_stub(path: Path, title: str) -> None:
    if path.is_file():
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        "\n".join(
            [
                f"# {title} — placeholder (pending BA7)",
                "",
                "> Written by BA6 report so paper-lab links resolve. "
                "BA7 replaces this with the formal freeze.",
                "",
                f"Ship claim: **{SHIP_CLAIM}**",
                "",
                "Do not invent Wave BB without lab-book reopen.",
                "",
            ]
        ),
        encoding="utf-8",
    )


def _ensure_freeze_stubs() -> None:
    _write_stub(_FREEZE_STUB, "BA-FREEZE")
    _write_stub(_FORMAL_FREEZE_STUB, "formal-habfreeze-ba-freeze")


def _write_public() -> None:
    _SUMMARY.parent.mkdir(parents=True, exist_ok=True)
    _ensure_freeze_stubs()
    _SUMMARY.write_text(render_wave_ba_summary(), encoding="utf-8")
    _PAPER.write_text(render_paper_lab_wave_ba(), encoding="utf-8")


def _status_word(decision: str) -> str:
    return (
        "PROMOTE"
        if decision == "PROMOTE" or decision.startswith("PROMOTE")
        else decision.split("(", 1)[0].strip()
    )


def _update_local_session(decision: str) -> None:
    if not _LOCAL_SESSION.parent.is_dir():
        return
    status = (
        "DONE — PROMOTE"
        if decision == "PROMOTE"
        else f"DONE — {decision}"
    )
    body = "\n".join(
        [
            f"# Wave BA session checklist (**OPEN** · BA6 {status})",
            "",
            "> Private under `.local/`. Lab: `.local/pesquisa.md`.  ",
            "> Ship lock: **"
            + SHIP_CLAIM
            + "** · ≤5M (no TAC / true-continue unlock).",
            "",
            "## Current stage",
            "",
            f"**BA6 — BA-REPORT ({status})** · Next: **BA7 BA-FREEZE**",
            "",
            "| Field | Value |",
            "|-------|--------|",
            "| Wave | **BA OPEN** (RESEARCH_COMPLETE pending FREEZE) |",
            f"| Decision | **{decision}** |",
            "| Public | `docs/results/nano-lm/wave-ba-summary.md` |",
            "| Paper-lab | `docs/results/nano-lm/paper-lab-wave-ba.md` |",
            "",
            "## Stage board",
            "",
            "| Stage | ID | Status |",
            "|------:|----|--------|",
            "| BA0 | SESSION | **DONE — PROMOTE** |",
            "| BA1 | H-REALGAIN | **DONE — PROMOTE** |",
            "| BA2 | H-FASTREAL | **DONE — PROMOTE** |",
            "| BA3 | H-CTXREAL2 | **DONE — PROMOTE** |",
            "| BA4 | H-NANOGEN11 | **DONE — DEFER** |",
            "| BA5 | BA-REAL-EVAL | **DONE — PROMOTE** |",
            f"| BA6 | BA-REPORT | **{status}** |",
            "| BA7 | BA-FREEZE | **NEXT** |",
            "",
        ]
    )
    _LOCAL_SESSION.write_text(body, encoding="utf-8")


def _patch_local_helpers(status: str) -> None:
    if _LOCAL_IMPL.is_file():
        text = _LOCAL_IMPL.read_text(encoding="utf-8")
        old = (
            "6. **BA5 BA-REAL-EVAL** — **DONE PROMOTE** "
            "(`npm run nano:ba:real-eval`) — battery live; gen locked "
            "(BA4 DEFER).  "
        )
        new = (
            "6. **BA5 BA-REAL-EVAL** — **DONE PROMOTE** "
            "(`npm run nano:ba:real-eval`) — battery live; gen locked "
            "(BA4 DEFER).  \n"
            f"7. **BA6 BA-REPORT** — **DONE {status}** "
            "(`npm run nano:ba:report`) · next **BA7 BA-FREEZE**.  "
        )
        if old in text and f"BA6 BA-REPORT** — **DONE {status}" not in text:
            _LOCAL_IMPL.write_text(text.replace(old, new, 1), encoding="utf-8")
    if _LOCAL_README.is_file():
        text = _LOCAL_README.read_text(encoding="utf-8")
        old = (
            "Session: `wave-ba/SESSION.md` (BA5 BA-REAL-EVAL "
            "**DONE — PROMOTE**; next BA6 BA-REPORT)."
        )
        new = (
            f"Session: `wave-ba/SESSION.md` (BA6 BA-REPORT "
            f"**DONE — {status}**; next BA7 BA-FREEZE)."
        )
        if old in text:
            _LOCAL_README.write_text(
                text.replace(old, new, 1), encoding="utf-8"
            )


def _patch_pesquisa(decision: str) -> None:
    if not _LOCAL_PESQUISA.is_file():
        return
    status = _status_word(decision)
    text = _LOCAL_PESQUISA.read_text(encoding="utf-8")
    text2, n = re.subn(
        r"\| BA6 \| \*\*BA-REPORT\*\* \|[^\n]+\| \*\*NEXT\*\* \|",
        (
            "| BA6 | **BA-REPORT** | Summary + paper-lab | "
            "anti-FP · HOLD/DEFER cited | "
            f"**DONE — {status}** |"
        ),
        text,
        count=1,
    )
    if n:
        text = text2
    text = text.replace(
        (
            "7. **BA6 BA-REPORT** — **NEXT** — summary + paper-lab.  \n"
            "8. **BA7 BA-FREEZE** — lock outcomes.  "
        ),
        (
            f"7. **BA6 BA-REPORT** — **DONE {status}** "
            "(`npm run nano:ba:report`) · next BA7 freeze.  \n"
            "8. **BA7 BA-FREEZE** — **NEXT** — lock outcomes.  "
        ),
        1,
    )
    text = text.replace(
        "> **Session:** `.local/wave-ba/SESSION.md` "
        "(BA5 BA-REAL-EVAL **DONE — PROMOTE**; next BA6 BA-REPORT).  ",
        "> **Session:** `.local/wave-ba/SESSION.md` "
        f"(BA6 BA-REPORT **DONE — {status}**; next BA7 BA-FREEZE).  ",
        1,
    )
    if "# next: nano:ba:report" in text:
        text = text.replace(
            "# next: nano:ba:report\n# npm run nano:ba:report",
            "npm run nano:ba:report\n# next: nano:ba:freeze",
            1,
        )
        if "# next: nano:ba:report" in text:
            text = text.replace(
                "# next: nano:ba:report",
                "npm run nano:ba:report\n# next: nano:ba:freeze",
                1,
            )
    _LOCAL_PESQUISA.write_text(text, encoding="utf-8")
    _patch_local_helpers(status)


def _ba_active_line(status: str) -> str:
    return (
        "BA0 [SESSION PROMOTE](wave-ba-session.md) · "
        "BA1 [H-REALGAIN PROMOTE](formal-hrealgain-realgain.md) · "
        "BA2 [H-FASTREAL PROMOTE](formal-hfastreal-ba2.md) · "
        "BA3 [H-CTXREAL2 PROMOTE](formal-hctxreal2-ctxreal2.md) · "
        "BA4 [H-NANOGEN11 DEFER](formal-hnanogen11-nanogen11.md) · "
        "BA5 [BA-REAL-EVAL PROMOTE](wave-ba-real-eval.md) · "
        f"BA6 [BA-REPORT {status}](wave-ba-summary.md) "
        "(`npm run nano:ba:report`) · [paper-lab-wave-ba.md]"
        "(paper-lab-wave-ba.md); next BA7 BA-FREEZE; ship remains "
        "**AF + AQ + AS trust + STRICT ablated DECODE**; ≤5M stays"
    )


def _patch_recipes(status: str) -> None:
    if not _RECIPES.is_file():
        return
    text = _RECIPES.read_text(encoding="utf-8")
    line = f"**Wave BA ACTIVE:** {_ba_active_line(status)}."
    text2, n = re.subn(
        r"\*\*Wave BA ACTIVE:\*\*[^\n]*",
        line,
        text,
        count=1,
    )
    if n:
        text = text2
    if "Wave BA6 BA-REPORT" not in text:
        needle = (
            "| Wave BA5 BA-REAL-EVAL | [wave-ba-real-eval.md]"
            "(wave-ba-real-eval.md) **PROMOTE** "
            "(`npm run nano:ba:real-eval`) — battery 10/10 · "
            "gen locked (BA4 DEFER) |\n"
        )
        row = (
            "| Wave BA6 BA-REPORT | [wave-ba-summary.md]"
            "(wave-ba-summary.md) · [paper-lab-wave-ba.md]"
            "(paper-lab-wave-ba.md) **PROMOTE** "
            "(`npm run nano:ba:report`) — anti-FP · BA4 DEFER · "
            "NANOGEN6/7 HOLD · NANOGEN8·9·10 DEFER cited |\n"
        )
        if needle in text:
            text = text.replace(needle, needle + row, 1)
    _RECIPES.write_text(text, encoding="utf-8")


def _patch_card(status: str) -> None:
    if not _CARD.is_file():
        return
    text = _CARD.read_text(encoding="utf-8")
    line = f"**Wave BA ACTIVE** — {_ba_active_line(status)}."
    text2, n = re.subn(
        r"\*\*Wave BA ACTIVE\*\* —[^\n]*",
        line,
        text,
        count=1,
    )
    if n:
        _CARD.write_text(text2, encoding="utf-8")


def _patch_agents(status: str) -> None:
    if not _AGENTS.is_file():
        return
    text = _AGENTS.read_text(encoding="utf-8")
    if f"BA-REPORT {status}" in text and "paper-lab-wave-ba.md" in text:
        return
    text2, n = re.subn(
        r"(- \*\*Wave BA ACTIVE\*\* —[^\n]*BA-REAL-EVAL PROMOTE[^\n]*?)"
        r"(; next BA6 BA-REPORT|; next BA6)",
        rf"\1 · BA6 [BA-REPORT {status}]"
        r"(docs/results/nano-lm/wave-ba-summary.md) "
        r"(`npm run nano:ba:report`) · "
        r"[paper-lab-wave-ba.md](docs/results/nano-lm/paper-lab-wave-ba.md)"
        r"; next BA7 BA-FREEZE",
        text,
        count=1,
    )
    if n:
        _AGENTS.write_text(text2, encoding="utf-8")


def _patch_agenda(status: str) -> None:
    if not _AGENDA.is_file():
        return
    text = _AGENDA.read_text(encoding="utf-8")
    ba_tail = text.split("| **BA** |", 1)[-1][:800]
    if f"BA-REPORT {status}" in ba_tail:
        return
    text2, n = re.subn(
        r"(\| \*\*BA\*\* \| \*\*ACTIVE\*\* \|[^\n]*BA-REAL-EVAL "
        r"PROMOTE[^\n]*?)(; next BA6 BA-REPORT|; next BA6)",
        rf"\1 · BA6 [BA-REPORT {status}]"
        r"(results/nano-lm/wave-ba-summary.md); "
        r"next BA7 BA-FREEZE",
        text,
        count=1,
    )
    if n:
        _AGENDA.write_text(text2, encoding="utf-8")


def _patch_evogen(status: str) -> None:
    if not _EVOGEN.is_file():
        return
    text = _EVOGEN.read_text(encoding="utf-8")
    if "wave-ba-summary.md" in text and "paper-lab-wave-ba.md" in text:
        return
    needle = "BA5 BA-REAL-EVAL PROMOTE; next BA6 BA-REPORT"
    repl = (
        f"BA5 BA-REAL-EVAL PROMOTE · BA6 BA-REPORT {status} "
        f"(wave-ba-summary.md / paper-lab-wave-ba.md); next BA7 BA-FREEZE"
    )
    if needle in text:
        _EVOGEN.write_text(text.replace(needle, repl, 1), encoding="utf-8")


def _patch_public_status(decision: str) -> None:
    if not decision.startswith("PROMOTE"):
        return
    status = "PROMOTE"
    _patch_recipes(status)
    _patch_card(status)
    _patch_agents(status)
    _patch_agenda(status)
    _patch_evogen(status)


def _gates(
    decision: str,
    *,
    markers: bool,
    board: bool,
    antifp: bool,
    realeval: bool,
    ask: dict[str, Any] | None,
) -> str:
    if ask is not None and not bool(ask.get("ok")):
        return "KILL (BA forever/modes smoke failed)"
    if decision.startswith("PROMOTE") and not markers:
        return "KILL (wave-ba-summary missing thesis markers)"
    if decision.startswith("PROMOTE") and not board:
        return "KILL (wave-ba-summary missing scoreboard)"
    if decision.startswith("PROMOTE") and not antifp:
        return "KILL (wave-ba-summary missing anti-FP evidence)"
    if decision.startswith("PROMOTE") and not realeval:
        return "KILL (wave-ba-summary missing real-eval section)"
    return decision


def run_ba_report(
    *, out: Path, skip_ask: bool = False, workers: int = 10
) -> dict[str, Any]:
    """
    GIVEN BA0–BA5 evidence
    WHEN writing public summary + paper-lab and checking anti-FP/mode smoke
    THEN PROMOTE iff evidence ∧ markers ∧ scoreboard ∧ antifp ∧ realeval ∧ smoke.
    """
    _write_public()
    evidence = _evidence_map()
    decision = decide_ba_report(evidence)
    report_text = _SUMMARY.read_text(encoding="utf-8")
    markers = report_markers_ok(report_text)
    board = scoreboard_ok(report_text)
    antifp = antifp_section_ok(report_text)
    realeval = realeval_section_ok(report_text)
    ask: dict[str, Any] | None = None
    if not skip_ask:
        ask = _smoke_ba_modes(workers=workers)
    decision = _gates(
        decision,
        markers=markers,
        board=board,
        antifp=antifp,
        realeval=realeval,
        ask=ask,
    )
    ok = (
        str(decision).startswith("PROMOTE")
        and markers
        and board
        and antifp
        and realeval
    )
    if ask is not None:
        ok = ok and bool(ask.get("ok"))
    final = "PROMOTE" if ok else decision
    _update_local_session(final)
    _patch_pesquisa(final)
    _patch_public_status(final)
    payload: dict[str, Any] = {
        "id": BA_ID,
        "hyp_id": BA_ID,
        "stage": "BA6",
        "thesis": BA_THESIS,
        "decision": final,
        "markers_ok": markers,
        "scoreboard_ok": board,
        "antifp_ok": antifp,
        "realeval_ok": realeval,
        "scoreboard": list(BA_SCOREBOARD),
        "evidence": evidence,
        "stage_facts": _stage_facts(),
        "ask_smoke": ask,
        "public_report": "docs/results/nano-lm/wave-ba-summary.md",
        "paper_lab": "docs/results/nano-lm/paper-lab-wave-ba.md",
        "wave_status": "RESEARCH_COMPLETE" if ok else "OPEN",
        "ship_claim": SHIP_CLAIM,
        "cpu_threads": int(os.environ.get("OMP_NUM_THREADS") or 0),
        "workers": int(workers),
        "finding": (
            f"{BA_ID}: decision={final}; "
            f"markers={markers}; scoreboard={board}; "
            f"antifp={antifp}; realeval={realeval}."
        ),
        "next": "BA7 BA-FREEZE",
    }
    write_json(out, payload)
    return payload


def main() -> int:
    _clear_proxy()
    ap = argparse.ArgumentParser(description="Wave BA6 BA-REPORT")
    ap.add_argument("--out", type=Path, default=_OUT)
    ap.add_argument("--skip-ask", action="store_true")
    args = ap.parse_args()
    threads, workers = _hardware()
    try:
        summary = run_ba_report(
            out=Path(args.out),
            skip_ask=bool(args.skip_ask),
            workers=workers,
        )
    except (OSError, RuntimeError, ValueError) as exc:
        print(json.dumps({"ok": False, "error": str(exc)}), file=sys.stderr)
        return 2
    ok = str(summary.get("decision")) == "PROMOTE"
    print(
        json.dumps(
            {
                "ok": ok,
                "hyp_id": BA_ID,
                "decision": summary.get("decision"),
                "markers_ok": summary.get("markers_ok"),
                "scoreboard_ok": summary.get("scoreboard_ok"),
                "antifp_ok": summary.get("antifp_ok"),
                "realeval_ok": summary.get("realeval_ok"),
                "ask_smoke_ok": (summary.get("ask_smoke") or {}).get("ok"),
                "cpu_threads": threads,
                "workers": workers,
                "out": str(args.out),
            }
        )
    )
    return 0 if ok else 2


if __name__ == "__main__":
    raise SystemExit(main())
