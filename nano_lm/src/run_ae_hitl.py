"""Wave AE5 AE-HITL-10 runner: final pack on declared AE stack (nano:ae:hitl)."""
...
from ae_session_ops import AE0_PACK, overlaps_prior_questions
...
_SUMMARY = REPO / "results/nano-lm/wave-ae/ae_hitl_summary.json"
...
def _apply_fix(
    *,
    ...
    score, err, notes = score_ae5_trial(
        mode=mode,
...
def _ctx_ok_for(
    ...
    if routed != "app-longdoc" or ctx is None:
        return None
    return bool(ctx.get("l_eff_ok")) and bool(ctx.get("ratio_ok"))
...
def run_ae_hitl(
    *,
    ...
    """
    GIVEN AE0 pack + declared AE stack
    WHEN ASK→EVAL→FIX×10 via APPMAX app router
    THEN mean≥7 · errors≤3 · no false-hit → PROMOTE|HOLD|KILL.
    """
    if len(AE0_PACK) != AE5_N:
    ...
    held_out_ok = len(overlaps_prior_questions(AE0_PACK)) == 0
    ...
    if len(payloads) != AE5_N:
        raise RuntimeError(f"expected {AE5_N} payloads")
    ...
    for i, (item, payload) in enumerate(
        ...
        trial: dict[str, Any] = {
            ...
            "question": item["question"],
            "source_id": item["source_id"],
            "recipe_id": payload.get("recipe_id") or "champion-ae-v0",
            "ckpt": None,
            "completion": completion,
            ...
            "manual_adjust": (
                "FIX: constrained SEMWRAP re-ASK"
                if fixed
                else "no change — AE5 final stack ok"
            ),
