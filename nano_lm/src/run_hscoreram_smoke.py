"""Smoke H-SCORERAM: disk/RAM pack score cache; warm wall↓; lp unchanged."""
...
def main() -> int:
    ...
    if device.type != "cuda":
        print("ERROR: H-SCORERAM requires CUDA", file=sys.stderr)
        return 2
    ...
    payload = {
        ...
        "max_new": _MAX_NEW,
        "cpu_threads": threads,
        "mode": "SCORERAM: cold fill disk cache then warm re-score pack",
        "mechanism": "PackScoreCache RAM+disk; TCACHE elig-only code on warm hit",
        "parent": "PFB2 K=2 banks (decode once; score cold vs warm)",
    }
    write_json(out / "hscoreram_smoke.json", payload)
    print(json.dumps({"decision": decision, "out": str(out / "hscoreram_smoke.json")}))
    return 0
