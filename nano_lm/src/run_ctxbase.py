"""Wave AP2 H-CTXBASE runner: trideca-doc dual-arm ASK→EVAL→FIX×10."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ap/ctxbase_summary.json"
...
def _seed_pack_golds(bank_path: Path, ao_bank: Path) -> int:
    ...
    for i, item in enumerate(AP0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AP-CTXBASE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = CTXBASE_ID
        row["judge_notes"] = [
            "CTXBASE seed alias for held-out AP ask",
...
def _build_ctxs(
    *, curated_root: Path, workers: int, tok: Any
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        rows = list(pool.map(_read, [dict(p) for p in AP0_PACK]))
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AP-CTXBASE-FIX-{i:02d}",
...
def _lookup_trial(
    *,
    ...
    tid = f"AP-CTXBASE-LOOKUP-HITL-{i:02d}"
    mode = str(payload.get("mode", ""))
    score, err, notes, usable = score_ctxbase_lookup(
        mode=mode,
...
def _gen_trial(
    *,
    ...
    tid = f"AP-CTXBASE-GEN-HITL-{i:02d}"
    score, err, notes, usable = score_ctxbase_gen(
        completion=str(payload.get("completion", "")),
        expected_gold=str(item["gold"]),
        meta=ctx,
        payload=payload,
...
def run_ctxbase(
    *,
    ...
    """
    GIVEN AP0 held-out asks + trideca curated docs
    WHEN K=27 ROLL/SUMCACHE + LOOKUP askfast + GENERATE raw
    THEN L_eff↑ vs CTXCORE · gen usable≥5 → PROMOTE|HOLD|KILL.
    """
    if len(AP0_PACK) != CTXBASE_N:
        raise ValueError("AP0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack_golds(bank_path, ao_bank)
    cfg = matrix_cfg()
    tok = load_tokenizer(str(cfg["tokenizer_id"]), cfg["cache"])
    bank = load_bank_rows(bank_path)
    ctxs = _build_ctxs(curated_root=curated_root, workers=workers, tok=tok)
    questions = [p["question"] for p in AP0_PACK]
    ...
    for i, (item, lp, gp, ctx) in enumerate(
        zip(AP0_PACK, lookup_payloads, gen_payloads, ctxs, strict=True),
        start=1,
    ):
    ...
    mean_l = float(sum(float(c["l_eff"]) for c in ctxs) / CTXBASE_N)
    mean_a = float(
        sum(float(c["sumcache_active"]) for c in ctxs) / CTXBASE_N
    )
    mean_s = float(sum(float(c["n_slices"]) for c in ctxs) / CTXBASE_N)
    mean_src = float(sum(float(c["n_sources"]) for c in ctxs) / CTXBASE_N)
    ...
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{CTXBASE_ID}: L_lookup={stats['lookup_mean']:.1f} "
            f"L_gen={stats['gen_mean']:.1f} "
            f"usable_L={stats['n_lookup_usable']}/10 "
            f"usable_G={stats['n_gen_usable']}/10 "
            f"L_eff={mean_l:.0f} (>CTXCORE {stats['ctxcore_mean_leff']:.0f}) "
            f"sources={mean_src:.1f} slices={mean_s:.1f} "
            f"false_hit={n_false} fix={fix_count} decision={decision}."
        ),
        "public_note": "docs/results/nano-lm/formal-hctxbase-ctxbase.md",
        "ship_claim": "AF packaged stack (trideca long-ctx; LOOKUP≠gen IQ)",
        "claim": (
            "trideca-doc longer usable ctx beyond CTXCORE parent — "
