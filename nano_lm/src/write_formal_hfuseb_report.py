"""Render formal H-FUSEB vs H-CHBAT (FUSE under CHBAT batch)."""
...
def render(path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "Commands: `npm run nano:formal:hfuseb` → "
            "`npm run nano:formal:hfuseb:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hfuseb/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hfuseb-vs-hchbat.md"),
    )
    args = p.parse_args()
