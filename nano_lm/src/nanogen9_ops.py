"""Wave AY3 H-NANOGEN9: gen-defer gate — not NANOGEN8+rename."""

from __future__ import annotations

from typing import Any, Mapping

from ay_session_ops import (
    AY0_ANTI_FP,
    AY0_GEN_STANCE,
    AY0_NORTH_STAR,
    AY0_SAFE_NOTE,
    AY0_SHIP_LOCK,
    AY0_TRUE_GEN_JUDGE,
)

__all__ = [
    "NANOGEN9_ID",
    "NANOGEN9_THESIS",
    "NANOGEN9_CLAIM",
    "NANOGEN9_SAFE_NOTE",
    "NANOGEN9_ANTI_FP",
    "NANOGEN9_STANCE",
    "NANOGEN9_METHOD",
    "TRUE_GEN_JUDGE",
    "PARENT_NANOGEN6_TRUE_CONTINUE",
    "PARENT_NANOGEN7_TRUE_CONTINUE",
    "PARENT_NANOGEN8_DEFER",
    "decide_nanogen9",
    "method_is_rename",
    "extract_nanogen9_board",
]

NANOGEN9_ID = "H-NANOGEN9"
NANOGEN9_THESIS = (
    "North-star generative gate under AY0 gen stance: PROMOTE only with a "
    "real new train/data/arch method AND true_continue; else DEFER/HOLD. "
    "CAPCHECK closed; never NANOGEN9 = NANOGEN8+rename; "
    "NANOGEN6·7 HOLD · NANOGEN8 DEFER stand; span-fallback ≠ gen IQ; "
    "mini-AGI locked while deferred"
)
NANOGEN9_CLAIM = AY0_SHIP_LOCK
NANOGEN9_SAFE_NOTE = AY0_SAFE_NOTE
NANOGEN9_ANTI_FP = AY0_ANTI_FP
NANOGEN9_STANCE = dict(AY0_GEN_STANCE)
NANOGEN9_METHOD: Mapping[str, object] = {
    "id": "gen-defer",
    "kind": "defer",
    "real_new_method": False,
    "capcheck": "closed",
    "renames_nanogen8": False,
    "renames_nanogen7_tac": False,
    "renames_nanogen6": False,
    "rationale": (
        "AY0 froze stance=defer: no real new train/data/arch ready; "
        "NANOGEN6·7 HOLD · NANOGEN8 DEFER stand; prefer product ship "
        "(PRODINT·SHIPAY) + honest paper over vanity NANOGEN9 clone"
    ),
}
TRUE_GEN_JUDGE = dict(AY0_TRUE_GEN_JUDGE)
PARENT_NANOGEN6_TRUE_CONTINUE = 0.0
PARENT_NANOGEN7_TRUE_CONTINUE = 0.0
PARENT_NANOGEN8_DEFER = True


def method_is_rename(method: Mapping[str, object]) -> bool:
    """
    GIVEN a claimed generative method
    WHEN checking anti-clone law
    THEN True iff it is NANOGEN6/7/8 rename theater.
    """
    if bool(method.get("renames_nanogen8")):
        return True
    if bool(method.get("renames_nanogen7_tac")):
        return True
    if bool(method.get("renames_nanogen6")):
        return True
    kind = str(method.get("kind") or "").lower()
    mid = str(method.get("id") or "").lower()
    if "nanogen8" in mid and "rename" in kind:
        return True
    if "nanogen7" in mid and "rename" in kind:
        return True
    if kind in {
        "tac_rename",
        "nanogen8_rename",
        "nanogen7_rename",
        "nanogen6_rename",
    }:
        return True
    return False


def extract_nanogen9_board(
    *,
    stance: Mapping[str, object] | None = None,
    method: Mapping[str, object] | None = None,
    true_continue_mean: float = 0.0,
    n_true_continue: int = 0,
    n_span_fallback: int = 0,
    parent6: float = PARENT_NANOGEN6_TRUE_CONTINUE,
    parent7: float = PARENT_NANOGEN7_TRUE_CONTINUE,
    parent8_defer: bool = PARENT_NANOGEN8_DEFER,
    live_modes_ok: bool = True,
) -> dict[str, Any]:
    """
    GIVEN AY0 stance + claimed method + optional live true_continue probe
    WHEN building NANOGEN9 board
    THEN publish defer/method/true_continue honesty fields.
    """
    st = dict(stance) if stance is not None else dict(NANOGEN9_STANCE)
    meth = dict(method) if method is not None else dict(NANOGEN9_METHOD)
    return {
        "stance": str(st.get("stance") or ""),
        "capcheck": str(st.get("capcheck") or meth.get("capcheck") or ""),
        "real_new_method": bool(meth.get("real_new_method")),
        "method_id": str(meth.get("id") or ""),
        "method_kind": str(meth.get("kind") or ""),
        "renames_forbidden": bool(st.get("nanogen9_rename_forbidden", True)),
        "is_rename": method_is_rename(meth),
        "true_continue_mean": float(true_continue_mean),
        "n_true_continue": int(n_true_continue),
        "n_span_fallback": int(n_span_fallback),
        "parent_nanogen6_true_continue": float(parent6),
        "parent_nanogen7_true_continue": float(parent7),
        "parent_nanogen8_defer": bool(parent8_defer),
        "span_fallback_neq_gen": bool(
            TRUE_GEN_JUDGE.get("span_fallback_neq_gen", True)
        ),
        "live_modes_ok": bool(live_modes_ok),
        "north_star": AY0_NORTH_STAR,
        "ship_lock": NANOGEN9_CLAIM,
    }


def decide_nanogen9(
    *,
    board: Mapping[str, Any],
    anti_fp_signed: bool = True,
) -> str:
    """
    GIVEN NANOGEN9 board (stance · method · true_continue)
    WHEN applying pesquisa §5 AY3
    THEN PROMOTE only real method + true_continue; else DEFER/HOLD;
         KILL on rename / unsigned anti-FP / mode regress.
    """
    if not anti_fp_signed:
        return "KILL (anti-FP charter not signed)"
    if bool(board.get("is_rename")):
        return "KILL (NANOGEN9 = NANOGEN8/7/6 rename forbidden)"
    if not bool(board.get("renames_forbidden", True)):
        return "KILL (nanogen9_rename_forbidden must stay True)"
    if not bool(board.get("live_modes_ok", True)):
        return "KILL (live product modes regress on gen gate path)"
    if not bool(board.get("span_fallback_neq_gen", True)):
        return "KILL (span-fallback ≠ gen law missing)"
    stance = str(board.get("stance") or "")
    real = bool(board.get("real_new_method"))
    tc = float(board.get("true_continue_mean") or 0.0)
    n_tc = int(board.get("n_true_continue") or 0)
    if real and n_tc > 0 and tc >= 5.5:
        return (
            f"PROMOTE ({NANOGEN9_ID}: real new method + "
            f"true_continue={tc:.1f})"
        )
    if stance == "defer" or not real:
        return (
            f"DEFER ({NANOGEN9_ID}: stance={stance or 'unset'}; "
            "CAPCHECK closed; no real new method; "
            "NANOGEN6·7 HOLD · NANOGEN8 DEFER stand; not NANOGEN8 rename)"
        )
    return (
        f"HOLD ({NANOGEN9_ID}: method claimed but true_continue "
        f"{tc:.1f} unmet; span-fallback ≠ gen)"
    )
