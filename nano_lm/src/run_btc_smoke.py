"""Smoke H-BTC: PACK tip gate on bitcoin domain (≤5M + TinyStories teacher)."""
...
def main() -> int:
    ...
    if device.type != "cuda":
        print("ERROR: H-BTC requires CUDA", file=sys.stderr)
        return 2
    ...
    write_json(out / "hbtc_smoke.json", payload)
    print(json.dumps({"decision": decision, "out": str(out / "hbtc_smoke.json")}))
    return 0
