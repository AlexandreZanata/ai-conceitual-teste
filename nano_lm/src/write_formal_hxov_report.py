"""Render formal H-XOV vs B2 comparison markdown."""
...
def render(path: Path) -> str:
    ...
    decision = decide_formal_vs_b2("H-XOV", stats)
    delta = stats["H-XOV"]["lp"] - stats["B2"]["lp"]
    collapsed = "yes" if stats["H-XOV"].get("collapsed", 0.0) > 0.0 else "no"
    lines = [
        ...
        f"{int(stats['B2']['n'])} |",
        f"| H-XOV | {stats['H-XOV']['lp']:.4f} | {delta:+.4f} | "
        f"{stats['H-XOV']['wall']:.0f} | {collapsed} | {int(stats['H-XOV']['n'])} |",
        "",
        f"**Decision:** {decision}",
        "",
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hxov-b2/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hxov-vs-b2.md"),
    )
    args = p.parse_args()
