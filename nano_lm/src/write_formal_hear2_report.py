"""Render formal H-EAR2 vs H-EARLY."""
...
def render(path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:hear2` → "
            "`npm run nano:formal:hear2:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hear2/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hear2-vs-hearly.md"),
    )
    args = p.parse_args()
