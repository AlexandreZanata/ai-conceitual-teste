"""Smoke H-PACK: SERVE + SROUTE packs vs tip H-EARLY."""
...
def main() -> int:
    ...
    if device.type != "cuda":
        print("ERROR: H-PACK requires CUDA", file=sys.stderr)
        return 2
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        ...
        for row in trio:
            if row["family"] == "H-SROUTE":
                write_json(out / f"HPACK_seed{seed}_eval.json", row)
        rows.extend(trio)
    write_json(
        out / "pack_smoke.json",
        {
    ...
    print(json.dumps({"n_rows": len(rows), "out": str(out / "pack_smoke.json")}))
    return 0
