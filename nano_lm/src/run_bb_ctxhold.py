"""Wave BB3 H-CTXHOLD runner (nano:bb:ctxhold) — content bars + anti-FP."""
...
from run_bb_fasthold import _measure_latency_tetrad
from run_z_ask import ask_once
from shipreal_ops import attach_shipreal
from tipd_pair import tune_cpu_threads

_SUMMARY = REPO / "results/nano-lm/wave-bb/bb_ctxhold_summary.json"
_TRIALS = REPO / "results/nano-lm/wave-bb/trials"
_Z_BANK = REPO / "results/nano-lm/wave-z/error_bank.jsonl"
_CHAMPION = REPO / "results/nano-lm/wave-z/models/champion"
_CURATED = REPO / "nano_lm/data/curated"
_PUBLIC = REPO / "docs/results/nano-lm/formal-hctxhold-ctxhold.md"
...
def _write_public(
    *,
    ...
    body = "\n".join(
        [
            f"# H-CTXHOLD — usable long/cite/howto (**DONE** — {status})",
            "",
            "> Lab: `.local/pesquisa.md` §2 · §8 BB3 · Session: "
            "`.local/wave-bb/SESSION.md`  ",
            "> Parent: [formal-hfasthold-fasthold.md](formal-hfasthold-fasthold.md) · "
            "BB0 ctx baseline (= BA H-CTXREAL2)  ",
            "> Module: `nano_lm/src/bb_ctxhold_ops.py` · "
            "Runner: `npm run nano:bb:ctxhold`  ",
            "> **Not** BA [formal-hctxreal2-ctxreal2.md](formal-hctxreal2-ctxreal2.md) "
            "(`npm run nano:ba:ctxreal2`) · **Not** AG "
            "[formal-hctxreal-ctxreal.md](formal-hctxreal-ctxreal.md) "
            ...
            *lat_rows,
            ...
            "3. INTENTGEN anti-FP hold: BB FH 0 · BA FH 0 · AZ hold · "
            "over-refuse 0.  ",
            "4. p50/p99 republished; L_eff alone **not** a win.  ",
            f"5. Wall clock ~{wall_s:.1f}s · max safe CPU (`cpus-6`, workers≤6).  ",
            ...
            "| Allowed | Forbidden |",
            "|---------|-----------|",
            f"| {CTXHOLD_CLAIM} | Open chat / mini-AGI |",
            ...
            "",
            f"SAFE note: {CTXHOLD_SAFE_NOTE}  ",
            f"Anti-FP: {CTXHOLD_ANTI_FP}",
            "",
...
def _write_session(decision: str, board: dict[str, Any]) -> None:
    ...
    body = "\n".join(
        [
            f"# Wave BB session checklist (**OPEN** · BB3 {status})",
            "",
            "> Private under `.local/`. Lab: `.local/pesquisa.md` "
            "(Wave BB **OPEN** · compositional anti-FP + ctx/speed + honest gen).  ",
            f"> Ship lock: **{CTXHOLD_CLAIM}** · ≤5M.",
            "",
            "## Current stage",
            "",
            f"**BB3 — H-CTXHOLD ({status})** · Next: **BB4 H-NANOGEN12**",
            "",
            "| Field | Value |",
            "|-------|--------|",
            f"| ctx_content_ok | **{board.get('ctx_content_ok_n')}/"
            ...
            f"| BB3 | H-CTXHOLD | **{status}** |",
            "| BB4 | H-NANOGEN12 | **NEXT** |",
...
def _patch_pesquisa(decision: str) -> None:
    ...
    bb3_done = (
        "| BB3 | **H-CTXHOLD** | Context content bars hold / improve **without** "
        "FP regress | content + §1 | **DONE — PROMOTE** |"
    )
    if bb3_next in text:
        text = text.replace(bb3_next, bb3_done, 1)
    bb4_todo = (
        "| BB4 | **H-NANOGEN12** | One real gen method M1\\|M2\\|M3 — else HOLD/DEFER "
        "| true_continue → PROMOTE else HOLD/DEFER | **TODO** |"
    )
    bb4_next = (
        "| BB4 | **H-NANOGEN12** | One real gen method M1\\|M2\\|M3 — else HOLD/DEFER "
        "| true_continue → PROMOTE else HOLD/DEFER | **NEXT** |"
    )
    if bb4_todo in text:
        text = text.replace(bb4_todo, bb4_next, 1)
    text = text.replace(
        "4. **BB3 H-CTXHOLD** — **NEXT** — context content hold/improve; "
        "no anti-FP regress.  ",
        "4. **BB3 H-CTXHOLD** — **DONE PROMOTE** "
        "(`npm run nano:bb:ctxhold`) — howto·cite·long content_ok.  ",
        1,
    )
    text = text.replace(
        "5. **BB4 H-NANOGEN12** — one real method or **HOLD/DEFER** "
        "(cite NANOGEN6–11).  ",
...
**BB0–BB3 DONE — PROMOTE** · **BB3 H-CTXHOLD** (`npm run nano:bb:ctxhold`).

## Next

1. BB0–BB3 done.  
2. **BB4 H-NANOGEN12** — **NEXT** — one real method or HOLD/DEFER.  
...
**Wave BB ACTIVE** — BB0–BB3 **PROMOTE** (H-CTXHOLD content bars).

Next: **BB4 H-NANOGEN12**. Parent: Wave BA **COMPLETE + FROZEN**.
...
def _bb_active_line() -> str:
    return (
        ...
        "(`npm run nano:bb:ctxhold`) — howto·cite·long content_ok; "
        "next BB4 H-NANOGEN12; ship remains **AF + AQ + AS trust + STRICT "
...
def _patch_recipes(board: dict[str, Any], line: str) -> None:
    ...
    insert = (
        "| Wave BB3 H-CTXHOLD | [formal-hctxhold-ctxhold.md]"
        "(formal-hctxhold-ctxhold.md) **PROMOTE** (`npm run nano:bb:ctxhold`) "
        f"— ctx {board.get('ctx_content_ok_n')}/{board.get('ctx_content_n')} · "
        f"BB FH {board.get('bb_forever_false_hit')} · BA FH "
        f"{board.get('ba_forever_false_hit')} · live FP {board.get('live_fp')} "
        "(≠ BA `nano:ba:ctxreal2` · ≠ AG `nano:ctxreal`) |"
    )
    if "Wave BB3 H-CTXHOLD" not in text2:
        marker = "| Wave BB2 H-FASTHOLD |"
        if marker in text2:
            text2 = text2.replace(marker, insert + "\n" + marker, 1)
    if n or "Wave BB3 H-CTXHOLD" in text2:
...
def _patch_card_agents_agenda(line: str, board: dict[str, Any]) -> None:
    ...
    if _AGENDA.is_file():
        ...
        row = (
            "| **BB** | **ACTIVE** | BB0–BB3 PROMOTE (H-CTXHOLD) "
            "(`npm run nano:bb:ctxhold`); next BB4 H-NANOGEN12; ≤5M |"
        )
        text2, n = re.subn(
            r"\| \*\*BB\*\* \| \*\*ACTIVE\*\* \|[^\n]+", row, text, count=1
        )
...
def run_bb_ctxhold(
    *,
    ...
    """
    GIVEN BB0 ctx baseline + INTENTGEN anti-FP
    WHEN scoring howto·cite·long + apps + hold packs
    THEN PROMOTE/KILL per pesquisa §8 BB3.
    """
    ...
    decision = decide_ctxhold(board=board, anti_fp_signed=True)
    wall_s = time.perf_counter() - t0
    payload: dict[str, Any] = {
        "id": CTXHOLD_ID,
        "stage": "BB3",
        "thesis": CTXHOLD_THESIS,
        "decision": decision,
        "board": board,
        "ctx_rows": ctx_rows,
        "apps_rows": apps_rows,
        ...
        "workers": workers,
        "claim": CTXHOLD_CLAIM,
        "public_note": "docs/results/nano-lm/formal-hctxhold-ctxhold.md",
        "ba_archive": "docs/results/nano-lm/formal-hctxreal2-ctxreal2.md",
        "ag_archive": "docs/results/nano-lm/formal-hctxreal-ctxreal.md",
        "next": "BB4 H-NANOGEN12",
        "anti_fp_signed": True,
    }
