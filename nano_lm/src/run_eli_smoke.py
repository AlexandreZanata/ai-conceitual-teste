"""Smoke H-ELI on matrix out dir; merge rows into matrix.json."""
...
def _merge(rows: list[dict[str, Any]], wall_s: float, out: Path) -> None:
    ...
    kept = [r for r in data.get("rows", []) if r.get("family") != "H-ELI"]
    data["rows"] = kept + rows
    data["eli_wall_s"] = wall_s
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        write_json(out / f"HELI_seed{seed}_eval.json", ev)
        rows.append(ev)
    wall_s = time.perf_counter() - t0
    write_json(out / "eli_smoke.json", {"rows": rows, "wall_s": wall_s})
    _merge(rows, wall_s, out)
    print(json.dumps({"n_rows": len(rows), "out": str(out / "eli_smoke.json")}))
    return 0
