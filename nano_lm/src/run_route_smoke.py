"""Smoke H-ROUTE: short→GALL, long→GRAPHF vs single arms."""
...
def main() -> int:
    ...
    if device.type != "cuda":
        print("ERROR: H-ROUTE requires CUDA", file=sys.stderr)
        return 2
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        ...
        for row in pair:
            if row["family"] == "H-ROUTE":
                write_json(out / f"HROUTE_seed{seed}_eval.json", row)
        rows.extend(pair)
    write_json(
        out / "route_smoke.json",
        {
            "rows": rows,
    ...
    print(json.dumps({"n_rows": len(rows), "out": str(out / "route_smoke.json")}))
    return 0
