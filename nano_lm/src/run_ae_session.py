"""Wave AE0 SESSION runner (nano:ae:session) — freeze held-out HITL×10."""
...
_OUT = REPO / "results/nano-lm/wave-ae/ae0_session.json"
_TRIALS = REPO / "results/nano-lm/wave-ae/trials"
_ERROR_BANK = REPO / "results/nano-lm/wave-ae/error_bank.jsonl"
...
def main() -> int:
    ...
    ids = [str(p["source_id"]) for p in AE0_PACK]
    ...
    trials_ready = Path(args.trials_dir).is_dir() and len(written) == AE0_N

    decision = _override_decision(
        decide_ae0_session(known_sources=known, trials_dir_ready=trials_ready),
        miss=miss,
        clash=clash,
    ...
    payload = {
        ...
        "ask_smoke": ask,
        "public_note": "docs/results/nano-lm/wave-ae-session.md",
        "rule": "pesquisa §5 AE0 · Cursor ASK→EVAL→FIX law",
        "next": "AE1 H-CTXMAX (ASK→EVAL→FIX×10)",
    }
