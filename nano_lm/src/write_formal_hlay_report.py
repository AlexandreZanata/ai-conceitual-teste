"""Render formal H-LAY vs H-EARLY (layer early-exit)."""
...
def render(path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "Commands: `npm run nano:formal:hlay` → "
            "`npm run nano:formal:hlay:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hlay/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hlay-vs-hearly.md"),
    )
    args = p.parse_args()
