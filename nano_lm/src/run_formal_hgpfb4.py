"""Formal H-ABS-GPFB4: freeze smoke GENC genes; PFB K=4 (fit≠eval ckpts)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hgpfb4"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-ABS-GPFB4 formal requires CUDA")
    out: Path = c["out"]
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "cpu_threads": threads,
            "mode": (
                "GPFB4 formal: freeze smoke GENC genes; "
                "PFB K=4 on formal ckpts"
            ),
            "mechanism": (
                "frozen GENC gene; serial decode on GENC ctx; PFB K=4 "
                "(GPFB K=2 KILL lesson)"
            ),
            "parent": "H-GENC-serial n=1 (same genome; formal HEARLY/B2)",
