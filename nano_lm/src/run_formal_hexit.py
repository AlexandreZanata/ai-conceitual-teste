"""Formal-budget H-EXIT vs H-EARLY tip (FLOP dual gate)."""
...
from exit_ops import clamp_exit_gene
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hexit"
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_exit(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=c["out"] / f"HEXIT_seed{seed}_train.json",
        )
        rows.append(
            _eval(
                "H-EXIT",
