"""Formal-budget H-LS vs B2 (label-smoothed KD), equal steps."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hls",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 4,
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        write_json(c["out"] / f"HLS_seed{seed}_train.json", ls_meta)
        rows.append(eval_ckpt(c, ckpt, seed, "H-LS"))
        if device.type == "cuda":
            torch.cuda.empty_cache()
    payload = {
        "rows": rows,
