"""Formal H-ABS-QPFB: PFB on QT-int8 vs H-QT (fit≠eval genes)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hqpfb"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-ABS-QPFB formal requires CUDA")
    out: Path = c["out"]
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "max_new": _MAX_NEW,
            "cpu_threads": threads,
            "mode": "QPFB formal: QT-int8 + PFB vs QT EARLY (fit≠eval)",
            "mechanism": "quantize_student_int8 then PFB (empty-elig→parent)",
            "parent": "H-QT int8 EARLY n=1 (formal genes)",
        },
    )
