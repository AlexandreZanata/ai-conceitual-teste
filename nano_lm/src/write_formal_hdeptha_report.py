"""Render formal H-DEPTHA vs H-ADAMF (DEPTH under ADAMF)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        "# Formal H-DEPTHA vs H-ADAMF (DEPTH under ADAMF)",
        "",
        f"Source: `{path}`",
        f"Wall clock: {data.get('wall_s', float('nan')):.1f}s",
        "",
        "Same ADAMF I/O path; only student depth differs (1-layer DEPTH vs tip).",
        "Fit≠eval. Gate: lp ≥ ADAMF−ε **and** train ms/step < ADAMF.",
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Tip H-ADAMF / H-DEPTH util unchanged. Thin+prune × train I/O.",
            "",
            "Commands: `npm run nano:formal:hdeptha` → "
            "`npm run nano:formal:hdeptha:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hdeptha/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hdeptha-vs-hadamf.md"),
    )
    args = p.parse_args()
