"""Wave AL2 H-CTXFRESH runner: nona-doc dual-arm ASK→EVAL→FIX×10."""
...
_SUMMARY = REPO / "results/nano-lm/wave-al/ctxfresh_summary.json"
...
def _seed_pack_golds(bank_path: Path, al_bank: Path) -> int:
    ...
    for i, item in enumerate(AL0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AL-CTXFRESH-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = CTXFRESH_ID
        row["judge_notes"] = [
            "CTXFRESH seed alias for held-out AL ask",
...
def _build_ctxs(
    *, curated_root: Path, workers: int, tok: Any
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        rows = list(pool.map(_read, [dict(p) for p in AL0_PACK]))
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AL-CTXFRESH-FIX-{i:02d}",
...
def _lookup_trial(
    *,
    ...
    tid = f"AL-CTXFRESH-LOOKUP-HITL-{i:02d}"
    mode = str(payload.get("mode", ""))
    score, err, notes, usable = score_ctxfresh_lookup(
        mode=mode,
...
def _gen_trial(
    *,
    ...
    tid = f"AL-CTXFRESH-GEN-HITL-{i:02d}"
    score, err, notes, usable = score_ctxfresh_gen(
        completion=str(payload.get("completion", "")),
        expected_gold=str(item["gold"]),
        meta=ctx,
        payload=payload,
...
def run_ctxfresh(
    *,
    ...
    """
    GIVEN AL0 held-out asks + octa curated docs
    WHEN K=19 ROLL/SUMCACHE + LOOKUP askfast + GENERATE raw
    THEN L_eff↑ vs CTXMORE · gen usable≥5 → PROMOTE|HOLD|KILL.
    """
    if len(AL0_PACK) != CTXFRESH_N:
        raise ValueError("AL0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack_golds(bank_path, al_bank)
    cfg = matrix_cfg()
    ...
    for i, (item, lp, gp, ctx) in enumerate(
        zip(AL0_PACK, lookup_payloads, gen_payloads, ctxs, strict=True),
        start=1,
    ):
    ...
    mean_l = float(sum(float(c["l_eff"]) for c in ctxs) / CTXFRESH_N)
    mean_a = float(
        sum(float(c["sumcache_active"]) for c in ctxs) / CTXFRESH_N
    )
    mean_s = float(sum(float(c["n_slices"]) for c in ctxs) / CTXFRESH_N)
    mean_src = float(sum(float(c["n_sources"]) for c in ctxs) / CTXFRESH_N)
    n_true = sum(1 for t in lookup_trials if t["lookup_kind"] == "TRUE_HIT")
    n_false = sum(
        1 for t in lookup_trials if t["lookup_kind"] == "FALSE_HIT"
    )
    stats = ctxfresh_stats(
    ...
    decision = decide_ctxfresh(stats)
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{CTXFRESH_ID}: L_lookup={stats['lookup_mean']:.1f} "
            f"L_gen={stats['gen_mean']:.1f} "
            f"usable_L={stats['n_lookup_usable']}/10 "
            f"usable_G={stats['n_gen_usable']}/10 "
            f"L_eff={mean_l:.0f} (>CTXMORE {stats['ctxmore_mean_leff']:.0f}) "
            f"sources={mean_src:.1f} slices={mean_s:.1f} "
            f"false_hit={n_false} fix={fix_count} decision={decision}."
        ),
        "public_note": "docs/results/nano-lm/formal-hctxfresh-ctxfresh.md",
        "ship_claim": "AF packaged stack (nona long-ctx; LOOKUP≠gen IQ)",
        "claim": (
            "nona-doc longer usable ctx beyond CTXMORE parent — "
