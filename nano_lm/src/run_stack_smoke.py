"""Smoke H-STACK on B2 ckpts; dual gate vs H-EARLY + H-DECM tips."""
...
from hyp_stack import run_h_stack
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_stack(
            ...
            lam=MIN_LAM,
            mix_m=MIX_M,
            out_meta=out / f"HSTACK_seed{seed}_train.json",
        )
        row = {
            "family": "H-STACK",
            "label": f"HSTACK_seed{seed}",
        ...
        write_json(out / f"HSTACK_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "stack_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "stack_smoke.json")}))
    return 0
