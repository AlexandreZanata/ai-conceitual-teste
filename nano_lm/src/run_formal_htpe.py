"""Formal-budget H-TPE vs H-TYP, shared B2 ckpts."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-htpe"
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        rows.append(
            {
                ...
                "teacher_mean_logprob": float(typ_meta["eval_fit"]),
                "mean_wall_ms": float(typ_meta["eval_wall_ms"]),
                "best_typ_mass": float(typ_meta["best_typ_mass"]),
                "seed": seed,
            }
        )
        if device.type == "cuda":
        ...
        meta = run_h_tpe(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=c["out"] / f"HTPE_seed{seed}_train.json",
        )
        row = {
            "family": "H-TPE",
            "label": f"HTPE_seed{seed}",
