"""Wave AJ0 SESSION runner (nano:aj:session) — freeze held-out HITL×10."""
...
_OUT = REPO / "results/nano-lm/wave-aj/aj0_session.json"
_TRIALS = REPO / "results/nano-lm/wave-aj/trials"
_ERROR_BANK = REPO / "results/nano-lm/wave-aj/error_bank.jsonl"
...
def main() -> int:
    ...
    ids = [str(p["source_id"]) for p in AJ0_PACK]
    ...
    trials_ready = Path(args.trials_dir).is_dir() and len(written) == AJ0_N

    decision = _override_decision(
        decide_aj0_session(known_sources=known, trials_dir_ready=trials_ready),
        miss=miss,
        clash=clash,
    ...
    payload = {
        ...
        "ask_smoke": ask,
        "public_note": "docs/results/nano-lm/wave-aj-session.md",
        "rule": "pesquisa §6 AJ0 · Cursor ASK→EVAL→FIX dual-arm law",
        "next": "AJ1 H-GENPEAK (gen lift + ASK dual-arm)",
        "anti_fp": (
