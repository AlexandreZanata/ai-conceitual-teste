"""Smoke H-QPACK: FLAYB quality pack vs tip H-POOL."""
...
def main() -> int:
    ...
    if device.type != "cuda":
        print("ERROR: H-QPACK requires CUDA", file=sys.stderr)
        return 2
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        ...
        for row in pair:
            if row["family"] == "H-FLAYB":
                write_json(out / f"HQPACK_seed{seed}_eval.json", row)
        rows.extend(pair)
    write_json(
        out / "qpack_smoke.json",
        {
            "rows": rows,
    ...
    print(json.dumps({"n_rows": len(rows), "out": str(out / "qpack_smoke.json")}))
    return 0
