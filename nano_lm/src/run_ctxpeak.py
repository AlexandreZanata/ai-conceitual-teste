"""Wave AJ2 H-CTXPEAK runner: hepta-doc dual-arm ASK→EVAL→FIX×10."""
...
_SUMMARY = REPO / "results/nano-lm/wave-aj/ctxpeak_summary.json"
...
def _seed_pack_golds(bank_path: Path, aj_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(AJ0_PACK, start=1):
        q = str(item["question"]).strip()
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AJ-CTXPEAK-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = CTXPEAK_ID
        row["judge_notes"] = [
            "CTXPEAK seed alias for held-out AJ ask",
...
def _build_ctxs(
    *, curated_root: Path, workers: int, tok: Any
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        rows = list(pool.map(_read, [dict(p) for p in AJ0_PACK]))
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AJ-CTXPEAK-FIX-{i:02d}",
...
def _lookup_trial(
    *,
    ...
    tid = f"AJ-CTXPEAK-LOOKUP-HITL-{i:02d}"
    mode = str(payload.get("mode", ""))
    ...
    return {
        ...
        "septenary_source": ctx.get("septenary_source"),
...
def _gen_trial(
    *,
    ...
    tid = f"AJ-CTXPEAK-GEN-HITL-{i:02d}"
    score, err, notes, usable = score_ctxpeak_gen(
        completion=str(payload.get("completion", "")),
        expected_gold=str(item["gold"]),
        meta=ctx,
        payload=payload,
...
def run_ctxpeak(
    *,
    ...
    """
    GIVEN AJ0 held-out asks + hepta curated docs
    WHEN K=15 ROLL/SUMCACHE + LOOKUP askfast + GENERATE raw
    THEN L_eff↑ vs CTXPUSH · gen usable≥5 → PROMOTE|HOLD|KILL.
    """
    if len(AJ0_PACK) != CTXPEAK_N:
        raise ValueError("AJ0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack_golds(bank_path, aj_bank)
    cfg = matrix_cfg()
    tok = load_tokenizer(str(cfg["tokenizer_id"]), cfg["cache"])
    bank = load_bank_rows(bank_path)
    ctxs = _build_ctxs(curated_root=curated_root, workers=workers, tok=tok)
    questions = [p["question"] for p in AJ0_PACK]
    ...
    for i, (item, lp, gp, ctx) in enumerate(
        zip(AJ0_PACK, lookup_payloads, gen_payloads, ctxs, strict=True),
        start=1,
    ):
    ...
    mean_l = float(sum(float(c["l_eff"]) for c in ctxs) / CTXPEAK_N)
    mean_a = float(
        sum(float(c["sumcache_active"]) for c in ctxs) / CTXPEAK_N
    )
    mean_s = float(sum(float(c["n_slices"]) for c in ctxs) / CTXPEAK_N)
    mean_src = float(sum(float(c["n_sources"]) for c in ctxs) / CTXPEAK_N)
