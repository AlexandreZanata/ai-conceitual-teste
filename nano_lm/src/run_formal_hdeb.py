"""Formal-budget H-DEB vs B2 (dual student; teacher picks)."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hdeb",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 2,
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        ckpt = c["out"] / f"HDEB_seed{seed}.pt"
        deb_meta = run_h_deb(
            teacher_id=c["teacher_id"],
            tokenizer_id=c["tokenizer_id"],
            cache_dir=c["cache"],
            ...
            alpha=0.5,
            out_path=ckpt,
            peer_weight=float(c["peer_weight"]),
        )
        write_json(c["out"] / f"HDEB_seed{seed}_train.json", deb_meta)
        rows.append(eval_ckpt(c, ckpt, seed, "H-DEB"))
        if device.type == "cuda":
            torch.cuda.empty_cache()
    payload = {
        "rows": rows,
