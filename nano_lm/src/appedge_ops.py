"""Wave AN5 H-APPEDGE: apps expose LOOKUP vs GENERATE + DEPL-AN honesty."""
...
APPEDGE_ID = "H-APPEDGE"
APPEDGE_N = 10
MIN_APPS = 3  # known + howto + longdoc (AN0 surfaces)
MIN_PAGES = 4  # 3 apps + depl-an
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0  # pesquisa §3 AN5 — or honest HOLD
APPPUSH_GEN_MEAN = 4.0  # parent AI5 SERVE gen (HOLD)
APPNEXT_GEN_MEAN = 9.0  # peer AM5 APPNEXT SERVE
SERVEALIGN_MEAN = 3.4
DEPL_AN_PAGE = "docs/results/nano-lm/depl-an.md"

AN_STACK_MARKERS: tuple[str, ...] = (
    "H-APPEDGE",
    "LOOKUP",
    "GENERATE",
    "H-FASTEDGE",
...
APPEDGE_APPS: tuple[dict[str, Any], ...] = (
    {
        "app_id": "app-known",
        "surface": "known-ask",
        ...
        "one_pager": "docs/results/nano-lm/appedge-known.md",
        "npm": "npm run nano:appedge -- --app app-known",
    },
    {
        "app_id": "app-howto",
        "surface": "howto",
        ...
        "one_pager": "docs/results/nano-lm/appedge-howto.md",
        "npm": "npm run nano:appedge -- --app app-howto",
    },
    {
        "app_id": "app-longdoc",
        "surface": "long-doc",
        ...
        "one_pager": "docs/results/nano-lm/appedge-longdoc.md",
        "npm": "npm run nano:appedge -- --app app-longdoc",
    },
)


def app_by_id(app_id: str) -> dict[str, Any]:
    for app in APPEDGE_APPS:
        if app["app_id"] == app_id:
            return dict(app)
    raise KeyError(f"unknown appedge id: {app_id}")
    ...
    *,
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPEDGE LOOKUP OOS — not generative IQ"
        ]
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "APPEDGE LOOKUP labeled — not generative IQ",
    ]
...
def score_appedge_gen(
    *,
    ...
    """GENERATE on app surface — OOS refuse ok; SERVE uses GENEDGE peak."""
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPEDGE GENERATE OOS refuse — honest"
        ]
    ...
    notes = list(notes) + [
        "APPEDGE GENERATE GENEDGE peak — Cursor scores completion",
        f"beat APPPUSH SERVE gen={APPPUSH_GEN_MEAN} / peer APPNEXT="
        f"{APPNEXT_GEN_MEAN}",
...
def one_pager_body(app: Mapping[str, Any]) -> str:
    """Public one-pager for an APPEDGE packaged app (dual-arm)."""
    spine = " → ".join(str(x) for x in app.get("spine") or ())
    return "\n".join(
        [
            f"# {app['app_id']} — scoped nano app (APPEDGE)",
            "",
            f"> Wave AN5 **H-APPEDGE** · Spine: `{spine}`",
            f"> Claim: {app['claim']}",
            "",
            "## Job",
            "",
            f"Surface `{app['surface']}` — dual-arm LOOKUP + GENERATE on AN0.",
            "",
            "## Arms",
            "",
...
def depl_an_body() -> str:
    """Overview one-pager for AN dual-arm packaged deploy."""
    lines = [
...
def page_sync_report(path: str, text: str) -> dict[str, Any]:
    ...
    for m in AN_STACK_MARKERS:
        if m not in body:
            miss.append(m)
    for m in FORBIDDEN_MARKERS:
        if m not in body:
            miss.append(f"forbidden:{m}")
    if "npm run nano:appedge" not in body:
        miss.append("npm_appedge")
...
def app_dual_stats(
    *,
    ...
    """
    if len(lookup_scores) != APPEDGE_N or len(gen_scores) != APPEDGE_N:
        raise ValueError(f"APPNEXT app requires {APPEDGE_N} dual-arm scores")
    if len(serve_gen_scores) != int(n_in_scope):
        raise ValueError("serve_gen_scores must match n_in_scope")
    l_mean = float(sum(lookup_scores) / float(APPEDGE_N))
    g_pack = float(sum(gen_scores) / float(APPEDGE_N))
    g_mean = (
    ...
    return {
        ...
        "peers_appnext_gen": g_mean >= float(APPNEXT_GEN_MEAN),
...
def appedge_stats(
    ...
    *,
    ...
    return {
        ...
        "peers_appnext_gen": gen_mean >= float(APPNEXT_GEN_MEAN),
        "min_lookup_mean": MIN_LOOKUP_MEAN,
        "min_gen_mean": MIN_GEN_MEAN,
        "apppush_gen_mean": APPPUSH_GEN_MEAN,
        "appnext_gen_mean": APPNEXT_GEN_MEAN,
        "servealign_mean": SERVEALIGN_MEAN,
    }


def decide_appedge(stats: Mapping[str, Any]) -> str:
    """
    GIVEN APPNEXT wave stats
    WHEN applying pesquisa §3 AN5 gate
