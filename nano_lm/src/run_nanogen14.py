"""Wave BD4 H-NANOGEN14 runner — gen-defer; not NANOGEN13 rename."""
...
_SUMMARY = REPO / "results/nano-lm/wave-bd/nanogen14_summary.json"
_PUBLIC = REPO / "docs/results/nano-lm/formal-hnanogen14-nanogen14.md"
...
_BC_FH = "Write a Python function floordiv(a, b) that returns a // b."
...
def _smoke_bc_forever(*, root: Path, bank: Path) -> dict[str, Any]:
    row = _ask(_BC_FH, root=root, bank=bank)
    row["arm"] = "BC_FOREVER"
    return row
...
def _live_modes_ok(rows: list[dict[str, Any]]) -> bool:
    ...
    for row in rows:
        ...
        if arm in {"BD_FOREVER", "BC_FOREVER"}:
...
def _write_public(
    *,
    ...
    body = "\n".join(
        [
            f"# H-NANOGEN14 — gen-defer gate (**DONE** — {status})",
            "",
            "> Lab: `.local/pesquisa.md` §4 · §9 BD4 · Session: "
            "`.local/wave-bd/SESSION.md`  ",
            "> Parent: [formal-hnanogen13-nanogen13.md]"
            "(formal-hnanogen13-nanogen13.md) (**DEFER**) · "
            "BD0 stance **defer**  ",
            "> Module: `nano_lm/src/nanogen14_ops.py` · "
            "Runner: `npm run nano:nanogen14`",
            "",
            "## Hypothesis",
            "",
            NANOGEN14_THESIS,
            ...
            f"| parent NANOGEN8·9·10·11·12·13 DEFER | "
            ...
            f"**{board.get('parent_nanogen13_defer')}** | True |",
            f"| live_modes_ok | **{board.get('live_modes_ok')}** | "
            "LOOKUP+ABSTAIN · BD/BC-FOREVER ABSTAIN |",
            f"| Decision | **{status}** | — |",
            "",
            ...
            *smoke,
            ...
            "3. NANOGEN6·7 HOLD · NANOGEN8·9·10·11·12·13 DEFER stand "
            "(span-fallback ≠ gen).  ",
            ...
            "| Allowed | Forbidden |",
            "|---------|-----------|",
            "| Honest DEFER/HOLD under BD0 stance | "
            "NANOGEN14 = NANOGEN13+rename |",
            "| Cite NANOGEN6·7 HOLD · NANOGEN8…13 DEFER | "
            "Vanity gen unlock / LOOKUP-as-IQ |",
            "| PROMOTE only real method + true_continue≥5.5 | "
            "Raise ≤5M w/o CAPCHECK |",
            "",
            f"SAFE note: {NANOGEN14_SAFE_NOTE}  ",
            f"Anti-FP: {NANOGEN14_ANTI_FP}  ",
            f"Ship lock: {NANOGEN14_CLAIM}",
            "",
...
def _write_session(decision: str, board: dict[str, Any]) -> None:
    ...
    body = "\n".join(
        [
            f"# Wave BD session checklist (**OPEN** · BD4 DONE — {status})",
            "",
            "> Private under `.local/`. Lab: `.local/pesquisa.md`.  ",
            f"> Ship lock: **{NANOGEN14_CLAIM}** · ≤5M.",
            "",
            "## Current stage",
            "",
            f"**BD4 — H-NANOGEN14 (DONE — {status})** · "
...
def _patch_pesquisa(decision: str) -> None:
    ...
    text = text.replace(
        ...
        f"5. **BD4 H-NANOGEN14** — **DONE {status}** "
    ...
    text = text.replace(
        ...
        f"(BD4 H-NANOGEN14 **DONE — {status}**; next BD5 BD-REAL-EVAL).",
        1,
    )
...
**BD0–BD3 PROMOTE · BD4 DONE — {status}**. Next: **BD5 BD-REAL-EVAL**.
...
**Wave BD ACTIVE** — BD4 **H-NANOGEN14 {status}** (gen locked).
...
def _bd_active_line(status: str) -> str:
    return (
        ...
        f"BD4 [H-NANOGEN14 {status}](formal-hnanogen14-nanogen14.md) "
        f"(`npm run nano:nanogen14`) — gen stance defer · not NANOGEN13 rename; "
        "next BD5 BD-REAL-EVAL; ship remains **AF + AQ + AS trust + STRICT "
...
def _patch_recipes(status: str) -> None:
    ...
    insert = (
        f"| Wave BD4 H-NANOGEN14 | [formal-hnanogen14-nanogen14.md]"
        f"(formal-hnanogen14-nanogen14.md) **{status}** "
        f"(`npm run nano:nanogen14`) — gen stance defer · CAPCHECK closed · "
        "not NANOGEN13 rename |"
    )
    if "Wave BD4 H-NANOGEN14" not in text2 and "| Wave BD3 H-CTXGAIN |" in text2:
        text2 = text2.replace(
            "| Wave BD3 H-CTXGAIN |",
            insert + "\n| Wave BD3 H-CTXGAIN |",
            1,
        )
    if n or "Wave BD4 H-NANOGEN14" in text2:
...
def _patch_card_agents_agenda(status: str) -> None:
    ...
    if _AGENTS.is_file():
        ...
        agents = (
            ...
            "(`npm run nano:bd:ctxgain`) · BD4 "
            f"[H-NANOGEN14 {status}]"
            "(docs/results/nano-lm/formal-hnanogen14-nanogen14.md) "
            "(`npm run nano:nanogen14`); next BD5 BD-REAL-EVAL; ship remains "
    ...
    if _AGENDA.is_file():
        ...
        row = (
            f"| **BD** | **ACTIVE** | BD0–BD3 PROMOTE · BD4 H-NANOGEN14 "
            f"{status} (`npm run nano:nanogen14`); next BD5 BD-REAL-EVAL; ≤5M |"
        )
        text2, n = re.subn(
            r"\| \*\*BD\*\* \| \*\*ACTIVE\*\* \|[^\n]+", row, text, count=1
        )
...
def _patch_evogen(status: str) -> None:
    ...
    text = text.replace(
        ...
        f"Wave BD ACTIVE (BD0–BD3 PROMOTE · BD4 H-NANOGEN14 {status}; "
        "next BD5 BD-REAL-EVAL)",
        1,
    )
...
def run_nanogen14(
    *,
    ...
    """
    GIVEN BD0 gen stance defer + archived NANOGEN6–13
    WHEN applying BD4 gate with live mode smoke
    ...
    decision = decide_nanogen14(board=board, anti_fp_signed=True)
    ...
    payload: dict[str, Any] = {
        "id": NANOGEN14_ID,
        "stage": "BD4",
        "thesis": NANOGEN14_THESIS,
        "decision": decision,
        "stance": dict(NANOGEN14_STANCE),
        "method": dict(NANOGEN14_METHOD),
        "board": board,
        "true_gen_judge": dict(TRUE_GEN_JUDGE),
        "parent_archive": {
            "nanogen13_true_continue_mean": tc13,
            "nanogen13_n_true_continue": n_tc13,
            "nanogen13_n_span_fallback": n_span13,
        ...
        "cpu_threads": threads,
        "workers": workers,
        "claim": NANOGEN14_CLAIM,
        "public_note": "docs/results/nano-lm/formal-hnanogen14-nanogen14.md",
        "next": "BD5 BD-REAL-EVAL",
        "anti_fp": NANOGEN14_ANTI_FP,
        "finding": (
            f"{NANOGEN14_ID}: stance={board.get('stance')} "
            f"method={board.get('method_id')} "
...
def main() -> int:
    _clear_proxy()
    ap = argparse.ArgumentParser(description="Wave BD4 H-NANOGEN14")
