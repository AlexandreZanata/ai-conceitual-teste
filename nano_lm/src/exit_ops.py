"""H-EXIT: earlier min_new codebook + forced n=1; FLOP dual gate."""
...
ExitGene = dict[str, Any]
EXIT_MIN_NEWS = (2, 4, 8)
PATIENCES = (1, 2, 3)


def clamp_exit_gene(gene: ExitGene) -> ExitGene:
    """
    GIVEN raw early-exit knobs
    WHEN clamping for H-EXIT
    THEN min_new on EXIT_MIN_NEWS; n forced to 1; conf/temp/top_p bounded.
    """
    mn = int(round(float(gene["min_new"])))
    min_new = min(EXIT_MIN_NEWS, key=lambda x: abs(x - mn))
...
def random_exit_gene(rng: random.Random) -> ExitGene:
    return clamp_exit_gene(
        {
            "min_new": rng.choice(EXIT_MIN_NEWS),
...
def mutate_exit_gene(gene: ExitGene, rng: random.Random) -> ExitGene:
    g = dict(clamp_exit_gene(gene))
    if rng.random() < 0.4:
        i = EXIT_MIN_NEWS.index(int(g["min_new"]))
        i = max(0, min(len(EXIT_MIN_NEWS) - 1, i + rng.choice([-1, 0, 1])))
        g["min_new"] = EXIT_MIN_NEWS[i]
    if rng.random() < 0.4:
...
def decide_hexit(
    ...
    return "PROMOTE (earlier exit FLOP win vs H-EARLY)"
