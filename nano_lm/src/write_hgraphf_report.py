"""Render H-GRAPHF smoke vs H-FLAYB (CUDA graph under FLAYB decode)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Same dual-budget FLAYB path on B2; non-KV arm uses full-depth per-T "
        "CUDAGraph replay (capture untimed). KV arm stays CPOOLB. "
        "Kill if |Δlp| > ε vs H-FLAYB or no wall win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/graphf_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hgraphf-vs-hflayb.md"),
    )
    args = p.parse_args()
