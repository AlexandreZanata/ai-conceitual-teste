"""Render H-CUR2 smoke n_stages ablation markdown."""
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/cur2_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hcur2-vs-hcur.md"),
    )
    args = p.parse_args()
