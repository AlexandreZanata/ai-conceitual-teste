"""Render formal H-SROUTE vs H-SERVE."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Fit≠eval. ROUTE vs frozen SERVE on (lp, wall). "
        "Kill if SERVE dominates; else PROMOTE.",
    ...
    lines.extend(
        [
            ...
            "",
            "Commands: `npm run nano:formal:hsroute` → "
            "`npm run nano:formal:hsroute:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hsroute/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hsroute-vs-hserve.md"),
    )
    args = p.parse_args()
