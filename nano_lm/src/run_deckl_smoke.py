"""Smoke H-DECKL on B2 ckpts; compare vs B4 (matrix) on Pareto."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_deckl(
            ...
            top_k=TOP_K,
            lam=LAM,
            out_meta=out / f"HDECKL_seed{seed}_train.json",
        )
        row = {
            "family": "H-DECKL",
        ...
        write_json(out / f"HDECKL_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "deckl_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "deckl_smoke.json")}))
    return 0
