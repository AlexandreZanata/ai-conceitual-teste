"""Wave AM5 H-APPNEXT runner: dual-arm apps + DEPL-AM (nano:appnext)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-am/appnext_summary.json"
...
def _seed_pack(bank_path: Path, am_bank: Path) -> int:
    ...
    for i, item in enumerate(AM0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AM-APPNEXT-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = APPNEXT_ID
        row["judge_notes"] = [
            "APPNEXT seed for AM held-out ask",
...
def _write_pages(docs: Path) -> list[dict[str, Any]]:
    ...
    depl_path = docs / Path(DEPL_AM_PAGE).name
    depl_body = depl_am_body()
    depl_path.write_text(depl_body, encoding="utf-8")
    reports.append(page_sync_report(DEPL_AM_PAGE, depl_body))
    return reports
...
def _run_one_app(
    *,
    ...
    if len(lookup_payloads) != APPNEXT_N or len(shared_gen) != APPNEXT_N:
    ...
    for i, item in enumerate(AM0_PACK, start=1):
        ...
        if not bool(route["in_scope"]):
            ...
            row = alias_bank_row(
                trial_id=f"AM-APPNEXT-FIX-{tag}-{i:02d}",
        ...
        l_score, l_err, l_notes = score_appnext_lookup(
            mode=l_mode,
            completion=l_comp,
        ...
        l_trial = {
            "trial_id": f"AM-APPNEXT-{tag}-LOOKUP-HITL-{i:02d}",
            "stage": "AM5",
            "hyp_id": APPNEXT_ID,
            "arm": "LOOKUP",
            "realapp_id": app_id,
        ...
        g_score, g_err, g_notes = score_appnext_gen(
        ...
        g_trial = {
            "trial_id": f"AM-APPNEXT-{tag}-GEN-HITL-{i:02d}",
            "stage": "AM5",
            "hyp_id": APPNEXT_ID,
            "arm": "GENERATE",
            "realapp_id": app_id,
...
def run_appnext(
    *,
    ...
    bank = load_bank_rows(bank_path)
    apps = [dict(a) for a in APPNEXT_APPS]
    if only_app:
        apps = [app_by_id(only_app)]

    # Shared GENTRUTH peak GENERATE once — max HW without 3× CUDA reload.
    gen_items = [dict(p) for p in AM0_PACK]
    ...
    summary: dict[str, Any] = {
        ...
        "public_note": "docs/results/nano-lm/formal-happnext-appnext.md",
