"""Render formal H-MID vs H-EARLY (FLOP dual gate)."""
...
from mid_ops import decide_hmid
...
def render(path: Path) -> str:
    ...
    decision = decide_hmid(stats.get("H-MID", {}), stats)
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:hmid` → "
            "`npm run nano:formal:hmid:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hmid/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hmid-vs-hearly.md"),
    )
    args = p.parse_args()
