"""Wave AL0 SESSION runner (nano:al:session) — freeze held-out HITL×10."""
...
_OUT = REPO / "results/nano-lm/wave-al/al0_session.json"
_TRIALS = REPO / "results/nano-lm/wave-al/trials"
_ERROR_BANK = REPO / "results/nano-lm/wave-al/error_bank.jsonl"
...
def main() -> int:
    ...
    ids = [str(p["source_id"]) for p in AL0_PACK]
    ...
    trials_ready = Path(args.trials_dir).is_dir() and len(written) == AL0_N

    decision = _override_decision(
        decide_al0_session(known_sources=known, trials_dir_ready=trials_ready),
        miss=miss,
        clash=clash,
    ...
    payload = {
        ...
        "ask_smoke": ask,
        "public_note": "docs/results/nano-lm/wave-al-session.md",
        "rule": "pesquisa §3 AL0 · Cursor ASK→EVAL→FIX dual-arm law",
        "next": "AL1 H-GENFRESH (smarter gen + ASK dual-arm)",
        "anti_fp": (
