"""H-LAY: search layer-exit knobs under frozen H-EARLY tip genes."""
...
from lay_fit import fitness_lay_detail
from lay_ops import (
    clamp_lay_gene,
    flop_aware_score,
    mutate_lay_gene,
...
def run_h_lay(
    *,
    ...
    pop = [random_lay_gene(rng) for _ in range(pop_size)]
    ...
    for gen in range(generations):
        ...
        history.append(
            {
                ...
                "best_max_skip": pop[ranked[0]]["max_skip"],
        ...
        if scores[ranked[0]] > best_score:
            ...
            best_gene = clamp_lay_gene(pop[ranked[0]])
        parents = [pop[i] for i in ranked[: max(1, pop_size // 2)]]
        pop = [
            mutate_lay_gene(parents[i % len(parents)], rng) for i in range(pop_size)
        ]
    eval_lp, eval_wall, eval_gf = fitness_lay_detail(
        best_gene,
        early_gene,
        teacher=teacher,
        student=student,
