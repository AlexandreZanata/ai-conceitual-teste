"""Formal-budget H-Q8 vs H-CURL (same EARLY decode; INT8 dynamic)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hq8"
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        rows.append(
            _score_row(
                c, family="H-CURL", ckpt=curl, gene=gene, seed=seed, quantize=False
            )
        )
        rows.append(
            _score_row(
                c, family="H-Q8", ckpt=curl, gene=gene, seed=seed, quantize=True
            )
        )
        if device.type == "cuda":
