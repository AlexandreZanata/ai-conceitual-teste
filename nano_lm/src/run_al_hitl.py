"""Wave AL6 AL-HITL-10 runner: final dual-arm verify (nano:al:hitl)."""
...
from al_session_ops import AL0_PACK, overlaps_prior_questions, pack_app_counts
...
_SUMMARY = REPO / "results/nano-lm/wave-al/al_hitl_summary.json"
...
def run_al_hitl(
    *,
    ...
    """
    GIVEN frozen AL0 held-out asks
    WHEN final LOOKUP + GENFRESH peak GENERATE ASK→EVAL→FIX×10
    THEN lookup≥7; gen≥5 → PROMOTE else documented HOLD.
    """
    if len(AL0_PACK) != AL6_N:
    ...
    del _ablated
    if len(lookup_payloads) != AL6_N or len(gen_payloads) != AL6_N:
    ...
    for i, (item, lp, gp) in enumerate(
        zip(AL0_PACK, lookup_payloads, gen_payloads, strict=True),
        start=1,
    ):
        ...
        l_score, l_err, l_notes = score_al6_lookup(
        ...
        lt = {
            "trial_id": f"AL-FINAL-LOOKUP-HITL-{i:02d}",
            "stage": "AL6",
            "hyp_id": AL6_ID,
            "arm": "LOOKUP",
            "app_id": item["app_id"],
        ...
        if g_tel["wall_ms"] > 0.0 and g_tel["n_new"] > 0:
            n_gen_wall_ok += 1
        g_score, g_err, g_notes = score_al6_gen(
            completion=str(gp.get("completion", "")),
            expected_gold=str(item["gold"]),
            payload=gp,
        )
        gt = {
            "trial_id": f"AL-FINAL-GEN-HITL-{i:02d}",
            "stage": "AL6",
            "hyp_id": AL6_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
