"""Formal H-SROUTE: ROUTE vs frozen SERVE (eval, fit≠eval)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hsroute"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-SROUTE formal requires CUDA")
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        rows.extend(run_seed_pair(c, seed, prompts, claim_offset=9494))
