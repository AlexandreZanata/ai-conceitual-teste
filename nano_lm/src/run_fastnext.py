"""Wave AM4 H-FASTNEXT runner: GENTRUTH peak-fast gen wall vs AL FASTNEXT."""
...
_SUMMARY = REPO / "results/nano-lm/wave-am/fastnext_summary.json"
...
def _seed_pack(bank_path: Path, am_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(AM0_PACK, start=1):
        q = str(item["question"]).strip()
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AM-FASTNEXT-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = FASTNEXT_ID
        row["judge_notes"] = [
            "FASTNEXT seed for AM held-out ask",
...
def _timed_gen(
    *,
    ...
    """GENERATE arm: GENTRUTH peak-extractive (wall_ms>0, no LOOKUP cache)."""
    ...
    *,
    ...
    for _ in range(max(1, rounds)):
        ...
        if wall < best_wall or (wall == best_wall and e2e < best_e2e):
            ...
            best = payloads
    if best is None:
        raise RuntimeError("FASTNEXT hot gen produced no payloads")
...
def _lookup_trial(
    *,
    ...
    score, err, notes = score_fastnext_lookup(
        mode=str(payload.get("mode", "")),
    ...
    return {
        "trial_id": f"AM-FASTNEXT-LOOKUP-HITL-{i:02d}",
        "stage": "AM4",
        "hyp_id": FASTNEXT_ID,
        "arm": "LOOKUP",
        "app_id": item["app_id"],
...
def _gen_trial(
    *,
    ...
    score, err, notes = score_fastnext_gen(
        completion=str(payload.get("completion", "")),
        expected_gold=str(item["gold"]),
        payload=payload,
    )
    tel = extract_telemetry(payload)
    return {
        "trial_id": f"AM-FASTNEXT-GEN-HITL-{i:02d}",
        "stage": "AM4",
        "hyp_id": FASTNEXT_ID,
        "arm": "GENERATE",
        "app_id": item["app_id"],
...
def run_fastnext(
    *,
    ...
    """
    GIVEN AM0 held-out asks
    WHEN LOOKUP quality + GENTRUTH peak-fast GENERATE cold/warm/hot
    THEN dual-arm FASTNEXT vs AL FASTFRESH → PROMOTE|HOLD|KILL.
    """
    if len(AM0_PACK) != FASTNEXT_N:
        raise ValueError("AM0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack(bank_path, am_bank)
    bank = load_bank_rows(bank_path)
    items = [dict(p) for p in AM0_PACK]
    ...
    for i, (item, payload) in enumerate(
        ...
        if kind != "TRUE_HIT":
            row = alias_bank_row(
                trial_id=f"AM-FASTNEXT-FIX-{i:02d}",
                question=item["question"],
                source_id=item["source_id"],
                gold=item["gold"],
            )
    ...
    decision = decide_fastnext(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "GENERATE PEAK_FAST+GENTRUTH (no student decode)",
            f"cold+warm+hot gen timing (hot rounds={_GEN_HOT_ROUNDS})",
            f"vs FASTFRESH hot {FASTFRESH_HOT_WALL_MS:.3f} ms",
        ],
        "forbidden": [
            "STREAM",
            "KVCACHE-Q",
        ...
        "timing": {
            ...
            "fastfresh_hot_wall_ms": FASTFRESH_HOT_WALL_MS,
        ...
        json.dumps(
            {
                ...
                "pass_vs_fastfresh": st["pass_vs_fastfresh"],
                ...
                "fastfresh_hot_wall_ms": st["fastfresh_hot_wall_ms"],
