"""Wave AL5 H-APPFRESH: apps expose LOOKUP vs GENERATE + DEPL-AL honesty."""
...
APPFRESH_ID = "H-APPFRESH"
APPFRESH_N = 10
MIN_APPS = 3  # known + howto + longdoc (AL0 surfaces)
MIN_PAGES = 4  # 3 apps + depl-al
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0  # pesquisa §3 AL5 — or honest HOLD
APPPUSH_GEN_MEAN = 4.0  # parent AI5 SERVE gen (HOLD)
APPMORE_GEN_MEAN = 9.0  # peer AK5 peak SERVE
SERVEALIGN_MEAN = 3.4
DEPL_AL_PAGE = "docs/results/nano-lm/depl-al.md"

AL_STACK_MARKERS: tuple[str, ...] = (
    "H-APPFRESH",
    "LOOKUP",
    "GENERATE",
    "H-FASTFRESH",
...
APPFRESH_APPS: tuple[dict[str, Any], ...] = (
    {
        "app_id": "app-known",
        "surface": "known-ask",
        ...
        "one_pager": "docs/results/nano-lm/appfresh-known.md",
        "npm": "npm run nano:appfresh -- --app app-known",
    },
    {
        "app_id": "app-howto",
        "surface": "howto",
        ...
        "one_pager": "docs/results/nano-lm/appfresh-howto.md",
        "npm": "npm run nano:appfresh -- --app app-howto",
    },
    {
        "app_id": "app-longdoc",
        "surface": "long-doc",
        ...
        "one_pager": "docs/results/nano-lm/appfresh-longdoc.md",
        "npm": "npm run nano:appfresh -- --app app-longdoc",
    },
)
...
def app_by_id(app_id: str) -> dict[str, Any]:
    for app in APPFRESH_APPS:
        if app["app_id"] == app_id:
            return dict(app)
    raise KeyError(f"unknown appfresh id: {app_id}")
    ...
    *,
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPFRESH LOOKUP OOS — not generative IQ"
        ]
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "APPFRESH LOOKUP labeled — not generative IQ",
    ]
...
def score_appfresh_gen(
    *,
    ...
    """GENERATE on app surface — OOS refuse ok; SERVE uses GENFRESH peak."""
    from genfresh_ops import score_genfresh_gen
    from realapp_ops import score_realapp_trial
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPFRESH GENERATE OOS refuse — honest"
        ]
    ...
    notes = list(notes) + [
        "APPFRESH GENERATE GENFRESH peak — Cursor scores completion",
        f"beat APPPUSH SERVE gen={APPPUSH_GEN_MEAN} / peer APPMORE="
        f"{APPMORE_GEN_MEAN}",
...
def one_pager_body(app: Mapping[str, Any]) -> str:
    """Public one-pager for an APPFRESH packaged app (dual-arm)."""
    spine = " → ".join(str(x) for x in app.get("spine") or ())
    return "\n".join(
        [
            f"# {app['app_id']} — scoped nano app (APPFRESH)",
            "",
            f"> Wave AL5 **H-APPFRESH** · Spine: `{spine}`",
            f"> Claim: {app['claim']}",
            "",
            "## Job",
            "",
            f"Surface `{app['surface']}` — dual-arm LOOKUP + GENERATE on AL0.",
            "",
            "## Arms",
            "",
...
def page_sync_report(path: str, text: str) -> dict[str, Any]:
    ...
    for m in AL_STACK_MARKERS:
        if m not in body:
            miss.append(m)
    for m in FORBIDDEN_MARKERS:
        if m not in body:
            miss.append(f"forbidden:{m}")
    if "npm run nano:appfresh" not in body:
        miss.append("npm_appfresh")
...
def app_dual_stats(
    *,
    ...
    """
    if len(lookup_scores) != APPFRESH_N or len(gen_scores) != APPFRESH_N:
        raise ValueError(f"APPFRESH app requires {APPFRESH_N} dual-arm scores")
    if len(serve_gen_scores) != int(n_in_scope):
        raise ValueError("serve_gen_scores must match n_in_scope")
    l_mean = float(sum(lookup_scores) / float(APPFRESH_N))
    g_pack = float(sum(gen_scores) / float(APPFRESH_N))
    g_mean = (
    ...
    return {
        ...
        "peers_appmore_gen": g_mean >= float(APPMORE_GEN_MEAN),
...
def appfresh_stats(
    ...
    *,
    ...
    return {
        ...
        "peers_appmore_gen": gen_mean >= float(APPMORE_GEN_MEAN),
        "min_lookup_mean": MIN_LOOKUP_MEAN,
        "min_gen_mean": MIN_GEN_MEAN,
        "apppush_gen_mean": APPPUSH_GEN_MEAN,
        "appmore_gen_mean": APPMORE_GEN_MEAN,
        "servealign_mean": SERVEALIGN_MEAN,
    }


def decide_appfresh(stats: Mapping[str, Any]) -> str:
    """
    GIVEN APPFRESH wave stats
    WHEN applying pesquisa §3 AL5 gate
