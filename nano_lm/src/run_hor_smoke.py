"""Smoke H-HOR: freeze tip n + horizon≤2 + FLOP search vs H-POOL tip."""
...
def _score_tip(
    ...
    return _row("H-POOL", f"HPOOL_hor_seed{seed}", lp, wall, gf, seed, gene), gene
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_hor(
            ...
            tip_gene=tip_gene,
            lam=LAM,
            out_meta=out / f"HHOR_seed{seed}_train.json",
        )
        row = _row(
            "H-HOR",
            f"HHOR_seed{seed}",
        ...
        write_json(out / f"HHOR_seed{seed}_eval.json", row)
        rows.append(row)
    write_json(
        out / "hor_smoke.json",
        {"rows": rows, "wall_s": time.perf_counter() - t0},
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "hor_smoke.json")}))
    return 0
