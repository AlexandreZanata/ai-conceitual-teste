"""Formal H-PINC: torch.compile under PIN vs eager PIN (formal KD budget)."""
...
from pinc_pair import run_seed_pair
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hpinc"
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        rows.extend(
            run_seed_pair(
                c, out, seed, device, vocab, steps, label_prefix="HPINC_formal"
            )
        )
        if device.type == "cuda":
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "steps": steps,
            "top_k": DEFAULT_TOP_K,
            "mode": "PIN train + torch.compile(default); warmup untimed",
        },
    )
