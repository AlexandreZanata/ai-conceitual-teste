"""Wave AI5 H-APPPUSH: apps expose LOOKUP vs GENERATE + DEPL-AI honesty."""
...
APPPUSH_ID = "H-APPPUSH"
APPPUSH_N = 10
MIN_APPS = 3  # known + howto + longdoc (AI0 surfaces)
MIN_PAGES = 4  # 3 apps + depl-ai
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0  # pesquisa §5 — or honest HOLD
APPLIFT_GEN_MEAN = 1.0  # parent AH5 SERVE gen (periods)
SERVEALIGN_MEAN = 3.4
DEPL_AI_PAGE = "docs/results/nano-lm/depl-ai.md"

AI_STACK_MARKERS: tuple[str, ...] = (
    "H-APPPUSH",
    "LOOKUP",
    "GENERATE",
    "H-FASTPUSH",
...
APPPUSH_APPS: tuple[dict[str, Any], ...] = (
    {
        "app_id": "app-known",
        "surface": "known-ask",
        ...
        "one_pager": "docs/results/nano-lm/apppush-known.md",
        "npm": "npm run nano:apppush -- --app app-known",
    },
    {
        "app_id": "app-howto",
        "surface": "howto",
        ...
        "one_pager": "docs/results/nano-lm/apppush-howto.md",
        "npm": "npm run nano:apppush -- --app app-howto",
    },
    {
        "app_id": "app-longdoc",
        "surface": "long-doc",
        ...
        "one_pager": "docs/results/nano-lm/apppush-longdoc.md",
        "npm": "npm run nano:apppush -- --app app-longdoc",
    },
)
...
def app_by_id(app_id: str) -> dict[str, Any]:
    for app in APPPUSH_APPS:
        if app["app_id"] == app_id:
            return dict(app)
    raise KeyError(f"unknown apppush id: {app_id}")
    ...
    *,
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPPUSH LOOKUP OOS — not generative IQ"
        ]
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "APPPUSH LOOKUP labeled — not generative IQ",
    ]
...
def score_apppush_gen(
    *,
    ...
    WHEN Cursor EVAL
    THEN OOS refuse ok; SERVE uses grounded genplus rubric (push vs APPLIFT).
    """
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPPUSH GENERATE OOS refuse — honest"
        ]
    ...
    notes = list(notes) + [
        "APPPUSH GENERATE grounded — Cursor scores completion",
        f"beat APPLIFT SERVE gen={APPLIFT_GEN_MEAN}",
    ]
    return float(score), bool(err), notes


def one_pager_body(app: Mapping[str, Any]) -> str:
    """Public one-pager for an APPPUSH packaged app (dual-arm)."""
    spine = " → ".join(str(x) for x in app.get("spine") or ())
    return "\n".join(
        [
            f"# {app['app_id']} — scoped nano app (APPPUSH)",
            "",
            f"> Wave AI5 **H-APPPUSH** · Spine: `{spine}`",
            f"> Claim: {app['claim']}",
            "",
            "## Job",
            "",
            f"Surface `{app['surface']}` — dual-arm LOOKUP + GENERATE on AI0.",
            "",
            "## Arms",
            "",
...
def depl_ai_body() -> str:
    """Overview one-pager for AI dual-arm packaged deploy."""
    lines = [
...
def page_sync_report(path: str, text: str) -> dict[str, Any]:
    ...
    for m in AI_STACK_MARKERS:
        if m not in body:
            miss.append(m)
    for m in FORBIDDEN_MARKERS:
        if m not in body:
            miss.append(f"forbidden:{m}")
    if "npm run nano:apppush" not in body:
        miss.append("npm_apppush")
...
def app_dual_stats(
    *,
    ...
    """
    if len(lookup_scores) != APPPUSH_N or len(gen_scores) != APPPUSH_N:
        raise ValueError(f"APPPUSH app requires {APPPUSH_N} dual-arm scores")
    if len(serve_gen_scores) != int(n_in_scope):
        raise ValueError("serve_gen_scores must match n_in_scope")
    l_mean = float(sum(lookup_scores) / float(APPPUSH_N))
    g_pack = float(sum(gen_scores) / float(APPPUSH_N))
    g_mean = (
    ...
    return {
        ...
        "beats_applift_gen": g_mean > float(APPLIFT_GEN_MEAN),
...
def apppush_stats(
    ...
    *,
    ...
    return {
        ...
        "beats_applift_gen": gen_mean > float(APPLIFT_GEN_MEAN),
        "min_lookup_mean": MIN_LOOKUP_MEAN,
        "min_gen_mean": MIN_GEN_MEAN,
        "applift_gen_mean": APPLIFT_GEN_MEAN,
        "servealign_mean": SERVEALIGN_MEAN,
    }


def decide_apppush(stats: Mapping[str, Any]) -> str:
    """
    GIVEN APPPUSH wave stats
    WHEN applying pesquisa §5 AI5 gate
