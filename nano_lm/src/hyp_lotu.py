"""H-LOTU: H-SEL scaffold; worst individual gets one free elite clone."""
...
from lotu_ops import underdog_gift
...
def run_h_lotu(
    *,
    ...
    if not data:
        raise RuntimeError("H-LOTU: no training batches")
    probe = data[0]
    best_state = None
    best_fit = float("-inf")
    history: list[float] = []
    gift_log: list[list[int]] = []
    for _gen in range(generations):
        ...
        under, elite = underdog_gift(fits)
        gift_log.append([under, elite])
        pop[under].load_state_dict(copy.deepcopy(pop[elite].state_dict()))
        pop[under].eval()
        fits[under] = fits[elite]
    ...
    return {
        "hypothesis": "H-LOTU",
        "lottery": "underdog_elite_clone",
        "gifts_per_gen": gift_log,
