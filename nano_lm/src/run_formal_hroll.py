"""Formal H-ROLL: PFB2 on rolled summary‖W vs EARLY (fit≠eval)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hroll"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-ROLL formal requires CUDA")
    out: Path = c["out"]
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "max_new": _MAX_NEW,
            "cpu_threads": threads,
            "mode": "ROLL formal: PFB2 on summary‖W vs EARLY (fit≠eval)",
            "mechanism": "stride-summary cache + window W; BEAMKV; not CTX",
            "parent": "H-EARLY n=1 on rolled ctx (formal genes)",
        },
    )
