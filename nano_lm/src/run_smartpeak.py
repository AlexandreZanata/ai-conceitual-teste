"""Wave AJ3 H-SMARTPEAK runner: hepta cite LOOKUP + GENPEAK GENERATE."""
...
from run_genpeak import _run_gen_peak
...
_SUMMARY = REPO / "results/nano-lm/wave-aj/smartpeak_summary.json"
...
def _seed_pack(bank_path: Path, aj_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(SMARTPEAK_PACK, start=1):
        for q_key, q_text in (
            ...
            if q in existing:
                continue
            row = alias_bank_row(
                trial_id=f"AJ-SMARTPEAK-SEED-{q_key}-{i:02d}",
                question=q,
                source_id=item["source_id"],
                gold=item["gold"],
            )
            row["hyp_id"] = SMARTPEAK_ID
            row["judge_notes"] = [
                "SMARTPEAK seed for hepta-hop paraphrase stress",
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AJ-SMARTPEAK-FIX-{i:02d}",
...
def run_smartpeak(
    *,
    ...
    """
    GIVEN AJ0 hepta-hop paraphrases + parent asks
    WHEN SEMWRAP LOOKUP + GENPEAK GENERATE dual-arm ×10
    THEN false-hit≈0; PROMOTE if gen≥5 else HOLD.
    """
    if len(SMARTPEAK_PACK) != SMARTPEAK_N:
        raise ValueError("SMARTPEAK pack must be 10")
    if not hard_paraphrase_ok():
        raise ValueError("paraphrases must differ from parents")
    if not has_adversarial_noise() or not has_hepta_hop_cues():
        raise ValueError("hepta-hop adversarial pack incomplete")

    trials_dir.mkdir(parents=True, exist_ok=True)
    t0 = time.perf_counter()
    seeded = _seed_pack(bank_path, aj_bank)
    bank = load_bank_rows(bank_path)
    paras = [p["paraphrase"] for p in SMARTPEAK_PACK]
    gen_items = [
        {
    ...
    for i, (item, lp) in enumerate(
        zip(SMARTPEAK_PACK, lookup_payloads, strict=True),
        start=1,
    ):
        ...
        score_l, err_l, notes_l, cited = score_smartpeak_lookup(
        ...
        lookup_trials.append(
            {
                "trial_id": f"AJ-SMARTPEAK-LOOKUP-HITL-{i:02d}",
                "stage": "AJ3",
                "hyp_id": SMARTPEAK_ID,
                "arm": "LOOKUP",
                "app_id": item["app_id"],
    ...
    for i, (item, gp) in enumerate(
        zip(SMARTPEAK_PACK, gen_payloads, strict=True)
    ):
        score_g, err_g, notes_g = score_smartpeak_gen(
    ...
    if weak_idx:
        re_items = [
            {
                "question": SMARTPEAK_PACK[i]["parent_question"],
                "source_id": SMARTPEAK_PACK[i]["source_id"],
                "gold": SMARTPEAK_PACK[i]["gold"],
            }
            for i in weak_idx
        ]
        ...
        for j, idx in enumerate(weak_idx):
            item = SMARTPEAK_PACK[idx]
            re_gp = re_payloads[j]
            score2, err2, notes2 = score_smartpeak_gen(
    ...
    for i, (item, gp) in enumerate(
        ...
        if bool(gp.get("peak_used")):
            n_peak += 1
        gt = {
            "trial_id": f"AJ-SMARTPEAK-GEN-HITL-{i:02d}",
            "stage": "AJ3",
            "hyp_id": SMARTPEAK_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
    ...
    decision = decide_smartpeak(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "hepta-hop paraphrase (CTXPEAK companions)",
        ...
        "finding": (
            f"{SMARTPEAK_ID}: L_lookup={stats['lookup_mean']:.1f} "
        ...
        "public_note": "docs/results/nano-lm/formal-hsmartpeak-smartpeak.md",
