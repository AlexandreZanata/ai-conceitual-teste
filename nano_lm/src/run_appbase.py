"""Wave AP5 H-APPBASE runner: dual-arm apps + DEPL-AP (nano:appbase)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ap/appbase_summary.json"
...
def _seed_pack(bank_path: Path, ao_bank: Path) -> int:
    ...
    for i, item in enumerate(AP0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AP-APPBASE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = APPBASE_ID
        row["judge_notes"] = [
            "APPBASE seed for AO held-out ask",
...
def _write_pages(docs: Path) -> list[dict[str, Any]]:
    ...
    depl_path = docs / Path(DEPL_AP_PAGE).name
    depl_body = depl_ap_body()
    depl_path.write_text(depl_body, encoding="utf-8")
    reports.append(page_sync_report(DEPL_AP_PAGE, depl_body))
    return reports
...
def _run_one_app(
    *,
    ...
    if len(lookup_payloads) != APPBASE_N or len(shared_gen) != APPBASE_N:
    ...
    for i, item in enumerate(AP0_PACK, start=1):
        ...
        if not bool(route["in_scope"]):
            ...
            row = alias_bank_row(
                trial_id=f"AP-APPBASE-FIX-{tag}-{i:02d}",
        ...
        l_score, l_err, l_notes = score_appbase_lookup(
            mode=l_mode,
            completion=l_comp,
        ...
        l_trial = {
            "trial_id": f"AP-APPBASE-{tag}-LOOKUP-HITL-{i:02d}",
            "stage": "AP5",
            "hyp_id": APPBASE_ID,
            "arm": "LOOKUP",
            "realapp_id": app_id,
        ...
        g_score, g_err, g_notes = score_appbase_gen(
        ...
        g_trial = {
            "trial_id": f"AP-APPBASE-{tag}-GEN-HITL-{i:02d}",
            "stage": "AP5",
            "hyp_id": APPBASE_ID,
            "arm": "GENERATE",
            "realapp_id": app_id,
...
def run_appbase(
    *,
    ...
    """
    GIVEN AP0 pack + APPBASE apps
    WHEN ASK→EVAL→FIX×10 dual-arm per surface + DEPL-AP
    THEN expose LOOKUP|GENERATE ∧ DEPL → PROMOTE|HOLD|KILL.
    """
    if len(AP0_PACK) != APPBASE_N:
    ...
    bank = load_bank_rows(bank_path)
    apps = [dict(a) for a in APPBASE_APPS]
    if only_app:
        apps = [app_by_id(only_app)]

    # Shared GENBASE peak GENERATE once — max HW without 3× CUDA reload.
    gen_items = [dict(p) for p in AP0_PACK]
    ...
    summary: dict[str, Any] = {
        ...
        "public_note": "docs/results/nano-lm/formal-happbase-appbase.md",
