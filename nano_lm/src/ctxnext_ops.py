"""Wave AM2 H-CTXNEXT: deca-doc beyond CTXFRESH; dual-arm Cursor EVAL."""
...
CTXNEXT_ID = "H-CTXNEXT"
CTXNEXT_N = 10
MIN_LOOKUP_USABLE = 7
MIN_GEN_USABLE = 5
MIN_SOURCES = 10  # deca-doc beyond CTXFRESH nona
# Deeper than CTXFRESH K=19.
TOP_K_SLICES_NEXT = 21
# Evidence: docs/results/nano-lm/formal-hctxfresh-ctxfresh.md
CTXFRESH_MEAN_LEFF = 200344.0

# Primary AM0 source_id → nine companions (all ten incl. primary distinct).
CTXNEXT_COMPANIONS: Mapping[
    str, tuple[str, str, str, str, str, str, str, str, str]
] = {
    "bip-0039": (
        "bip-0032",
        "bip-0141",
...
def companions_for(
    ...
    """
    GIVEN AM0 primary source_id
    WHEN resolving CTXNEXT companions
    THEN return nine distinct companion ids.
    """
    sid = str(source_id)
    if sid not in CTXNEXT_COMPANIONS:
        raise KeyError(f"no CTXNEXT companions for {sid}")
    return CTXNEXT_COMPANIONS[sid]


def ctxnext_doc_meta(
    doc_token_ids: Sequence[Sequence[int]],
    question_ids: Sequence[int],
    *,
    source_ids: Sequence[str],
    k: int = TOP_K_SLICES_NEXT,
) -> dict[str, Any]:
    """
    GIVEN ten curated docs + question tokens
    WHEN building CTXNEXT context
    THEN deca multi-slice metas with n_sources≥10 and L_eff vs CTXFRESH.
    """
    if len(doc_token_ids) != MIN_SOURCES or len(source_ids) != MIN_SOURCES:
        raise ValueError(f"CTXNEXT requires {MIN_SOURCES} docs/sources")
    if len(set(source_ids)) != MIN_SOURCES:
        raise ValueError("CTXNEXT sources must be distinct")
    parts: dict[str, Any] = {}
    combined_l = 0
    combined_slices = 0
    ...
    return {
        ...
        **{f"{k}_source": parts[f"{k}_source"] for k in _DOC_KEYS},
        ...
        "multi_source": True,
        "above_ctxfresh_leff": float(combined_l) > float(CTXFRESH_MEAN_LEFF),
        "deeper_than_ctxfresh_k": int(k) > int(TOP_K_SLICES_FRESH),
    }
...
def score_ctxnext_lookup(
    *,
    ...
    """LOOKUP arm + deca-doc — labeled LOOKUP ≠ gen IQ."""
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        (
            f"CTXNEXT deca L_eff={meta.get('l_eff')} "
    ...
    if not ctx_ok and not err:
        return score, True, notes + ["FIX: deca-doc/ctx gate failed"], False
    return float(score), bool(err), notes, bool(usable)


def score_ctxnext_gen(
    *,
    completion: str,
    expected_gold: str,
    meta: Mapping[str, Any],
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        (
            f"CTXNEXT gen long-ctx L_eff={meta.get('l_eff')} "
...
def ctxnext_stats(
    *,
    ...
    """Summarize AM2: L_eff↑ vs CTXFRESH · deca · gen usable≥5."""
    if len(lookup_scores) != CTXNEXT_N or len(gen_scores) != CTXNEXT_N:
        raise ValueError(f"CTXNEXT requires {CTXNEXT_N} dual-arm scores")
    l_mean = float(sum(lookup_scores) / float(CTXNEXT_N))
    g_mean = float(sum(gen_scores) / float(CTXNEXT_N))
    ...
    return {
        ...
        "pass_leff_up": float(mean_l_eff) > float(CTXFRESH_MEAN_LEFF),
        "pass_active": float(mean_active) <= float(ACTIVE_CAP),
        "pass_slices": float(mean_slices) >= float(TOP_K_SLICES_NEXT),
...
def decide_ctxnext(stats: Mapping[str, Any]) -> str:
    """
    GIVEN CTXNEXT stats
    WHEN applying pesquisa §3 AM2 gate
    THEN PROMOTE if L_eff↑ vs CTXFRESH ∧ deca ∧ gen usable≥5 ∧ lookup ok;
