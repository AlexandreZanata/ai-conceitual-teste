"""Render H-EARS smoke vs H-EARLY tip."""
...
def render(smoke_path: Path, early_path: Path) -> str:
    ...
    s = stats.get("H-EARS", {})
    decision = decide_hears(s, stats) if s else "needs H-EARS rows"
...
def main() -> int:
    ...
    p.add_argument("--smoke", type=Path, default=root / "ears_smoke.json")
    p.add_argument("--early", type=Path, default=root / "early_smoke.json")
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hears-vs-hearly.md"),
    )
    args = p.parse_args()
