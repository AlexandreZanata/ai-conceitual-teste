"""Wave AK3 H-SMARTMORE: octa-hop cite + GENTRUE peak gen; kill SEMWRAP FPs."""
...
from ctxmore_ops import companions_for
...
SMARTMORE_ID = "H-SMARTMORE"
SMARTMORE_N = 10
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0  # pesquisa §3 AK3 — or honest HOLD
MIN_CITE_OK = 8
# Parent Wave AI SMARTPUSH open-gen mean under dual-arm Cursor EVAL.
SMARTPUSH_GEN_MEAN = 4.0
# Peer Wave AJ SMARTPEAK peak-overlay mean (extractive — labeled).
SMARTPEAK_GEN_MEAN = 9.0
...
# Octa-hop adversarial paraphrases of AK0 (CTXMORE companions as distractors).
SMARTMORE_PACK: tuple[dict[str, str], ...] = (
    _row(
        pack_i=0,
        paraphrase=(
            "skip BIP-32 chain-code + SegWit marker + Schnorr + TLS + CBOR + "
            "BIP-1 + BIPs index — BIP-39 allowed ENT size range bits "
            "(low-high)??"
        ),
    ),
    _row(
        pack_i=1,
        paraphrase=(
            "ignore BIP-39 ENT range + P2WSH marker + Schnorr + TLS + CBOR + "
            "BIP-1 + JSON-RPC — BIP-32 chain code: how many bytes??"
        ),
    ),
    _row(
        pack_i=2,
        paraphrase=(
            "not BIP-39 ENT + not BIP-32 chain + not Schnorr + not TLS + not "
            "CBOR + not BIPs index + not BIP-1 — BIP-141 witness marker hex??"
        ),
    ),
    _row(
        pack_i=3,
        paraphrase=(
            "forget getattr + break + intro + open() + IHL + Rust bool + "
            "ownership — remove all items from list a, one method call??"
        ),
    ),
    _row(
        pack_i=4,
        paraphrase=(
            "skip getattr + clear() + intro + io + IHL + Rust ch03 + "
            "structs — statement that breaks out of the innermost loop??"
        ),
    ),
    _row(
        pack_i=5,
        paraphrase=(
            "ignore clear() + break + intro + io + IHL + field access + "
            "Rust types — built-in that fetches a named attribute "
            "(with setattr/delattr)??"
        ),
    ),
    _row(
        pack_i=6,
        paraphrase=(
            ...
            "not TLS + not Python intro + not control — Rust Boolean "
            "type name??"
        ),
    ),
    _row(
        pack_i=7,
        paraphrase=(
            "skip bool type + ownership + variables + CBOR + TLS + "
            "getattr + intro — Rust struct field access notation??"
        ),
    ),
    _row(
        pack_i=8,
        paraphrase=(
            ...
            "BIP-32 + TLS + CBOR + BIP-1 — REST GET path for mempool "
            "info JSON??"
        ),
    ),
    _row(
        ...
        paraphrase=(
            ...
            "not JSON-RPC + not BIP-1 + not REST mempool — RFC 791 Version "
            "field how many bits??"
        ),
    ),
)
...
def hard_paraphrase_ok(
    ...
    rows = pack if pack is not None else SMARTMORE_PACK
    if len(rows) != SMARTMORE_N:
        return False
    for item in rows:
...
def has_octa_hop_cues(
    pack: Sequence[Mapping[str, str]] | None = None,
) -> bool:
    """
    GIVEN SMARTMORE pack
    WHEN checking octa-hop distractor wiring
    THEN True iff every row maps CTXMORE companions and ≥8 carry cues.
    """
    cues = (
        "bip-39",
        "bip-32",
...
def score_smartmore_lookup(
    *,
    ...
    notes = list(notes) + [
        ...
        "SMARTMORE LOOKUP octa paraphrase cite — not generative IQ",
...
def score_smartmore_gen(
    *,
    ...
    """
    GIVEN GENERATE arm (GENTRUE extractive peak, labeled)
    WHEN Cursor EVAL completion vs gold
    THEN gentrue peak rubric; peak ≠ open-chat IQ label.
    """
    ...
    notes = list(notes) + [
        "SMARTMORE gen GENTRUE peak (extractive) — Cursor scores completion",
        f"beat SMARTPUSH gen={SMARTPUSH_GEN_MEAN} / peer SMARTPEAK="
        f"{SMARTPEAK_GEN_MEAN}",
    ]
...
def smartmore_stats(
    *,
    ...
    if len(lookup_scores) != SMARTMORE_N or len(gen_scores) != SMARTMORE_N:
        raise ValueError(f"SMARTMORE requires {SMARTMORE_N} dual-arm scores")
    l_mean = float(sum(lookup_scores) / float(SMARTMORE_N))
    g_mean = float(sum(gen_scores) / float(SMARTMORE_N))
    ...
    return {
        ...
        "smartpeak_gen_mean": SMARTPEAK_GEN_MEAN,
...
def decide_smartmore(stats: Mapping[str, Any]) -> str:
    """
    GIVEN SMARTMORE dual-arm stats
    WHEN applying pesquisa §3 AK3 gate
    THEN KILL if false-hit; PROMOTE if lookup+cite+gen≥5; else HOLD.
    """
    if int(stats.get("n_false_hit", 0)) > 0:
        return "KILL"
    if not bool(stats.get("dual_arm")) or not bool(stats.get("octa_hop")):
        return "KILL"
