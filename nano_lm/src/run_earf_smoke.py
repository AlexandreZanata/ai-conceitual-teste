"""Smoke H-EARF: FLOP-aware early search vs frozen H-EARLY tip genes."""
...
from hyp_earf import run_h_earf
...
def _score_tip(
    ...
    return _row("H-EARLY", f"HEARLY_earf_seed{seed}", lp, wall, gf, seed, gene)
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        rows.append(_score_tip(out, teacher, prompts, max_new, seed))
        ckpt = out / f"B2_seed{seed}.pt"
        meta = run_h_earf(
            student_ckpt=ckpt,
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=out / f"HEARF_seed{seed}_train.json",
        )
        row = _row(
            "H-EARF",
            f"HEARF_seed{seed}",
        ...
        write_json(out / f"HEARF_seed{seed}_eval.json", row)
        rows.append(row)
    write_json(
        out / "earf_smoke.json",
        {"rows": rows, "wall_s": time.perf_counter() - t0},
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "earf_smoke.json")}))
    return 0
