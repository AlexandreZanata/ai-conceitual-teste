"""Wave BF1 H-PREDINT: BF-FOREVER FH 0 · BA…BE/AZ hold · live board."""
...
PREDINT_ID = "H-PREDINT"
PREDINT_THESIS = (
    "Drive BF-FOREVER held-out FH → 0 (predicate/boolean even≠add · "
    "paraphrases · predicate/schema neighbors) via predicate SEMWRAP "
    "predicate/schema gate — not bank stuffing; hold BA…BE-FOREVER + AZ "
    "div·sub·BIP FH 0 + a.clear() LOOKUP; live ask scoreboard "
    "OK|FP|MISS|ABSTAIN-OK (≥10 novel); modes · p50/p99 · DECODE law"
)
PREDINT_CLAIM = BF0_SHIP_LOCK
PREDINT_SAFE_NOTE = BF0_SAFE_NOTE
PREDINT_ANTI_FP = BF0_ANTI_FP
...
NOVEL_PROBES: tuple[dict[str, str], ...] = (
    {
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Python: is_odd(n) returning a boolean for odd integers."
        ),
    },
    {
        "id": "BF-NOVEL-02",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Does integer x have even parity? Answer True or False only."
        ),
    },
    {
        "id": "BF-NOVEL-03",
        ...
        "expect_mode": "ABSTAIN",
        "question": "Return bool: is k an even integer?",
    },
    {
        "id": "BF-NOVEL-04",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Boolean indicating whether value n is positive in Python."
        ),
    },
    {
        "id": "BF-NOVEL-05",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Write a function that returns True if x equals zero."
        ),
    },
    {
        "id": "BF-NOVEL-06",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "How do I test if integer z is divisible by 2 (boolean result)?"
        ),
    },
    {
        "id": "BF-NOVEL-07",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Implement a helper that returns whether n is odd."
        ),
    },
    {
        "id": "BF-NOVEL-08",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Python predicate: is a greater than b? return bool only."
        ),
    },
    {
        "id": "BF-NOVEL-09",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Boolean: does integer a leave remainder 0 mod 2?"
        ),
    },
    {
        "id": "BF-NOVEL-10",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Return True iff p and q have same parity."
        ),
    },
)
...
def bars_from_scoreboard(
    ...
    """
    GIVEN BE0 §1 scoreboard
    WHEN reading bars
    THEN return typed bar dict for SEMINT gate.
    """
    src = suite if suite is not None else BF0_SCOREBOARD
    bars = src.get("bars")
...
def score_live_row(row: Mapping[str, Any], *, expect_mode: str) -> str:
    ...
    """
    mode = str(
        row.get("product_mode") or map_bf_product_mode(str(row.get("mode", "")))
    )
...
def extract_predint_board(
    *,
    ...
    """
    GIVEN BF-FOREVER + BA…BE hold + AZ hold + over-refuse + live scores
    WHEN building PREDINT board
    THEN publish BF FH · BA/BB/BC/BD hold · AZ hold · over-refuse · live.
    """
...
def _gate_core_flags(board: Mapping[str, Any], bar: Mapping[str, Any]) -> str | None:
    ...
    if not bool(bar.get("ba_bb_bc_bd_be_pass_neq_bf_forever", True)):
        return "KILL (BA…BE PASS ≠ BF forever bar missing)"
...
def decide_predint(
    *,
    ...
    WHEN applying pesquisa §9 BF1
    THEN PROMOTE iff BE FH 0 + BA…BE hold 0 + AZ hold 0 + over-refuse 0
         + live FP 0.
    """
    ...
    err = (
        _gate_forever_pack(
            ...
            bar=bar,
            key="bf_forever",
            min_n=int(bar.get("bf_forever_min_n", 12)),
    ...
    if err:
        return err
    return (
        f"PROMOTE ({PREDINT_ID}: BF-FOREVER FH 0; BA…BE hold 0; "
