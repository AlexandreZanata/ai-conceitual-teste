"""Wave AI6 AI-HITL-10 runner: final dual-arm verify (nano:ai:hitl)."""
...
from ai_session_ops import AI0_PACK, overlaps_prior_questions, pack_app_counts
...
_SUMMARY = REPO / "results/nano-lm/wave-ai/ai_hitl_summary.json"
...
def run_ai_hitl(
    *,
    ...
    """
    GIVEN frozen AI0 held-out asks
    WHEN final LOOKUP + grounded GENERATE ASK→EVAL→FIX×10
    THEN lookup≥7; gen≥5 → PROMOTE else documented HOLD.
    """
    if len(AI0_PACK) != AI6_N:
    ...
    if len(lookup_payloads) != AI6_N or len(gen_payloads) != AI6_N:
    ...
    for i, (item, lp, gp) in enumerate(
        zip(AI0_PACK, lookup_payloads, gen_payloads, strict=True),
        start=1,
    ):
        ...
        l_score, l_err, l_notes = score_ai6_lookup(
        ...
        lt = {
            "trial_id": f"AI-FINAL-LOOKUP-HITL-{i:02d}",
            "stage": "AI6",
            "hyp_id": AI6_ID,
            "arm": "LOOKUP",
            "app_id": item["app_id"],
        ...
        if g_tel["wall_ms"] > 0.0 and g_tel["n_new"] > 0:
            n_gen_wall_ok += 1
        g_score, g_err, g_notes = score_ai6_gen(
            completion=str(gp.get("completion", "")),
            expected_gold=str(item["gold"]),
            payload=gp,
        )
        gt = {
            "trial_id": f"AI-FINAL-GEN-HITL-{i:02d}",
            "stage": "AI6",
            "hyp_id": AI6_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
