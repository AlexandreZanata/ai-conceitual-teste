"""Render H-LS smoke vs B2 markdown."""
...
def render(smoke_path: Path, matrix_path: Path) -> str:
    ...
    kept = [r for r in matrix.get("rows", []) if r.get("family") != "H-LS"]
    stats = mean_by_family(kept + smoke["rows"])
    s = stats.get("H-LS", {})
    decision = decide_hls(s, stats) if s else "needs H-LS rows"
    b2 = stats.get("B2", {})
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/ls_smoke.json"),
    )
    p.add_argument(
        "--matrix",
        type=Path,
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hls-vs-b2.md"),
    )
    args = p.parse_args()
