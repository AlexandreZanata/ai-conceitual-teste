"""Formal H-ETRAIN: PRE3 e2e vs live H-STAG (formal KD budget)."""
...
from etrain_pair import run_seed_pair
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hetrain"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-ETRAIN formal requires CUDA")
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        rows.extend(
            run_seed_pair(
                c, out, seed, device, vocab, steps, label_prefix="HETRAIN_formal"
            )
        )
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "steps": steps,
            "top_k": DEFAULT_TOP_K,
            "mode": "PRE3 e2e (cache_build+train) vs live STAG",
        },
    )
