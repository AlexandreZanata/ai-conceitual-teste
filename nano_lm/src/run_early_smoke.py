"""Smoke H-EARLY on B2 ckpts; dual gate vs B4 (matrix)."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_early(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=out / f"HEARLY_seed{seed}_train.json",
        )
        row = {
            "family": "H-EARLY",
        ...
        write_json(out / f"HEARLY_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "early_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "early_smoke.json")}))
    return 0
