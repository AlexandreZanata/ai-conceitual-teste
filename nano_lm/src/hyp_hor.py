"""H-HOR: FLOP-aware search; tip n frozen; horizon ≤ 2."""
...
from hor_ops import clamp_hor_gene, mutate_hor_gene, seed_hor_from_tip
...
def _init_pop(rng: random.Random, tip: dict[str, Any], pop_size: int) -> list[dict]:
    frozen_n = int(tip["n"])
    pop = [clamp_hor_gene(tip, frozen_n)]
    while len(pop) < pop_size:
        pop.append(seed_hor_from_tip(tip, rng))
    return pop
    ...
    *,
    ...
    for gen in range(generations):
        ...
        history.append(
            {
                ...
                "best_n": frozen_n,
                "best_horizon": int(pop[ranked[0]]["horizon"]),
        ...
        if scores[ranked[0]] > best_score:
            ...
            best_gene = clamp_hor_gene(pop[ranked[0]], frozen_n)
        parents = [pop[i] for i in ranked[: max(1, pop_size // 2)]]
        pop = [
            mutate_hor_gene(parents[i % len(parents)], frozen_n, rng)
