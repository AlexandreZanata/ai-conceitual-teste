"""Wave AJ4 H-FASTPEAK runner: peak-fast gen wall vs AI FASTPUSH."""
...
_SUMMARY = REPO / "results/nano-lm/wave-aj/fastpeak_summary.json"
_CHAMPION = REPO / "results/nano-lm/wave-z/models/champion"
_CURATED = REPO / "nano_lm/data/curated"
_JUDGE = "cursor-composer-frontier-chat"
_BY_ID = {str(s["id"]): s for s in SOURCES}
# Extra hot rounds — peak speed under max safe HW.
_GEN_HOT_ROUNDS = 12
...
def _seed_pack(bank_path: Path, aj_bank: Path) -> int:
    ...
    for i, item in enumerate(AJ0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AJ-FASTPEAK-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = FASTPEAK_ID
        row["judge_notes"] = [
            "FASTPEAK seed for AJ held-out ask",
...
def _timed_gen(
    *,
    ...
    """GENERATE arm: peak-extractive (wall_ms>0, no LOOKUP cache)."""
    ...
    *,
    ...
    for _ in range(max(1, rounds)):
        ...
        if wall < best_wall or (wall == best_wall and e2e < best_e2e):
            ...
            best = payloads
    if best is None:
        raise RuntimeError("FASTPEAK hot gen produced no payloads")
...
def _lookup_trial(
    *,
    ...
    return {
        "trial_id": f"AJ-FASTPEAK-LOOKUP-HITL-{i:02d}",
        "stage": "AJ4",
        "hyp_id": FASTPEAK_ID,
        "arm": "LOOKUP",
        "app_id": item["app_id"],
...
def _gen_trial(
    *,
    ...
    tel = extract_telemetry(payload)
    return {
        "trial_id": f"AJ-FASTPEAK-GEN-HITL-{i:02d}",
        "stage": "AJ4",
        "hyp_id": FASTPEAK_ID,
        "arm": "GENERATE",
        "app_id": item["app_id"],
...
def run_fastpeak(
    *,
    ...
    """
    GIVEN AJ0 held-out asks
    WHEN LOOKUP quality + peak-fast GENERATE cold/warm/hot (wall_ms>0)
    THEN dual-arm FASTPEAK vs AI FASTPUSH hot → PROMOTE|HOLD|KILL.
    """
    if len(AJ0_PACK) != FASTPEAK_N:
        raise ValueError("AJ0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack(bank_path, aj_bank)
    bank = load_bank_rows(bank_path)
    items = [dict(p) for p in AJ0_PACK]
    ...
    for i, (item, payload) in enumerate(
        ...
        if kind != "TRUE_HIT":
            row = alias_bank_row(
                trial_id=f"AJ-FASTPEAK-FIX-{i:02d}",
                question=item["question"],
                source_id=item["source_id"],
                gold=item["gold"],
            )
    ...
    decision = decide_fastpeak(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "GENERATE PEAK_FAST+EXTRACTIVE (no student decode)",
            f"cold+warm+hot gen timing (hot rounds={_GEN_HOT_ROUNDS})",
            f"vs FASTPUSH hot {FASTPUSH_HOT_WALL_MS:.3f} ms",
        ],
        "forbidden": [
            "STREAM",
            "KVCACHE-Q",
        ...
        "timing": {
            ...
            "fastpush_hot_wall_ms": FASTPUSH_HOT_WALL_MS,
        ...
        "finding": (
            f"{FASTPEAK_ID}: gen wall "
            f"{cold_wall:.1f}/{warm_wall:.1f}/{hot_wall:.1f}ms "
            f"(FASTPUSH hot {FASTPUSH_HOT_WALL_MS:.1f}) "
            f"e2e {cold_e2e:.0f}/{warm_e2e:.0f}/{hot_e2e:.0f} "
            f"L={stats['lookup_mean']:.1f} G={stats['gen_mean']:.1f} "
            f"wall_ok={n_gen_wall_ok}/10 "
            f"vs_fp={stats['pass_vs_fastpush']} → {decision}"
        ),
        "public_note": "docs/results/nano-lm/formal-hfastpeak-fastpeak.md",
        "ship_claim": "AF packaged stack until AJ6 gen bar",
        "claim": (
            "faster peak-extractive gen vs FASTPUSH with wall_ms>0 — "
