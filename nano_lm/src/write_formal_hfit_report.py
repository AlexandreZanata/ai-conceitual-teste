"""Render formal H-FIT vs B2 comparison markdown."""
...
def render(path: Path) -> str:
    ...
    decision = decide_formal_vs_b2("H-FIT", stats)
    delta = stats["H-FIT"]["lp"] - stats["B2"]["lp"]
    overfit = "yes" if stats["H-FIT"].get("overfit", 0.0) > 0.0 else "no"
    lines = [
        ...
        f"{int(stats['B2']['n'])} |",
        f"| H-FIT | {stats['H-FIT']['lp']:.4f} | {delta:+.4f} | "
        f"{stats['H-FIT']['wall']:.0f} | {overfit} | {int(stats['H-FIT']['n'])} |",
        "",
        f"**Decision:** {decision}",
        "",
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hfit-b2/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hfit-vs-b2.md"),
    )
    args = p.parse_args()
