"""Render formal H-CAP vs H-POOL."""

from __future__ import annotations

import argparse
import json
from pathlib import Path

from cap_ops import decide_hcap
from formal_ops import means_by_family


def render(path: Path) -> str:
    data = json.loads(path.read_text(encoding="utf-8"))
    fam = means_by_family(data["rows"])
    stats = {
        name: {"mean_lp": st["lp"], "mean_wall": st["wall"], "n": st["n"]}
        for name, st in fam.items()
    }
    decision = decide_hcap(stats.get("H-CAP", {}), stats)
    tip = stats.get("H-POOL", {})
    hyp = stats.get("H-CAP", {})
    d_lp = hyp.get("mean_lp", float("nan")) - tip.get("mean_lp", float("nan"))
    lines = [
        "# Formal H-CAP vs H-POOL (hard max_new/n caps)",
        "",
        f"Source: `{path}`",
        f"Wall clock: {data.get('wall_s', float('nan')):.1f}s",
        "",
        "Formal POOL tip genes; search max_new∈{8,12,16} with n≤2. Fit≠eval.",
        "Kill if quality < POOL−ε or no wall save vs H-POOL.",
        "",
        "| family | mean teacher_lp | mean wall_ms | Δ lp vs POOL | n |",
        "|--------|-----------------|--------------|--------------|---|",
    ]
    for name in ("H-POOL", "H-CAP"):
        if name not in stats:
            continue
        st = stats[name]
        d = "—" if name == "H-POOL" else f"{d_lp:+.4f}"
        lines.append(
            f"| {name} | {st['mean_lp']:.4f} | {st['mean_wall']:.0f} | {d} | "
            f"{int(st['n'])} |"
        )
    lines.extend(
        [
            "",
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:hcap` → "
            "`npm run nano:formal:hcap:report`.",
            "",
        ]
    )
    return "\n".join(lines)


def main() -> int:
    p = argparse.ArgumentParser()
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hcap/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hcap-vs-hpool.md"),
    )
    args = p.parse_args()
    text = render(args.formal)
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(text, encoding="utf-8")
    print(f"wrote {args.out}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
