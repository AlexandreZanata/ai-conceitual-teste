"""Formal-budget H-CURL: seq_lo∈{8,16,32} equal KD steps."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hcurl",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 4,
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        for seq_lo in CURL_LOS:
            ckpt = c["out"] / f"HCURL_lo{seq_lo}_seed{seed}.pt"
            meta = run_h_cur(
            ...
            write_json(
                c["out"] / f"HCURL_lo{seq_lo}_seed{seed}_train.json", meta
            )
