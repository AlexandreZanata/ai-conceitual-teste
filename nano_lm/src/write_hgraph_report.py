"""Render H-GRAPH smoke vs H-LAYB (CUDA graph under LAYB decode)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Same dual-budget LAYB path on B2; non-KV arm uses full-depth per-T "
        "CUDAGraph replay (capture untimed). KV arm stays CHBAT. "
        "Kill if |Δlp| > ε vs H-LAYB or no wall win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/graph_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hgraph-vs-hlayb.md"),
    )
    args = p.parse_args()
