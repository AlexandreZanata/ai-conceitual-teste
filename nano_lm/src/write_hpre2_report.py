"""Render H-PRE2 smoke vs H-ADAMF (2-deep prefetch under ADAMF)."""
...
def render(path: Path) -> str:
    ...
    s = stats.get("H-PRE2", {})
    decision = decide_hpre2(s, stats) if s else "needs H-PRE2 rows"
    ...
    lines = [
        ...
        "",
        "Same top-k soft cache, STAG curriculum, HALF wire, and fused AdamW; "
        "only prefetch_depth differs (2 vs 1). Kill if |Δlp| > ε or no ms/step win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/pre2_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hpre2-vs-hadamf.md"),
    )
    args = p.parse_args()
