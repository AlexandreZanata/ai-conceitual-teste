"""Wave AN0 SESSION runner (nano:an:session) — freeze held-out HITL×10."""
...
_OUT = REPO / "results/nano-lm/wave-an/an0_session.json"
_TRIALS = REPO / "results/nano-lm/wave-an/trials"
_ERROR_BANK = REPO / "results/nano-lm/wave-an/error_bank.jsonl"
_CURATED = REPO / "nano_lm/data/curated"
_PUBLIC = REPO / "docs/results/nano-lm/wave-an-session.md"
...
def _write_public_note() -> None:
    rows = "\n".join(
        f"| {p['id']} | {p['app_id']} | {p['source_id']} |" for p in AN0_PACK
    )
    body = "\n".join(
        [
            ...
            "`.local/wave-an/SESSION.md`  ",
            "> Module: `nano_lm/src/an_session_ops.py` · "
            "Runner: `npm run nano:an:session`  ",
            "> Parent: [am-freeze.md](am-freeze.md) "
            "(Wave AN reopened explicitly via lab-book reopen 2026-07-27)",
            "",
            "## Decision",
            "",
            "**PROMOTE** — Freeze **10 held-out HITL questions** "
            "(source_id + app_id + gold) for every AN model/stack.  ",
            "Questions are **not** verbatim copies of AB…AM HITL-01…10.  ",
            "Topics: BIP-39 ENT=192 → 18 words · BIP-32 child-number "
            "bytes · BIP-141 witnessScript ≤10000 · `a.remove(x)` · "
            "`range` · `__dict__` · Rust tuples/arrays · tuple structs · "
            "REST headers · RFC 791 Total Length bits.",
            "",
            "## Mix",
            "",
            "| app_id | Count | Trials |",
            ...
            "`results/nano-lm/wave-an/an0_session.json` · "
            "`results/nano-lm/wave-an/trials/AN-HITL-*.json` · "
            "`error_bank.jsonl`.  ",
            "Contract: `nano_lm/tests/test_an_session.py`.",
            "",
            "## Claims",
            "",
            "- Held-out scoped app assist HITL set (13th pack) — "
            "**not** open chat LM.  ",
            "- Default ship claim until proven otherwise still "
            "**AF packaged stack**.  ",
            "- Forbidden: QI · STREAM · KVCACHE-Q · GENCACHE · MIXD · "
            "GPFB-K=2 · naive CTX · ZPREF · invent Wave AO · "
            "PROMOTE LOOKUP-only as generative IQ.",
            "",
            "Next: **AN1 H-GENEDGE** — smarter usable gen + dual-arm + "
            "ablation gate.",
            "",
        ]
    )
...
def _override_decision(
    ...
    *,
    ...
    if clash:
        return f"KILL (verbatim AB…AM questions: {','.join(clash)})"
    if not curated_ok:
...
def main() -> int:
    ...
    ids = [str(p["source_id"]) for p in AN0_PACK]
    ...
    trials_ready = Path(args.trials_dir).is_dir() and len(written) == AN0_N
    _write_public_note()

    decision = _override_decision(
        decide_an0_session(known_sources=known, trials_dir_ready=trials_ready),
        miss=miss,
        clash=clash,
    ...
    payload = {
        ...
        "ask_smoke": ask,
        "public_note": "docs/results/nano-lm/wave-an-session.md",
        "rule": "pesquisa §3 AN0 · Cursor ASK→EVAL→FIX dual-arm law",
        "next": "AN1 H-GENEDGE (smarter gen + ASK dual-arm + ablation)",
        "anti_fp": (
