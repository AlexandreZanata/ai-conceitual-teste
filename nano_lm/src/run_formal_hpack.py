"""Formal H-PACK: SERVE + SROUTE packs vs EARLY (eval, fit≠eval)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hpack"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-PACK formal requires CUDA")
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        rows.extend(run_seed_trio(c, seed, prompts, claim_offset=9898))
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "mode": "SERVE=min-wall + SROUTE=Pareto vs EARLY",
        },
    )
