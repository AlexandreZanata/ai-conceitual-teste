"""Wave BC REPORT runner: public closeout + BC-FOREVER/modes live smoke."""
...
from bc_real_eval_ops import battery_row_ok
...
from bc_session_ops import BC0_ASK_BATTERY
from matrix_common import REPO, write_json
from tipd_pair import tune_cpu_threads

_OUT = REPO / "results/nano-lm/wave-bc/bc_report_summary.json"
_SUMMARY = REPO / "docs/results/nano-lm/wave-bc-summary.md"
_PAPER = REPO / "docs/results/nano-lm/paper-lab-wave-bc.md"
_FREEZE_STUB = REPO / "docs/results/nano-lm/bc-freeze.md"
_FORMAL_FREEZE_STUB = REPO / "docs/results/nano-lm/formal-habcfreeze-bc-freeze.md"
...
# Compact live smoke: LOOKUP · OOD · BC-FOREVER · over-refuse (max HW, low RAM).
_SMOKE_IDS = frozenset(
    {"BC-ASK-01", "BC-ASK-02", "BC-ASK-07", "BC-ASK-08"}
)
...
def _evidence_map() -> dict[str, bool]:
    return {p: (REPO / p).is_file() for p in BC_EVIDENCE}
...
def _stage_facts() -> dict[str, Any]:
    keys = {
        "session": "results/nano-lm/wave-bc/bc0_session.json",
        "opsfam": "results/nano-lm/wave-bc/opsfam_summary.json",
        "fastlift": "results/nano-lm/wave-bc/bc_fastlift_summary.json",
        "ctxlift2": "results/nano-lm/wave-bc/bc_ctxlift2_summary.json",
        "nanogen13": "results/nano-lm/wave-bc/nanogen13_summary.json",
        "real_eval": "results/nano-lm/wave-bc/bc_real_eval_summary.json",
    }
    out: dict[str, Any] = {}
...
def _smoke_bc_modes(*, workers: int) -> dict[str, Any]:
    """LOOKUP · OOD · BC-FOREVER FP · over-refuse via BC ask path."""
    from run_bc_real_eval import _ask_row

    items = [dict(p) for p in BC0_ASK_BATTERY if p["id"] in _SMOKE_IDS]
    ...
    return {
        "ok": ok,
        "decision": "PROMOTE" if ok else "KILL (BC forever/modes smoke)",
...
def _write_stub(path: Path, title: str) -> None:
    ...
    path.write_text(
        "\n".join(
            [
                f"# {title} — placeholder (pending BC7)",
                "",
                "> Written by BC6 report so paper-lab links resolve. "
                "BC7 replaces this with the formal freeze.",
                "",
...
def _ensure_freeze_stubs() -> None:
    _write_stub(_FREEZE_STUB, "BC-FREEZE")
    _write_stub(_FORMAL_FREEZE_STUB, "formal-habcfreeze-bc-freeze")
...
def _update_local_session(decision: str) -> None:
    ...
    body = "\n".join(
        [
            f"# Wave BC session checklist (**OPEN** · BC6 {status})",
            "",
            ...
            "## Current stage",
            "",
            f"**BC6 — BC-REPORT ({status})** · Next: **BC7 BC-FREEZE**",
            "",
            "| Field | Value |",
            "|-------|--------|",
            "| Wave | **BC OPEN** (RESEARCH_COMPLETE pending FREEZE) |",
            f"| Decision | **{decision}** |",
            "| Public | `docs/results/nano-lm/wave-bc-summary.md` |",
            "| Paper-lab | `docs/results/nano-lm/paper-lab-wave-bc.md` |",
            "",
            "## Stage board",
            "",
            "| Stage | ID | Status |",
...
**BC0–BC3 PROMOTE · BC4 DEFER · BC5–BC6 DONE — {status}**. Next: **BC7 BC-FREEZE**.
...
**Wave BC ACTIVE** — BC6 **BC-REPORT {status}** (gen locked).
...
def _patch_pesquisa(decision: str) -> None:
    ...
    text2, n = re.subn(
        r"\| BC6 \| \*\*BC-REPORT\*\* \|[^\n]+\| \*\*NEXT\*\* \|",
        (
            "| BC6 | **BC-REPORT** | Summary + paper-lab "
    ...
    text = text.replace(
        ...
        "(BC5 BC-REAL-EVAL **DONE — PROMOTE**; next BC6 BC-REPORT).  ",
        "> **Session:** `.local/wave-bc/SESSION.md` "
        f"(BC6 BC-REPORT **DONE — {status}**; next BC7 BC-FREEZE).  ",
        1,
    )
    if "# next: nano:bc:report" in text:
        text = text.replace(
            "# next: nano:bc:report\n# npm run nano:bc:report\n",
            "npm run nano:bc:report\n# next: nano:bc:freeze\n",
            1,
...
def _bc_active_line(status: str) -> str:
    return (
        "BC0 [SESSION PROMOTE](wave-bc-session.md) · "
        ...
        f"BC6 [BC-REPORT {status}](wave-bc-summary.md) "
        "(`npm run nano:bc:report`) · [paper-lab-wave-bc.md]"
        "(paper-lab-wave-bc.md); next BC7 BC-FREEZE; ship remains "
...
def _patch_recipes(status: str) -> None:
    ...
    line = f"**Wave BC ACTIVE:** {_bc_active_line(status)}."
    text2, n = re.subn(
        r"\*\*Wave BC ACTIVE:\*\*[^\n]*",
        line,
        text,
    ...
    if n:
        text = text2
    if "Wave BC6 BC-REPORT" not in text:
        needle = (
            "| Wave BC5 BC-REAL-EVAL | [wave-bc-real-eval.md]"
            "(wave-bc-real-eval.md) **PROMOTE** "
            "(`npm run nano:bc:real-eval`) — battery 13/13 · "
            "gen locked (BC4 DEFER) |\n"
        )
        row = (
        ...
        elif "| Wave BC5 BC-REAL-EVAL |" in text:
            text2, n2 = re.subn(
                r"(\| Wave BC5 BC-REAL-EVAL \|[^\n]+\|\n)",
                rf"\1{row}",
                text,
                count=1,
            )
...
def _patch_agents(status: str) -> None:
    ...
    agents = (
        ...
        "(`npm run nano:bc:report`) · "
        "[paper-lab-wave-bc.md](docs/results/nano-lm/paper-lab-wave-bc.md); "
        "next BC7 BC-FREEZE; ship remains "
        "**AF + AQ + AS trust + STRICT ablated DECODE**; ≤5M stays."
    )
    text2, n = re.subn(
        r"- \*\*Wave BC ACTIVE\*\* —[^\n]+", agents, text, count=1
...
def _patch_agenda(status: str) -> None:
    ...
    row = (
        ...
        f"BC-REAL-EVAL PROMOTE · BC6 BC-REPORT {status} "
        f"(`npm run nano:bc:report`); next BC7 BC-FREEZE; ≤5M |"
    )
    text2, n = re.subn(
        r"\| \*\*BC\*\* \| \*\*ACTIVE\*\* \|[^\n]+", row, text, count=1
    )
...
def _patch_evogen(status: str) -> None:
    ...
    text = text.replace(
        (
            "Wave BC ACTIVE (BC0–BC3 PROMOTE · BC4 DEFER · "
            "BC5 BC-REAL-EVAL PROMOTE; next BC6 BC-REPORT)"
        ),
        (
            f"Wave BC ACTIVE (BC0–BC3 PROMOTE · BC4 DEFER · "
            f"BC5 BC-REAL-EVAL PROMOTE · BC6 BC-REPORT {status}; "
            f"next BC7 BC-FREEZE)"
        ),
        1,
    )
...
def _gates(
    ...
    *,
    ...
    if ask is not None and not bool(ask.get("ok")):
        return "KILL (BC forever/modes smoke failed)"
    if decision.startswith("PROMOTE") and not markers:
        return "KILL (wave-bc-summary missing thesis markers)"
    if decision.startswith("PROMOTE") and not board:
        return "KILL (wave-bc-summary missing scoreboard)"
    if decision.startswith("PROMOTE") and not antifp:
        return "KILL (wave-bc-summary missing anti-FP evidence)"
    if decision.startswith("PROMOTE") and not realeval:
        return "KILL (wave-bc-summary missing real-eval section)"
    return decision
...
def run_bc_report(
    *, out: Path, skip_ask: bool = False, workers: int = 8
    ...
    decision = decide_bc_report(evidence)
    ...
    payload: dict[str, Any] = {
        ...
        "scoreboard": list(BC_SCOREBOARD),
        "evidence": evidence,
        "stage_facts": _stage_facts(),
        "ask_smoke": ask,
        "public_report": "docs/results/nano-lm/wave-bc-summary.md",
        "paper_lab": "docs/results/nano-lm/paper-lab-wave-bc.md",
        "wave_status": "RESEARCH_COMPLETE" if ok else "OPEN",
        "ship_claim": SHIP_CLAIM,
        "cpu_threads": int(os.environ.get("OMP_NUM_THREADS") or 0),
        "workers": int(workers),
        "finding": (
            f"{BC_ID}: decision={final}; "
...
def main() -> int:
    _clear_proxy()
    ap = argparse.ArgumentParser(description="Wave BC6 BC-REPORT")
    ap.add_argument("--out", type=Path, default=_OUT)
    ap.add_argument("--skip-ask", action="store_true")
    args = ap.parse_args()
    threads, workers = _hardware()
    try:
        summary = run_bc_report(
            out=Path(args.out),
