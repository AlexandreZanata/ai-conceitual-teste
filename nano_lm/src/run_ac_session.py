"""Wave AC0 SESSION runner (nano:ac:session) — freeze held-out HITL×10."""
...
_OUT = REPO / "results/nano-lm/wave-ac/ac0_session.json"
_TRIALS = REPO / "results/nano-lm/wave-ac/trials"
_ERROR_BANK = REPO / "results/nano-lm/wave-ac/error_bank.jsonl"
...
def main() -> int:
    ...
    # Max HW without starving desktop: leave ~4 cores free (16→12).
    ...
    clash = overlaps_ab_questions()
    ids = [str(p["source_id"]) for p in AC0_PACK]
    ...
    trials_ready = Path(args.trials_dir).is_dir() and len(written) == AC0_N

    decision = _override_decision(
        decide_ac0_session(known_sources=known, trials_dir_ready=trials_ready),
        miss=miss,
        clash=clash,
    ...
    payload = {
        ...
        "ab_question_overlap": clash,
        "curated_checks": curated_checks,
        "trials_written": written,
        "error_bank": str(_ERROR_BANK.relative_to(REPO)),
        "ask_smoke": ask,
        "public_note": "docs/results/nano-lm/wave-ac-session.md",
        "rule": "pesquisa §8.5 AC0 · §12.2",
        "next": "AC1 H-CTXPLUS (ASK→EVAL→FIX×10)",
    }
