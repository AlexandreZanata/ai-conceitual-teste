"""Formal H-PRE2: 2-deep prefetch under ADAMF vs H-ADAMF (formal KD)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hpre2"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-PRE2 formal requires CUDA")
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        rows.extend(
            run_seed_pair(
                c, out, seed, device, vocab, steps, label_prefix="HPRE2_formal"
            )
        )
