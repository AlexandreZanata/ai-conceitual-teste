"""Wave AO3 H-SMARTCORE runner: dodeca cite LOOKUP + GENCORE peak GENERATE."""
...
from run_gencore import _run_gen_ablation
...
_AO_BANK = REPO / "results/nano-lm/wave-ao/error_bank.jsonl"
_TRIALS = REPO / "results/nano-lm/wave-ao/trials"
_SUMMARY = REPO / "results/nano-lm/wave-ao/smartcore_summary.json"
...
def _seed_pack(bank_path: Path, ao_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(SMARTCORE_PACK, start=1):
        for q_key, q_text in (
            ...
            if q in existing:
                continue
            row = alias_bank_row(
                trial_id=f"AO-SMARTCORE-SEED-{q_key}-{i:02d}",
                question=q,
                source_id=item["source_id"],
                gold=item["gold"],
            )
            row["hyp_id"] = SMARTCORE_ID
            row["judge_notes"] = [
                "SMARTCORE seed for dodeca-hop paraphrase stress",
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AO-SMARTCORE-FIX-{i:02d}",
...
def run_smartcore(
    *,
    ...
    """
    GIVEN AO0 dodeca-hop paraphrases + parent asks
    WHEN SEMWRAP LOOKUP + GENCORE peak GENERATE dual-arm ×10
    THEN false-hit≈0; PROMOTE if gen≥5 else HOLD.
    """
    if len(SMARTCORE_PACK) != SMARTCORE_N:
        raise ValueError("SMARTCORE pack must be 10")
    if not hard_paraphrase_ok():
        raise ValueError("paraphrases must differ from parents")
    if not has_adversarial_noise() or not has_dodeca_hop_cues():
        raise ValueError("dodeca-hop adversarial pack incomplete")

    trials_dir.mkdir(parents=True, exist_ok=True)
    t0 = time.perf_counter()
    seeded = _seed_pack(bank_path, ao_bank)
    bank = load_bank_rows(bank_path)
    paras = [p["paraphrase"] for p in SMARTCORE_PACK]
    gen_items = [
        {
    ...
    for i, (item, lp) in enumerate(
        zip(SMARTCORE_PACK, lookup_payloads, strict=True),
        start=1,
    ):
        ...
        score_l, err_l, notes_l, cited = score_smartcore_lookup(
        ...
        lookup_trials.append(
            {
                "trial_id": f"AO-SMARTCORE-LOOKUP-HITL-{i:02d}",
                "stage": "AO3",
                "hyp_id": SMARTCORE_ID,
                "arm": "LOOKUP",
                "app_id": item["app_id"],
    ...
    for i, (item, gp) in enumerate(
        zip(SMARTCORE_PACK, gen_payloads, strict=True)
    ):
        score_g, err_g, notes_g = score_smartcore_gen(
    ...
    if weak_idx:
        re_items = [
            {
                "question": SMARTCORE_PACK[i]["parent_question"],
                "source_id": SMARTCORE_PACK[i]["source_id"],
                "gold": SMARTCORE_PACK[i]["gold"],
            }
            for i in weak_idx
        ]
        ...
        for j, idx in enumerate(weak_idx):
            item = SMARTCORE_PACK[idx]
            re_gp = re_payloads[j]
            score2, err2, notes2 = score_smartcore_gen(
            ...
            if score2 > gen_scores[idx]:
                ...
                gen_notes[idx] = list(notes2) + [
                    "FIX: re-peak GENCORE grounded decode"
                ]
                gen_fix[idx] = 1
                fix_count += 1
            else:
                gen_notes[idx] = list(gen_notes[idx]) + [
                    "FIX attempted: re-peak GENCORE (no lift)"
                ]
    ...
    for i, (item, gp) in enumerate(
        ...
        if bool(gp.get("peak_used")):
            n_peak += 1
        gt = {
            "trial_id": f"AO-SMARTCORE-GEN-HITL-{i:02d}",
            "stage": "AO3",
            "hyp_id": SMARTCORE_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
    ...
    decision = decide_smartcore(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "dodeca-hop paraphrase (CTXCORE companions)",
            "QPFB2+BEAMKV+GROUNDED+ANTI_PERIOD+GENCORE_PEAK GENERATE",
        ],
        "forbidden": [
            "QI",
            "STREAM",
        ...
        "finding": (
            f"{SMARTCORE_ID}: L_lookup={stats['lookup_mean']:.1f} "
            f"L_gen={stats['gen_mean']:.1f} cite={stats['n_cite_ok']}/10 "
            f"false_hit={n_false} period={n_period} peak={n_peak} "
            f"beats_gencore_ablated={stats['beats_gencore_ablated']} "
            f"peers_smartedge={stats['peers_smartedge_gen']} "
            f"pass_gen={stats['pass_gen']} fix={fix_count} → {decision}"
        ),
        "public_note": "docs/results/nano-lm/formal-hsmartcore-smartcore.md",
        "ship_claim": "AF packaged stack (dodeca cite+peak; LOOKUP≠gen IQ)",
    }
