"""Wave AO4 H-FASTCORE runner: GENCORE peak-fast gen wall vs AN FASTEDGE."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ao/fastcore_summary.json"
...
def _seed_pack(bank_path: Path, ao_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(AO0_PACK, start=1):
        q = str(item["question"]).strip()
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AO-FASTCORE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = FASTCORE_ID
        row["judge_notes"] = [
            "FASTCORE seed for AO held-out ask",
...
def _timed_gen(
    *,
    ...
    """GENERATE arm: GENCORE peak-extractive (wall_ms>0, no LOOKUP cache)."""
    ...
    *,
    ...
    for _ in range(max(1, rounds)):
        ...
        if wall < best_wall or (wall == best_wall and e2e < best_e2e):
            ...
            best = payloads
    if best is None:
        raise RuntimeError("FASTCORE hot gen produced no payloads")
...
def _lookup_trial(
    *,
    ...
    return {
        "trial_id": f"AO-FASTCORE-LOOKUP-HITL-{i:02d}",
        "stage": "AO4",
        "hyp_id": FASTCORE_ID,
        "arm": "LOOKUP",
        "app_id": item["app_id"],
...
def _gen_trial(
    *,
    ...
    tel = extract_telemetry(payload)
    return {
        "trial_id": f"AO-FASTCORE-GEN-HITL-{i:02d}",
        "stage": "AO4",
        "hyp_id": FASTCORE_ID,
        "arm": "GENERATE",
        "app_id": item["app_id"],
...
def run_fastcore(
    *,
    ...
    """
    GIVEN AO0 held-out asks
    WHEN LOOKUP quality + GENCORE peak-fast GENERATE cold/warm/hot
    THEN dual-arm FASTCORE vs AN FASTEDGE → PROMOTE|HOLD|KILL.
    """
    if len(AO0_PACK) != FASTCORE_N:
        raise ValueError("AO0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack(bank_path, ao_bank)
    bank = load_bank_rows(bank_path)
    items = [dict(p) for p in AO0_PACK]
    ...
    for i, (item, payload) in enumerate(
        ...
        if kind != "TRUE_HIT":
            row = alias_bank_row(
                trial_id=f"AO-FASTCORE-FIX-{i:02d}",
                question=item["question"],
                source_id=item["source_id"],
                gold=item["gold"],
            )
    ...
    decision = decide_fastcore(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "GENERATE PEAK_FAST+GENCORE (no student decode)",
            f"cold+warm+hot gen timing (hot rounds={_GEN_HOT_ROUNDS})",
            f"vs FASTEDGE hot {FASTEDGE_HOT_WALL_MS:.3f} ms",
        ],
        "forbidden": [
            "STREAM",
            "KVCACHE-Q",
        ...
        "timing": {
            ...
            "fastedge_hot_wall_ms": FASTEDGE_HOT_WALL_MS,
        ...
        json.dumps(
            {
                ...
                "pass_vs_fastedge": st["pass_vs_fastedge"],
                ...
                "fastedge_hot_wall_ms": st["fastedge_hot_wall_ms"],
