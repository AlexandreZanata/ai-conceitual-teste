"""Smoke H-POOL3: FLOP-aware n≤3 search vs frozen H-POOL tip."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_pool3(
            ...
            tip_gene=tip_gene,
            lam=LAM,
            out_meta=out / f"HPOOL3_seed{seed}_train.json",
        )
        row = _row(
            "H-POOL3",
            f"HPOOL3_seed{seed}",
        ...
        write_json(out / f"HPOOL3_seed{seed}_eval.json", row)
        rows.append(row)
    write_json(
        out / "pool3_smoke.json",
        {"rows": rows, "wall_s": time.perf_counter() - t0},
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "pool3_smoke.json")}))
    return 0
