"""Wave AN-FREEZE runner (nano:an:freeze) — lock AN; no Wave AO invent."""
...
from an_report_ops import render_paper_lab_wave_an, render_wave_an_summary
from an_session_ops import AN0_PACK
from antifp_ops import classify_arm, extract_telemetry
from matrix_common import REPO, write_json
from tipd_pair import tune_cpu_threads

_OUT = REPO / "results/nano-lm/wave-an/an_freeze.json"
_FREEZE_DOC = REPO / "docs/results/nano-lm/an-freeze.md"
_FORMAL = REPO / "docs/results/nano-lm/formal-hanfreeze-an-freeze.md"
...
def _write_freeze_docs() -> None:
    ...
    _FREEZE_DOC.write_text(render_an_freeze(), encoding="utf-8")
    _FORMAL.write_text(
        "\n".join(
            [
                "# AN-FREEZE — Wave AN lock (**DONE** — PROMOTE)",
                "",
                "> Lab: `.local/pesquisa.md` §3 AN8 · "
                "Public note: [an-freeze.md](an-freeze.md)  ",
                "> After: [wave-an-summary.md](wave-an-summary.md) / "
                "[paper-lab-wave-an.md](paper-lab-wave-an.md)",
                "",
                "## Hypothesis",
                "",
                "After AN-REPORT, freeze Wave AN the same way AM-FREEZE "
                "locked AM: **outcomes stay** (edge dual-arm PROMOTEs + "
                "GENEDGE HOLD); **no Wave AO** without an explicit "
                "reopen agenda.",
                "",
                "## Gate",
                "",
                "| Check | Result |",
                "|-------|--------|",
                "| AN formals keep GENEDGE HOLD · CTXEDGE…APPEDGE · "
                "HITL · REPORT decisions | **ok** |",
                "| `wave-an-summary` · `paper-lab-wave-an` · `an-freeze` "
                "contain **COMPLETE** | **ok** |",
                "| RECIPES + champion-card contain **H-CTXEDGE** · "
                "**AN-HITL-10** · **COMPLETE** | **ok** |",
...
def run_an_freeze(*, out: Path, skip_ask: bool = False) -> dict[str, Any]:
    """
    GIVEN AN formals + COMPLETE closeout
    WHEN locking Wave AN
    THEN PROMOTE iff decisions ∧ public COMPLETE ∧ product markers ∧ smoke.
    """
    _write_freeze_docs()
    formal_paths = [p for _, (p, _) in AN_DECISIONS.items()]
    read_paths = list(
        dict.fromkeys([*formal_paths, *AN_PUBLIC, *AN_PRODUCT_DOCS])
    )
    ...
    public_texts = {p: texts.get(p, "") for p in AN_PUBLIC}
    product_texts = {p: texts.get(p, "") for p in AN_PRODUCT_DOCS}
    decision = decide_an_freeze(
    ...
    payload: dict[str, Any] = {
        ...
        "ask_smoke": ask,
        "public_note": "docs/results/nano-lm/an-freeze.md",
        "formal_note": "docs/results/nano-lm/formal-hanfreeze-an-freeze.md",
        "wave_an_summary": "docs/results/nano-lm/wave-an-summary.md",
        "rule": "pesquisa §3 AN-FREEZE",
