"""Smoke H-LAT2 on B2 ckpts; compare vs B4 (matrix) on wall+quality."""
...
from hyp_lat2 import run_h_lat2
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_lat2(
            ...
            seed=seed,
            lam=LAM,
            out_meta=out / f"HLAT2_seed{seed}_train.json",
        )
        row = {
            "family": "H-LAT2",
            "label": f"HLAT2_seed{seed}",
        ...
        write_json(out / f"HLAT2_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "lat2_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "lat2_smoke.json")}))
    return 0
