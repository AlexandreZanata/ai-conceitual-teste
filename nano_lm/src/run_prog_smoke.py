"""Smoke H-PROG: PACK tip gate on programming domain (≤5M + TinyStories teacher)."""
...
def main() -> int:
    # Leave 2 cores free so the laptop stays responsive under smoke load.
    threads = tune_cpu_threads(max(4, int(os.cpu_count() or 4) - 2))
    c = matrix_cfg()
    device = resolve_device(True)
    if device.type != "cuda":
        print("ERROR: H-PROG requires CUDA", file=sys.stderr)
        return 2
    ...
    write_json(out / "hprog_smoke.json", payload)
    print(json.dumps({"decision": decision, "out": str(out / "hprog_smoke.json")}))
    return 0
