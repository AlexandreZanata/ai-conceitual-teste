"""Smoke H-NGRE: stack tip metas; dual gate vs H-NGRAM + H-EARLY."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        early_train = out / f"HEARLY_seed{seed}_train.json"
        ngram_train = out / f"HNGRAM_seed{seed}_train.json"
        for p in (ckpt, early_train, ngram_train):
            if not p.is_file():
        ...
        meta = run_h_ngre(
            ...
            out_meta=out / f"HNGRE_seed{seed}_train.json",
        )
        row = {
            "family": "H-NGRE",
        ...
        write_json(out / f"HNGRE_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "ngre_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "ngre_smoke.json")}))
    return 0
