"""Formal H-BPACK: SKIP + LAYB packs vs EARLY (eval, fit≠eval)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hbpack"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-BPACK formal requires CUDA")
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        rows.extend(run_seed_trio(c, seed, prompts, claim_offset=9922))
