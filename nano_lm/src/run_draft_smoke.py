"""Smoke H-DRAFT on B2 ckpts; dual gate vs B4 (matrix)."""
...
from hyp_draft import run_h_draft
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_draft(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=out / f"HDRAFT_seed{seed}_train.json",
        )
        row = {
            "family": "H-DRAFT",
            "label": f"HDRAFT_seed{seed}",
        ...
        write_json(out / f"HDRAFT_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "draft_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "draft_smoke.json")}))
    return 0
