"""Render formal H-NGDM vs tip markdown."""
...
def render(path: Path) -> str:
    ...
    decision = decide_hngdm(stats.get("H-NGDM", {}), stats)
    max_lp = tip_max_lp(stats) or float("nan")
    lines = [
        "# Formal H-NGDM vs H-NGRAM × H-DECM",
        "",
        f"Source: `{path}`",
    ...
    for name in ("H-NGRAM", "H-DECM", "H-NGDM"):
        if name not in stats:
            continue
        st = stats[name]
        delta = "—" if name != "H-NGDM" else f"{st['mean_lp'] - max_lp:+.4f}"
        lines.append(
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:ngdm` → "
            "`npm run nano:formal:ngdm:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hngdm/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hngdm-vs-tips.md"),
    )
    args = p.parse_args()
