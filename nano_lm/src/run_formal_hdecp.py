"""Formal-budget H-DECP vs B4 + GLOBAL (fit≠eval), shared B2 ckpts."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hdecp"
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_decp(
            ...
            seed=seed,
            out_meta=c["out"] / f"HDECP_seed{seed}_train.json",
        )
        decp = {
            "family": "H-DECP",
            ...
            "n_prompts": len(load_prompt_ids(c["prompts"])),
            "seed": seed,
            "bank_n": len(meta["bank"]),
            "picks": meta["picks"],
        }
        attach_overfit(decp, float(meta["best_fit"]))
        rows.append(decp)
        rows.append(
            {
                "family": "GLOBAL",
