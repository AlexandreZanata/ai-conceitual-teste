"""Formal equal-budget: B2 KD + H-BAL vs H-LAM (Lamarck), 3 seeds."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hlam-b2",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 4,
...
def _plastic(
    ...
    return run_fn(
        ...
        pop_size=c["pop"],
        generations=c["gens"],
        lifetime_steps=c["lifetime_steps"],
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        for fam, tag, fn in (
            ("H-BAL", "HBAL", run_h_bal),
            ("H-LAM", "HLAM", run_h_lam),
        ):
            ckpt = c["out"] / f"{tag}_seed{seed}.pt"
            meta = _plastic(fn, c, device, seed, ckpt)
            write_json(c["out"] / f"{tag}_seed{seed}_train.json", meta)
            row = _eval(c, ckpt, seed, fam)
            row["unstable"] = bool(meta.get("unstable"))
            rows.append(row)
            if device.type == "cuda":
                torch.cuda.empty_cache()
