"""Render H-CHUNK smoke vs H-EARLY / H-FLASH (chunked prefill)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Same EARLY tip genes; long prompts (`target_tokens`); prefill in "
        "blocks of `chunk_size` under SDPA + KV, then generate.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/chunk_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hchunk-vs-hflash.md"),
    )
    args = p.parse_args()
