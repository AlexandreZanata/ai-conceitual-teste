"""Render H-HALF smoke vs H-PRE (fp16-wire H2D under PRE)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Same top-k soft cache, STAG curriculum, and PRE prefetch; "
        "H-HALF keeps topk_val fp16 across H2D then casts on GPU before expand. "
        "Kill if |Δlp| > ε vs H-PRE or no ms/step win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/half_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hhalf-vs-hpre.md"),
    )
    args = p.parse_args()
