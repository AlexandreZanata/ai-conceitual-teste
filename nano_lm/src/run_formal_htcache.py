"""Formal H-TCACHE: memo + eligible-only code on PFB2 (fit≠eval)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-htcache"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-TCACHE formal requires CUDA")
    out: Path = c["out"]
    ...
    tc_rows, _, code_wall_t = commit_pfb_rows_tcache(
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "max_new": _MAX_NEW,
            "cpu_threads": threads,
            "mode": "TCACHE formal: memo + eligible-only code (fit≠eval)",
            "mechanism": "TeacherLpMemo by completion id; code forwards only if story-eligible",
            "parent": "H-EARLY n=1 greedy on B2 (formal genes; PFB2 freeze)",
        },
    )
