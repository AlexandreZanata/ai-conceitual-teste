"""Render formal H-DEPTH vs H-STAG (shallow STAG + PRUN)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Formal STAG tip control; H-DEPTH trains `n_layers=1` under STAG recipe, "
        "then mag-prune + KD recover. Fit≠eval (`eval_prompts`).",
    ...
    lines.extend(
        [
            ...
            "",
            "Commands: `npm run nano:formal:hdepth` → "
            "`npm run nano:formal:hdepth:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hdepth/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hdepth-vs-hstag.md"),
    )
    args = p.parse_args()
