"""Wave AG REPORT runner: write public closeout + anti-FP gate (nano:ag:report)."""
...
_OUT = REPO / "results/nano-lm/wave-ag/ag_report_summary.json"
...
def _evidence_map() -> dict[str, bool]:
    return {p: (REPO / p).is_file() for p in AG_EVIDENCE}
...
def _stage_facts() -> dict[str, Any]:
    keys = {
        "antifp": "results/nano-lm/wave-ag/antifp_summary.json",
        "ctxreal": "results/nano-lm/wave-ag/ctxreal_summary.json",
        "smartreal": "results/nano-lm/wave-ag/smartreal_summary.json",
        "fastreal": "results/nano-lm/wave-ag/fastreal_summary.json",
        "appreal": "results/nano-lm/wave-ag/appreal_summary.json",
        "ag_hitl": "results/nano-lm/wave-ag/ag_hitl_summary.json",
    }
    out: dict[str, Any] = {}
...
def _smoke_dual_arm() -> dict[str, Any]:
    """LOOKUP + GENERATE smoke on first AG0 ask (anti-FP telemetry)."""
...
def run_ag_report(*, out: Path, skip_ask: bool = False) -> dict[str, Any]:
    """
    GIVEN AG0–AG6 evidence
    ...
    decision = decide_ag_report(evidence)
    ...
    if decision.startswith("PROMOTE") and not markers:
        decision = "KILL (wave-ag-summary missing thesis markers)"
    if decision.startswith("PROMOTE") and not board:
        decision = "KILL (wave-ag-summary missing dual-arm FIX scoreboard)"
    if decision.startswith("PROMOTE") and not antifp:
        decision = "KILL (wave-ag-summary missing anti-FP evidence)"
    ok = (
    ...
    payload: dict[str, Any] = {
        ...
        "scoreboard_ok": board,
        "antifp_ok": antifp,
        "scoreboard": list(AG_HITL_SCOREBOARD),
        "evidence": evidence,
        "stage_facts": _stage_facts(),
        "ask_smoke": ask,
        "public_report": "docs/results/nano-lm/wave-ag-summary.md",
        "paper_lab": "docs/results/nano-lm/paper-lab-wave-ag.md",
        "wave_status": "RESEARCH_COMPLETE" if ok else "OPEN",
        "ship_claim": "scoped AF packaged stack — not open chat LM",
        "cpu_threads": int(os.environ.get("OMP_NUM_THREADS") or 0),
        "finding": (
            f"{AG_ID}: decision={'PROMOTE' if ok else decision}; "
