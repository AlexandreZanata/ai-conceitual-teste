"""Wave AC REPORT runner: write public closeout + evidence gate (nano:ac:report)."""
...
_OUT = REPO / "results/nano-lm/wave-ac/ac_report_summary.json"
...
def _evidence_map() -> dict[str, bool]:
    return {p: (REPO / p).is_file() for p in AC_EVIDENCE}
...
def _stage_facts() -> dict[str, Any]:
    keys = {
        "ctxplus": "results/nano-lm/wave-ac/ctxplus_summary.json",
        "smartplus": "results/nano-lm/wave-ac/smartplus_summary.json",
        "fastplus": "results/nano-lm/wave-ac/fastplus_summary.json",
        "appplus": "results/nano-lm/wave-ac/appplus_summary.json",
        "ac_hitl": "results/nano-lm/wave-ac/ac_hitl_summary.json",
    }
    out: dict[str, Any] = {}
...
def run_ac_report(*, out: Path, skip_ask: bool = False) -> dict[str, Any]:
    """
    GIVEN AC0–AC5 evidence
    ...
    decision = decide_ac_report(evidence)
    ...
    if decision.startswith("PROMOTE") and not markers:
        decision = "KILL (wave-ac-summary missing thesis markers)"
    if decision.startswith("PROMOTE") and not board:
        decision = "KILL (wave-ac-summary missing per-model FIX scoreboard)"
    ...
    payload: dict[str, Any] = {
        ...
        "markers_ok": markers,
        "scoreboard_ok": board,
        "scoreboard": list(AC_HITL_SCOREBOARD),
        "evidence": evidence,
        "stage_facts": _stage_facts(),
        "ask_smoke": ask,
        "public_report": "docs/results/nano-lm/wave-ac-summary.md",
        "paper_lab": "docs/results/nano-lm/paper-lab-wave-ac.md",
        "wave_status": "COMPLETE" if ok else "OPEN",
        "cpu_threads": int(os.environ.get("OMP_NUM_THREADS") or 0),
        "finding": (
            f"{AC_ID}: decision={'PROMOTE' if ok else decision}; "
...
def main() -> int:
    ...
    try:
        summary = run_ac_report(out=Path(args.out), skip_ask=bool(args.skip_ask))
