"""Formal-budget H-EARS vs H-EARLY tip (scheduled early thr)."""
...
from hyp_ears import run_h_ears
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hears"
    return base


def _eval_ears(c: dict[str, Any], ckpt: Path, gene: dict, seed: int) -> dict:
    ...
    lp, wall_ms = fitness_ears_detail(
        gene,
        teacher=teacher,
        student=student,
        prompts=texts,
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_ears(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=c["out"] / f"HEARS_seed{seed}_train.json",
        )
        row = _eval_ears(c, b2, meta["best_gene"], seed)
        row["lam"] = MIN_LAM
