"""Wave AF0 SESSION runner (nano:af:session) — freeze held-out HITL×10."""
...
_OUT = REPO / "results/nano-lm/wave-af/af0_session.json"
_TRIALS = REPO / "results/nano-lm/wave-af/trials"
_ERROR_BANK = REPO / "results/nano-lm/wave-af/error_bank.jsonl"
...
def main() -> int:
    ...
    ids = [str(p["source_id"]) for p in AF0_PACK]
    ...
    trials_ready = Path(args.trials_dir).is_dir() and len(written) == AF0_N

    decision = _override_decision(
        decide_af0_session(known_sources=known, trials_dir_ready=trials_ready),
        miss=miss,
        clash=clash,
    ...
    payload = {
        ...
        "ask_smoke": ask,
        "public_note": "docs/results/nano-lm/wave-af-session.md",
        "rule": "pesquisa §5 AF0 · Cursor ASK→EVAL→FIX law",
        "next": "AF1 H-CTXULTRA (ASK→EVAL→FIX×10)",
    }
