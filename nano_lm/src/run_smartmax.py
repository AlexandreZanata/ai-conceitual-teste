"""Wave AE2 H-SMARTMAX runner: multi-hop cite beyond HARDPARA (nano:smartmax)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ae/smartmax_summary.json"
...
def _seed_parent_and_para(bank_path: Path, ae_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(SMARTMAX_PACK, start=1):
        for q_key, q_text in (
            ...
            if q in existing:
                continue
            row = alias_bank_row(
                trial_id=f"AE-SMARTMAX-SEED-{q_key}-{i:02d}",
                question=q,
                source_id=item["source_id"],
                gold=item["gold"],
            )
            row["hyp_id"] = SMARTMAX_ID
            row["judge_notes"] = [
                "SMARTMAX seed for multi-hop paraphrase stress",
...
def _fix_alias(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AE-SMARTMAX-FIX-{i:02d}",
...
def _build_trial(
    *,
    ...
    tid = f"AE-SMARTMAX-HITL-{i:02d}"
    mode = str(payload.get("mode", ""))
    score, err, notes, cite_flag = score_smartmax_trial(
        mode=mode,
        completion=text,
    ...
    trial: dict[str, Any] = {
        ...
        "judge_notes": notes,
        "manual_adjust": (
            "no change — SMARTMAX true-hit + cite"
            if lookup_kind == "TRUE_HIT" and cited and not err
            else "FIX: multi-hop alias / primary cite under distractor"
        ),
...
def run_smartmax(
    *,
    ...
    """
    GIVEN AE0 multi-hop adversarial paraphrases
    WHEN SEMWRAP retrieve + cite primary + FIX
    THEN mean≥7 · false-hit=0 · cite≥7 → PROMOTE|HOLD|KILL.
    """
    if len(SMARTMAX_PACK) != SMARTMAX_N:
        raise ValueError("SMARTMAX pack must be 10")
    if not hard_paraphrase_ok():
        raise ValueError("SMARTMAX paraphrases must differ from parents")
    if not has_adversarial_noise():
        raise ValueError("SMARTMAX pack must carry adversarial noise cues")
    if not has_multihop_cues():
        raise ValueError("SMARTMAX pack must carry multi-hop distractor cues")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_parent_and_para(bank_path, ae_bank)
    bank = load_bank_rows(bank_path)

    questions = [p["paraphrase"] for p in SMARTMAX_PACK]
    ...
    for i, (item, payload) in enumerate(
        ...
        if kind != "TRUE_HIT" or not cited0:
            _fix_alias(
                i=i, item=dict(item), bank_path=bank_path, ae_bank=ae_bank
            )
            bank = load_bank_rows(bank_path)
            fix_count += 1
            fix_pass = 1
    ...
    decision = decide_smartmax(stats)
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{SMARTMAX_ID}: mean={stats['mean']:.1f} "
            f"false_hit={n_false} true_hit={n_true} "
            f"cite_ok={stats['n_cite_ok']}/10 "
            f"fix={fix_count} decision={decision}."
        ),
        "public_note": "docs/results/nano-lm/formal-hsmartmax-smartmax.md",
        "claim": "multi-hop scoped retrieve/cite — not open chat LM",
    }
