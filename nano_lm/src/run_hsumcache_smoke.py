"""Smoke H-SUMCACHE: PFB2 on summary+tail vs EARLY; wall < full@512."""
...
def main() -> int:
    ...
    if device.type != "cuda":
        print("ERROR: H-SUMCACHE requires CUDA", file=sys.stderr)
        return 2
    ...
    write_json(
        out / "hsumcache_smoke.json",
        {
            ...
            "max_new": _MAX_NEW,
            "cpu_threads": threads,
            "mode": "SUMCACHE: PFB2 on coarse‖fine‖tail vs EARLY; wall vs full@512",
            "mechanism": "GENC-scale hierarchical compress; BEAMKV; not CTX",
            "parent": "H-EARLY n=1 on summary+tail",
        },
    )
    print(
        json.dumps(
            {"decision": decision, "out": str(out / "hsumcache_smoke.json")}
        )
    )
    return 0
