"""Wave BB3 H-CTXHOLD: hold howto·cite·long content · no anti-FP regress."""
...
CTXHOLD_THESIS = (
    ...
    "(BB-FOREVER FH 0 · BA-FOREVER hold · AZ hold · over-refuse 0 · live FP 0), "
    "p50/p99 published, modes 4/4 — L_eff alone ≠ win; ≠ BA H-CTXREAL2 rename · "
    "≠ AG H-CTXREAL archive"
)
CTXHOLD_CLAIM = BB0_SHIP_LOCK
CTXHOLD_SAFE_NOTE = BB0_SAFE_NOTE
CTXHOLD_ANTI_FP = BB0_ANTI_FP
CTXHOLD_CTX_BASELINE = BB0_CTX_BASELINE
CTXHOLD_SPEED_BASELINE = BB0_SPEED_BASELINE
...
def ctx_row_content_ok(row: Mapping[str, Any]) -> bool:
    """
    GIVEN a BB CTX content row (howto|cite|long)
...
def apps_ctx_content_ok(apps: Sequence[Mapping[str, Any]]) -> bool:
    ...
    WHEN applying BB3 content bars
...
def extract_ctxhold_board(
    *,
    ...
    """
    GIVEN ctx content + apps + BB/BA/AZ anti-FP + latency
    WHEN building BB3 CTXHOLD board
    ...
    return {
        ...
        **kinds,
        ...
        "l_eff_alone_insufficient": bool(
            CTXHOLD_CTX_BASELINE.get("l_eff_alone_insufficient")
        ),
        "content_bars_required": bool(
            CTXHOLD_CTX_BASELINE.get("content_bars_required")
        ),
        "bank_stuff_forbidden": True,
        "ba_ctxreal2_archive_untouched": True,
        "ag_ctxreal_archive_untouched": True,
        "baseline_source": str(CTXHOLD_CTX_BASELINE.get("source", "")),
    }
...
def decide_ctxhold(
    *,
    ...
    """
    GIVEN BB3 CTXHOLD board
    WHEN applying pesquisa §2 + §8 BB3
    ...
    if err:
        return err
    return (
        f"PROMOTE ({CTXHOLD_ID}: howto·cite·long content_ok; "
        "BB/BA/AZ anti-FP hold; p50/p99 published; L_eff alone ≠ win)"
    )
