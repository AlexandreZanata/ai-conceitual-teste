"""Wave AK0 SESSION runner (nano:ak:session) — freeze held-out HITL×10."""
...
_OUT = REPO / "results/nano-lm/wave-ak/ak0_session.json"
_TRIALS = REPO / "results/nano-lm/wave-ak/trials"
_ERROR_BANK = REPO / "results/nano-lm/wave-ak/error_bank.jsonl"
...
def main() -> int:
    ...
    ids = [str(p["source_id"]) for p in AK0_PACK]
    ...
    trials_ready = Path(args.trials_dir).is_dir() and len(written) == AK0_N

    decision = _override_decision(
        decide_ak0_session(known_sources=known, trials_dir_ready=trials_ready),
        miss=miss,
        clash=clash,
    ...
    payload = {
        ...
        "ask_smoke": ask,
        "public_note": "docs/results/nano-lm/wave-ak-session.md",
        "rule": "pesquisa §3 AK0 · Cursor ASK→EVAL→FIX dual-arm law",
        "next": "AK1 H-GENTRUE (smarter gen + ASK dual-arm)",
        "anti_fp": (
