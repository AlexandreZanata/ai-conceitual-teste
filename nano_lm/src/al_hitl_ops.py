"""Wave AL6 AL-HITL-10: final dual-arm verify on frozen AL0 pack."""
...
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0
# Declared AL wave stack (fresh spines + SEMWRAP/ASKFAST inherit).
...
STACK_CLAIM = (
    "scoped AL dual-arm fresh verify on AF packaged stack — "
    "LOOKUP ≠ generative IQ; not open chat LM"
)
SHIP_CLAIM_AF = (
    "scoped AF packaged stack — not open chat LM "
    "(AL gen arm below bar; ship claim unchanged)"
)
...
def score_al6_lookup(
    *,
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "AL6 LOOKUP final — not generative IQ",
    ]
    if arm != "LOOKUP":
...
def score_al6_gen(
    *,
    ...
    """GENERATE final — wall_ms>0 ∧ n_new>0; GENFRESH peak rubric."""
    ...
    notes = list(notes) + [
        "AL6 GENERATE final — Cursor scores completion",
        "GENFRESH grounded+peak path (labeled ≠ open-chat IQ)",
    ]
    return float(score), bool(err), notes
...
def al6_stats(
    *,
    ...
    WHEN summarizing AL6
    THEN lookup≥7 · errors≤3/arm · gen≥5 or HOLD path · held-out ok.
    """
    if len(lookup_scores) != AL6_N or len(gen_scores) != AL6_N:
        raise ValueError(f"AL6 requires {AL6_N} dual-arm scores")
    l_mean = float(sum(lookup_scores) / float(AL6_N))
    g_mean = float(sum(gen_scores) / float(AL6_N))
    ...
    return {
        ...
        "pass_gen_telemetry": int(n_gen_wall_ok) >= AL6_N,
        "pass_mean": PASS_MEAN,
...
def decide_al6(stats: Mapping[str, Any]) -> str:
    """
    GIVEN AL6 final dual-arm stats
    WHEN applying pesquisa §3 AL6 gate
