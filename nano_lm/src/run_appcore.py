"""Wave AO5 H-APPCORE runner: dual-arm apps + DEPL-AO (nano:appcore)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ao/appcore_summary.json"
...
def _seed_pack(bank_path: Path, ao_bank: Path) -> int:
    ...
    for i, item in enumerate(AO0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AO-APPCORE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = APPCORE_ID
        row["judge_notes"] = [
            "APPCORE seed for AO held-out ask",
...
def _write_pages(docs: Path) -> list[dict[str, Any]]:
    ...
    depl_path = docs / Path(DEPL_AO_PAGE).name
    depl_body = depl_ao_body()
    depl_path.write_text(depl_body, encoding="utf-8")
    reports.append(page_sync_report(DEPL_AO_PAGE, depl_body))
    return reports
...
def _run_one_app(
    *,
    ...
    if len(lookup_payloads) != APPCORE_N or len(shared_gen) != APPCORE_N:
    ...
    for i, item in enumerate(AO0_PACK, start=1):
        ...
        if not bool(route["in_scope"]):
            ...
            row = alias_bank_row(
                trial_id=f"AO-APPCORE-FIX-{tag}-{i:02d}",
        ...
        l_score, l_err, l_notes = score_appcore_lookup(
            mode=l_mode,
            completion=l_comp,
        ...
        l_trial = {
            "trial_id": f"AO-APPCORE-{tag}-LOOKUP-HITL-{i:02d}",
            "stage": "AO5",
            "hyp_id": APPCORE_ID,
            "arm": "LOOKUP",
            "realapp_id": app_id,
        ...
        g_score, g_err, g_notes = score_appcore_gen(
        ...
        g_trial = {
            "trial_id": f"AO-APPCORE-{tag}-GEN-HITL-{i:02d}",
            "stage": "AO5",
            "hyp_id": APPCORE_ID,
            "arm": "GENERATE",
            "realapp_id": app_id,
...
def run_appcore(
    *,
    ...
    """
    GIVEN AO0 pack + APPCORE apps
    WHEN ASK→EVAL→FIX×10 dual-arm per surface + DEPL-AO
    THEN expose LOOKUP|GENERATE ∧ DEPL → PROMOTE|HOLD|KILL.
    """
    if len(AO0_PACK) != APPCORE_N:
    ...
    bank = load_bank_rows(bank_path)
    apps = [dict(a) for a in APPCORE_APPS]
    if only_app:
        apps = [app_by_id(only_app)]

    # Shared GENCORE peak GENERATE once — max HW without 3× CUDA reload.
    gen_items = [dict(p) for p in AO0_PACK]
    ...
    summary: dict[str, Any] = {
        ...
        "public_note": "docs/results/nano-lm/formal-happcore-appcore.md",
        "ship_claim": "AF packaged stack until AO6 HITL bar",
        "claim": (
