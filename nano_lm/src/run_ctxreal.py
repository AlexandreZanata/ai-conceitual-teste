"""Wave AG2 H-CTXREAL runner: quad-doc dual-arm ASK→EVAL→FIX×10."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ag/ctxreal_summary.json"
...
def _seed_pack_golds(bank_path: Path, ag_bank: Path) -> int:
    ...
    for i, item in enumerate(AG0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AG-CTXREAL-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = CTXREAL_ID
        row["judge_notes"] = [
            "CTXREAL seed alias for held-out AG ask",
...
def _build_ctxs(
    *, curated_root: Path, workers: int, tok: Any
    ...
    def _read(item: dict[str, str]) -> tuple[dict[str, str], str, str, str, str]:
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        rows = list(pool.map(_read, [dict(p) for p in AG0_PACK]))
    ctxs: list[dict[str, Any]] = []
    for item, p_text, s_text, t_text, q_text in rows:
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AG-CTXREAL-FIX-{i:02d}",
...
def _lookup_trial(
    *,
    ...
    tid = f"AG-CTXREAL-LOOKUP-HITL-{i:02d}"
    mode = str(payload.get("mode", ""))
...
def _gen_trial(
    *,
    ...
    tid = f"AG-CTXREAL-GEN-HITL-{i:02d}"
...
def run_ctxreal(
    *,
    ...
    """
    GIVEN AG0 held-out asks + quad curated docs
    WHEN K=9 ROLL/SUMCACHE + LOOKUP askfast + GENERATE raw
    THEN L_eff↑ vs CTXULTRA · gen usable≥5 → PROMOTE|HOLD|KILL.
    """
    if len(AG0_PACK) != CTXREAL_N:
        raise ValueError("AG0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack_golds(bank_path, ag_bank)
    cfg = matrix_cfg()
    ...
    for i, (item, lp, gp, ctx) in enumerate(
        zip(AG0_PACK, lookup_payloads, gen_payloads, ctxs, strict=True),
        start=1,
    ):
    ...
    mean_l = float(sum(float(c["l_eff"]) for c in ctxs) / CTXREAL_N)
    mean_a = float(
        sum(float(c["sumcache_active"]) for c in ctxs) / CTXREAL_N
    )
    mean_s = float(sum(float(c["n_slices"]) for c in ctxs) / CTXREAL_N)
    mean_src = float(sum(float(c["n_sources"]) for c in ctxs) / CTXREAL_N)
    ...
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{CTXREAL_ID}: L_lookup={stats['lookup_mean']:.1f} "
            f"L_gen={stats['gen_mean']:.1f} "
            f"usable_L={stats['n_lookup_usable']}/10 "
            f"usable_G={stats['n_gen_usable']}/10 "
            f"L_eff={mean_l:.0f} (>CTXULTRA {stats['ctxultra_mean_leff']:.0f}) "
            f"sources={mean_src:.1f} slices={mean_s:.1f} "
            f"false_hit={n_false} fix={fix_count} decision={decision}."
        ),
        "public_note": "docs/results/nano-lm/formal-hctxreal-ctxreal.md",
        "claim": (
            "quad-doc longer usable ctx beyond CTXULTRA — "
