"""Wave AK5 H-APPMORE runner: dual-arm apps + DEPL-AK (nano:appmore)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ak/appmore_summary.json"
...
def _seed_pack(bank_path: Path, ak_bank: Path) -> int:
    ...
    for i, item in enumerate(AK0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AK-APPMORE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = APPMORE_ID
        row["judge_notes"] = [
            "APPMORE seed for AK held-out ask",
...
def _write_pages(docs: Path) -> list[dict[str, Any]]:
    ...
    depl_path = docs / Path(DEPL_AK_PAGE).name
    depl_body = depl_ak_body()
    depl_path.write_text(depl_body, encoding="utf-8")
    reports.append(page_sync_report(DEPL_AK_PAGE, depl_body))
    return reports
...
def _run_one_app(
    *,
    ...
    if len(lookup_payloads) != APPMORE_N or len(shared_gen) != APPMORE_N:
    ...
    for i, item in enumerate(AK0_PACK, start=1):
        ...
        if not bool(route["in_scope"]):
            ...
            row = alias_bank_row(
                trial_id=f"AK-APPMORE-FIX-{tag}-{i:02d}",
        ...
        l_score, l_err, l_notes = score_appmore_lookup(
            mode=l_mode,
            completion=l_comp,
        ...
        l_trial = {
            "trial_id": f"AK-APPMORE-{tag}-LOOKUP-HITL-{i:02d}",
            "stage": "AK5",
            "hyp_id": APPMORE_ID,
            "arm": "LOOKUP",
            "realapp_id": app_id,
        ...
        g_score, g_err, g_notes = score_appmore_gen(
        ...
        g_trial = {
            "trial_id": f"AK-APPMORE-{tag}-GEN-HITL-{i:02d}",
            "stage": "AK5",
            "hyp_id": APPMORE_ID,
            "arm": "GENERATE",
            "realapp_id": app_id,
...
def run_appmore(
    *,
    ...
    """
    GIVEN AK0 pack + APPMORE apps
    WHEN ASK→EVAL→FIX×10 dual-arm per surface + DEPL-AK
    THEN expose LOOKUP|GENERATE ∧ DEPL → PROMOTE|HOLD|KILL.
    """
    if len(AK0_PACK) != APPMORE_N:
    ...
    bank = load_bank_rows(bank_path)
    apps = [dict(a) for a in APPMORE_APPS]
    if only_app:
        apps = [app_by_id(only_app)]

    # Shared GENTRUE peak GENERATE once — max HW without 3× CUDA reload.
    gen_items = [dict(p) for p in AK0_PACK]
    ...
    summary: dict[str, Any] = {
        ...
        "public_note": "docs/results/nano-lm/formal-happmore-appmore.md",
