"""Smoke H-TPE on B2 ckpts; dual gate vs H-TYP."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_tpe(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=out / f"HTPE_seed{seed}_train.json",
        )
        row = {
            "family": "H-TPE",
        ...
        write_json(out / f"HTPE_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "tpe_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "tpe_smoke.json")}))
    return 0
