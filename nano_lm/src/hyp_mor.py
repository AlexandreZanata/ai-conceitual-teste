"""H-MOR: H-SEL scaffold; cull bottom quartile before breed."""
...
def run_h_mor(
    *,
    ...
    vocab = len(tok)
    k = mortality_k(pop_size, fraction=mortality_fraction)
    ...
    if not data:
        raise RuntimeError("H-MOR: no training batches")
    probe = data[0]
    best_state = None
    best_fit = float("-inf")
    history: list[float] = []
    cull_log: list[list[int]] = []
    for _gen in range(generations):
        ...
        survivors, culled = cull_worst(fits, k)
        cull_log.append(culled)
        ranked = sorted(survivors, key=lambda i: fits[i], reverse=True)
