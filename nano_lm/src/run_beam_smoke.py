"""Smoke H-BEAM on B2 ckpts; dual gate vs B4 (matrix)."""
...
from hyp_beam import run_h_beam
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_beam(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=out / f"HBEAM_seed{seed}_train.json",
        )
        row = {
            "family": "H-BEAM",
            "label": f"HBEAM_seed{seed}",
        ...
        write_json(out / f"HBEAM_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "beam_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "beam_smoke.json")}))
    return 0
