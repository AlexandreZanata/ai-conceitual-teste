"""Render formal H-GALL vs H-GRAPH (CUDA graph all budgets)."""
...
def render(path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "Commands: `npm run nano:formal:hgall` → "
            "`npm run nano:formal:hgall:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hgall/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hgall-vs-hgraph.md"),
    )
    args = p.parse_args()
