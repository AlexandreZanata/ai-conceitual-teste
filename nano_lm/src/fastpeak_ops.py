"""Wave AJ4 H-FASTPEAK: faster generative ask (wall_ms>0) vs AI FASTPUSH."""
...
from fastpush_ops import AF_RAW_OPEN_WALL_MS
...
__all__ = [
    ...
    "FASTPUSH_WARM_WALL_MS",
    "FASTPUSH_COLD_WALL_MS",
    "FASTPUSH_HOT_E2E_MS",
    "WALL_DROP_MIN",
    "PASS_MEAN",
    "PASS_MAX_ERRORS",
...
FASTPEAK_ID = "H-FASTPEAK"
FASTPEAK_N = 10
# Published AI4 FASTPUSH means (formal-hfastpush-fastpush.md) — gen arm only.
FASTPUSH_COLD_WALL_MS = 24.5
FASTPUSH_WARM_WALL_MS = 10.6
FASTPUSH_HOT_WALL_MS = 10.7
FASTPUSH_HOT_E2E_MS = 1209.0


def fastpeak_generate(
    *,
    question: str,
    chunks: Sequence[str],
    ...
    WHEN peaking an extractive span under wall clock
    THEN GENERATE payload with wall_ms>0 ∧ n_new>0 (peak-fast product).
    """
    t0 = time.perf_counter()
    hits = peak_top_k_chunks(question, chunks, max(1, int(k_retrieve)))
    ctx = "\n\n".join(hits)[:_CTX_CAP]
    peak = extract_peak_answer(question, ctx)
...
def score_fastpeak_lookup(
    *,
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "FASTPEAK LOOKUP product path — NOT speed IQ "
        "(vs H-FASTPUSH wall=0 claim ban)",
    ]
    if arm != "LOOKUP":
...
def score_fastpeak_gen(
    *,
    ...
    notes = list(notes) + [
        f"arm={arm} mode={tel['mode']} wall_ms={tel['wall_ms']} "
        f"n_new={tel['n_new']}",
        f"FASTPEAK gen vs H-FASTPUSH hot baseline "
        f"{FASTPUSH_HOT_WALL_MS:.3f} ms",
    ]
...
def fastpeak_stats(
    *,
    ...
    """
    GIVEN dual-arm FASTPEAK timings
    WHEN summarizing AJ4
    THEN gen wall_ms>0 ∧ (warm|hot)↓ vs cold ∧ vs FASTPUSH hot.
    """
    if len(lookup_scores) != FASTPEAK_N or len(gen_scores) != FASTPEAK_N:
        raise ValueError(f"FASTPEAK requires {FASTPEAK_N} dual-arm scores")
    l_mean = float(sum(lookup_scores) / float(FASTPEAK_N))
    g_mean = float(sum(gen_scores) / float(FASTPEAK_N))
    ...
    drop_vs_fp = wall_reduction(FASTPUSH_HOT_WALL_MS, hot_wall_ms)
    ...
    beats_fp_wall = float(hot_wall_ms) < float(FASTPUSH_HOT_WALL_MS) or (
        float(warm_wall_ms) < float(FASTPUSH_WARM_WALL_MS)
    )
    beats_fp_e2e = float(hot_e2e_ms) < float(FASTPUSH_HOT_E2E_MS)
    return {
        "n_trials": FASTPEAK_N,
        "lookup_mean": l_mean,
        "gen_mean": g_mean,
        ...
        "fastpush_hot_wall_ms": float(FASTPUSH_HOT_WALL_MS),
        "fastpush_warm_wall_ms": float(FASTPUSH_WARM_WALL_MS),
        "fastpush_cold_wall_ms": float(FASTPUSH_COLD_WALL_MS),
        "fastpush_hot_e2e_ms": float(FASTPUSH_HOT_E2E_MS),
        "wall_drop_vs_cold": float(drop_cold),
        "wall_drop_vs_fastpush": float(drop_vs_fp),
        "wall_drop_min": float(WALL_DROP_MIN),
        "pass_gen_telemetry": int(n_gen_wall_ok) >= FASTPEAK_N,
        "pass_speed": bool(wall_down or e2e_down or ttft_down),
        "pass_vs_fastpush": bool(
            beats_fp_wall
            or beats_fp_e2e
...
def decide_fastpeak(stats: Mapping[str, Any]) -> str:
    """
    GIVEN FASTPEAK stats
    WHEN applying pesquisa §3 AJ4 gate
    THEN KILL if false-hit or gen wall=0; PROMOTE if vs-FASTPUSH∧telemetry;
    ...
    if not bool(stats.get("dual_arm")):
        return "KILL"
    if bool(stats.get("pass_vs_fastpush")) and bool(
