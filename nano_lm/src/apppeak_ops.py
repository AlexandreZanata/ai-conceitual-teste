"""Wave AJ5 H-APPPEAK: apps expose LOOKUP vs GENERATE + DEPL-AJ honesty."""
...
APPPEAK_ID = "H-APPPEAK"
APPPEAK_N = 10
MIN_APPS = 3  # known + howto + longdoc (AJ0 surfaces)
MIN_PAGES = 4  # 3 apps + depl-aj
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0  # pesquisa §3 AJ5 — or honest HOLD
APPPUSH_GEN_MEAN = 4.0  # parent AI5 SERVE gen (HOLD)
SERVEALIGN_MEAN = 3.4
DEPL_AJ_PAGE = "docs/results/nano-lm/depl-aj.md"

AJ_STACK_MARKERS: tuple[str, ...] = (
    "H-APPPEAK",
    "LOOKUP",
    "GENERATE",
    "H-FASTPEAK",
...
APPPEAK_APPS: tuple[dict[str, Any], ...] = (
    {
        "app_id": "app-known",
        "surface": "known-ask",
        ...
        "one_pager": "docs/results/nano-lm/apppeak-known.md",
        "npm": "npm run nano:apppeak -- --app app-known",
    },
    {
        "app_id": "app-howto",
        "surface": "howto",
        ...
        "one_pager": "docs/results/nano-lm/apppeak-howto.md",
        "npm": "npm run nano:apppeak -- --app app-howto",
    },
    {
        "app_id": "app-longdoc",
        "surface": "long-doc",
        ...
        "one_pager": "docs/results/nano-lm/apppeak-longdoc.md",
        "npm": "npm run nano:apppeak -- --app app-longdoc",
    },
)
...
def app_by_id(app_id: str) -> dict[str, Any]:
    for app in APPPEAK_APPS:
        if app["app_id"] == app_id:
            return dict(app)
    raise KeyError(f"unknown apppeak id: {app_id}")
    ...
    *,
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPPEAK LOOKUP OOS — not generative IQ"
        ]
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "APPPEAK LOOKUP labeled — not generative IQ",
    ]
...
def score_apppeak_gen(
    *,
    ...
    WHEN Cursor EVAL
    THEN OOS refuse ok; SERVE uses GENPEAK peak rubric (vs APPPUSH).
    """
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPPEAK GENERATE OOS refuse — honest"
        ]
    ...
    notes = list(notes) + [
        "APPPEAK GENERATE peak — Cursor scores completion",
        f"beat APPPUSH SERVE gen={APPPUSH_GEN_MEAN}",
    ]
    return float(score), bool(err), notes


def one_pager_body(app: Mapping[str, Any]) -> str:
    """Public one-pager for an APPPEAK packaged app (dual-arm)."""
    spine = " → ".join(str(x) for x in app.get("spine") or ())
    return "\n".join(
        [
            f"# {app['app_id']} — scoped nano app (APPPEAK)",
            "",
            f"> Wave AJ5 **H-APPPEAK** · Spine: `{spine}`",
            f"> Claim: {app['claim']}",
            "",
            "## Job",
            "",
            f"Surface `{app['surface']}` — dual-arm LOOKUP + GENERATE on AJ0.",
            "",
            "## Arms",
            "",
...
def depl_aj_body() -> str:
    """Overview one-pager for AJ dual-arm packaged deploy."""
    lines = [
...
def page_sync_report(path: str, text: str) -> dict[str, Any]:
    ...
    for m in AJ_STACK_MARKERS:
        if m not in body:
            miss.append(m)
    for m in FORBIDDEN_MARKERS:
        if m not in body:
            miss.append(f"forbidden:{m}")
    if "npm run nano:apppeak" not in body:
        miss.append("npm_apppeak")
...
def app_dual_stats(
    *,
    ...
    """
    if len(lookup_scores) != APPPEAK_N or len(gen_scores) != APPPEAK_N:
        raise ValueError(f"APPPEAK app requires {APPPEAK_N} dual-arm scores")
    if len(serve_gen_scores) != int(n_in_scope):
        raise ValueError("serve_gen_scores must match n_in_scope")
    l_mean = float(sum(lookup_scores) / float(APPPEAK_N))
    g_pack = float(sum(gen_scores) / float(APPPEAK_N))
    g_mean = (
...
def decide_apppeak(stats: Mapping[str, Any]) -> str:
    """
    GIVEN APPPEAK wave stats
    WHEN applying pesquisa §3 AJ5 gate
