"""Render H-CUR smoke vs B2 markdown."""
...
def render(smoke_path: Path, matrix_path: Path) -> str:
    ...
    kept = [r for r in matrix.get("rows", []) if r.get("family") != "H-CUR"]
    stats = mean_by_family(kept + smoke["rows"])
    s = stats.get("H-CUR", {})
    decision = decide_hcur(s, stats) if s else "needs H-CUR rows"
    b2 = stats.get("B2", {})
...
def main() -> int:
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hcur-vs-b2.md"),
    )
    args = p.parse_args()
