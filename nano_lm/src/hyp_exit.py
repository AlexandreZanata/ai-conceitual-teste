"""H-EXIT: evolve earlier-exit (min_new↓, n=1) with FLOP-aware score."""
...
from exit_ops import clamp_exit_gene, mutate_exit_gene, random_exit_gene
...
def run_h_exit(
    *,
    ...
    pop = [random_exit_gene(rng) for _ in range(pop_size)]
    ...
    for gen in range(generations):
        ...
        if scores[ranked[0]] > best_score:
            ...
            best_gene = clamp_exit_gene(pop[ranked[0]])
        parents = [pop[i] for i in ranked[: max(1, pop_size // 2)]]
        pop = [
            mutate_exit_gene(parents[i % len(parents)], rng) for i in range(pop_size)
        ]
