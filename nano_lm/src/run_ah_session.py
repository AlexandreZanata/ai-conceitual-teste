"""Wave AH0 SESSION runner (nano:ah:session) — freeze held-out HITL×10."""
...
_OUT = REPO / "results/nano-lm/wave-ah/ah0_session.json"
_TRIALS = REPO / "results/nano-lm/wave-ah/trials"
_ERROR_BANK = REPO / "results/nano-lm/wave-ah/error_bank.jsonl"
...
def main() -> int:
    ...
    ids = [str(p["source_id"]) for p in AH0_PACK]
    ...
    trials_ready = Path(args.trials_dir).is_dir() and len(written) == AH0_N

    decision = _override_decision(
        decide_ah0_session(known_sources=known, trials_dir_ready=trials_ready),
        miss=miss,
        clash=clash,
    ...
    payload = {
        ...
        "ask_smoke": ask,
        "public_note": "docs/results/nano-lm/wave-ah-session.md",
        "rule": "pesquisa §5 AH0 · Cursor ASK→EVAL→FIX dual-arm law",
        "next": "AH1 H-GENLIFT (gen lift + ASK dual-arm)",
        "anti_fp": (
