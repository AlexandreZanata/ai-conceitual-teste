"""Wave AP5 H-APPBASE: apps expose LOOKUP vs GENERATE + DEPL-AP honesty."""
...
APPBASE_ID = "H-APPBASE"
APPBASE_N = 10
MIN_APPS = 3  # known + howto + longdoc (AP0 surfaces)
MIN_PAGES = 4  # 3 apps + depl-ap
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0  # pesquisa §3 AP5 — or honest HOLD
APPPUSH_GEN_MEAN = 4.0  # parent AI5 SERVE gen (HOLD)
APPCORE_GEN_MEAN = 9.0  # peer AO5 APPCORE SERVE
SERVEALIGN_MEAN = 3.4
DEPL_AP_PAGE = "docs/results/nano-lm/depl-ap.md"

AP_STACK_MARKERS: tuple[str, ...] = (
    "H-APPBASE",
    "LOOKUP",
    "GENERATE",
    "H-FASTBASE",
...
APPBASE_APPS: tuple[dict[str, Any], ...] = (
    {
        "app_id": "app-known",
        "surface": "known-ask",
        ...
        "one_pager": "docs/results/nano-lm/appbase-known.md",
        "npm": "npm run nano:appbase -- --app app-known",
    },
    {
        "app_id": "app-howto",
        "surface": "howto",
        ...
        "one_pager": "docs/results/nano-lm/appbase-howto.md",
        "npm": "npm run nano:appbase -- --app app-howto",
    },
    {
        "app_id": "app-longdoc",
        "surface": "long-doc",
        ...
        "one_pager": "docs/results/nano-lm/appbase-longdoc.md",
        "npm": "npm run nano:appbase -- --app app-longdoc",
    },
)


def app_by_id(app_id: str) -> dict[str, Any]:
    for app in APPBASE_APPS:
        if app["app_id"] == app_id:
            return dict(app)
    raise KeyError(f"unknown appbase id: {app_id}")
    ...
    *,
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPBASE LOOKUP OOS — not generative IQ"
        ]
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "APPBASE LOOKUP labeled — not generative IQ",
    ]
...
def score_appbase_gen(
    *,
    ...
    """GENERATE on app surface — OOS refuse ok; SERVE uses GENBASE peak."""
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPBASE GENERATE OOS refuse — honest"
        ]
    ...
    if gold == ".." and text == "..":
        ...
        notes = [
            ...
            "APPBASE GENERATE GENBASE peak — Cursor scores completion",
            f"beat APPPUSH SERVE gen={APPPUSH_GEN_MEAN} / peer APPCORE="
            f"{APPCORE_GEN_MEAN}",
...
def one_pager_body(app: Mapping[str, Any]) -> str:
    """Public one-pager for an APPBASE packaged app (dual-arm)."""
    spine = " → ".join(str(x) for x in app.get("spine") or ())
    return "\n".join(
        [
            f"# {app['app_id']} — scoped nano app (APPBASE)",
            "",
            f"> Wave AP5 **H-APPBASE** · Spine: `{spine}`",
            f"> Claim: {app['claim']}",
            "",
            "## Job",
            "",
            f"Surface `{app['surface']}` — dual-arm LOOKUP + GENERATE on AP0.",
            "",
            "## Arms",
            "",
...
def depl_ap_body() -> str:
    """Overview one-pager for AO dual-arm packaged deploy."""
    lines = [
...
def page_sync_report(path: str, text: str) -> dict[str, Any]:
    ...
    for m in AP_STACK_MARKERS:
        if m not in body:
            miss.append(m)
    for m in FORBIDDEN_MARKERS:
        if m not in body:
            miss.append(f"forbidden:{m}")
    if "npm run nano:appbase" not in body:
        miss.append("npm_appbase")
...
def app_dual_stats(
    *,
    ...
    """
    if len(lookup_scores) != APPBASE_N or len(gen_scores) != APPBASE_N:
        raise ValueError(f"APPBASE app requires {APPBASE_N} dual-arm scores")
    if len(serve_gen_scores) != int(n_in_scope):
        raise ValueError("serve_gen_scores must match n_in_scope")
    l_mean = float(sum(lookup_scores) / float(APPBASE_N))
    g_pack = float(sum(gen_scores) / float(APPBASE_N))
    g_mean = (
    ...
    return {
        ...
        "peers_appcore_gen": g_mean >= float(APPCORE_GEN_MEAN),
...
def appbase_stats(
    ...
    *,
    ...
    return {
        ...
        "peers_appcore_gen": gen_mean >= float(APPCORE_GEN_MEAN),
        "min_lookup_mean": MIN_LOOKUP_MEAN,
        "min_gen_mean": MIN_GEN_MEAN,
        "apppush_gen_mean": APPPUSH_GEN_MEAN,
        "appcore_gen_mean": APPCORE_GEN_MEAN,
        "servealign_mean": SERVEALIGN_MEAN,
    }


def decide_appbase(stats: Mapping[str, Any]) -> str:
    """
    GIVEN APPBASE wave stats
    WHEN applying pesquisa §3 AP5 gate
