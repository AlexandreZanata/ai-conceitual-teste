"""Wave AP4 H-FASTBASE runner: GENBASE peak-fast gen wall vs AN FASTCORE."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ap/fastbase_summary.json"
...
def _load_chunks(source_id: str, curated: Path) -> tuple[str, list[str]]:
    ...
    return doc, chunk_doc(doc, win=400, stride=160)
...
def _seed_pack(bank_path: Path, ao_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(AP0_PACK, start=1):
        q = str(item["question"]).strip()
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AP-FASTBASE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = FASTBASE_ID
        row["judge_notes"] = [
            "FASTBASE seed for AO held-out ask",
...
def _timed_gen(
    *,
    ...
    """GENERATE arm: GENBASE peak-extractive (wall_ms>0, no LOOKUP cache)."""
    t0 = time.perf_counter()
    payloads: list[dict[str, Any]] = []
    for item in items:
        doc, parts = chunks[item["source_id"]]
        payloads.append(
            fastbase_generate(
    ...
    *,
    ...
    for _ in range(max(1, rounds)):
        ...
        if wall < best_wall or (wall == best_wall and e2e < best_e2e):
            ...
            best = payloads
    if best is None:
        raise RuntimeError("FASTBASE hot gen produced no payloads")
...
def _lookup_trial(
    *,
    ...
    return {
        "trial_id": f"AP-FASTBASE-LOOKUP-HITL-{i:02d}",
        "stage": "AP4",
        "hyp_id": FASTBASE_ID,
        "arm": "LOOKUP",
        "app_id": item["app_id"],
...
def _gen_trial(
    *,
    ...
    tel = extract_telemetry(payload)
    return {
        "trial_id": f"AP-FASTBASE-GEN-HITL-{i:02d}",
        "stage": "AP4",
        "hyp_id": FASTBASE_ID,
        "arm": "GENERATE",
        "app_id": item["app_id"],
...
def run_fastbase(
    *,
    ...
    """
    GIVEN AP0 held-out asks
    WHEN LOOKUP quality + GENBASE peak-fast GENERATE cold/warm/hot
    THEN dual-arm FASTBASE vs AN FASTCORE → PROMOTE|HOLD|KILL.
    """
    if len(AP0_PACK) != FASTBASE_N:
        raise ValueError("AP0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack(bank_path, ao_bank)
    bank = load_bank_rows(bank_path)
    items = [dict(p) for p in AP0_PACK]
    ...
    for i, (item, payload) in enumerate(
        ...
        if kind != "TRUE_HIT":
            row = alias_bank_row(
                trial_id=f"AP-FASTBASE-FIX-{i:02d}",
                question=item["question"],
                source_id=item["source_id"],
                gold=item["gold"],
            )
    ...
    decision = decide_fastbase(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "GENERATE PEAK_FAST+GENBASE (no student decode)",
            f"cold+warm+hot gen timing (hot rounds={_GEN_HOT_ROUNDS})",
            f"vs FASTCORE hot {FASTCORE_HOT_WALL_MS:.3f} ms",
        ],
        "forbidden": [
            "STREAM",
            "KVCACHE-Q",
        ...
        "timing": {
            ...
            "fastcore_hot_wall_ms": FASTCORE_HOT_WALL_MS,
        ...
        "finding": (
            f"{FASTBASE_ID}: gen wall "
            f"{cold_wall:.2f}/{warm_wall:.2f}/{hot_wall:.2f}ms "
            f"(FASTCORE hot {FASTCORE_HOT_WALL_MS:.2f}) "
            f"e2e {cold_e2e:.0f}/{warm_e2e:.0f}/{hot_e2e:.0f} "
            f"L={stats['lookup_mean']:.1f} G={stats['gen_mean']:.1f} "
            f"wall_ok={n_gen_wall_ok}/10 "
            f"vs_fe={stats['pass_vs_fastcore']} "
            f"floor={stats['pass_quality_floor']} → {decision}"
        ),
        "public_note": "docs/results/nano-lm/formal-hfastbase-fastbase.md",
        "ship_claim": "AF packaged stack (peak-fast; LOOKUP≠speed IQ)",
        "claim": (
            "faster GENBASE peak-extractive gen vs FASTCORE with wall_ms>0 — "
        ...
        json.dumps(
            {
                ...
                "pass_vs_fastcore": st["pass_vs_fastcore"],
                ...
                "fastcore_hot_wall_ms": st["fastcore_hot_wall_ms"],
