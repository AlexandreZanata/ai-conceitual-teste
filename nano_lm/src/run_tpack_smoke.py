"""Smoke H-TPACK: PRE3 ms/step pack vs live H-STAG (not e2e)."""
...
def main() -> int:
    ...
    if device.type != "cuda":
        print("ERROR: H-TPACK requires CUDA", file=sys.stderr)
        return 2
    out = c["out"]
    ...
    print(json.dumps({"n_rows": len(rows), "out": str(out / "tpack_smoke.json")}))
    return 0
