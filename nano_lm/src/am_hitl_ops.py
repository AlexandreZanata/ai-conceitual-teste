"""Wave AM6 AM-HITL-10: final dual-arm verify on frozen AM0 pack."""
...
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0
# Declared AM wave stack (fresh spines + SEMWRAP/ASKFAST inherit).
...
STACK_CLAIM = (
    "scoped AM dual-arm next verify on AF packaged stack — "
    "LOOKUP ≠ generative IQ; not open chat LM"
)
SHIP_CLAIM_AF = (
    "scoped AF packaged stack — not open chat LM "
    "(AM gen arm below bar; ship claim unchanged)"
)
...
def score_am6_lookup(
    *,
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "AM6 LOOKUP final — not generative IQ",
    ]
    if arm != "LOOKUP":
...
def score_am6_gen(
    *,
    ...
    """GENERATE final — wall_ms>0 ∧ n_new>0; GENTRUTH peak rubric."""
    ...
    notes = list(notes) + [
        "AM6 GENERATE final — Cursor scores completion",
        "GENTRUTH grounded+peak path (labeled ≠ open-chat IQ)",
    ]
    return float(score), bool(err), notes
...
def am6_stats(
    *,
    ...
    WHEN summarizing AM6
    THEN lookup≥7 · errors≤3/arm · gen≥5 or HOLD path · held-out ok.
    """
    if len(lookup_scores) != AM6_N or len(gen_scores) != AM6_N:
        raise ValueError(f"AM6 requires {AM6_N} dual-arm scores")
    l_mean = float(sum(lookup_scores) / float(AM6_N))
    g_mean = float(sum(gen_scores) / float(AM6_N))
    ...
    return {
        ...
        "pass_gen_telemetry": int(n_gen_wall_ok) >= AM6_N,
        "pass_mean": PASS_MEAN,
...
def decide_am6(stats: Mapping[str, Any]) -> str:
    """
    GIVEN AM6 final dual-arm stats
    WHEN applying pesquisa §3 AM6 gate
