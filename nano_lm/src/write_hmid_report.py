"""Render H-MID smoke vs H-EARLY tip (FLOP dual gate)."""
...
def render(path: Path) -> str:
    ...
    s = stats.get("H-MID", {})
    decision = decide_hmid(s, stats) if s else "needs H-MID rows"
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/mid_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hmid-vs-hearly.md"),
    )
    args = p.parse_args()
