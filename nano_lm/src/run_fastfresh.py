"""Wave AL4 H-FASTFRESH runner: GENFRESH peak-fast gen wall vs AK FASTMORE."""
...
_SUMMARY = REPO / "results/nano-lm/wave-al/fastfresh_summary.json"
...
def _seed_pack(bank_path: Path, al_bank: Path) -> int:
    ...
    for i, item in enumerate(AL0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AL-FASTFRESH-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = FASTFRESH_ID
        row["judge_notes"] = [
            "FASTFRESH seed for AL held-out ask",
...
def _timed_gen(
    *,
    ...
    """GENERATE arm: GENFRESH peak-extractive (wall_ms>0, no LOOKUP cache)."""
    ...
    *,
    ...
    for _ in range(max(1, rounds)):
        ...
        if wall < best_wall or (wall == best_wall and e2e < best_e2e):
            ...
            best = payloads
    if best is None:
        raise RuntimeError("FASTFRESH hot gen produced no payloads")
...
def _lookup_trial(
    *,
    ...
    return {
        "trial_id": f"AL-FASTFRESH-LOOKUP-HITL-{i:02d}",
        "stage": "AL4",
        "hyp_id": FASTFRESH_ID,
        "arm": "LOOKUP",
        "app_id": item["app_id"],
...
def _gen_trial(
    *,
    ...
    tel = extract_telemetry(payload)
    return {
        "trial_id": f"AL-FASTFRESH-GEN-HITL-{i:02d}",
        "stage": "AL4",
        "hyp_id": FASTFRESH_ID,
        "arm": "GENERATE",
        "app_id": item["app_id"],
...
def run_fastfresh(
    *,
    ...
    """
    GIVEN AL0 held-out asks
    WHEN LOOKUP quality + GENFRESH peak-fast GENERATE cold/warm/hot
    THEN dual-arm FASTFRESH vs AK FASTMORE → PROMOTE|HOLD|KILL.
    """
    if len(AL0_PACK) != FASTFRESH_N:
        raise ValueError("AL0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack(bank_path, al_bank)
    bank = load_bank_rows(bank_path)
    items = [dict(p) for p in AL0_PACK]
    ...
    for i, (item, payload) in enumerate(
        ...
        if kind != "TRUE_HIT":
            row = alias_bank_row(
                trial_id=f"AL-FASTFRESH-FIX-{i:02d}",
                question=item["question"],
                source_id=item["source_id"],
                gold=item["gold"],
            )
    ...
    decision = decide_fastfresh(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "GENERATE PEAK_FAST+GENFRESH (no student decode)",
            f"cold+warm+hot gen timing (hot rounds={_GEN_HOT_ROUNDS})",
            f"vs FASTMORE hot {FASTMORE_HOT_WALL_MS:.3f} ms",
        ],
        "forbidden": [
            "STREAM",
            "KVCACHE-Q",
        ...
        "timing": {
            ...
            "fastmore_hot_wall_ms": FASTMORE_HOT_WALL_MS,
        ...
        json.dumps(
            {
                ...
                "pass_vs_fastmore": st["pass_vs_fastmore"],
                ...
                "fastmore_hot_wall_ms": st["fastmore_hot_wall_ms"],
