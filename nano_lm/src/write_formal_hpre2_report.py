"""Render formal H-PRE2 vs H-ADAMF (2-deep prefetch under ADAMF)."""
...
def render(path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "Commands: `npm run nano:formal:hpre2` → "
            "`npm run nano:formal:hpre2:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hpre2/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hpre2-vs-hadamf.md"),
    )
    args = p.parse_args()
