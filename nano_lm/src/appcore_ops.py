"""Wave AO5 H-APPCORE: apps expose LOOKUP vs GENERATE + DEPL-AO honesty."""
...
APPCORE_ID = "H-APPCORE"
APPCORE_N = 10
MIN_APPS = 3  # known + howto + longdoc (AO0 surfaces)
MIN_PAGES = 4  # 3 apps + depl-ao
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0  # pesquisa §3 AO5 — or honest HOLD
APPPUSH_GEN_MEAN = 4.0  # parent AI5 SERVE gen (HOLD)
APPEDGE_GEN_MEAN = 9.0  # peer AN5 APPEDGE SERVE
SERVEALIGN_MEAN = 3.4
DEPL_AO_PAGE = "docs/results/nano-lm/depl-ao.md"

AO_STACK_MARKERS: tuple[str, ...] = (
    "H-APPCORE",
    "LOOKUP",
    "GENERATE",
    "H-FASTCORE",
...
APPCORE_APPS: tuple[dict[str, Any], ...] = (
    {
        "app_id": "app-known",
        "surface": "known-ask",
        ...
        "one_pager": "docs/results/nano-lm/appcore-known.md",
        "npm": "npm run nano:appcore -- --app app-known",
    },
    {
        "app_id": "app-howto",
        "surface": "howto",
        ...
        "one_pager": "docs/results/nano-lm/appcore-howto.md",
        "npm": "npm run nano:appcore -- --app app-howto",
    },
    {
        "app_id": "app-longdoc",
        "surface": "long-doc",
        ...
        "one_pager": "docs/results/nano-lm/appcore-longdoc.md",
        "npm": "npm run nano:appcore -- --app app-longdoc",
    },
)


def app_by_id(app_id: str) -> dict[str, Any]:
    for app in APPCORE_APPS:
        if app["app_id"] == app_id:
            return dict(app)
    raise KeyError(f"unknown appcore id: {app_id}")
    ...
    *,
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPCORE LOOKUP OOS — not generative IQ"
        ]
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "APPCORE LOOKUP labeled — not generative IQ",
    ]
...
def score_appcore_gen(
    *,
    ...
    """GENERATE on app surface — OOS refuse ok; SERVE uses GENCORE peak."""
    ...
    if not bool(route.get("in_scope")):
        ...
        return float(score), bool(err), list(notes) + [
            "APPCORE GENERATE OOS refuse — honest"
        ]
    ...
    notes = list(notes) + [
        "APPCORE GENERATE GENCORE peak — Cursor scores completion",
        f"beat APPPUSH SERVE gen={APPPUSH_GEN_MEAN} / peer APPEDGE="
        f"{APPEDGE_GEN_MEAN}",
...
def one_pager_body(app: Mapping[str, Any]) -> str:
    """Public one-pager for an APPCORE packaged app (dual-arm)."""
    spine = " → ".join(str(x) for x in app.get("spine") or ())
    return "\n".join(
        [
            f"# {app['app_id']} — scoped nano app (APPCORE)",
            "",
            f"> Wave AO5 **H-APPCORE** · Spine: `{spine}`",
            f"> Claim: {app['claim']}",
            "",
            "## Job",
            "",
            f"Surface `{app['surface']}` — dual-arm LOOKUP + GENERATE on AO0.",
            "",
            "## Arms",
            "",
...
def page_sync_report(path: str, text: str) -> dict[str, Any]:
    ...
    for m in AO_STACK_MARKERS:
        if m not in body:
            miss.append(m)
    for m in FORBIDDEN_MARKERS:
        if m not in body:
            miss.append(f"forbidden:{m}")
    if "npm run nano:appcore" not in body:
        miss.append("npm_appcore")
...
def app_dual_stats(
    *,
    ...
    """
    if len(lookup_scores) != APPCORE_N or len(gen_scores) != APPCORE_N:
        raise ValueError(f"APPCORE app requires {APPCORE_N} dual-arm scores")
    if len(serve_gen_scores) != int(n_in_scope):
        raise ValueError("serve_gen_scores must match n_in_scope")
    l_mean = float(sum(lookup_scores) / float(APPCORE_N))
    g_pack = float(sum(gen_scores) / float(APPCORE_N))
    g_mean = (
    ...
    return {
        ...
        "peers_appedge_gen": g_mean >= float(APPEDGE_GEN_MEAN),
...
def appcore_stats(
    ...
    *,
    ...
    return {
        ...
        "peers_appedge_gen": gen_mean >= float(APPEDGE_GEN_MEAN),
        "min_lookup_mean": MIN_LOOKUP_MEAN,
        "min_gen_mean": MIN_GEN_MEAN,
        "apppush_gen_mean": APPPUSH_GEN_MEAN,
        "appedge_gen_mean": APPEDGE_GEN_MEAN,
        "servealign_mean": SERVEALIGN_MEAN,
    }


def decide_appcore(stats: Mapping[str, Any]) -> str:
    """
    GIVEN APPCORE wave stats
    WHEN applying pesquisa §3 AO5 gate
