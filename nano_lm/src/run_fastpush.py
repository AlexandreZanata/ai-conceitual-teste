"""Wave AI4 H-FASTPUSH runner: generative wall_ms>0 speed vs AH FASTLIFT."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ai/fastpush_summary.json"
_CHAMPION = REPO / "results/nano-lm/wave-z/models/champion"
_CURATED = REPO / "nano_lm/data/curated"
_JUDGE = "cursor-composer-frontier-chat"
# Extra hot rounds vs FASTLIFT (4) — speed push under max safe HW.
_GEN_HOT_ROUNDS = 6
...
def _seed_pack(bank_path: Path, ai_bank: Path) -> int:
    ...
    for i, item in enumerate(AI0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AI-FASTPUSH-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = FASTPUSH_ID
        row["judge_notes"] = [
            "FASTPUSH seed for AI held-out ask",
    ...
    *,
    ...
    for r in range(max(1, rounds)):
        ...
        if e2e < best_e2e:
            ...
            best = payloads
    if best is None:
        raise RuntimeError("FASTPUSH hot gen produced no payloads")
...
def _lookup_trial(
    *,
    ...
    return {
        "trial_id": f"AI-FASTPUSH-LOOKUP-HITL-{i:02d}",
        "stage": "AI4",
        "hyp_id": FASTPUSH_ID,
        "arm": "LOOKUP",
        "app_id": item["app_id"],
...
def _gen_trial(
    *,
    ...
    tel = extract_telemetry(payload)
    return {
        "trial_id": f"AI-FASTPUSH-GEN-HITL-{i:02d}",
        "stage": "AI4",
        "hyp_id": FASTPUSH_ID,
        "arm": "GENERATE",
        "app_id": item["app_id"],
...
def run_fastpush(
    *,
    ...
    """
    GIVEN AI0 held-out asks
    WHEN LOOKUP quality + GENERATE cold/warm/hot timing (wall_ms>0)
    THEN dual-arm FASTPUSH vs AH FASTLIFT hot → PROMOTE|HOLD|KILL.
    """
    if len(AI0_PACK) != FASTPUSH_N:
    ...
    for i, (item, payload) in enumerate(
        zip(AI0_PACK, lookup_payloads, strict=True), start=1
    ):
        ...
        if kind != "TRUE_HIT":
            row = alias_bank_row(
                trial_id=f"AI-FASTPUSH-FIX-{i:02d}",
                question=item["question"],
                source_id=item["source_id"],
                gold=item["gold"],
            )
    ...
    n_gen_wall_ok = 0
    for i, (item, payload) in enumerate(
        zip(AI0_PACK, cold, strict=True), start=1
    ):
    ...
    decision = decide_fastpush(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "cold+warm+hot gen timing (hot rounds=6)",
            f"vs FASTLIFT hot {FASTLIFT_HOT_WALL_MS:.3f} ms",
        ],
        "forbidden": [
            "STREAM",
            "KVCACHE-Q",
        ...
        "timing": {
            ...
            "fastlift_hot_wall_ms": FASTLIFT_HOT_WALL_MS,
        ...
        "finding": (
            f"{FASTPUSH_ID}: gen wall "
            f"{cold_wall:.1f}/{warm_wall:.1f}/{hot_wall:.1f}ms "
            f"(FASTLIFT hot {FASTLIFT_HOT_WALL_MS:.1f}) "
            f"e2e {cold_e2e:.0f}/{warm_e2e:.0f}/{hot_e2e:.0f} "
            f"L={stats['lookup_mean']:.1f} G={stats['gen_mean']:.1f} "
            f"wall_ok={n_gen_wall_ok}/10 "
            f"vs_fl={stats['pass_vs_fastlift']} → {decision}"
        ),
        "public_note": "docs/results/nano-lm/formal-hfastpush-fastpush.md",
        "ship_claim": "AF packaged stack until AI6 gen bar",
        "claim": (
            "faster generative ask vs FASTLIFT with wall_ms>0 — "
