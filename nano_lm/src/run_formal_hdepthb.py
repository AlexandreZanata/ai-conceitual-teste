"""Formal H-DEPTHB: DEPTH_prun under LAYB vs tip LAYB on B2 (eval)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hdepthb"
...
def _run_seed(
    ...
    depth_ckpt = c["depth_dir"] / f"HDEPTH_prun_seed{seed}.pt"
    if not depth_ckpt.is_file():
        raise FileNotFoundError(f"missing formal DEPTH_prun: {depth_ckpt}")
    ...
    row = tip_row("H-DEPTHB", f"HDEPTHB_formal_seed{seed}", depthb, seed, g)
    row["density"] = dens
    return [
        tip_row("H-LAYB", f"HLAYB_depthb_formal_seed{seed}", layb, seed, g_tip),
        row,
    ]
...
def run_formal() -> int:
    ...
    payload = {
        ...
        "mode": "DEPTH_prun under LAYB vs B2+LAYB; dual-budget; long eval",
    }
