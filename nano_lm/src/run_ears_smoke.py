"""Smoke H-EARS: scheduled early-exit thr vs H-EARLY tip."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_ears(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=out / f"HEARS_seed{seed}_train.json",
        )
        row = {
            "family": "H-EARS",
        ...
        write_json(out / f"HEARS_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "ears_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "ears_smoke.json")}))
    return 0
