"""Wave AP3 H-SMARTBASE runner: trideca cite LOOKUP + GENBASE peak GENERATE."""
...
_AO_BANK = REPO / "results/nano-lm/wave-ap/error_bank.jsonl"
_TRIALS = REPO / "results/nano-lm/wave-ap/trials"
_SUMMARY = REPO / "results/nano-lm/wave-ap/smartbase_summary.json"
...
def _seed_pack(bank_path: Path, ao_bank: Path) -> int:
    bank_path.parent.mkdir(parents=True, exist_ok=True)
    ao_bank.parent.mkdir(parents=True, exist_ok=True)
    if not ao_bank.is_file():
        ao_bank.write_text("", encoding="utf-8")
    existing = {
        str(r.get("question", "")).strip() for r in load_bank_rows(bank_path)
    }
    n = 0
    for i, item in enumerate(SMARTBASE_PACK, start=1):
        for q_key, q_text in (
            ...
            if q in existing:
                continue
            row = alias_bank_row(
                trial_id=f"AP-SMARTBASE-SEED-{q_key}-{i:02d}",
                question=q,
                source_id=item["source_id"],
                gold=item["gold"],
            )
            row["hyp_id"] = SMARTBASE_ID
            row["judge_notes"] = [
                "SMARTBASE seed for trideca-hop paraphrase stress",
                "LOOKUP product path — not generative IQ",
                "no student weight update",
            ]
            append_error_row(bank_path, row)
            append_error_row(ao_bank, row)
            existing.add(q)
            n += 1
    return n
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AP-SMARTBASE-FIX-{i:02d}",
...
def run_smartbase(
    *,
    ...
    """
    GIVEN AP0 trideca-hop paraphrases + parent asks
    WHEN SEMWRAP LOOKUP + GENBASE peak GENERATE dual-arm ×10
    THEN false-hit≈0; PROMOTE if gen≥5 else HOLD.
    """
    if len(SMARTBASE_PACK) != SMARTBASE_N:
        raise ValueError("SMARTBASE pack must be 10")
    if not hard_paraphrase_ok():
        raise ValueError("paraphrases must differ from parents")
    if not has_adversarial_noise() or not has_trideca_hop_cues():
        raise ValueError("trideca-hop adversarial pack incomplete")

    trials_dir.mkdir(parents=True, exist_ok=True)
    t0 = time.perf_counter()
    seeded = _seed_pack(bank_path, ao_bank)
    bank = load_bank_rows(bank_path)
    paras = [p["paraphrase"] for p in SMARTBASE_PACK]
    gen_items = [
        {
    ...
    for i, (item, lp) in enumerate(
        zip(SMARTBASE_PACK, lookup_payloads, strict=True),
        start=1,
    ):
        ...
        score_l, err_l, notes_l, cited = score_smartbase_lookup(
        ...
        lookup_trials.append(
            {
                "trial_id": f"AP-SMARTBASE-LOOKUP-HITL-{i:02d}",
                "stage": "AP3",
                "hyp_id": SMARTBASE_ID,
                "arm": "LOOKUP",
                "app_id": item["app_id"],
                ...
                "duodenary_source": item["duodenary_source"],
                "tredenary_source": item["tredenary_source"],
    ...
    for i, (item, gp) in enumerate(
        zip(SMARTBASE_PACK, gen_payloads, strict=True)
    ):
        score_g, err_g, notes_g = score_smartbase_gen(
            completion=str(gp.get("completion", "")),
            expected_gold=item["gold"],
            payload=gp,
        )
        comp = str(gp.get("completion", ""))
        gold = str(item["gold"]).strip()
        # Struct-update `..` exact gold is not period collapse.
        if is_period_collapse(comp) and not (gold == ".." and comp.strip() == ".."):
            n_period += 1
    ...
    if weak_idx:
        re_items = [
            {
                "question": SMARTBASE_PACK[i]["parent_question"],
                "source_id": SMARTBASE_PACK[i]["source_id"],
                "gold": SMARTBASE_PACK[i]["gold"],
            }
            for i in weak_idx
        ]
        ...
        for j, idx in enumerate(weak_idx):
            item = SMARTBASE_PACK[idx]
            re_gp = re_payloads[j]
            score2, err2, notes2 = score_smartbase_gen(
            ...
            if score2 > gen_scores[idx]:
                ...
                gen_notes[idx] = list(notes2) + [
                    "FIX: re-peak GENBASE grounded decode"
                ]
                gen_fix[idx] = 1
                fix_count += 1
            else:
                gen_notes[idx] = list(gen_notes[idx]) + [
                    "FIX attempted: re-peak GENBASE (no lift)"
                ]
    ...
    for i, (item, gp) in enumerate(
        ...
        if bool(gp.get("peak_used")):
            n_peak += 1
        gt = {
            "trial_id": f"AP-SMARTBASE-GEN-HITL-{i:02d}",
            "stage": "AP3",
            "hyp_id": SMARTBASE_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
    ...
    decision = decide_smartbase(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "trideca-hop paraphrase (CTXBASE companions)",
            "QPFB2+BEAMKV+GROUNDED+ANTI_PERIOD+GENBASE_PEAK GENERATE",
        ],
        "forbidden": [
            "QI",
            "STREAM",
        ...
        "finding": (
            f"{SMARTBASE_ID}: L_lookup={stats['lookup_mean']:.1f} "
            f"L_gen={stats['gen_mean']:.1f} cite={stats['n_cite_ok']}/10 "
            f"false_hit={n_false} period={n_period} peak={n_peak} "
            f"beats_genbase_ablated={stats['beats_genbase_ablated']} "
            f"peers_smartcore={stats['peers_smartcore_gen']} "
            f"pass_gen={stats['pass_gen']} fix={fix_count} → {decision}"
        ),
        "public_note": "docs/results/nano-lm/formal-hsmartbase-smartbase.md",
        "ship_claim": "AF packaged stack (trideca cite+peak; LOOKUP≠gen IQ)",
    }
...
def main() -> int:
    ...
    ap.add_argument("--ao-bank", type=Path, default=_AO_BANK)
    ...
    try:
        summary = run_smartbase(
            bank_path=Path(args.bank),
            ao_bank=Path(args.ao_bank),
            root=Path(args.root),
            out=Path(args.out),
