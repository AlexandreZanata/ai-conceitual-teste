"""Wave AP2 H-CTXBASE: trideca-doc beyond CTXCORE; dual-arm Cursor EVAL."""
...
from ctxcore_ops import TOP_K_SLICES_CORE
...
__all__ = [
    ...
    "companions_for",
    "ctxbase_doc_meta",
    "score_ctxbase_lookup",
    "score_ctxbase_gen",
    "ctxbase_stats",
    "decide_ctxbase",
]

CTXBASE_ID = "H-CTXBASE"
CTXBASE_N = 10
MIN_LOOKUP_USABLE = 7
MIN_GEN_USABLE = 5
MIN_SOURCES = 13  # trideca-doc beyond CTXCORE dodeca
# Deeper than CTXCORE K=25.
TOP_K_SLICES_BASE = 27
# Evidence: docs/results/nano-lm/formal-hctxcore-ctxcore.md
CTXCORE_MEAN_LEFF = 253105.4

# Primary AP0 source_id → twelve companions (all thirteen incl. primary distinct).
CTXBASE_COMPANIONS: Mapping[
    str,
...
def companions_for(
    ...
    """
    GIVEN AP0 primary source_id
    WHEN resolving CTXBASE companions
    THEN return twelve distinct companion ids.
    """
    sid = str(source_id)
    if sid not in CTXBASE_COMPANIONS:
        raise KeyError(f"no CTXBASE companions for {sid}")
    return CTXBASE_COMPANIONS[sid]


def ctxbase_doc_meta(
    doc_token_ids: Sequence[Sequence[int]],
    question_ids: Sequence[int],
    *,
    source_ids: Sequence[str],
    k: int = TOP_K_SLICES_BASE,
) -> dict[str, Any]:
    """
    GIVEN thirteen curated docs + question tokens
    WHEN building CTXBASE context
    THEN trideca multi-slice metas with n_sources≥13 and L_eff vs CTXCORE.
    """
    if len(doc_token_ids) != MIN_SOURCES or len(source_ids) != MIN_SOURCES:
        raise ValueError(f"CTXBASE requires {MIN_SOURCES} docs/sources")
    if len(set(source_ids)) != MIN_SOURCES:
        raise ValueError("CTXBASE sources must be distinct")
    parts: dict[str, Any] = {}
    combined_l = 0
    combined_slices = 0
    ...
    return {
        ...
        **{f"{k}_source": parts[f"{k}_source"] for k in _DOC_KEYS},
        ...
        "multi_source": True,
        "above_ctxcore_leff": float(combined_l) > float(CTXCORE_MEAN_LEFF),
        "deeper_than_ctxcore_k": int(k) > int(TOP_K_SLICES_CORE),
    }
...
def score_ctxbase_lookup(
    *,
    ...
    """LOOKUP arm + trideca-doc — labeled LOOKUP ≠ gen IQ."""
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        (
            f"CTXBASE trideca L_eff={meta.get('l_eff')} "
    ...
    if not ctx_ok and not err:
        return score, True, notes + ["FIX: trideca-doc/ctx gate failed"], False
    return float(score), bool(err), notes, bool(usable)


def score_ctxbase_gen(
    *,
    completion: str,
    expected_gold: str,
    meta: Mapping[str, Any],
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        (
            f"CTXBASE gen long-ctx L_eff={meta.get('l_eff')} "
...
def ctxbase_stats(
    *,
    ...
    """Summarize AP2: L_eff↑ vs CTXCORE · trideca · gen usable≥5."""
    if len(lookup_scores) != CTXBASE_N or len(gen_scores) != CTXBASE_N:
        raise ValueError(f"CTXBASE requires {CTXBASE_N} dual-arm scores")
    l_mean = float(sum(lookup_scores) / float(CTXBASE_N))
    g_mean = float(sum(gen_scores) / float(CTXBASE_N))
    ...
    return {
        ...
        "pass_leff_up": float(mean_l_eff) > float(CTXCORE_MEAN_LEFF),
        "pass_active": float(mean_active) <= float(ACTIVE_CAP),
        "pass_slices": float(mean_slices) >= float(TOP_K_SLICES_BASE),
...
def decide_ctxbase(stats: Mapping[str, Any]) -> str:
    """
    GIVEN CTXBASE stats
    WHEN applying pesquisa §3 AP2 gate
    THEN PROMOTE if L_eff↑ vs CTXCORE ∧ trideca ∧ gen usable≥5 ∧ lookup ok;
