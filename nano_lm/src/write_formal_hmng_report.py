"""Render formal H-MNG vs tip markdown."""
...
def render(path: Path) -> str:
    ...
    decision = decide_hmng(stats.get("H-MNG", {}), stats)
    max_lp = tip_max_lp(stats) or float("nan")
    lines = [
        "# Formal H-MNG vs H-MINP × H-NGRAM",
        "",
        f"Source: `{path}`",
    ...
    for name in ("H-MINP", "H-NGRAM", "H-MNG"):
        if name not in stats:
            continue
        st = stats[name]
        delta = "—" if name != "H-MNG" else f"{st['mean_lp'] - max_lp:+.4f}"
        lines.append(
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:mng` → "
            "`npm run nano:formal:mng:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hmng/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hmng-vs-tips.md"),
    )
    args = p.parse_args()
