"""Render formal H-HOP vs B2 markdown."""
...
def render(path: Path) -> str:
    ...
    decision = decide_hhop(stats.get("H-HOP", {}), stats)
    hyp = stats.get("H-HOP", {})
    b2 = stats.get("B2", {})
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:hhop` → "
            "`npm run nano:formal:hhop:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hhop/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hhop-vs-b2.md"),
    )
    args = p.parse_args()
