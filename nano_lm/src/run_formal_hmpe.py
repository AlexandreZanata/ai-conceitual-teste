"""Formal-budget H-MPE vs H-MINP, shared B2 ckpts."""
...
from hyp_minp import run_h_minp
from hyp_mpe import run_h_mpe
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hmpe"
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        minp_meta = run_h_minp(
            ...
            seed=seed,
            out_meta=c["out"] / f"HMINP_seed{seed}_train.json",
        )
        rows.append(
            {
                "family": "H-MINP",
                "label": f"HMINP_seed{seed}",
                "teacher_mean_logprob": float(minp_meta["eval_fit"]),
                "mean_wall_ms": float(minp_meta["eval_wall_ms"]),
                "best_min_p": float(minp_meta["best_min_p"]),
                "seed": seed,
            }
        )
        if device.type == "cuda":
        ...
        meta = run_h_mpe(
            ...
            seed=seed,
            lam=MIN_LAM,
            out_meta=c["out"] / f"HMPE_seed{seed}_train.json",
        )
        row = {
            "family": "H-MPE",
            "label": f"HMPE_seed{seed}",
