"""Wave AK3 H-SMARTMORE runner: octa cite LOOKUP + GENTRUE peak GENERATE."""
...
from run_gentrue import _run_gen_ablation
...
_SUMMARY = REPO / "results/nano-lm/wave-ak/smartmore_summary.json"
...
def _seed_pack(bank_path: Path, ak_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(SMARTMORE_PACK, start=1):
        for q_key, q_text in (
            ...
            if q in existing:
                continue
            row = alias_bank_row(
                trial_id=f"AK-SMARTMORE-SEED-{q_key}-{i:02d}",
                question=q,
                source_id=item["source_id"],
                gold=item["gold"],
            )
            row["hyp_id"] = SMARTMORE_ID
            row["judge_notes"] = [
                "SMARTMORE seed for octa-hop paraphrase stress",
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AK-SMARTMORE-FIX-{i:02d}",
...
def run_smartmore(
    *,
    ...
    """
    GIVEN AK0 octa-hop paraphrases + parent asks
    WHEN SEMWRAP LOOKUP + GENTRUE peak GENERATE dual-arm ×10
    THEN false-hit≈0; PROMOTE if gen≥5 else HOLD.
    """
    if len(SMARTMORE_PACK) != SMARTMORE_N:
        raise ValueError("SMARTMORE pack must be 10")
    if not hard_paraphrase_ok():
        raise ValueError("paraphrases must differ from parents")
    if not has_adversarial_noise() or not has_octa_hop_cues():
        raise ValueError("octa-hop adversarial pack incomplete")

    trials_dir.mkdir(parents=True, exist_ok=True)
    t0 = time.perf_counter()
    seeded = _seed_pack(bank_path, ak_bank)
    bank = load_bank_rows(bank_path)
    paras = [p["paraphrase"] for p in SMARTMORE_PACK]
    gen_items = [
        {
    ...
    for i, (item, lp) in enumerate(
        zip(SMARTMORE_PACK, lookup_payloads, strict=True),
        start=1,
    ):
        ...
        score_l, err_l, notes_l, cited = score_smartmore_lookup(
        ...
        lookup_trials.append(
            {
                "trial_id": f"AK-SMARTMORE-LOOKUP-HITL-{i:02d}",
                "stage": "AK3",
                "hyp_id": SMARTMORE_ID,
                "arm": "LOOKUP",
                "app_id": item["app_id"],
    ...
    for i, (item, gp) in enumerate(
        zip(SMARTMORE_PACK, gen_payloads, strict=True)
    ):
        score_g, err_g, notes_g = score_smartmore_gen(
    ...
    if weak_idx:
        re_items = [
            {
                "question": SMARTMORE_PACK[i]["parent_question"],
                "source_id": SMARTMORE_PACK[i]["source_id"],
                "gold": SMARTMORE_PACK[i]["gold"],
            }
            for i in weak_idx
        ]
        ...
        for j, idx in enumerate(weak_idx):
            item = SMARTMORE_PACK[idx]
            re_gp = re_payloads[j]
            score2, err2, notes2 = score_smartmore_gen(
            ...
            if score2 > gen_scores[idx]:
                ...
                gen_notes[idx] = list(notes2) + [
                    "FIX: re-peak GENTRUE grounded decode"
                ]
                gen_fix[idx] = 1
                fix_count += 1
            else:
                gen_notes[idx] = list(gen_notes[idx]) + [
                    "FIX attempted: re-peak GENTRUE (no lift)"
                ]
    ...
    for i, (item, gp) in enumerate(
        ...
        if bool(gp.get("peak_used")):
            n_peak += 1
        gt = {
            "trial_id": f"AK-SMARTMORE-GEN-HITL-{i:02d}",
            "stage": "AK3",
            "hyp_id": SMARTMORE_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
    ...
    decision = decide_smartmore(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "octa-hop paraphrase (CTXMORE companions)",
            "QPFB2+BEAMKV+GROUNDED+ANTI_PERIOD+GENTRUE_PEAK GENERATE",
        ],
        "forbidden": [
            "QI",
            "STREAM",
        ...
        "finding": (
            f"{SMARTMORE_ID}: L_lookup={stats['lookup_mean']:.1f} "
        ...
        "public_note": "docs/results/nano-lm/formal-hsmartmore-smartmore.md",
