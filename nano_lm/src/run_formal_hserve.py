"""Formal H-SERVE: best frozen stack vs tip EARLY alone (eval)."""
...
from serve_pair import SMOKE_BUDGETS, run_seed_pair


def formal_cfg() -> dict[str, Any]:
    base = hgraphf_formal_cfg()
    base["out"] = REPO / "results/nano-lm/formal-hserve"
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-SERVE formal requires CUDA")
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        rows.extend(run_seed_pair(c, seed, prompts, claim_offset=9090))
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "mode": "best of GALL-speed / GRAPHF-quality vs serial EARLY",
        },
    )
