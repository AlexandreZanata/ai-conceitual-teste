"""Formal H-PIN: pinned H2D vs pageable H-TOP (formal KD budget)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hpin"
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        rows.extend(
            run_seed_pair(
                c, out, seed, device, vocab, steps, label_prefix="HPIN_formal"
            )
        )
        if device.type == "cuda":
