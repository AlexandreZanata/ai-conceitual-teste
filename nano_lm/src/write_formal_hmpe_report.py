"""Render formal H-MPE vs H-MINP markdown."""
...
def render(path: Path) -> str:
    ...
    decision = decide_hmpe(stats.get("H-MPE", {}), stats)
    hyp = stats.get("H-MPE", {})
    ...
    for name in ("H-MINP", "H-MPE"):
        if name not in stats:
            continue
        st = stats[name]
        delta = "—" if name == "H-MINP" else f"{d_lp:+.4f}"
        lines.append(
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:mpe` → "
            "`npm run nano:formal:mpe:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hmpe/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hmpe-vs-hminp.md"),
    )
    args = p.parse_args()
