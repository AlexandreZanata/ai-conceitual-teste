"""H-POOL3: FLOP-aware POOL search with n≤3 + tip warm-start."""
...
def _init_pop(rng: random.Random, tip: dict | None, pop_size: int) -> list[dict]:
    if tip is None:
        return [random_pool3_gene(rng) for _ in range(pop_size)]
    warm = max(1, pop_size // 2)
    pop = [seed_pool3_from_tip(tip, rng) for _ in range(warm)]
    while len(pop) < pop_size:
        pop.append(random_pool3_gene(rng))
    return pop
    ...
    *,
    ...
    for gen in range(generations):
        ...
        if scores[ranked[0]] > best_score:
            ...
            best_gene = clamp_pool3_gene(pop[ranked[0]])
        parents = [pop[i] for i in ranked[: max(1, pop_size // 2)]]
        pop = [
            mutate_pool3_gene(parents[i % len(parents)], rng) for i in range(pop_size)
        ]
