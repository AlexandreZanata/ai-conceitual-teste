"""Formal-budget H-ANN vs KD-cos (+ B2), equal KD steps."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hann",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 4,
...
def _train_eval(
    ...
    *,
    ...
    ckpt = c["out"] / f"{prefix}_seed{seed}.pt"
    meta = schedule_fn(
        ...
        lr=c["lr"],
        seed=seed,
        temperature=float(c["ann_temp_start"]),
        alpha=0.5,
        out_path=ckpt,
        temp_end=float(c["ann_temp_end"]),
    )
    write_json(c["out"] / f"{prefix}_seed{seed}_train.json", meta)
    return eval_ckpt(c, ckpt, seed, family)
