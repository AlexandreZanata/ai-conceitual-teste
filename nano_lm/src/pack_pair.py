"""Shared H-EARLY / H-SERVE / H-SROUTE seed trio for H-PACK."""
...
from pack_score import SMOKE_BUDGETS, score_pack_trio, tip_row
from poolb_score import throughput_gene

__all__ = ["run_seed_trio", "PACK_CHUNK", "SMOKE_BUDGETS"]
...
def run_seed_trio(
    ...
    prompts: list[str],
    *,
    claim_offset: int = 9797,
    budgets: tuple[int, ...] = SMOKE_BUDGETS,
) -> list[dict[str, Any]]:
    """
    GIVEN shared B2 + tip genes
    WHEN scoring EARLY, SERVE, and SROUTE packs
    ...
    recipe, early_m, serve, sroute = score_pack_trio(
        teacher=teacher,
        student=student,
        prompts=prompts,
        early_gene=early,
