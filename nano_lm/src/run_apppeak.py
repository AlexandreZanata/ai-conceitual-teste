"""Wave AJ5 H-APPPEAK runner: dual-arm apps + DEPL-AJ (nano:apppeak)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-aj/apppeak_summary.json"
...
def _seed_pack(bank_path: Path, aj_bank: Path) -> int:
    ...
    for i, item in enumerate(AJ0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AJ-APPPEAK-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = APPPEAK_ID
        row["judge_notes"] = [
            "APPPEAK seed for AJ held-out ask",
...
def _write_pages(docs: Path) -> list[dict[str, Any]]:
    ...
    depl_path = docs / Path(DEPL_AJ_PAGE).name
    depl_body = depl_aj_body()
    depl_path.write_text(depl_body, encoding="utf-8")
    reports.append(page_sync_report(DEPL_AJ_PAGE, depl_body))
    return reports
...
def _run_one_app(
    *,
    ...
    if len(lookup_payloads) != APPPEAK_N or len(shared_gen) != APPPEAK_N:
    ...
    for i, item in enumerate(AJ0_PACK, start=1):
        ...
        if not bool(route["in_scope"]):
            ...
            row = alias_bank_row(
                trial_id=f"AJ-APPPEAK-FIX-{tag}-{i:02d}",
        ...
        l_score, l_err, l_notes = score_apppeak_lookup(
            mode=l_mode,
            completion=l_comp,
        ...
        l_trial = {
            "trial_id": f"AJ-APPPEAK-{tag}-LOOKUP-HITL-{i:02d}",
            "stage": "AJ5",
            "hyp_id": APPPEAK_ID,
            "arm": "LOOKUP",
            "realapp_id": app_id,
        ...
        g_score, g_err, g_notes = score_apppeak_gen(
        ...
        g_trial = {
            "trial_id": f"AJ-APPPEAK-{tag}-GEN-HITL-{i:02d}",
            "stage": "AJ5",
            "hyp_id": APPPEAK_ID,
            "arm": "GENERATE",
            "realapp_id": app_id,
...
def run_apppeak(
    *,
    ...
    """
    GIVEN AJ0 pack + APPPEAK apps
    WHEN ASK→EVAL→FIX×10 dual-arm per surface + DEPL-AJ
    THEN expose LOOKUP|GENERATE ∧ DEPL → PROMOTE|HOLD|KILL.
    """
    if len(AJ0_PACK) != APPPEAK_N:
    ...
    bank = load_bank_rows(bank_path)
    apps = [dict(a) for a in APPPEAK_APPS]
    if only_app:
        apps = [app_by_id(only_app)]

    # Shared GENPEAK GENERATE once — max HW without 3× CUDA reload.
    gen_items = [dict(p) for p in AJ0_PACK]
    shared_gen = _run_gen_peak(
        champ=root,
        items=gen_items,
    ...
    summary: dict[str, Any] = {
        ...
        "public_note": "docs/results/nano-lm/formal-happpeak-apppeak.md",
