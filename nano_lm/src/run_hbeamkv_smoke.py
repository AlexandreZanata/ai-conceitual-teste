"""Smoke H-BEAMKV: shared KV prefill for PFB K=2 on QT; wall↓ vs indep."""
...
def main() -> int:
    ...
    # Leave 2 cores free — avoid host thrash on 16-thread laptop.
    threads = tune_cpu_threads(max(4, int(os.cpu_count() or 4) - 2))
    c = matrix_cfg()
    device = resolve_device(True)
    if device.type != "cuda":
        print("ERROR: H-BEAMKV requires CUDA", file=sys.stderr)
        return 2
    ...
    payload = {
        ...
        "max_new": _MAX_NEW,
        "cpu_threads": threads,
        "mode": "BEAMKV: shared prompt KV vs indep prefills on QT+PFB K=2",
        "mechanism": "prefill once + expand past; wall gate vs K indep KV prefills",
        "parent": "H-QT int8 EARLY n=1 on B2 (QPFB2 recipe freeze)",
    }
    write_json(out / "hbeamkv_smoke.json", payload)
    print(json.dumps({"decision": decision, "out": str(out / "hbeamkv_smoke.json")}))
    return 0
