"""Wave AG6 AG-HITL-10: final dual-arm verify on frozen AG0 pack."""
...
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0
# Declared AG wave stack (anti-FP dual-arm + AF product inherit).
...
STACK_CLAIM = (
    "scoped AG dual-arm verify on AF packaged stack — "
    "LOOKUP ≠ generative IQ; not open chat LM"
)
SHIP_CLAIM_AF = (
    "scoped AF packaged stack — not open chat LM "
    "(AG gen arm below bar; ship claim unchanged)"
)
...
def score_ag6_lookup(
    *,
    ...
    """
    GIVEN LOOKUP arm on final AG pack
    WHEN Cursor EVAL
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "AG6 LOOKUP final — not generative IQ",
    ]
    if arm != "LOOKUP":
...
def score_ag6_gen(
    *,
    ...
    """
    GIVEN GENERATE arm on final AG pack
    WHEN Cursor EVAL
    ...
    notes = list(notes) + [
        f"arm={arm} mode={tel['mode']} wall_ms={tel['wall_ms']} "
        f"n_new={tel['n_new']}",
        "AG6 GENERATE final — Cursor scores completion",
    ]
...
def ag6_stats(
    *,
    ...
    WHEN summarizing AG6
    THEN lookup≥7 · errors≤3/arm · gen≥5 or HOLD path · held-out ok.
    """
    if len(lookup_scores) != AG6_N or len(gen_scores) != AG6_N:
        raise ValueError(f"AG6 requires {AG6_N} dual-arm scores")
    l_mean = float(sum(lookup_scores) / float(AG6_N))
    g_mean = float(sum(gen_scores) / float(AG6_N))
    ...
    return {
        ...
        "pass_gen_telemetry": int(n_gen_wall_ok) >= AG6_N,
        "pass_mean": PASS_MEAN,
...
def decide_ag6(stats: Mapping[str, Any]) -> str:
    """
    GIVEN AG6 final dual-arm stats
    WHEN applying pesquisa §5 AG6 gate
