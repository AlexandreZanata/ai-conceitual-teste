"""Wave AN1 H-GENEDGE runner: dual-arm + peak ablation true-gen gate."""
...
_SUMMARY = REPO / "results/nano-lm/wave-an/genedge_summary.json"
...
def _seed_pack(bank_path: Path, an_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(GENEDGE_PACK, start=1):
        q = str(item["question"]).strip()
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AN-GENEDGE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = GENEDGE_ID
        row["judge_notes"] = [
            "GENEDGE seed for LOOKUP arm",
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AN-GENEDGE-FIX-{i:02d}",
...
def run_genedge(
    *,
    ...
    """
    if len(GENEDGE_PACK) != GENEDGE_N:
        raise ValueError("GENEDGE pack must be 10")

    trials_dir.mkdir(parents=True, exist_ok=True)
    t0 = time.perf_counter()
    seeded = _seed_pack(bank_path, an_bank)
    bank = load_bank_rows(bank_path)
    questions = [p["question"] for p in GENEDGE_PACK]
    ...
    gen_off, gen_on = _run_gen_ablation(
        champ=root,
        items=[dict(p) for p in GENEDGE_PACK],
    ...
    for i, (item, lp) in enumerate(
        zip(GENEDGE_PACK, lookup_payloads, strict=True),
        start=1,
    ):
        ...
        score_l, err_l, notes_l = score_genedge_lookup(
        ...
        lookup_trials.append(
            {
                "trial_id": f"AN-GENEDGE-LOOKUP-HITL-{i:02d}",
                "stage": "AN1",
                "hyp_id": GENEDGE_ID,
                "arm": "LOOKUP",
                "app_id": item["app_id"],
    ...
    for i, (item, off, on) in enumerate(
        zip(GENEDGE_PACK, gen_off, gen_on, strict=True)
    ):
        score_g, err_g, notes_g = score_genedge_gen(
            completion=str(off.get("completion", "")),
            expected_gold=item["gold"],
            payload=off,
            peak_ablated=True,
        )
        score_p, _err_p, notes_p = score_genedge_gen(
    ...
    fix_attempts = 0
    if weak_idx:
        re_items = [dict(GENEDGE_PACK[i]) for i in weak_idx]
        ...
        for j, idx in enumerate(weak_idx):
            item = GENEDGE_PACK[idx]
            score2, err2, notes2 = score_genedge_gen(
                completion=str(re_off[j].get("completion", "")),
                expected_gold=item["gold"],
                payload=re_off[j],
                peak_ablated=True,
            )
            score_p2, _, _ = score_genedge_gen(
    ...
    for i, (item, off, on) in enumerate(
        ...
        if bool(on.get("peak_used")):
            n_peak += 1
        gt = {
            "trial_id": f"AN-GENEDGE-GEN-HITL-{i:02d}",
            "stage": "AN1",
            "hyp_id": GENEDGE_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
    ...
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{GENEDGE_ID}: L_lookup={stats['lookup_mean']:.1f} "
        ...
        "public_note": "docs/results/nano-lm/formal-hgenedge-genedge.md",
