"""Render formal H-THIN vs H-CURL (same EARLY decode)."""
...
def render(path: Path) -> str:
    ...
    decision = decide_hthin(stats.get("H-THIN", {}), stats)
    tip = stats.get("H-CURL", {})
    hyp = stats.get("H-THIN", {})
    ...
    lines.extend(
        [
            ...
            f"**Decision:** {decision}",
            "",
            "Commands: `npm run nano:formal:hthin` → "
            "`npm run nano:formal:hthin:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hthin/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hthin-vs-hcurl.md"),
    )
    args = p.parse_args()
