"""Wave AD1 H-HARDPARA runner: adversarial para SEMWRAP+SMARTPLUS (nano:hardpara)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ad/hardpara_summary.json"
...
def _seed_parent_and_para(bank_path: Path, ad_bank: Path) -> int:
    """Seed AD0 parents + HARDPARA paraphrases into wrap banks."""
    ...
    n = 0
    for i, item in enumerate(HARDPARA_PACK, start=1):
        for q_key, q_text in (
            ...
            if q in existing:
                continue
            row = alias_bank_row(
                trial_id=f"AD-HARDPARA-SEED-{q_key}-{i:02d}",
                question=q,
                source_id=item["source_id"],
                gold=item["gold"],
            )
            row["hyp_id"] = HARDPARA_ID
            row["judge_notes"] = [
                "HARDPARA seed for adversarial paraphrase stress",
...
def _classify(
    ...
    text, route = route_hardpara(completion, mode=mode)
...
def _fix_alias(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AD-HARDPARA-FIX-{i:02d}",
...
def _build_trial(
    *,
    ...
    trial: dict[str, Any] = {
        ...
        "judge_notes": notes,
        "manual_adjust": (
            "no change — HARDPARA true-hit"
            if lookup_kind == "TRUE_HIT" and not err
            else "FIX: adversarial paraphrase alias / SEMWRAP margin"
        ),
...
def run_hardpara(
    *,
    ...
    """
    GIVEN AD0 adversarial paraphrases
    WHEN SEMWRAP retrieve + ASKSMART polish + FIX
    THEN mean≥7 · false-hit=0 → PROMOTE|HOLD|KILL.
    """
    if len(HARDPARA_PACK) != HARDPARA_N:
        raise ValueError("HARDPARA pack must be 10")
    if not hard_paraphrase_ok():
        raise ValueError("HARDPARA paraphrases must differ from parents")
    if not has_adversarial_noise():
        raise ValueError("HARDPARA pack must carry adversarial noise cues")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_parent_and_para(bank_path, ad_bank)
    bank = load_bank_rows(bank_path)

    questions = [p["paraphrase"] for p in HARDPARA_PACK]
    ...
    decision = decide_hardpara(stats)
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{HARDPARA_ID}: mean={stats['mean']:.1f} "
            f"false_hit={n_false} true_hit={n_true} "
            f"fix={fix_count} decision={decision}."
        ),
        "public_note": "docs/results/nano-lm/formal-hhardpara-hardpara.md",
        "claim": "adversarial-paraphrase scoped retrieve — not open chat LM",
    }
