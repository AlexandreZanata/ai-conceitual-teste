"""Wave AN4 H-FASTEDGE runner: GENEDGE peak-fast gen wall vs AM FASTNEXT."""
...
_SUMMARY = REPO / "results/nano-lm/wave-an/fastedge_summary.json"
...
def _seed_pack(bank_path: Path, an_bank: Path) -> int:
    ...
    for i, item in enumerate(AN0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AN-FASTEDGE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = FASTEDGE_ID
        row["judge_notes"] = [
            "FASTEDGE seed for AN held-out ask",
...
def _timed_gen(
    *,
    ...
    """GENERATE arm: GENEDGE peak-extractive (wall_ms>0, no LOOKUP cache)."""
    ...
    *,
    ...
    for _ in range(max(1, rounds)):
        ...
        if wall < best_wall or (wall == best_wall and e2e < best_e2e):
            ...
            best = payloads
    if best is None:
        raise RuntimeError("FASTEDGE hot gen produced no payloads")
...
def _lookup_trial(
    *,
    ...
    return {
        "trial_id": f"AN-FASTEDGE-LOOKUP-HITL-{i:02d}",
        "stage": "AN4",
        "hyp_id": FASTEDGE_ID,
        "arm": "LOOKUP",
        "app_id": item["app_id"],
...
def _gen_trial(
    *,
    ...
    tel = extract_telemetry(payload)
    return {
        "trial_id": f"AN-FASTEDGE-GEN-HITL-{i:02d}",
        "stage": "AN4",
        "hyp_id": FASTEDGE_ID,
        "arm": "GENERATE",
        "app_id": item["app_id"],
...
def run_fastedge(
    *,
    ...
    """
    GIVEN AN0 held-out asks
    WHEN LOOKUP quality + GENEDGE peak-fast GENERATE cold/warm/hot
    THEN dual-arm FASTEDGE vs AM FASTNEXT → PROMOTE|HOLD|KILL.
    """
    if len(AN0_PACK) != FASTEDGE_N:
        raise ValueError("AN0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack(bank_path, an_bank)
    bank = load_bank_rows(bank_path)
    items = [dict(p) for p in AN0_PACK]
    ...
    for i, (item, payload) in enumerate(
        ...
        if kind != "TRUE_HIT":
            row = alias_bank_row(
                trial_id=f"AN-FASTEDGE-FIX-{i:02d}",
                question=item["question"],
                source_id=item["source_id"],
                gold=item["gold"],
            )
    ...
    decision = decide_fastedge(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "GENERATE PEAK_FAST+GENEDGE (no student decode)",
            f"cold+warm+hot gen timing (hot rounds={_GEN_HOT_ROUNDS})",
            f"vs FASTNEXT hot {FASTNEXT_HOT_WALL_MS:.3f} ms",
        ],
        "forbidden": [
            "STREAM",
            "KVCACHE-Q",
        ...
        "timing": {
            ...
            "fastnext_hot_wall_ms": FASTNEXT_HOT_WALL_MS,
        ...
        json.dumps(
            {
                ...
                "pass_vs_fastnext": st["pass_vs_fastnext"],
                ...
                "fastnext_hot_wall_ms": st["fastnext_hot_wall_ms"],
