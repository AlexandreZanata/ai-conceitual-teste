"""Render formal H-CURL2 fine seq_lo grid vs tip lo=8."""
...
def render(path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            f"Best seq_lo: {best}.",
            "",
            "Commands: `npm run nano:formal:curl2` → "
            "`npm run nano:formal:curl2:report`.",
            "",
        ]
    )
    return "\n".join(lines)
...
def main() -> int:
    ...
    p.add_argument(
        "--formal",
        type=Path,
        default=Path("results/nano-lm/formal-hcurl2/formal.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/formal-hcurl2-vs-hcurl.md"),
    )
    args = p.parse_args()
