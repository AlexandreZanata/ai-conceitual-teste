"""Score H-GRAPHF: dual-budget FLAYB path with CUDA-graph non-KV arm."""
...
from decode_pool_lay_graph import decode_pool_lay_graph
...
def score_batch_graphf_arm(
    *,
    ...
    """Batched CUDA-graph BoN+LAY arm under caller SDPA."""
    g = _policy(gene)
    ...
    results, wall_ms = decode_pool_lay_graph(
        student,
        tok,
        prompts,
        n=int(g["n"]),
...
def score_batch_graphf(
    *,
    ...
    seed: int,
    kv_threshold: int,
    chunk_size: int = GRAPHF_CHUNK,
    budgets: tuple[int, ...] = SMOKE_BUDGETS,
) -> dict[str, float]:
    """
    GIVEN POOL + LAY + KVSEL under SDPA
    WHEN dual budgets (CPOOLB iff KV on; else CUDA-graph BoN+LAY)
