"""Formal H-ABS-PFB: parent-fallback story-floor BoN (fit≠eval genes)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hpfb"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-ABS-PFB formal requires CUDA")
    out: Path = c["out"]
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "max_new": _MAX_NEW,
            "cpu_threads": threads,
            "mode": "PFB formal: story≥P_s−ε → max code else parent (fit≠eval)",
