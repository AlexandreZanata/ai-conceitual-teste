"""Formal equal-budget: B2 KD vs H-SYM (obligate pair), 3 seeds."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hsym-b2",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 4,
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        hsym = c["out"] / f"HSYM_seed{seed}.pt"
        meta = run_h_sym(
            tokenizer_id=c["tokenizer_id"],
            cache_dir=c["cache"],
            device=device,
            pop_size=c["sym_pop"],
            generations=c["sym_gens"],
        ...
        write_json(c["out"] / f"HSYM_seed{seed}_train.json", meta)
        row = _eval(c, hsym, seed, "H-SYM")
        row["sterile_gens"] = int(meta.get("sterile_gens", 0))
        rows.append(row)
        if device.type == "cuda":
            torch.cuda.empty_cache()
