"""Wave AG4 H-FASTREAL runner: generative wall_ms>0 speed vs AF raw."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ag/fastreal_summary.json"
...
def _seed_pack(bank_path: Path, ag_bank: Path) -> int:
    ...
    for i, item in enumerate(AG0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AG-FASTREAL-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = FASTREAL_ID
        row["judge_notes"] = [
            "FASTREAL seed for AG held-out ask",
    ...
    *,
    ...
    for r in range(max(1, rounds)):
        ...
        if e2e < best_e2e:
            ...
            best = payloads
    if best is None:
        raise RuntimeError("FASTREAL hot gen produced no payloads")
...
def _lookup_trial(
    *,
    ...
    return {
        "trial_id": f"AG-FASTREAL-LOOKUP-HITL-{i:02d}",
        "stage": "AG4",
        "hyp_id": FASTREAL_ID,
        "arm": "LOOKUP",
        "app_id": item["app_id"],
...
def _gen_trial(
    *,
    ...
    tel = extract_telemetry(payload)
    return {
        "trial_id": f"AG-FASTREAL-GEN-HITL-{i:02d}",
        "stage": "AG4",
        "hyp_id": FASTREAL_ID,
        "arm": "GENERATE",
        "app_id": item["app_id"],
...
def run_fastreal(
    *,
    ...
    """
    GIVEN AG0 held-out asks
    WHEN LOOKUP quality + GENERATE cold/warm/hot timing (wall_ms>0)
    THEN dual-arm FASTREAL vs AF raw open baseline → PROMOTE|HOLD|KILL.
    """
    if len(AG0_PACK) != FASTREAL_N:
    ...
    # LOOKUP arm — product path (NOT speed IQ).
    ...
    for i, (item, payload) in enumerate(
        zip(AG0_PACK, lookup_payloads, strict=True), start=1
    ):
        ...
        if kind != "TRUE_HIT":
            row = alias_bank_row(
                trial_id=f"AG-FASTREAL-FIX-{i:02d}",
                question=item["question"],
                source_id=item["source_id"],
                gold=item["gold"],
            )
    ...
    # GENERATE arm — open decode; never AskCompletionCache (would force wall=0).
    ...
    n_gen_wall_ok = 0
    for i, (item, payload) in enumerate(
        zip(AG0_PACK, cold, strict=True), start=1
    ):
    ...
    decision = decide_fastreal(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "cold+warm+hot gen timing",
            f"AF raw open baseline {AF_RAW_OPEN_WALL_MS:.3f} ms",
        ],
        "forbidden": [
            "STREAM",
            "KVCACHE-Q",
        ...
        "finding": (
            f"{FASTREAL_ID}: gen wall "
            f"{cold_wall:.1f}/{warm_wall:.1f}/{hot_wall:.1f}ms "
            f"(AF raw {AF_RAW_OPEN_WALL_MS:.1f}) "
            f"e2e {cold_e2e:.0f}/{warm_e2e:.0f}/{hot_e2e:.0f} "
            f"L={stats['lookup_mean']:.1f} G={stats['gen_mean']:.1f} "
            f"wall_ok={n_gen_wall_ok}/10 decision={decision}."
        ),
        "public_note": "docs/results/nano-lm/formal-hfastreal-fastreal.md",
        "claim": (
            "faster generative ask with wall_ms>0 — "
