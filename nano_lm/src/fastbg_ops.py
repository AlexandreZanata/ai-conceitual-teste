"""Wave BG3 H-FASTBG: prod-path p50/p99 hold/lift · no anti-FP regress."""
...
FASTBG_THESIS = (
    ...
    "PROMOTE only if §1 anti-FP holds (BG-FOREVER FH 0 · BA…BF forever "
    "hold · AZ hold · over-refuse 0 · live FP 0) and live p99 does not "
    "regress vs BG0/H-FASTBF baseline — never warm-cache vanity; "
    "LOOKUP wall=0 ≠ speed IQ; sub-ms PEAK walls ≠ speed IQ; "
    "≠ BF H-FASTBF archive · ≠ BE H-FASTBE · ≠ BD H-FASTGAIN · "
    "≠ BC/AH H-FASTLIFT · ≠ BB H-FASTHOLD · ≠ BA H-FASTREAL · ≠ FP-for-ms"
)
FASTBG_CLAIM = BG0_SHIP_LOCK
FASTBG_SAFE_NOTE = BG0_SAFE_NOTE
FASTBG_ANTI_FP = BG0_ANTI_FP
FASTBG_BASELINE = BG0_SPEED_BASELINE
...
def extract_fastbg_board(
    *,
    ...
    """
    GIVEN latency tetrad + BG/BF/BE/BA…BD/AZ anti-FP hold packs
    WHEN building BG3 FASTBG board
    THEN publish p50/p99 · BG/BF/BE/BA…BD/AZ/orf · regress flags.
    """
    ...
    return {
        ...
        "baseline_source": str(FASTBG_BASELINE.get("source", "")),
        ...
        "bf_fastbf_archive_untouched": True,
...
def _gate_tel_paths(board: Mapping[str, Any]) -> str | None:
    ...
    if not isinstance(latency, dict) or set(latency) != BD0_MODES:
        return (
            "KILL (latency paths incomplete — need "
...
def _gate_latency_flags(board: Mapping[str, Any]) -> str | None:
    ...
    if bool(board.get("p99_regress")):
        ...
        return f"KILL (live p99 regress vs H-FASTBF baseline: {paths})"
    flags = (
        (
            "lookup_wall_neq_speed_iq",
            "KILL (LOOKUP wall=0 must not be sold as speed IQ)",
        ),
        (
            "warm_cache_vanity_forbidden",
            "KILL (warm-cache vanity must stay forbidden)",
        ),
        ("fp_for_ms_forbidden", "KILL (FP-for-ms must stay forbidden)"),
        (
            "bf_fastbf_archive_untouched",
            "KILL (BF H-FASTBF archive must stay untouched)",
        ),
        (
...
def decide_fastbg(
    *,
    ...
    """
    GIVEN BG3 FASTBG board
    WHEN applying pesquisa §4 + §9 BG3
    ...
    if err:
        return err
    return (
        f"PROMOTE ({FASTBG_ID}: prod p50/p99 hold/lift; "
        "anti-FP hold; no live p99 regress vs H-FASTBF baseline)"
    )
