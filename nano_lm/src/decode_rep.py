"""EARLY decode with repetition-penalty / no-repeat n-gram."""
...
from rep_ops import apply_repetition_penalty, ban_ngram_logits
    ...
    for step in range(max_new_tokens):
        with torch.no_grad():
            ...
            logits = apply_repetition_penalty(
                logits, ids, penalty=float(rep_penalty)
            )
            logits = ban_ngram_logits(
                logits, ids, ngram=int(no_repeat_ngram)
            )
