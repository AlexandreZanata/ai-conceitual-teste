"""Wave AN6 AN-HITL-10: final dual-arm verify on frozen AN0 pack."""
...
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0
# Declared AN wave stack (edge spines + SEMWRAP/ASKFAST inherit).
...
STACK_CLAIM = (
    "scoped AN dual-arm edge verify on AF packaged stack — "
    "LOOKUP ≠ generative IQ; not open chat LM"
)
SHIP_CLAIM_AF = (
    "scoped AF packaged stack — not open chat LM "
    "(AN gen arm below bar; ship claim unchanged)"
)
...
def score_an6_lookup(
    *,
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "AN6 LOOKUP final — not generative IQ",
    ]
    if arm != "LOOKUP":
...
def score_an6_gen(
    *,
    ...
    """GENERATE final — wall_ms>0 ∧ n_new>0; GENEDGE peak rubric."""
    ...
    notes = list(notes) + [
        "AN6 GENERATE final — Cursor scores completion",
        "GENEDGE grounded+peak path (labeled ≠ open-chat IQ)",
    ]
    return float(score), bool(err), notes
...
def an6_stats(
    *,
    ...
    WHEN summarizing AN6
    THEN lookup≥7 · errors≤3/arm · gen≥5 or HOLD path · held-out ok.
    """
    if len(lookup_scores) != AN6_N or len(gen_scores) != AN6_N:
        raise ValueError(f"AN6 requires {AN6_N} dual-arm scores")
    l_mean = float(sum(lookup_scores) / float(AN6_N))
    g_mean = float(sum(gen_scores) / float(AN6_N))
    ...
    return {
        ...
        "pass_gen_telemetry": int(n_gen_wall_ok) >= AN6_N,
        "pass_mean": PASS_MEAN,
...
def decide_an6(stats: Mapping[str, Any]) -> str:
    """
    GIVEN AN6 final dual-arm stats
    WHEN applying pesquisa §3 AN6 gate
