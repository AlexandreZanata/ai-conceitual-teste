"""Wave AC5 AC-HITL-10 runner: final pack on declared AC stack (nano:ac:hitl)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ac/ac_hitl_summary.json"
...
def run_ac_hitl(
    *,
    ...
    """
    GIVEN AC0 pack + declared AC stack
    WHEN ASK→EVAL→FIX×10 via app router (known/howto/longdoc)
    THEN mean≥7 · errors≤3 · no false-hit → PROMOTE|HOLD|KILL.
    """
    if len(AC0_PACK) != AC5_N:
    ...
    if len(payloads) != AC5_N:
        raise RuntimeError(f"expected {AC5_N} payloads")
    ...
    for i, (item, payload) in enumerate(
        ...
        score_before, err_before, notes_before = score_ac5_trial(
            mode=str(payload.get("mode", "")),
            completion=str(payload.get("completion", "")),
            expected_gold=str(item["gold"]),
            lookup_kind=kind,
        ...
        trial: dict[str, Any] = {
            ...
            "question": item["question"],
            "source_id": item["source_id"],
            "recipe_id": payload.get("recipe_id") or "champion-ac-v0",
            "ckpt": None,
            "completion": completion,
            ...
            "manual_adjust": (
                "FIX: constrained SEMWRAP re-ASK"
                if fixed
                else "no change — AC5 final stack ok"
            ),
    ...
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{AC5_ID}: mean={stats['mean']:.1f} "
            f"errors={stats['n_errors']}/10 "
            f"false_hit={n_false} fix={fix_count} "
            f"decision={decision}."
        ),
        "public_note": "docs/results/nano-lm/wave-ac-hitl.md",
    }
