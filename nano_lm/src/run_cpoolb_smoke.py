"""Smoke H-CPOOLB: chunked prefill under POOLB vs flat H-POOLB (tok/s)."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        rows.append(
            tip_row("H-POOLB", f"HPOOLB_cpoolb_seed{seed}", poolb, seed, gene)
        )
        ...
        row = tip_row("H-CPOOLB", f"HCPOOLB_seed{seed}", cpoolb, seed, g)
        write_json(out / f"HCPOOLB_seed{seed}_eval.json", row)
        rows.append(row)
    write_json(
        out / "cpoolb_smoke.json",
        {
            "rows": rows,
            ...
            "mode": "POOL tip top_p; n=1 near-greedy; long prompts",
        },
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "cpoolb_smoke.json")}))
    return 0
