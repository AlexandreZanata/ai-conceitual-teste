"""Wave AP REPORT runner: write public closeout + anti-FP gate (nano:ap:report)."""
...
_OUT = REPO / "results/nano-lm/wave-ap/ap_report_summary.json"
_SUMMARY = REPO / "docs/results/nano-lm/wave-ap-summary.md"
_PAPER = REPO / "docs/results/nano-lm/paper-lab-wave-ap.md"
# Stub freeze path so AP7 evidence links resolve before AP8 writes real freeze.
_FREEZE_STUB = REPO / "docs/results/nano-lm/ap-freeze.md"
...
def _evidence_map() -> dict[str, bool]:
    return {p: (REPO / p).is_file() for p in AP_EVIDENCE}
...
def _stage_facts() -> dict[str, Any]:
    keys = {
        "genbase": "results/nano-lm/wave-ap/genbase_summary.json",
        "ctxbase": "results/nano-lm/wave-ap/ctxbase_summary.json",
        "smartbase": "results/nano-lm/wave-ap/smartbase_summary.json",
        "fastbase": "results/nano-lm/wave-ap/fastbase_summary.json",
        "appbase": "results/nano-lm/wave-ap/appbase_summary.json",
        "ap_hitl": "results/nano-lm/wave-ap/ap_hitl_summary.json",
    }
    out: dict[str, Any] = {}
...
def _smoke_dual_arm() -> dict[str, Any]:
    """LOOKUP + GENERATE smoke on first AP0 ask (anti-FP telemetry)."""
...
def _ensure_freeze_stub() -> None:
    """AP7 links ap-freeze.md; AP8 replaces with the formal freeze."""
    ...
    _FREEZE_STUB.write_text(
        "\n".join(
            [
                "# AP-FREEZE — placeholder (pending AP8)",
                "",
                "> Written by AP7 report so paper-lab links resolve. "
                "AP8 replaces this with the formal freeze.",
                "",
...
def run_ap_report(*, out: Path, skip_ask: bool = False) -> dict[str, Any]:
    """
    GIVEN AP0–AP6 evidence
    ...
    decision = decide_ap_report(evidence)
    ...
    if decision.startswith("PROMOTE") and not markers:
        decision = "KILL (wave-ap-summary missing thesis markers)"
    if decision.startswith("PROMOTE") and not board:
        decision = "KILL (wave-ap-summary missing dual-arm FIX scoreboard)"
    if decision.startswith("PROMOTE") and not antifp:
        decision = "KILL (wave-ap-summary missing anti-FP evidence)"
    ok = (
    ...
    payload: dict[str, Any] = {
        ...
        "scoreboard_ok": board,
        "antifp_ok": antifp,
        "scoreboard": list(AP_HITL_SCOREBOARD),
        "evidence": evidence,
        "stage_facts": _stage_facts(),
        "ask_smoke": ask,
        "public_report": "docs/results/nano-lm/wave-ap-summary.md",
        "paper_lab": "docs/results/nano-lm/paper-lab-wave-ap.md",
        "wave_status": "RESEARCH_COMPLETE" if ok else "OPEN",
        "ship_claim": "scoped AF packaged stack — not open chat LM",
        "cpu_threads": int(os.environ.get("OMP_NUM_THREADS") or 0),
        "finding": (
            f"{AP_ID}: decision={'PROMOTE' if ok else decision}; "
