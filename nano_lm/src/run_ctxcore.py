"""Wave AO2 H-CTXCORE runner: dodeca-doc dual-arm ASK→EVAL→FIX×10."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ao/ctxcore_summary.json"
...
def _seed_pack_golds(bank_path: Path, ao_bank: Path) -> int:
    ...
    for i, item in enumerate(AO0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AO-CTXCORE-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = CTXCORE_ID
        row["judge_notes"] = [
            "CTXCORE seed alias for held-out AN ask",
...
def _build_ctxs(
    *, curated_root: Path, workers: int, tok: Any
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        rows = list(pool.map(_read, [dict(p) for p in AO0_PACK]))
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AO-CTXCORE-FIX-{i:02d}",
...
def _lookup_trial(
    *,
    ...
    tid = f"AO-CTXCORE-LOOKUP-HITL-{i:02d}"
    mode = str(payload.get("mode", ""))
...
def _gen_trial(
    *,
    ...
    tid = f"AO-CTXCORE-GEN-HITL-{i:02d}"
    score, err, notes, usable = score_ctxcore_gen(
        completion=str(payload.get("completion", "")),
        expected_gold=str(item["gold"]),
        meta=ctx,
        payload=payload,
...
def run_ctxcore(
    *,
    ...
    """
    GIVEN AO0 held-out asks + dodeca curated docs
    WHEN K=25 ROLL/SUMCACHE + LOOKUP askfast + GENERATE raw
    THEN L_eff↑ vs CTXEDGE · gen usable≥5 → PROMOTE|HOLD|KILL.
    """
    if len(AO0_PACK) != CTXCORE_N:
    ...
    questions = [p["question"] for p in AO0_PACK]
    ...
    for i, (item, lp, gp, ctx) in enumerate(
        zip(AO0_PACK, lookup_payloads, gen_payloads, ctxs, strict=True),
        start=1,
    ):
    ...
    mean_l = float(sum(float(c["l_eff"]) for c in ctxs) / CTXCORE_N)
    mean_a = float(
        sum(float(c["sumcache_active"]) for c in ctxs) / CTXCORE_N
    )
    mean_s = float(sum(float(c["n_slices"]) for c in ctxs) / CTXCORE_N)
    mean_src = float(sum(float(c["n_sources"]) for c in ctxs) / CTXCORE_N)
    ...
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{CTXCORE_ID}: L_lookup={stats['lookup_mean']:.1f} "
            f"L_gen={stats['gen_mean']:.1f} "
            f"usable_L={stats['n_lookup_usable']}/10 "
            f"usable_G={stats['n_gen_usable']}/10 "
            f"L_eff={mean_l:.0f} (>CTXEDGE {stats['ctxedge_mean_leff']:.0f}) "
            f"sources={mean_src:.1f} slices={mean_s:.1f} "
            f"false_hit={n_false} fix={fix_count} decision={decision}."
        ),
        "public_note": "docs/results/nano-lm/formal-hctxcore-ctxcore.md",
        "ship_claim": "AF packaged stack (dodeca long-ctx; LOOKUP≠gen IQ)",
        "claim": (
            "dodeca-doc longer usable ctx beyond CTXEDGE parent — "
