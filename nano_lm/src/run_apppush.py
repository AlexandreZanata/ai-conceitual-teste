"""Wave AI5 H-APPPUSH runner: dual-arm apps + DEPL-AI (nano:apppush)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ai/apppush_summary.json"
...
def _seed_pack(bank_path: Path, ai_bank: Path) -> int:
    ...
    for i, item in enumerate(AI0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AI-APPPUSH-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = APPPUSH_ID
        row["judge_notes"] = [
            "APPPUSH seed for AI held-out ask",
...
def _write_pages(docs: Path) -> list[dict[str, Any]]:
    ...
    depl_path = docs / Path(DEPL_AI_PAGE).name
    depl_body = depl_ai_body()
    depl_path.write_text(depl_body, encoding="utf-8")
    reports.append(page_sync_report(DEPL_AI_PAGE, depl_body))
    return reports
...
def _run_one_app(
    *,
    ...
    if len(lookup_payloads) != APPPUSH_N or len(shared_gen) != APPPUSH_N:
    ...
    for i, item in enumerate(AI0_PACK, start=1):
        ...
        if not bool(route["in_scope"]):
            ...
            row = alias_bank_row(
                trial_id=f"AI-APPPUSH-FIX-{tag}-{i:02d}",
        ...
        l_score, l_err, l_notes = score_apppush_lookup(
            mode=l_mode,
            completion=l_comp,
        ...
        l_trial = {
            "trial_id": f"AI-APPPUSH-{tag}-LOOKUP-HITL-{i:02d}",
            "stage": "AI5",
            "hyp_id": APPPUSH_ID,
            "arm": "LOOKUP",
            "realapp_id": app_id,
        ...
        g_score, g_err, g_notes = score_apppush_gen(
        ...
        g_trial = {
            "trial_id": f"AI-APPPUSH-{tag}-GEN-HITL-{i:02d}",
            "stage": "AI5",
            "hyp_id": APPPUSH_ID,
            "arm": "GENERATE",
            "realapp_id": app_id,
...
def run_apppush(
    *,
    ...
    """
    GIVEN AI0 pack + APPPUSH apps
    WHEN ASK→EVAL→FIX×10 dual-arm per surface + DEPL-AI
    THEN expose LOOKUP|GENERATE ∧ DEPL → PROMOTE|HOLD|KILL.
    """
    if len(AI0_PACK) != APPPUSH_N:
    ...
    bank = load_bank_rows(bank_path)
    apps = [dict(a) for a in APPPUSH_APPS]
    if only_app:
        apps = [app_by_id(only_app)]

    # Shared grounded GENERATE once — max HW without 3× CUDA reload.
    gen_items = [
        {
            "question": p["question"],
    ...
    if only_app:
        wave = {
            ...
            "pass_gen": False,
            "pass_product": False,
            "beats_applift_gen": float(app_results[0]["gen_mean"]) > 1.0,
        }
        decision = (
            "KILL"
            if wave["n_kill"]
    ...
    summary: dict[str, Any] = {
        ...
        "finding": (
            ...
            f"beats_applift={wave.get('beats_applift_gen')} → {decision}"
        ),
        "public_note": "docs/results/nano-lm/formal-happpush-apppush.md",
        ...
        json.dumps(
            {
                ...
                "beats_applift_gen": st.get("beats_applift_gen"),
