"""Wave AH2 H-CTXLIFT runner: penta-doc dual-arm ASK→EVAL→FIX×10."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ah/ctxlift_summary.json"
...
def _seed_pack_golds(bank_path: Path, ah_bank: Path) -> int:
    ...
    for i, item in enumerate(AH0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AH-CTXLIFT-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = CTXLIFT_ID
        row["judge_notes"] = [
            "CTXLIFT seed alias for held-out AH ask",
...
def _build_ctxs(
    *, curated_root: Path, workers: int, tok: Any
    ...
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        rows = list(pool.map(_read, [dict(p) for p in AH0_PACK]))
    ctxs: list[dict[str, Any]] = []
    for item, p_text, s_text, t_text, q_text, n_text in rows:
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AH-CTXLIFT-FIX-{i:02d}",
...
def _lookup_trial(
    *,
    ...
    tid = f"AH-CTXLIFT-LOOKUP-HITL-{i:02d}"
    mode = str(payload.get("mode", ""))
...
def _gen_trial(
    *,
    ...
    tid = f"AH-CTXLIFT-GEN-HITL-{i:02d}"
...
def run_ctxlift(
    *,
    ...
    """
    GIVEN AH0 held-out asks + penta curated docs
    WHEN K=11 ROLL/SUMCACHE + LOOKUP askfast + GENERATE raw
    THEN L_eff↑ vs CTXREAL · gen usable≥5 → PROMOTE|HOLD|KILL.
    """
    if len(AH0_PACK) != CTXLIFT_N:
        raise ValueError("AH0 pack must be 10")
    trials_dir.mkdir(parents=True, exist_ok=True)
    seeded = _seed_pack_golds(bank_path, ah_bank)
    cfg = matrix_cfg()
    ...
    for i, (item, lp, gp, ctx) in enumerate(
        zip(AH0_PACK, lookup_payloads, gen_payloads, ctxs, strict=True),
        start=1,
    ):
    ...
    mean_l = float(sum(float(c["l_eff"]) for c in ctxs) / CTXLIFT_N)
    mean_a = float(
        sum(float(c["sumcache_active"]) for c in ctxs) / CTXLIFT_N
    )
    mean_s = float(sum(float(c["n_slices"]) for c in ctxs) / CTXLIFT_N)
    mean_src = float(sum(float(c["n_sources"]) for c in ctxs) / CTXLIFT_N)
