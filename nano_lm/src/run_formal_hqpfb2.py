"""Formal H-ABS-QPFB2: PFB K=2 on QT; wall↓ vs QPFB k=4 (fit≠eval)."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hqpfb2"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-ABS-QPFB2 formal requires CUDA")
    out: Path = c["out"]
    ...
    qpfb4_rows = commit_pfb_rows(
        code, banks4, parent_code_by_key=parent_code, family="H-ABS-QPFB",
        weight_bytes=nbytes,
    )
    qpfb2_rows = commit_pfb_rows(
        code, banks2, parent_code_by_key=parent_code, family="H-ABS-QPFB2",
        weight_bytes=nbytes,
    )
    _free_cuda(code)
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "max_new": _MAX_NEW,
            "cpu_threads": threads,
            "mode": "QPFB2 formal: QT + PFB K=2; wall↓ vs QPFB k=4 (fit≠eval)",
            "mechanism": "quantize_student_int8 then PFB K=2; wall gate vs QPFB k=4",
            "parent": "H-QT int8 EARLY n=1 on B2 (formal genes)",
        },
    )
