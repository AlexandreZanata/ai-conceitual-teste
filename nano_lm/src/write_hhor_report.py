"""Render H-HOR smoke vs H-POOL tip (frozen-n + horizon≤2 FLOP search)."""
...
from hor_ops import decide_hhor
...
def render(path: Path) -> str:
    ...
    s = stats.get("H-HOR", {})
    decision = decide_hhor(s, stats) if s else "needs H-HOR rows"
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/hor_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hhor-vs-hpool.md"),
    )
    args = p.parse_args()
