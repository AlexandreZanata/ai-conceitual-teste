"""H-EAR2: evolve widened early-exit gene vs H-EARLY tip."""
...
from ear2_ops import clamp_ear2_gene, mutate_ear2_gene, random_ear2_gene
...
def run_h_ear2(
    *,
    ...
    pop = [random_ear2_gene(rng) for _ in range(pop_size)]
    ...
    for gen in range(generations):
        ...
        history.append(
            {
                ...
                "best_metric": pop[ranked[0]]["conf_metric"],
        ...
        if scores[ranked[0]] > best_score:
            ...
            best_gene = clamp_ear2_gene(pop[ranked[0]])
        parents = [pop[i] for i in ranked[: max(1, pop_size // 2)]]
        pop = [
            mutate_ear2_gene(parents[i % len(parents)], rng) for i in range(pop_size)
        ]
    eval_lp, eval_wall = fitness_ear2_detail(
        best_gene,
        teacher=teacher,
        student=student,
