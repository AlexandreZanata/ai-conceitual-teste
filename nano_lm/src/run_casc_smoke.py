"""Smoke H-CASC on B2 ckpts; write casc_smoke.json (compare vs B4 in report)."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_casc(
            ...
            mid_k=MID_K,
            final_k=FINAL_K,
            out_meta=out / f"HCASC_seed{seed}_train.json",
        )
        row = {
            "family": "H-CASC",
            "label": f"HCASC_seed{seed}",
        ...
        write_json(out / f"HCASC_seed{seed}_eval.json", row)
        rows.append(row)
    wall_s = time.perf_counter() - t0
    write_json(out / "casc_smoke.json", {"rows": rows, "wall_s": wall_s})
    print(json.dumps({"n_rows": len(rows), "out": str(out / "casc_smoke.json")}))
    return 0
