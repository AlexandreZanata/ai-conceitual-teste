"""Wave AK6 AK-HITL-10: final dual-arm verify on frozen AK0 pack."""
...
MIN_LOOKUP_MEAN = 7.0
MIN_GEN_MEAN = 5.0
# Declared AK wave stack (more spines + SEMWRAP/ASKFAST inherit).
...
STACK_CLAIM = (
    "scoped AK dual-arm more verify on AF packaged stack — "
    "LOOKUP ≠ generative IQ; not open chat LM"
)
SHIP_CLAIM_AF = (
    "scoped AF packaged stack — not open chat LM "
    "(AK gen arm below bar; ship claim unchanged)"
)
...
def score_ak6_lookup(
    *,
    ...
    notes = list(notes) + [
        ...
        f"n_new={tel['n_new']}",
        "AK6 LOOKUP final — not generative IQ",
    ]
    if arm != "LOOKUP":
...
def score_ak6_gen(
    *,
    ...
    """GENERATE final — wall_ms>0 ∧ n_new>0; GENTRUE peak rubric."""
    ...
    notes = list(notes) + [
        "AK6 GENERATE final — Cursor scores completion",
        "GENTRUE grounded+peak path (labeled ≠ open-chat IQ)",
    ]
    return float(score), bool(err), notes
...
def ak6_stats(
    *,
    ...
    WHEN summarizing AK6
    THEN lookup≥7 · errors≤3/arm · gen≥5 or HOLD path · held-out ok.
    """
    if len(lookup_scores) != AK6_N or len(gen_scores) != AK6_N:
        raise ValueError(f"AK6 requires {AK6_N} dual-arm scores")
    l_mean = float(sum(lookup_scores) / float(AK6_N))
    g_mean = float(sum(gen_scores) / float(AK6_N))
    ...
    return {
        ...
        "pass_gen_telemetry": int(n_gen_wall_ok) >= AK6_N,
        "pass_mean": PASS_MEAN,
...
def decide_ak6(stats: Mapping[str, Any]) -> str:
    """
    GIVEN AK6 final dual-arm stats
    WHEN applying pesquisa §3 AK6 gate
