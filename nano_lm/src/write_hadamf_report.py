"""Render H-ADAMF smoke vs H-HALF (fused AdamW under HALF)."""
...
def render(path: Path) -> str:
    ...
    s = stats.get("H-ADAMF", {})
    decision = decide_hadamf(s, stats) if s else "needs H-ADAMF rows"
    ...
    lines = [
        ...
        "",
        "Same top-k soft cache, STAG curriculum, PRE prefetch, and fp16-wire H2D; "
        "only AdamW fused=True differs. Kill if |Δlp| > ε vs H-HALF or no ms/step win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/adamf_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hadamf-vs-hhalf.md"),
    )
    args = p.parse_args()
