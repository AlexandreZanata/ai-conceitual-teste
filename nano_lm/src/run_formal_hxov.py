"""Formal equal-budget: B2 KD vs H-XOV (weight crossover), 3 seeds."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hxov-b2",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 4,
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        hxov = c["out"] / f"HXOV_seed{seed}.pt"
        meta = run_h_xov(
            tokenizer_id=c["tokenizer_id"],
            cache_dir=c["cache"],
            device=device,
            pop_size=c["xov_pop"],
            generations=c["xov_gens"],
        ...
        write_json(c["out"] / f"HXOV_seed{seed}_train.json", meta)
        row = _eval(c, hxov, seed, "H-XOV")
        row["diversity_collapsed"] = bool(meta.get("diversity_collapsed"))
        rows.append(row)
        if device.type == "cuda":
            torch.cuda.empty_cache()
