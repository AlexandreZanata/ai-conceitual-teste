"""H-MPE: evolve min_p + T/top_p; dual gate vs H-MINP."""
...
from mpe_fit import fitness_mpe_detail
from mpe_ops import clamp_mpe_gene, mutate_mpe_gene, random_mpe_gene
...
def run_h_mpe(
    *,
    ...
    pop = [random_mpe_gene(rng) for _ in range(pop_size)]
        ...
        history.append(
            {
                ...
                "best_min_p": pop[ranked[0]]["min_p"],
            }
        )
        if scores[ranked[0]] > best_score:
            best_score = scores[ranked[0]]
            best_lp = details[ranked[0]][0]
            best_gene = clamp_mpe_gene(pop[ranked[0]])
        parents = [pop[i] for i in ranked[: max(1, pop_size // 2)]]
        pop = [
            mutate_mpe_gene(parents[i % len(parents)], rng) for i in range(pop_size)
        ]
    eval_lp, eval_wall = fitness_mpe_detail(
        best_gene,
        teacher=teacher,
        student=student,
