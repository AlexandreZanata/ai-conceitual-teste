"""Formal H-GPFB4-LONG: freeze GENC genes; GPFB4∘ROLL on formal ckpts."""
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hgpfb4long"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-GPFB4-LONG formal requires CUDA")
    out: Path = c["out"]
    ...
    write_json(
        out / "formal.json",
        {
            ...
            "cpu_threads": threads,
            "mode": (
                "GPFB4-LONG formal: freeze GENC genes; ROLL+full wall; "
                "formal ckpts"
            ),
            "mechanism": (
                "compose GPFB4+ROLL; K=4 only; wall_roll≤full+slack"
            ),
            "parent": "H-GENC-serial n=1 on rolled ctx (formal HEARLY/B2)",
