"""Wave AE REPORT runner: write public closeout + evidence gate (nano:ae:report)."""
...
_OUT = REPO / "results/nano-lm/wave-ae/ae_report_summary.json"
...
def _evidence_map() -> dict[str, bool]:
    return {p: (REPO / p).is_file() for p in AE_EVIDENCE}
...
def _stage_facts() -> dict[str, Any]:
    keys = {
        "ctxmax": "results/nano-lm/wave-ae/ctxmax_summary.json",
        "smartmax": "results/nano-lm/wave-ae/smartmax_summary.json",
        "fastmax": "results/nano-lm/wave-ae/fastmax_summary.json",
        "appmax": "results/nano-lm/wave-ae/appmax_summary.json",
        "ae_hitl": "results/nano-lm/wave-ae/ae_hitl_summary.json",
    }
    out: dict[str, Any] = {}
...
def _smoke_askfast() -> dict[str, Any]:
    ...
    # AE0 HITL-01 phrasing (held-out bip-0032).
    q = (
...
def run_ae_report(*, out: Path, skip_ask: bool = False) -> dict[str, Any]:
    """
    GIVEN AE0–AE5 evidence
    ...
    decision = decide_ae_report(evidence)
    ...
    if decision.startswith("PROMOTE") and not markers:
        decision = "KILL (wave-ae-summary missing thesis markers)"
    if decision.startswith("PROMOTE") and not board:
        decision = "KILL (wave-ae-summary missing per-model FIX scoreboard)"
    ...
    payload: dict[str, Any] = {
        ...
        "markers_ok": markers,
        "scoreboard_ok": board,
        "scoreboard": list(AE_HITL_SCOREBOARD),
        "evidence": evidence,
        "stage_facts": _stage_facts(),
        "ask_smoke": ask,
        "public_report": "docs/results/nano-lm/wave-ae-summary.md",
        "paper_lab": "docs/results/nano-lm/paper-lab-wave-ae.md",
        "wave_status": "RESEARCH_COMPLETE" if ok else "OPEN",
        "cpu_threads": int(os.environ.get("OMP_NUM_THREADS") or 0),
        "finding": (
            f"{AE_ID}: decision={'PROMOTE' if ok else decision}; "
