"""Render H-PINC smoke vs H-PIN (torch.compile under PIN train)."""
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/pinc_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hpinc-vs-hpin.md"),
    )
    args = p.parse_args()
