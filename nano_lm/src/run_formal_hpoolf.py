"""Formal-budget H-POOLF vs H-POOL tip (FLOP dual gate)."""
...
from poolf_ops import clamp_poolf_gene
...
def formal_cfg() -> dict[str, Any]:
    ...
    base["out"] = REPO / "results/nano-lm/formal-hpoolf"
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_poolf(
            ...
            tip_gene=tip,
            lam=LAM,
            out_meta=c["out"] / f"HPOOLF_seed{seed}_train.json",
        )
        # Re-eval on claim prompts (run_h_poolf eval used fit prompts).
        rows.append(
            _eval(
                "H-POOLF",
                f"HPOOLF_seed{seed}",
