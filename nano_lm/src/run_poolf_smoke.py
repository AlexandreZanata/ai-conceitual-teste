"""Smoke H-POOLF: FLOP-aware n≤2 search vs frozen H-POOL tip."""
...
from hyp_poolf import run_h_poolf
...
def _score_tip(
    ...
    return _row("H-POOL", f"HPOOL_flop_seed{seed}", lp, wall, gf, seed, gene), gene
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_poolf(
            ...
            tip_gene=tip_gene,
            lam=LAM,
            out_meta=out / f"HPOOLF_seed{seed}_train.json",
        )
        row = _row(
            "H-POOLF",
            f"HPOOLF_seed{seed}",
        ...
        write_json(out / f"HPOOLF_seed{seed}_eval.json", row)
        rows.append(row)
    write_json(
        out / "poolf_smoke.json",
        {"rows": rows, "wall_s": time.perf_counter() - t0},
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "poolf_smoke.json")}))
    return 0
