"""Wave BD3 H-CTXGAIN: hold howto·cite·long content · no anti-FP regress."""
...
CTXGAIN_THESIS = (
    ...
    "(BD-FOREVER FH 0 · BA/BB/BC forever hold · AZ hold · over-refuse 0 · "
    "live FP 0), p50/p99 published, modes 4/4 — L_eff alone ≠ win; "
    "≠ AH H-CTXLIFT archive · ≠ BC H-CTXLIFT2 rename · ≠ BB H-CTXHOLD "
    "archive · ≠ BA H-CTXREAL2 archive"
)
CTXGAIN_CLAIM = BD0_SHIP_LOCK
CTXGAIN_SAFE_NOTE = BD0_SAFE_NOTE
CTXGAIN_ANTI_FP = BD0_ANTI_FP
CTXGAIN_CTX_BASELINE = BD0_CTX_BASELINE
CTXGAIN_SPEED_BASELINE = BD0_SPEED_BASELINE
...
def extract_ctxgain_board(
    *,
    ...
    """
    GIVEN ctx content + apps + BD/BA/BB/BC/AZ anti-FP + latency
    WHEN building BD3 CTXGAIN board
    ...
    return {
        ...
        **kinds,
        ...
        "l_eff_alone_insufficient": bool(
            CTXGAIN_CTX_BASELINE.get("l_eff_alone_insufficient")
        ),
        "content_bars_required": bool(
            CTXGAIN_CTX_BASELINE.get("content_bars_required")
        ),
        ...
        "baseline_source": str(CTXGAIN_CTX_BASELINE.get("source", "")),
    }
...
def decide_ctxgain(
    *,
    ...
    """
    GIVEN BD3 CTXGAIN board
    WHEN applying pesquisa §9 BD3
    ...
    if err:
        return err
    return (
        f"PROMOTE ({CTXGAIN_ID}: howto·cite·long content_ok; "
        "BD/BA/BB/BC/AZ anti-FP hold; p50/p99 published; L_eff alone ≠ win)"
    )
