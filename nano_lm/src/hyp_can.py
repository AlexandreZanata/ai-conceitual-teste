"""H-CAN: H-SEL scaffold; winner copies loser's LayerNorm stats each gen."""
...
from can_ops import copy_layernorm, pick_winner_loser, state_has_nan
...
def run_h_can(
    *,
    ...
    if not data:
        raise RuntimeError("H-CAN: no training batches")
    probe = data[0]
    best_state = None
    best_fit = float("-inf")
    history: list[float] = []
    pairs_log: list[list[int]] = []
    had_nan = False
    for _gen in range(generations):
        ...
        wi, li = pick_winner_loser(fits)
        pairs_log.append([wi, li])
        eaten = copy_layernorm(pop[wi].state_dict(), pop[li].state_dict())
        if state_has_nan(eaten):
            had_nan = True
        pop[wi].load_state_dict(eaten)
        pop[wi].eval()
    ...
    return {
        "hypothesis": "H-CAN",
        "cannibal": "winner_copies_loser_ln",
