"""AR decode with typical sampling filter."""
...
def decode_typ(
    ...
    *,
    ...
    """
    GIVEN a causal LM and typ_mass in (0,1]
    WHEN sampling a continuation
    THEN apply typical then top-p; mass=1 is AR+top_p.
    """
    if not (0.0 < float(typ_mass) <= 1.0):
        raise ValueError("decode_typ: typ_mass must be in (0,1]")
