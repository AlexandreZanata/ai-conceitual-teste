"""Wave BD2 H-FASTGAIN: prod-path p50/p99 hold/lift · no anti-FP regress."""
...
FASTGAIN_ID = "H-FASTGAIN"
FASTGAIN_THESIS = (
    "Hold/improve prod-ask p50/p99 for LOOKUP·PEAK·DECODE·ABSTAIN; "
    "PROMOTE only if §1 anti-FP holds (BD-FOREVER FH 0 · BA/BB/BC forever "
    "hold · AZ hold · over-refuse 0 · live FP 0) and live p99 does not "
    "regress vs BD0/H-FASTLIFT baseline — never warm-cache vanity; "
    "LOOKUP wall=0 ≠ speed IQ; sub-ms PEAK walls ≠ speed IQ; "
    "≠ AH H-FASTLIFT archive · ≠ BC H-FASTLIFT rename · "
    "≠ BB H-FASTHOLD archive · ≠ BA H-FASTREAL archive · ≠ FP-for-ms"
)
FASTGAIN_CLAIM = BD0_SHIP_LOCK
FASTGAIN_SAFE_NOTE = BD0_SAFE_NOTE
FASTGAIN_ANTI_FP = BD0_ANTI_FP
FASTGAIN_BASELINE = BD0_SPEED_BASELINE
...
def extract_fastgain_board(
    *,
    ...
    WHEN building BD2 FASTGAIN board
    ...
    return {
        ...
        "baseline_source": str(FASTGAIN_BASELINE.get("source", "")),
...
def _gate_antifp(board: Mapping[str, Any]) -> str | None:
    return (
        _gate_forever(board, key="bd_forever", min_n=12)
...
def _gate_latency_flags(board: Mapping[str, Any]) -> str | None:
    ...
    if bool(board.get("p99_regress")):
        ...
        return f"KILL (live p99 regress vs H-FASTLIFT baseline: {paths})"
    flags = (
...
def decide_fastgain(
    *,
    ...
    """
    GIVEN BD2 FASTGAIN board
    WHEN applying pesquisa §4 + §9 BD2
    ...
    if err:
        return err
    return (
        f"PROMOTE ({FASTGAIN_ID}: prod p50/p99 hold/lift; "
        "anti-FP hold; no live p99 regress vs H-FASTLIFT baseline)"
    )
