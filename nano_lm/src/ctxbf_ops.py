"""Wave BF4 H-CTXBF: hold howto·cite·long content · no anti-FP regress."""
...
CTXBF_THESIS = (
    ...
    "(BF-FOREVER FH 0 · BA…BE forever hold · AZ hold · over-refuse 0 · "
    "live FP 0), p50/p99 published, modes 4/4 — L_eff alone ≠ win; "
    "≠ BE H-CTXBE archive · ≠ BD H-CTXGAIN · ≠ AH H-CTXLIFT · "
    "≠ BC H-CTXLIFT2 · ≠ BB H-CTXHOLD · ≠ BA H-CTXREAL2"
)
CTXBF_CLAIM = BF0_SHIP_LOCK
CTXBF_SAFE_NOTE = BF0_SAFE_NOTE
CTXBF_ANTI_FP = BF0_ANTI_FP
CTXBF_CTX_BASELINE = BF0_CTX_BASELINE
CTXBF_SPEED_BASELINE = BF0_SPEED_BASELINE
...
def extract_ctxbf_board(
    *,
    ...
    """
    GIVEN ctx content + apps + BF/BE/BA…BD/AZ anti-FP + latency
    WHEN building BF4 CTXBF board
    ...
    return {
        ...
        **kinds,
        ...
        "l_eff_alone_insufficient": bool(
            CTXBF_CTX_BASELINE.get("l_eff_alone_insufficient")
        ),
        "content_bars_required": bool(
            CTXBF_CTX_BASELINE.get("content_bars_required")
        ),
        ...
        "baseline_source": str(CTXBF_CTX_BASELINE.get("source", "")),
    }
...
def decide_ctxbf(
    *,
    ...
    """
    GIVEN BF4 CTXBF board
    WHEN applying pesquisa §9 BF4
    ...
    if err:
        return err
    return (
        f"PROMOTE ({CTXBF_ID}: howto·cite·long content_ok; "
        "BF/BE/BD/BA/BB/BC/AZ anti-FP hold; p50/p99 published; "
        "L_eff alone ≠ win)"
    )
