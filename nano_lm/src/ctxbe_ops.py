"""Wave BE4 H-CTXBE: hold howto·cite·long content · no anti-FP regress."""
...
CTXBE_THESIS = (
    ...
    "(BE-FOREVER FH 0 · BA…BD forever hold · AZ hold · over-refuse 0 · "
    "live FP 0), p50/p99 published, modes 4/4 — L_eff alone ≠ win; "
    "≠ BD H-CTXGAIN archive · ≠ AH H-CTXLIFT · ≠ BC H-CTXLIFT2 · "
    "≠ BB H-CTXHOLD · ≠ BA H-CTXREAL2"
)
CTXBE_CLAIM = BE0_SHIP_LOCK
CTXBE_SAFE_NOTE = BE0_SAFE_NOTE
CTXBE_ANTI_FP = BE0_ANTI_FP
CTXBE_CTX_BASELINE = BE0_CTX_BASELINE
CTXBE_SPEED_BASELINE = BE0_SPEED_BASELINE
    ...
    return {
        ...
        **kinds,
        ...
        "l_eff_alone_insufficient": bool(
            CTXBE_CTX_BASELINE.get("l_eff_alone_insufficient")
        ),
        "content_bars_required": bool(
            CTXBE_CTX_BASELINE.get("content_bars_required")
        ),
        ...
        "baseline_source": str(CTXBE_CTX_BASELINE.get("source", "")),
    }
...
def decide_ctxbe(
    *,
    ...
    """
    GIVEN BE4 CTXBE board
    WHEN applying pesquisa §9 BE4
    ...
    if err:
        return err
    return (
        f"PROMOTE ({CTXBE_ID}: howto·cite·long content_ok; "
        "BE/BD/BA/BB/BC/AZ anti-FP hold; p50/p99 published; L_eff alone ≠ win)"
    )
