"""Smoke H-BPACK: SKIP + LAYB throughput packs vs tip H-EARLY."""
...
from bpack_pair import SMOKE_BUDGETS, run_seed_trio
...
def main() -> int:
    ...
    if device.type != "cuda":
        print("ERROR: H-BPACK requires CUDA", file=sys.stderr)
        return 2
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        ...
        for row in trio:
            if row["family"] == "H-LAYB":
                write_json(out / f"HBPACK_seed{seed}_eval.json", row)
        rows.extend(trio)
    write_json(
        out / "bpack_smoke.json",
        {
            "rows": rows,
            ...
            "mode": "SKIP+LAYB throughput packs vs EARLY",
        },
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "bpack_smoke.json")}))
    return 0
