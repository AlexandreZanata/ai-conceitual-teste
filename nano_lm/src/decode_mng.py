"""AR decode with min-p and no-repeat n-gram (MINP × NGRAM)."""
...
def decode_mng(
    ...
    *,
    ...
    """
    GIVEN min_p in [0,1) and ngram_size ≥ 0
    WHEN sampling
    THEN ban n-grams, apply min-p, then top-p.
    """
    if not (0.0 <= float(min_p) < 1.0):
        raise ValueError("decode_mng: min_p must be in [0,1)")
    if int(ngram_size) < 0:
        raise ValueError("decode_mng: ngram_size must be >= 0")
