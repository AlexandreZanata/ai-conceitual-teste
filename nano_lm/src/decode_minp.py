"""AR decode with min-p sampling filter."""
...
def decode_minp(
    ...
    *,
    ...
    """
    GIVEN a causal LM and min_p in [0,1)
    WHEN sampling a continuation
    THEN apply min-p then top-p; min_p=0 is AR+top_p.
    """
    if not (0.0 <= float(min_p) < 1.0):
        raise ValueError("decode_minp: min_p must be in [0,1)")
