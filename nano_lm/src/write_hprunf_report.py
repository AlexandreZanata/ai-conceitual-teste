"""Render H-PRUNF smoke vs H-FLAYB (PRUN under FLAYB decode)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Same dual-budget FLAYB path on frozen `HPRUN` (mag-prune + recover) "
        "vs control H-FLAYB on B2. GFLOPs density-scaled. "
        "Kill if |Δlp| > ε vs H-FLAYB or no wall/GFLOPs win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/prunf_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hprunf-vs-hflayb.md"),
    )
    args = p.parse_args()
