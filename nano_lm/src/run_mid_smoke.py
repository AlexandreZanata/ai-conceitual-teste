"""Smoke H-MID: mid min_new + tip warm-start vs frozen H-EARLY."""
...
def _score_tip(
    ...
    row = _row("H-EARLY", f"HEARLY_mid_seed{seed}", lp, wall, gf, seed, gene)
    return row, gene
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        meta = run_h_mid(
            ...
            tip_gene=tip_gene,
            lam=MIN_LAM,
            out_meta=out / f"HMID_seed{seed}_train.json",
        )
        row = _row(
            "H-MID",
            f"HMID_seed{seed}",
        ...
        write_json(out / f"HMID_seed{seed}_eval.json", row)
        rows.append(row)
    write_json(
        out / "mid_smoke.json",
        {"rows": rows, "wall_s": time.perf_counter() - t0},
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "mid_smoke.json")}))
    return 0
