"""Formal H-HALF: fp16-wire H2D under PRE vs H-PRE (formal KD budget)."""
...
from run_formal_hpre import formal_cfg as hpre_formal_cfg
...
def formal_cfg() -> dict[str, Any]:
    base = hpre_formal_cfg()
    base["out"] = REPO / "results/nano-lm/formal-hhalf"
    return base
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-HALF formal requires CUDA")
    out: Path = c["out"]
    ...
    for seed in c["seeds"]:
        rows.extend(
            run_seed_pair(
                c, out, seed, device, vocab, steps, label_prefix="HHALF_formal"
            )
        )
