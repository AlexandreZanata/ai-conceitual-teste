"""Wave AL3 H-SMARTFRESH runner: nona cite LOOKUP + GENFRESH peak GENERATE."""
...
_SUMMARY = REPO / "results/nano-lm/wave-al/smartfresh_summary.json"
...
def _seed_pack(bank_path: Path, al_bank: Path) -> int:
    ...
    n = 0
    for i, item in enumerate(SMARTFRESH_PACK, start=1):
        for q_key, q_text in (
            ...
            if q in existing:
                continue
            row = alias_bank_row(
                trial_id=f"AL-SMARTFRESH-SEED-{q_key}-{i:02d}",
                question=q,
                source_id=item["source_id"],
                gold=item["gold"],
            )
            row["hyp_id"] = SMARTFRESH_ID
            row["judge_notes"] = [
                "SMARTFRESH seed for nona-hop paraphrase stress",
...
def _fix_lookup(
    *,
    ...
    row = alias_bank_row(
        trial_id=f"AL-SMARTFRESH-FIX-{i:02d}",
...
def run_smartfresh(
    *,
    ...
    """
    GIVEN AL0 nona-hop paraphrases + parent asks
    WHEN SEMWRAP LOOKUP + GENFRESH peak GENERATE dual-arm ×10
    THEN false-hit≈0; PROMOTE if gen≥5 else HOLD.
    """
    if len(SMARTFRESH_PACK) != SMARTFRESH_N:
        raise ValueError("SMARTFRESH pack must be 10")
    if not hard_paraphrase_ok():
        raise ValueError("paraphrases must differ from parents")
    if not has_adversarial_noise() or not has_nona_hop_cues():
        raise ValueError("nona-hop adversarial pack incomplete")

    trials_dir.mkdir(parents=True, exist_ok=True)
    t0 = time.perf_counter()
    seeded = _seed_pack(bank_path, al_bank)
    bank = load_bank_rows(bank_path)
    paras = [p["paraphrase"] for p in SMARTFRESH_PACK]
    gen_items = [
        {
    ...
    for i, (item, lp) in enumerate(
        zip(SMARTFRESH_PACK, lookup_payloads, strict=True),
        start=1,
    ):
        ...
        score_l, err_l, notes_l, cited = score_smartfresh_lookup(
        ...
        lookup_trials.append(
            {
                "trial_id": f"AL-SMARTFRESH-LOOKUP-HITL-{i:02d}",
                "stage": "AL3",
                "hyp_id": SMARTFRESH_ID,
                "arm": "LOOKUP",
                "app_id": item["app_id"],
                "question": item["paraphrase"],
    ...
    for i, (item, gp) in enumerate(
        zip(SMARTFRESH_PACK, gen_payloads, strict=True)
    ):
        score_g, err_g, notes_g = score_smartfresh_gen(
    ...
    if weak_idx:
        re_items = [
            {
                "question": SMARTFRESH_PACK[i]["parent_question"],
                "source_id": SMARTFRESH_PACK[i]["source_id"],
                "gold": SMARTFRESH_PACK[i]["gold"],
            }
            for i in weak_idx
        ]
        ...
        for j, idx in enumerate(weak_idx):
            item = SMARTFRESH_PACK[idx]
            re_gp = re_payloads[j]
            score2, err2, notes2 = score_smartfresh_gen(
            ...
            if score2 > gen_scores[idx]:
                ...
                gen_notes[idx] = list(notes2) + [
                    "FIX: re-peak GENFRESH grounded decode"
                ]
                gen_fix[idx] = 1
                fix_count += 1
            else:
                gen_notes[idx] = list(gen_notes[idx]) + [
                    "FIX attempted: re-peak GENFRESH (no lift)"
                ]
    ...
    for i, (item, gp) in enumerate(
        ...
        if bool(gp.get("peak_used")):
            n_peak += 1
        gt = {
            "trial_id": f"AL-SMARTFRESH-GEN-HITL-{i:02d}",
            "stage": "AL3",
            "hyp_id": SMARTFRESH_ID,
            "arm": "GENERATE",
            "app_id": item["app_id"],
    ...
    decision = decide_smartfresh(stats)
    summary: dict[str, Any] = {
        ...
        "compose": [
            ...
            "nona-hop paraphrase (CTXFRESH companions)",
            "QPFB2+BEAMKV+GROUNDED+ANTI_PERIOD+GENFRESH_PEAK GENERATE",
        ],
        "forbidden": [
            "QI",
            "STREAM",
        ...
        "finding": (
            f"{SMARTFRESH_ID}: L_lookup={stats['lookup_mean']:.1f} "
            ...
            f"peers_smartmore={stats['peers_smartmore_gen']} "
            f"pass_gen={stats['pass_gen']} fix={fix_count} → {decision}"
        ),
        "public_note": "docs/results/nano-lm/formal-hsmartfresh-smartfresh.md",
