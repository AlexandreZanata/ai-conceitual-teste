"""Formal H-GALLF: graph-all-budgets vs tip GRAPHF on B2 (eval)."""
...
from run_formal_hgraphf import formal_cfg as hgraphf_formal_cfg


def formal_cfg() -> dict[str, Any]:
    base = hgraphf_formal_cfg()
    base["out"] = REPO / "results/nano-lm/formal-hgallf"
    return base
...
def _run_seed(
    ...
    return [
        tip_row("H-GRAPHF", f"HGRAPHF_gallf_formal_seed{seed}", graphed, seed, g_tip),
        tip_row("H-GALLF", f"HGALLF_formal_seed{seed}", gallf, seed, g),
    ]
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-GALLF formal requires CUDA")
    ...
    payload = {
        ...
        "mode": "CUDA graph all budgets (never KV) vs GRAPHF; long eval",
    }
