"""Wave BG1 H-UNARYINT: BG-FOREVER FH 0 · BA…BF/AZ hold · live board."""
...
UNARYINT_ID = "H-UNARYINT"
UNARYINT_THESIS = (
    "Drive BG-FOREVER held-out FH → 0 (unary/math abs≠add · factorial≠add · "
    "string-transform upper≠f-string · aggregate all-truthy≠clear · "
    "paraphrases · arity/transform neighbors) via SEMWRAP unary/transform/"
    "arity gate — not bank stuffing; hold BA…BF-FOREVER + AZ div·sub·BIP "
    "FH 0 + a.clear() LOOKUP; live ask scoreboard OK|FP|MISS|ABSTAIN-OK "
    "(≥10 novel); modes · p50/p99 · DECODE law"
)
UNARYINT_CLAIM = BG0_SHIP_LOCK
UNARYINT_SAFE_NOTE = BG0_SAFE_NOTE
UNARYINT_ANTI_FP = BG0_ANTI_FP
BG_FOREVER_ROWS = BG0_FOREVER_ROWS
BF_FOREVER_ROWS = BF0_FOREVER_ROWS
BE_FOREVER_ROWS = BE0_FOREVER_ROWS
...
# Novel probes outside exact BA…BG forever seeds (≥10 · anti pack theater).
NOVEL_PROBES: tuple[dict[str, str], ...] = (
    {
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Python helper abs1(x) that returns the absolute value of x."
        ),
    },
    {
        "id": "BG-NOVEL-02",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Implement fact(n) returning n factorial for non-negative n."
        ),
    },
    {
        "id": "BG-NOVEL-03",
        ...
        "expect_mode": "ABSTAIN",
        "question": "Return |x| for integer x in a short Python function.",
    },
    {
        "id": "BG-NOVEL-04",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Make every letter in string t uppercase — one method call."
        ),
    },
    {
        "id": "BG-NOVEL-05",
        ...
        "expect_mode": "ABSTAIN",
        "question": "How do I turn string u into lowercase letters?",
    },
    {
        "id": "BG-NOVEL-06",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Return True when every element of list xs is truthy."
        ),
    },
    {
        "id": "BG-NOVEL-07",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Write ceil1(x) that returns the ceiling of float x."
        ),
    },
    {
        "id": "BG-NOVEL-08",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Boolean: is any element of list xs truthy?"
        ),
    },
    {
        "id": "BG-NOVEL-09",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Compute math.floor of float y and return the int."
        ),
    },
    {
        "id": "BG-NOVEL-10",
        ...
        "expect_mode": "ABSTAIN",
        "question": (
            "Convert text w to UPPERCASE characters in Python."
        ),
    },
)
...
def bars_from_scoreboard(
    ...
    """
    GIVEN BG0 §1 scoreboard
    WHEN reading bars
    THEN return typed bar dict for UNARYINT gate.
    """
    src = suite if suite is not None else BG0_SCOREBOARD
    bars = src.get("bars")
...
def score_live_row(row: Mapping[str, Any], *, expect_mode: str) -> str:
    ...
    """
    mode = str(
        row.get("product_mode") or map_bg_product_mode(str(row.get("mode", "")))
    )
...
def extract_unaryint_board(
    *,
    ...
    """
    GIVEN BG-FOREVER + BA…BF hold + AZ hold + over-refuse + live scores
    WHEN building UNARYINT board
    THEN publish BG FH · BA…BF hold · AZ hold · over-refuse · live.
    """
    n_bg, n_bg_fh, n_bg_ok = _pack_fh(bg_rows)
...
def _gate_core_flags(board: Mapping[str, Any], bar: Mapping[str, Any]) -> str | None:
    ...
    if not bool(bar.get("ba_bb_bc_bd_be_bf_pass_neq_bg_forever", True)):
        return "KILL (BA…BF PASS ≠ BG forever bar missing)"
    if not bool(bar.get("unary_transform_gate_preferred", True)):
        return "KILL (unary/transform gate must be preferred)"
...
def decide_unaryint(
    *,
    ...
    """
    GIVEN UNARYINT metrics board
    WHEN applying pesquisa §9 BG1
    THEN PROMOTE iff BG FH 0 + BA…BF hold 0 + AZ hold 0 + over-refuse 0
         + live FP 0.
    """
    ...
    err = (
        _gate_forever_pack(
            ...
            bar=bar,
            key="bg_forever",
            min_n=int(bar.get("bg_forever_min_n", 12)),
            fh_bar_key="bg_forever_false_hit_max",
        )
        ...
        or _gate_forever_pack(
            ...
            key="be_forever",
            min_n=12,
            fh_bar_key="be_forever_false_hit_max",
        )
        or _gate_forever_pack(
            board=board,
            bar=bar,
            key="bf_forever",
            min_n=12,
            fh_bar_key="bf_forever_false_hit_max",
        )
        or _gate_forever_pack(
            board=board,
            bar=bar,
    ...
    if err:
        return err
    return (
        f"PROMOTE ({UNARYINT_ID}: BG-FOREVER FH 0; BA…BF hold 0; "
