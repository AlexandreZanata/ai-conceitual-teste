"""Smoke H-LAYB: LAY under FUSEB dual-budget batch vs tip FUSEB."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        rows.append(tip_row("H-FUSEB", f"HFUSEB_layb_seed{seed}", fuseb, seed, g_tip))
        ...
        row = tip_row("H-LAYB", f"HLAYB_seed{seed}", layb, seed, g)
        write_json(out / f"HLAYB_seed{seed}_eval.json", row)
        rows.append(row)
    write_json(
        out / "layb_smoke.json",
        {
            ...
            "mode": "dual-budget LAY under FUSEB; n=1 near-greedy; long prompts",
        },
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "layb_smoke.json")}))
    return 0
