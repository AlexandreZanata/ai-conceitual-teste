"""Formal equal-budget: B2 KD vs H-FIT (teacher_lp fitness, holdout), 3 seeds."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hfit-b2",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 4,
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        hfit = c["out"] / f"HFIT_seed{seed}.pt"
        meta = run_h_fit(
            teacher_id=c["teacher_id"],
            tokenizer_id=c["tokenizer_id"],
            ...
            device=device,
            pop_size=c["fit_pop"],
            generations=c["fit_gens"],
            mutate_scale=c["mutate_scale"],
            max_new_fit=c["max_new_fit"],
            seed=seed,
            out_path=hfit,
        )
        write_json(c["out"] / f"HFIT_seed{seed}_train.json", meta)
        row = _eval(c, hfit, seed, "H-FIT")
