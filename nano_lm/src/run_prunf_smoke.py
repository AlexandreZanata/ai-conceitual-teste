"""Smoke H-PRUNF: PRUN ckpt under FLAYB vs tip FLAYB on B2."""
...
def main() -> int:
    ...
    for seed in c["seeds"]:
        ...
        rows.append(
            tip_row("H-FLAYB", f"HFLAYB_prunf_seed{seed}", flayb, seed, g_tip)
        )
        ...
        row = tip_row("H-PRUNF", f"HPRUNF_seed{seed}", prunf, seed, g)
        row["density"] = dens
        write_json(out / f"HPRUNF_seed{seed}_eval.json", row)
        rows.append(row)
    write_json(
        out / "prunf_smoke.json",
        {
            "rows": rows,
            ...
            "mode": "PRUN under FLAYB vs B2+FLAYB; dual-budget; long prompts",
        },
    )
    print(json.dumps({"n_rows": len(rows), "out": str(out / "prunf_smoke.json")}))
    return 0
