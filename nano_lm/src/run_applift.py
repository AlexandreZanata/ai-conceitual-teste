"""Wave AH5 H-APPLIFT runner: dual-arm apps + DEPL-AH (nano:applift)."""
...
_SUMMARY = REPO / "results/nano-lm/wave-ah/applift_summary.json"
...
def _seed_pack(bank_path: Path, ah_bank: Path) -> int:
    ...
    for i, item in enumerate(AH0_PACK, start=1):
        ...
        if q in existing:
            continue
        row = alias_bank_row(
            trial_id=f"AH-APPLIFT-SEED-{i:02d}",
            question=q,
            source_id=item["source_id"],
            gold=item["gold"],
        )
        row["hyp_id"] = APPLIFT_ID
        row["judge_notes"] = [
            "APPLIFT seed for AH held-out ask",
...
def _write_pages(docs: Path) -> list[dict[str, Any]]:
    ...
    depl_path = docs / Path(DEPL_AH_PAGE).name
    depl_body = depl_ah_body()
    depl_path.write_text(depl_body, encoding="utf-8")
    reports.append(page_sync_report(DEPL_AH_PAGE, depl_body))
    return reports
...
def _run_one_app(
    *,
    ...
    if len(lookup_payloads) != APPLIFT_N or len(gen_payloads) != APPLIFT_N:
    ...
    for i, item in enumerate(AH0_PACK, start=1):
        ...
        if not bool(route["in_scope"]):
            ...
            row = alias_bank_row(
                trial_id=f"AH-APPLIFT-FIX-{tag}-{i:02d}",
        ...
        l_score, l_err, l_notes = score_applift_lookup(
            mode=l_mode,
            completion=l_comp,
        ...
        l_trial = {
            "trial_id": f"AH-APPLIFT-{tag}-LOOKUP-HITL-{i:02d}",
            "stage": "AH5",
            "hyp_id": APPLIFT_ID,
            "arm": "LOOKUP",
            "realapp_id": app_id,
        ...
        g_score, g_err, g_notes = score_applift_gen(
        ...
        g_trial = {
            "trial_id": f"AH-APPLIFT-{tag}-GEN-HITL-{i:02d}",
            "stage": "AH5",
            "hyp_id": APPLIFT_ID,
            "arm": "GENERATE",
            "realapp_id": app_id,
...
def run_applift(
    *,
    ...
    """
    GIVEN AH0 pack + APPLIFT apps
    WHEN ASK→EVAL→FIX×10 dual-arm per surface + DEPL-AH
    THEN expose LOOKUP|GENERATE ∧ DEPL → PROMOTE|HOLD|KILL.
    """
    if len(AH0_PACK) != APPLIFT_N:
    ...
    bank = load_bank_rows(bank_path)
    apps = [dict(a) for a in APPLIFT_APPS]
    if only_app:
    ...
    summary: dict[str, Any] = {
        ...
        "finding": (
            f"{APPLIFT_ID}: apps={wave['n_apps']} "
            f"L={wave.get('lookup_mean_across', 0):.1f} "
            f"G={wave.get('gen_mean_across', 0):.1f} "
            f"expose={wave.get('pass_expose')} "
            f"depl={wave.get('depl_ok')} → {decision}"
        ),
        "public_note": "docs/results/nano-lm/formal-happlift-applift.md",
