"""Render H-LAYB smoke vs H-FUSEB (LAY under FUSEB batch)."""
...
def render(path: Path) -> str:
    ...
    lines = [
        ...
        "",
        "Dual-budget mean: FUSEB path with batched LAY on the non-KV arm "
        f"(CHBAT B=`{data.get('chunk_size')}` when `max_new > kv_threshold`).",
        "Frozen LAY tip `max_skip` / `lay_conf`. Kill if |Δlp| > ε vs H-FUSEB or "
        "no tok/s/wall win.",
...
def main() -> int:
    ...
    p.add_argument(
        "--smoke",
        type=Path,
        default=Path("results/nano-lm/student-matrix/layb_smoke.json"),
    )
    p.add_argument(
        "--out",
        type=Path,
        default=Path("docs/results/nano-lm/hlayb-vs-hfuseb.md"),
    )
    args = p.parse_args()
