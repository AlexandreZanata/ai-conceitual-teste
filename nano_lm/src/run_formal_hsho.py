"""Formal equal-budget: B2 KD vs H-SHO (layer shock), 3 seeds."""
...
def formal_cfg() -> dict[str, Any]:
    return {
        ...
        "cache": ROOT / ".cache",
        "out": REPO / "results/nano-lm/formal-hsho-b2",
        "steps_kd": 120,
        "max_examples": 300,
        "seq_len": 128,
        "batch_size": 4,
...
def run_formal() -> int:
    ...
    for seed in c["seeds"]:
        ...
        hsho = c["out"] / f"HSHO_seed{seed}.pt"
        meta = run_h_sho(
            tokenizer_id=c["tokenizer_id"],
            cache_dir=c["cache"],
            device=device,
            pop_size=c["sho_pop"],
            generations=c["sho_gens"],
        ...
        write_json(c["out"] / f"HSHO_seed{seed}_train.json", meta)
        rows.append(_eval(c, hsho, seed, "H-SHO"))
        if device.type == "cuda":
            torch.cuda.empty_cache()
