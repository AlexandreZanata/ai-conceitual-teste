"""Formal H-GALL: graph-all-budgets vs tip GRAPH on B2 (eval)."""
...
from run_formal_hgraph import formal_cfg as hgraph_formal_cfg


def formal_cfg() -> dict[str, Any]:
    base = hgraph_formal_cfg()
    base["out"] = REPO / "results/nano-lm/formal-hgall"
    return base
...
def _run_seed(
    ...
    return [
        tip_row("H-GRAPH", f"HGRAPH_gall_formal_seed{seed}", graphed, seed, g_tip),
        tip_row("H-GALL", f"HGALL_formal_seed{seed}", gall, seed, g),
    ]
...
def run_formal() -> int:
    ...
    if device.type != "cuda":
        raise RuntimeError("H-GALL formal requires CUDA")
    ...
    payload = {
        ...
        "mode": "CUDA graph all budgets (never KV) vs GRAPH; long eval",
    }
