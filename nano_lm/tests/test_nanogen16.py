"""Contract: Wave BF5 H-NANOGEN16 — gen-SKIP stop rule; not empty DEFER."""
...
def test_given_contract_when_constants_then_match_bf5() -> None:
    # GIVEN/WHEN/THEN: pesquisa §9 BF5 — SKIP unless plan + tc
    assert NANOGEN16_ID == "H-NANOGEN16"
    assert NANOGEN16_STANCE["stance"] == "skip"
    assert NANOGEN16_STANCE["capcheck"] == "closed"
    assert NANOGEN16_STANCE["named_hyp"] == "H-NANOGEN16"
    assert NANOGEN16_STANCE["method_plan_attached"] is False
    assert NANOGEN16_STANCE["nanogen16_rename_forbidden"] is True
    assert NANOGEN16_STANCE["skip_gen_stop_rule"] is True
    assert NANOGEN16_STANCE["nanogen15_defer_cited"] is True
    assert NANOGEN16_METHOD["real_new_method"] is False
    assert NANOGEN16_METHOD["method_plan_attached"] is False
    assert NANOGEN16_METHOD["empty_defer_letter"] is False
    assert "skip" in NANOGEN16_THESIS.lower()
    assert "nanogen15" in NANOGEN16_THESIS.lower() or "DEFER" in NANOGEN16_THESIS
    assert "gibberish-tail" in NANOGEN16_CLAIM
    ...
    assert "SKIP" in NANOGEN16_ANTI_FP or "skip" in NANOGEN16_ANTI_FP.lower()
...
def test_given_rename_method_when_check_then_true() -> None:
    ...
    assert method_is_rename({"kind": "nanogen15_rename", "id": "x"}) is True
    assert method_is_rename(dict(NANOGEN16_METHOD)) is False


def test_given_skip_stance_when_decide_then_skip() -> None:
    board = extract_nanogen16_board()
    out = decide_nanogen16(board=board, anti_fp_signed=True)
    assert out.startswith("SKIP")
    assert NANOGEN16_ID in out
...
def test_given_nanogen15_rename_when_decide_then_kill() -> None:
    board = extract_nanogen16_board(
        method=_real_method(
            id="nanogen15-clone",
            kind="nanogen15_rename",
            renames_nanogen15=True,
            candidate="M1",
        ),
...
def test_given_unsigned_when_decide_then_kill() -> None:
    out = decide_nanogen16(
        board=extract_nanogen16_board(), anti_fp_signed=False
    )
    assert out.startswith("KILL")
...
def test_given_modes_regress_when_decide_then_kill() -> None:
    board = extract_nanogen16_board(live_modes_ok=False)
    out = decide_nanogen16(board=board, anti_fp_signed=True)
    assert out.startswith("KILL")
