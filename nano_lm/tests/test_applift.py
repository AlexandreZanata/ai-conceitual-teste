"""Contract: Wave AH5 H-APPLIFT — dual-arm apps + DEPL-AH honesty."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §5 AH5 H-APPLIFT
    assert APPLIFT_ID == "H-APPLIFT"
    assert APPLIFT_N == 10
    assert MIN_APPS == 3
    assert MIN_PAGES == 4
    ...
    ids = {a["app_id"] for a in APPLIFT_APPS}
...
def test_given_one_pager_when_body_then_dual_arm_and_honesty() -> None:
    ...
    assert "npm run nano:applift" in body
    assert "Not an open chat LM" in body
    assert "H-APPLIFT" in body
    assert "H-FASTLIFT" in body
    report = page_sync_report(
        "docs/results/nano-lm/applift-howto.md", body
    )
    assert report["ok"] is True, report["missing"]


def test_given_depl_ah_when_body_then_sync_ok() -> None:
    body = depl_ah_body()
    assert "DEPL-AH" in body
    assert "LOOKUP" in body and "GENERATE" in body
    report = page_sync_report("docs/results/nano-lm/depl-ah.md", body)
    assert report["ok"] is True, report["missing"]
...
def test_given_lookup_serve_when_score_then_not_iq() -> None:
    ...
    score, err, notes = score_applift_lookup(
        mode="WRAP_LOOKUP",
        completion="gold",
        expected_gold="gold",
...
def test_given_gen_serve_zero_wall_when_score_then_error() -> None:
    ...
    route = {"in_scope": True, "route": "SERVE"}
    _score, err, notes = score_applift_gen(
        completion="x",
        expected_gold="gold",
        route=route,
        payload=payload,
