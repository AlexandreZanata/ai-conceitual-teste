"""Contract: Wave BC4 H-NANOGEN13 — gen-defer; not NANOGEN12 rename."""
...
def test_given_contract_when_constants_then_match_bc4() -> None:
    # GIVEN/WHEN/THEN: pesquisa §9 BC4 — defer unless real method + true_continue
    assert NANOGEN13_ID == "H-NANOGEN13"
    assert NANOGEN13_STANCE["stance"] == "defer"
    assert NANOGEN13_STANCE["capcheck"] == "closed"
    assert NANOGEN13_STANCE["named_hyp"] == "H-NANOGEN13"
    assert NANOGEN13_STANCE["nanogen13_rename_forbidden"] is True
    assert NANOGEN13_STANCE["nanogen12_defer_cited"] is True
    assert NANOGEN13_METHOD["real_new_method"] is False
    assert NANOGEN13_METHOD["renames_nanogen12"] is False
    assert "defer" in NANOGEN13_THESIS.lower()
    assert "nanogen12" in NANOGEN13_THESIS.lower()
    assert "gibberish-tail" in NANOGEN13_CLAIM
    ...
    assert "NANOGEN13" in NANOGEN13_ANTI_FP or "nanogen13" in NANOGEN13_ANTI_FP.lower()
...
def test_given_rename_method_when_check_then_true() -> None:
    ...
    assert method_is_rename({"kind": "nanogen12_rename", "id": "x"}) is True
    assert method_is_rename(dict(NANOGEN13_METHOD)) is False


def test_given_defer_stance_when_decide_then_defer() -> None:
    board = extract_nanogen13_board()
    out = decide_nanogen13(board=board, anti_fp_signed=True)
    assert out.startswith("DEFER")
    assert NANOGEN13_ID in out
    assert "nanogen12" in out.lower() or "defer" in out.lower()


def test_given_nanogen12_rename_when_decide_then_kill() -> None:
    board = extract_nanogen13_board(
        method={
            "id": "nanogen12-clone",
            "kind": "nanogen12_rename",
            "real_new_method": True,
            "renames_nanogen12": True,
...
def test_given_real_method_true_continue_when_decide_then_promote() -> None:
    board = extract_nanogen13_board(
        stance={**dict(NANOGEN13_STANCE), "stance": "M1"},
        method={
            "id": "distill-x",
            "kind": "M1",
            "real_new_method": True,
...
def test_given_method_no_true_continue_when_decide_then_hold() -> None:
    board = extract_nanogen13_board(
        stance={**dict(NANOGEN13_STANCE), "stance": "M2"},
        method={
            "id": "hybrid-x",
            "kind": "M2",
            "real_new_method": True,
...
def test_given_unsigned_when_decide_then_kill() -> None:
    out = decide_nanogen13(
        board=extract_nanogen13_board(), anti_fp_signed=False
    )
    assert out.startswith("KILL")
...
def test_given_modes_regress_when_decide_then_kill() -> None:
    board = extract_nanogen13_board(live_modes_ok=False)
    out = decide_nanogen13(board=board, anti_fp_signed=True)
    assert out.startswith("KILL")
