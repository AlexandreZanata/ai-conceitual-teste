"""Contract: Wave AE5 AE-HITL-10 — final pack mean≥7.0 · errors≤3."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §5 AE5 AE-HITL-10
    assert AE5_ID == "AE-HITL-10"
    assert AE5_N == 10
    assert PASS_MEAN == 7.0
    assert PASS_MAX_ERRORS == 3
    assert "H-CTXMAX" in DECLARED_STACK
    assert "H-SMARTMAX" in DECLARED_STACK
    assert "H-FASTMAX" in DECLARED_STACK
    assert "H-APPMAX" in DECLARED_STACK
...
def test_given_true_hit_when_score_then_shippable() -> None:
    ...
    assert any("AE5" in n for n in notes)
...
def test_given_ctx_fail_when_score_then_error() -> None:
    ...
    assert any("CTXMAX_CTX_FAIL" in n for n in notes)
...
def test_given_pass_bar_when_decide_then_promote() -> None:
    ...
    assert decide_ae5(stats) == "PROMOTE"
