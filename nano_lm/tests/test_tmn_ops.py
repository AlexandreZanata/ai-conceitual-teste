"""
Contract: H-TMN dual gate vs H-TYP and H-MINP tips.
"""

from __future__ import annotations

from tmn_ops import decide_htmn, tip_max_lp, tip_min_wall


def test_given_tips_when_max_min_then_correct():
    stats = {
        "H-TYP": {"mean_lp": -16.5, "mean_wall": 45.0},
...
def test_given_quality_or_wall_miss_when_decide_then_kill():
    ...
    assert (
        decide_htmn({"mean_lp": -16.6, "mean_wall": 40.0}, stats)
        == "KILL (≤ max tip quality)"
    )
    assert (
        decide_htmn({"mean_lp": -16.35, "mean_wall": 44.0}, stats)
        == "KILL (no dual wall win)"
    )


def test_given_missing_tip_when_decide_then_needs():
    assert decide_htmn({"mean_lp": -11.0}, {}) == "needs H-TYP+H-MINP controls"
