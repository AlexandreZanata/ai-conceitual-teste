"""Contract: Wave BG3 H-FASTBG — prod p50/p99 hold · anti-FP (≠ BF FASTBF)."""
...
def test_given_id_when_read_then_h_fastbg_not_bf_clone() -> None:
    # GIVEN/WHEN/THEN: pesquisa §9 BG3 — hyp H-FASTBG; not BF FASTBF clone
    assert FASTBG_ID == "H-FASTBG"
    assert "BG-FOREVER" in FASTBG_THESIS or "anti-FP" in FASTBG_THESIS
    assert "FASTBF" in FASTBG_THESIS
    assert "gibberish-tail" in FASTBG_CLAIM
    assert "LOOKUP" in FASTBG_ANTI_FP or "BG-FOREVER" in FASTBG_ANTI_FP
    paths = FASTBG_BASELINE["paths"]
    assert set(paths) == BD0_MODES
    assert "FASTBF" in str(FASTBG_BASELINE.get("source", ""))
...
def test_given_packs_when_extract_then_board() -> None:
    ...
    assert board["bf_fastbf_archive_untouched"] is True


def test_given_all_ok_when_decide_then_promote() -> None:
    out = decide_fastbg(board=_board(), anti_fp_signed=True)
    assert out.startswith("PROMOTE")
    assert FASTBG_ID in out
...
def test_given_p99_regress_when_decide_then_kill() -> None:
    ...
    out = decide_fastbg(board=board, anti_fp_signed=True)
    assert out.startswith("KILL")
    assert "p99" in out.lower() or "regress" in out.lower()


def test_given_unsigned_when_decide_then_kill() -> None:
    out = decide_fastbg(board=_board(), anti_fp_signed=False)
    assert out.startswith("KILL")
