"""Contract: Wave BA5 BA-REAL-EVAL — product+ctx+speed + live battery; gen if BA4 PROMOTE."""
...
def test_given_contract_when_constants_then_match_ba5_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §8 BA5 — product pass · gen if BA4 PROMOTE
    assert BA_REAL_EVAL_ID == "BA-REAL-EVAL"
    assert len(ASK_BATTERY) >= 10
    ...
    assert "nanogen11" in claim_rule
    assert "true_continue" in claim_rule or "true-continue" in claim_rule
    assert "rename" in claim_rule
    assert "real eval" in BA_REAL_EVAL_THESIS.lower()
    assert "REALGAIN" in BA_REAL_EVAL_THESIS
    assert PARENT_NANOGEN11 == "DEFER"
    assert claim_is_honest(BA_REAL_EVAL_CLAIM, nanogen11_decision="DEFER")
    assert not gen_claim_allowed(BA_REAL_EVAL_CLAIM)
...
def test_given_forever_fp_when_lookup_then_fail() -> None:
    row = {
        "id": "BA-ASK-07",
...
def test_given_all_pillars_when_decide_then_promote() -> None:
    ...
    out = decide_ba_real_eval(
        realgain_decision="PROMOTE (x)",
        fastreal_decision="PROMOTE (x)",
        ctxreal2_decision="PROMOTE (x)",
        nanogen11_decision="DEFER (x)",
        battery_ok=True,
        claim=BA_REAL_EVAL_CLAIM,
    )
    assert out.startswith("PROMOTE")
    assert nanogen11_outcome_ok("DEFER (x)")
...
def test_given_gen_claim_when_defer_then_kill() -> None:
    out = decide_ba_real_eval(
        ...
        nanogen11_decision="DEFER (x)",
        battery_ok=True,
        claim=BA_REAL_EVAL_CLAIM + " · mini-AGI unlocked",
    )
