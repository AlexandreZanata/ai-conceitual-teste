"""Contract: H-SHORTB dual gate vs H-FUSEB + batched SHORT emits tokens."""
...
from shortb_ops import SHORTB_CHUNK, decide_hshortb
...
def test_given_batch_when_short_decode_then_emits_tokens() -> None:
    assert SHORTB_CHUNK == 256
    m = build_student(50257).eval()
    device = torch.device("cpu")
    tok = _Tok()
    prompts = [" ".join(["once"] * 12), " ".join(["upon"] * 8)]
    results, wall = decode_short_batch(
        m,
        tok,
        prompts,
        n=1,
