"""Contract: Wave AP5 H-APPBASE — dual-arm apps + DEPL-AP honesty."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §3 AP5 H-APPBASE
    assert APPBASE_ID == "H-APPBASE"
    assert APPBASE_N == 10
    assert MIN_APPS == 3
    assert MIN_PAGES == 4
    ...
    assert APPCORE_GEN_MEAN == 9.0
    assert SERVEALIGN_MEAN == 3.4
    ids = {a["app_id"] for a in APPBASE_APPS}
...
def test_given_one_pager_when_body_then_dual_arm_and_honesty() -> None:
    ...
    assert "npm run nano:appbase" in body
    assert "Not an open chat LM" in body
    assert "H-APPBASE" in body
    assert "H-FASTBASE" in body
    report = page_sync_report(
        "docs/results/nano-lm/appbase-howto.md", body
    )
    assert report["ok"] is True, report["missing"]


def test_given_depl_ap_when_body_then_sync_ok() -> None:
    body = depl_ap_body()
    assert "DEPL-AP" in body
    assert "LOOKUP" in body and "GENERATE" in body
    assert "H-APPBASE" in body
    report = page_sync_report("docs/results/nano-lm/depl-ap.md", body)
    assert report["ok"] is True, report["missing"]
...
def test_given_lookup_serve_when_score_then_not_iq() -> None:
    ...
    score, err, notes = score_appbase_lookup(
        mode="WRAP_LOOKUP",
        completion="gold",
        expected_gold="gold",
...
def test_given_gen_serve_zero_wall_when_score_then_error() -> None:
    ...
    route = {"in_scope": True, "route": "SERVE"}
    _score, err, notes = score_appbase_gen(
        completion="x",
        expected_gold="gold",
        route=route,
        payload=payload,
...
def test_given_wave_product_when_decide_then_promote() -> None:
    ...
    wave = appbase_stats(apps, n_pages_ok=4, n_pages=4)
    ...
    assert decide_appbase(wave) == "PROMOTE"
