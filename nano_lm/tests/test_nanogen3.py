"""Contract: Wave AS7 H-NANOGEN3 — ablated gen gate vs NANOGEN2 4.3."""
...
def test_given_contract_when_constants_then_match_as7_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §5 AS7 — ablated≥5 PROMOTE else HOLD
    assert NANOGEN3_ID == "H-NANOGEN3"
    assert NANOGEN3_N == 10
    assert MIN_LOOKUP_MEAN == 7.0
    assert MIN_GEN_MEAN == 5.0
    assert PARENT_NANOGEN2_ABLATED == 4.3
    assert len(NANOGEN3_PACK) == 10
    assert "ablated" in NANOGEN3_HYPOTHESIS.lower()
    assert "5.0" in NANOGEN3_HYPOTHESIS
    assert "4.3" in NANOGEN3_HYPOTHESIS
    assert "ablated" in NANOGEN3_THESIS.lower()


def test_given_pack_when_kinds_then_held_and_para() -> None:
    kinds = [p["kind"] for p in NANOGEN3_PACK]
...
def test_given_bank_grounded_when_ablated_score_then_excluded() -> None:
    ...
    assert any("NANOGEN3" in n for n in notes)
...
def test_given_ablated_pass_when_decide_then_promote() -> None:
    ...
    assert stats["beats_nanogen2_ablated"] is True
    assert decide_nanogen3(stats) == "PROMOTE"
...
def test_given_ablated_low_when_decide_then_hold() -> None:
    ...
    assert stats["beats_nanogen2_ablated"] is False
    assert decide_nanogen3(stats) == "HOLD"
...
def test_given_false_hit_when_decide_then_kill() -> None:
    ...
    assert decide_nanogen3(stats) == "KILL"
...
def test_given_lookup_true_hit_when_score_then_ok() -> None:
    ...
    assert any("NANOGEN3 LOOKUP" in n for n in notes)
