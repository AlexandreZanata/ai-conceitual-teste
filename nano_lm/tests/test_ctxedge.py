"""Contract: Wave AN2 H-CTXEDGE — undeca-doc dual-arm (pesquisa §3)."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §3 AN2 H-CTXEDGE
    assert CTXEDGE_ID == "H-CTXEDGE"
    assert CTXEDGE_N == 10
    assert MIN_LOOKUP_USABLE == 7
    assert MIN_GEN_USABLE == 5
    assert MIN_SOURCES == 11
    assert TOP_K_SLICES_EDGE == 23
    assert TOP_K_SLICES_EDGE > TOP_K_SLICES_NEXT
    assert CTXNEXT_MEAN_LEFF == 213147.0


def test_given_an0_sources_when_pair_then_all_mapped_distinct() -> None:
    ...
    for item in AN0_PACK:
        ...
        assert len(comps) == 10
        assert len({primary, *comps}) == 11
        assert CTXEDGE_COMPANIONS[primary] == comps


def test_given_undeca_docs_when_meta_then_multi_source_and_deeper_k() -> None:
    docs = [list(range(i * 6000, i * 6000 + 5000)) for i in range(11)]
    q = [10, 20, 400, 800]
    sources = [f"s{i}" for i in range(11)]
    meta = ctxedge_doc_meta(docs, q, source_ids=sources)
    assert meta["n_sources"] == 11
    assert meta["k_slices"] == 23
    assert meta["deeper_than_ctxnext_k"] is True
    assert meta["l_eff"] >= 0
...
def test_given_gen_telemetry_when_ctx_ok_then_usable() -> None:
    ...
    _score, _err, notes, usable = score_ctxedge_gen(
        completion="........",
...
def test_given_ready_stats_when_decide_then_promote() -> None:
    ...
    assert decide_ctxedge(stats) == "PROMOTE"
...
def test_given_low_leff_when_decide_then_hold() -> None:
    ...
    assert decide_ctxedge(stats) == "HOLD"
...
def test_given_false_hit_when_decide_then_kill() -> None:
    ...
    assert decide_ctxedge(stats) == "KILL"
