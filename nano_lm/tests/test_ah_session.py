"""Contract: Wave AH0 SESSION — freeze 10 held-out HITL asks (pesquisa §5)."""

from __future__ import annotations

from ah_session_ops import (
    AH0_APP_IDS,
    AH0_ID,
    AH0_MIX,
    AH0_N,
...
def test_given_pack_when_count_then_exactly_ten() -> None:
    # GIVEN/WHEN/THEN: pesquisa §5 AH0 — freeze held-out HITL×10
    assert len(AH0_PACK) == AH0_N == 10


def test_given_pack_when_ids_then_unique() -> None:
    assert unique_trial_ids(AH0_PACK) is True


def test_given_pack_when_mix_then_three_five_two() -> None:
    assert mix_ok(AH0_PACK) is True
    assert pack_app_counts(AH0_PACK) == dict(AH0_MIX)
...
def test_given_copied_ac_q_when_overlap_then_listed() -> None:
    ...
    assert overlaps_prior_questions(bad) == ["AH-HITL-02"]
...
def test_given_copied_ad_q_when_overlap_then_listed() -> None:
    ...
    assert overlaps_prior_questions(bad) == ["AH-HITL-03"]
...
def test_given_copied_ae_q_when_overlap_then_listed() -> None:
    ...
    assert overlaps_prior_questions(bad) == ["AH-HITL-04"]
...
def test_given_copied_af_q_when_overlap_then_listed() -> None:
    ...
    assert overlaps_prior_questions(bad) == ["AH-HITL-05"]
...
def test_given_copied_ag_q_when_overlap_then_listed() -> None:
    ...
    assert overlaps_prior_questions(bad) == ["AH-HITL-06"]


def test_given_ready_when_decide_then_promote() -> None:
    known = {p["source_id"] for p in AH0_PACK}
    out = decide_ah0_session(known_sources=known, trials_dir_ready=True)
    assert out.startswith("PROMOTE")
    assert AH0_ID in out
    assert "GENLIFT" in AH0_THESIS or "AH1" in AH0_THESIS


def test_given_no_trials_when_decide_then_kill() -> None:
    known = {p["source_id"] for p in AH0_PACK}
    out = decide_ah0_session(known_sources=known, trials_dir_ready=False)
...
def test_given_pack_when_fields_then_app_and_gold() -> None:
    for item in AH0_PACK:
        assert item["app_id"] in AH0_APP_IDS
