"""Contract: Wave AL5 H-APPFRESH — dual-arm apps + DEPL-AL honesty."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §3 AL5 H-APPFRESH
    assert APPFRESH_ID == "H-APPFRESH"
    assert APPFRESH_N == 10
    assert MIN_APPS == 3
    assert MIN_PAGES == 4
    assert MIN_LOOKUP_MEAN == 7.0
    assert MIN_GEN_MEAN == 5.0
    assert APPPUSH_GEN_MEAN == 4.0
    assert APPMORE_GEN_MEAN == 9.0
    assert SERVEALIGN_MEAN == 3.4
    ids = {a["app_id"] for a in APPFRESH_APPS}
...
def test_given_one_pager_when_body_then_dual_arm_and_honesty() -> None:
    ...
    assert "npm run nano:appfresh" in body
    assert "Not an open chat LM" in body
    assert "H-APPFRESH" in body
    assert "H-FASTFRESH" in body
    report = page_sync_report(
        "docs/results/nano-lm/appfresh-howto.md", body
    )
    assert report["ok"] is True, report["missing"]


def test_given_depl_al_when_body_then_sync_ok() -> None:
    body = depl_al_body()
    assert "DEPL-AL" in body
    assert "LOOKUP" in body and "GENERATE" in body
    report = page_sync_report("docs/results/nano-lm/depl-al.md", body)
    assert report["ok"] is True, report["missing"]
...
def test_given_lookup_serve_when_score_then_not_iq() -> None:
    ...
    score, err, notes = score_appfresh_lookup(
        mode="WRAP_LOOKUP",
        completion="gold",
        expected_gold="gold",
...
def test_given_gen_serve_zero_wall_when_score_then_error() -> None:
    ...
    route = {"in_scope": True, "route": "SERVE"}
    _score, err, notes = score_appfresh_gen(
        completion="x",
        expected_gold="gold",
        route=route,
        payload=payload,
...
def test_given_wave_product_when_decide_then_promote() -> None:
    ...
    wave = appfresh_stats(apps, n_pages_ok=4, n_pages=4)
    ...
    assert decide_appfresh(wave) == "PROMOTE"
