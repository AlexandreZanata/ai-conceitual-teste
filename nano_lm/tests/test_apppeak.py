"""Contract: Wave AJ5 H-APPPEAK — dual-arm apps + DEPL-AJ honesty."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §3 AJ5 H-APPPEAK
    assert APPPEAK_ID == "H-APPPEAK"
    assert APPPEAK_N == 10
    assert MIN_APPS == 3
    assert MIN_PAGES == 4
    ...
    ids = {a["app_id"] for a in APPPEAK_APPS}
...
def test_given_one_pager_when_body_then_dual_arm_and_honesty() -> None:
    ...
    assert "npm run nano:apppeak" in body
    assert "Not an open chat LM" in body
    assert "H-APPPEAK" in body
    assert "H-FASTPEAK" in body
    report = page_sync_report(
        "docs/results/nano-lm/apppeak-howto.md", body
    )
    assert report["ok"] is True, report["missing"]


def test_given_depl_aj_when_body_then_sync_ok() -> None:
    body = depl_aj_body()
    assert "DEPL-AJ" in body
    assert "LOOKUP" in body and "GENERATE" in body
    report = page_sync_report("docs/results/nano-lm/depl-aj.md", body)
    assert report["ok"] is True, report["missing"]
...
def test_given_lookup_serve_when_score_then_not_iq() -> None:
    ...
    score, err, notes = score_apppeak_lookup(
        mode="WRAP_LOOKUP",
        completion="gold",
        expected_gold="gold",
...
def test_given_gen_serve_zero_wall_when_score_then_error() -> None:
    ...
    route = {"in_scope": True, "route": "SERVE"}
    _score, err, notes = score_apppeak_gen(
        completion="x",
        expected_gold="gold",
        route=route,
        payload=payload,
...
def test_given_wave_product_when_decide_then_promote() -> None:
    ...
    wave = apppeak_stats(apps, n_pages_ok=4, n_pages=4)
    ...
    assert decide_apppeak(wave) == "PROMOTE"
