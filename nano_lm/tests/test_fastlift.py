"""Contract: Wave AH4 H-FASTLIFT — generative wall_ms>0 vs AG FASTREAL."""
...
from fastreal_ops import AF_RAW_OPEN_WALL_MS as FR_AF


def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §5 AH4 H-FASTLIFT
    assert FASTLIFT_ID == "H-FASTLIFT"
    assert FASTLIFT_N == 10
    assert AF_RAW_OPEN_WALL_MS == FR_AF
    assert FASTREAL_HOT_WALL_MS == 16.1
...
def test_given_gen_zero_wall_when_score_then_error() -> None:
    ...
    _score, err, notes = score_fastlift_gen(
        completion="anything",
        expected_gold="gold",
        payload=payload,
    )
...
def test_given_gen_periods_when_score_then_low_but_telemetry_ok() -> None:
    ...
    assert any("FASTREAL" in n for n in notes)


def test_given_beats_fastreal_when_decide_then_promote() -> None:
    ...
    assert stats["pass_vs_fastreal"] is True
    assert decide_fastlift(stats) == "PROMOTE"


def test_given_no_beat_fastreal_when_decide_then_hold() -> None:
    # warm/hot vs cold ok, but slower than FASTREAL hot 16.1
    stats = fastlift_stats(
        lookup_scores=[9.0] * 10,
        lookup_errors=[False] * 10,
        gen_scores=[1.0] * 10,
    ...
    assert stats["pass_vs_fastreal"] is False
    assert decide_fastlift(stats) == "HOLD"
...
def test_given_false_hit_or_zero_wall_when_decide_then_kill() -> None:
    ...
    assert decide_fastlift(fastlift_stats(**base)) == "KILL"
    base["n_false_hit"] = 0
    base["n_true_hit"] = 10
