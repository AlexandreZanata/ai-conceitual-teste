"""Contract: Wave AB6 AB-HITL-10 — final pack mean≥7.0 · errors≤3."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §8.3 AB6 AB-HITL-10
    assert AB6_ID == "AB-HITL-10"
    assert AB6_N == 10
    assert PASS_MEAN == 7.0
    assert PASS_MAX_ERRORS == 3
    assert "H-REALAPP" in DECLARED_STACK
    assert "H-SEMWRAP" in DECLARED_STACK
...
def test_given_item_app_when_select_then_route() -> None:
    ...
    assert select_app("howto") == "app-known"
...
def test_given_false_hit_when_score_then_error() -> None:
    score, err, _notes = score_ab6_trial(
        mode="SEMWRAP_LOOKUP",
        completion="wrong",
        expected_gold="gold",
...
def test_given_longapp_fail_when_score_then_error() -> None:
    score, err, notes = score_ab6_trial(
        mode="ASKFAST_CACHE",
        completion="BIP 9",
        expected_gold="BIP 9",
    ...
    assert any("LONGAPP_CTX_FAIL" in n for n in notes)
...
def test_given_pass_bar_when_decide_then_promote() -> None:
    ...
    assert decide_ab6(stats) == "PROMOTE"
