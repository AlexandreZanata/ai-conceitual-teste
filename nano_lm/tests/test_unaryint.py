"""Contract: Wave BG1 H-UNARYINT — BG-FOREVER FH 0 via unary/transform gate."""
...
def test_given_bg_unary_traps_when_contrast_then_reject_family() -> None:
    # GIVEN/WHEN/THEN: BG1 forever gate — unary/transform ≠ wrong-bank
    add_q = (
        "Write a short Python function named add that returns the sum "
        "of two integers a and b."
    )
    add_g = "def add(a, b):\n    return a + b"
    f_g = "Begin the string with f or F before the opening quotation mark."
    clear_g = "a.clear()"
    ...
    assert contrastive_reject(
        "How do I uppercase string s in Python?",
    ...
    assert intent_ask_must_abstain("How do I uppercase string s in Python?")
    assert intent_ask_must_abstain(
...
def test_given_forever_when_scan_then_all_abstain_detectors() -> None:
    ...
    for row in BG0_FOREVER_ROWS:
        assert intent_ask_must_abstain(str(row["question"]))
    assert all(
        intent_ask_must_abstain(str(p["question"])) for p in BF0_FOREVER_ROWS
    )


def test_given_novel_when_scan_then_disjoint_and_min_ten() -> None:
    forever_q = {str(p["question"]).strip() for p in BG0_FOREVER_ROWS}
...
def test_given_ready_board_when_decide_then_promote() -> None:
    out = decide_unaryint(board=_board(), anti_fp_signed=True)
    assert out.startswith("PROMOTE")
    assert UNARYINT_ID in out


def test_given_bg_fp_when_decide_then_kill() -> None:
    out = decide_unaryint(
        board=_board(bg_forever_false_hit=1), anti_fp_signed=True
    )
...
def test_given_unsigned_when_decide_then_kill() -> None:
    out = decide_unaryint(board=_board(), anti_fp_signed=False)
    assert out.startswith("KILL")
    assert "anti-FP" in out


def test_given_notes_when_read_then_claim_and_thesis() -> None:
    assert "≤5M" in UNARYINT_CLAIM or "AF" in UNARYINT_CLAIM
    assert "unary" in UNARYINT_THESIS.lower() or "BG-FOREVER" in UNARYINT_THESIS
    assert "LOOKUP" in UNARYINT_ANTI_FP
...
def test_given_extract_when_all_ok_then_board_keys() -> None:
    ...
    assert board["bg_forever_false_hit"] == 0
