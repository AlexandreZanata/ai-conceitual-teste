"""Contract: Wave AM4 H-FASTNEXT — generative wall_ms>0 vs AL FASTFRESH."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §3 AM4 H-FASTNEXT
    assert FASTNEXT_ID == "H-FASTNEXT"
    assert FASTNEXT_N == 10
    assert AF_RAW_OPEN_WALL_MS == FP_AF
    assert FASTFRESH_HOT_WALL_MS == 0.2
...
def test_given_gen_zero_wall_when_score_then_error() -> None:
    ...
    _score, err, notes = score_fastnext_gen(
        completion="anything",
        expected_gold="gold",
        payload=payload,
    )
...
def test_given_gen_periods_when_score_then_low_but_telemetry_ok() -> None:
    payload = {"mode": "PEAK_FAST+GENTRUTH", "wall_ms": 0.15, "n_new": 16}
    ...
    assert any("FASTFRESH" in n for n in notes)


def test_given_chunks_when_fastnext_generate_then_wall_and_peak() -> None:
    # GIVEN curated-like chunks WHEN GENTRUTH peak-fast THEN wall>0 ∧ n_new>0
    chunks = [
        "| ENT | CS | ENT+CS | MS |\n| 160 | 5 | 165 | 15 |",
        "BIP-39 mnemonic: 160-bit ENT yields 15 words.",
    ]
    payload = fastnext_generate(
        question="BIP-39: for 160-bit ENT, how many mnemonic words?",
        chunks=chunks,
    )
    assert payload["wall_ms"] > 0.0
    assert int(payload["n_new"]) > 0
    assert payload["mode"] == "PEAK_FAST+GENTRUTH"
    assert "15" in str(payload["completion"])


def test_given_beats_fastfresh_when_decide_then_promote() -> None:
    ...
    assert stats["pass_vs_fastfresh"] is True
    assert stats["pass_quality_floor"] is True
    assert decide_fastnext(stats) == "PROMOTE"
...
def test_given_low_gen_when_decide_then_hold() -> None:
    ...
    assert decide_fastnext(stats) == "HOLD"


def test_given_no_beat_fastfresh_when_decide_then_hold() -> None:
    stats = fastnext_stats(
        lookup_scores=[9.0] * 10,
        lookup_errors=[False] * 10,
        gen_scores=[7.0] * 10,
    ...
    assert stats["pass_vs_fastfresh"] is False
    assert decide_fastnext(stats) == "HOLD"
...
def test_given_false_hit_when_decide_then_kill() -> None:
    ...
    assert decide_fastnext(stats) == "KILL"
