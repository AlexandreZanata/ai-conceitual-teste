"""Contract: Wave AK2 H-CTXMORE — octa-doc dual-arm (pesquisa §3)."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §3 AK2 H-CTXMORE
    assert CTXMORE_ID == "H-CTXMORE"
    assert CTXMORE_N == 10
    assert MIN_LOOKUP_USABLE == 7
    assert MIN_GEN_USABLE == 5
    assert MIN_SOURCES == 8
    assert TOP_K_SLICES_MORE == 17
    assert TOP_K_SLICES_MORE > TOP_K_SLICES_PEAK
    assert CTXPEAK_MEAN_LEFF == 177809.0


def test_given_ak0_sources_when_pair_then_all_mapped_distinct() -> None:
    ...
    for item in AK0_PACK:
        ...
        assert len(comps) == 7
        assert len({primary, *comps}) == 8
        assert CTXMORE_COMPANIONS[primary] == comps


def test_given_octa_docs_when_meta_then_multi_source_and_deeper_k() -> None:
    docs = [list(range(i * 6000, i * 6000 + 5000)) for i in range(8)]
    q = [10, 20, 400, 800]
    sources = [f"s{i}" for i in range(8)]
    meta = ctxmore_doc_meta(docs, q, source_ids=sources)
    assert meta["n_sources"] == 8
    assert meta["k_slices"] == 17
    assert meta["deeper_than_ctxpeak_k"] is True
    assert meta["l_eff"] >= 0
...
def test_given_gen_telemetry_when_ctx_ok_then_usable() -> None:
    ...
    _score, _err, notes, usable = score_ctxmore_gen(
        completion="........",
...
def test_given_ready_stats_when_decide_then_promote() -> None:
    ...
    assert decide_ctxmore(stats) == "PROMOTE"
...
def test_given_low_leff_when_decide_then_hold() -> None:
    ...
    assert decide_ctxmore(stats) == "HOLD"
...
def test_given_false_hit_when_decide_then_kill() -> None:
    ...
    assert decide_ctxmore(stats) == "KILL"
