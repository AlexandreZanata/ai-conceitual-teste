"""Contract: Wave AM6 AM-HITL-10 — final dual-arm lookup≥7 · gen≥5|HOLD."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §3 AM6 AM-HITL-10
    assert AM6_ID == "AM-HITL-10"
    assert AM6_N == 10
    assert MIN_LOOKUP_MEAN == 7.0
    assert MIN_GEN_MEAN == 5.0
    assert PASS_MAX_ERRORS == 3
    assert "H-GENTRUTH" in DECLARED_STACK
    assert "H-APPNEXT" in DECLARED_STACK
    assert "H-FASTNEXT" in DECLARED_STACK
    assert "H-CTXNEXT" in DECLARED_STACK
    assert "H-SMARTNEXT" in DECLARED_STACK
...
def test_given_lookup_true_hit_when_score_then_not_iq() -> None:
    ...
    score, err, notes = score_am6_lookup(
        mode="WRAP_LOOKUP",
        completion="gold",
        expected_gold="gold",
...
def test_given_gen_zero_wall_when_score_then_error() -> None:
    ...
    _score, err, notes = score_am6_gen(
        completion="x",
        expected_gold="gold",
        payload=payload,
    )
...
def test_given_lookup_and_gen_pass_when_decide_then_promote() -> None:
    ...
    assert decide_am6(stats) == "PROMOTE"
...
def test_given_false_hit_or_overlap_when_decide_then_kill() -> None:
    ...
    assert decide_am6(am6_stats(**base)) == "KILL"
    base["n_false_hit"] = 0
    base["n_true_hit"] = 10
