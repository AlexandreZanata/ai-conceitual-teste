"""Contract: Wave AN6 AN-HITL-10 — final dual-arm lookup≥7 · gen≥5|HOLD."""
...
def test_given_contract_when_constants_then_match_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §3 AN6 AN-HITL-10
    assert AN6_ID == "AN-HITL-10"
    assert AN6_N == 10
    assert MIN_LOOKUP_MEAN == 7.0
    assert MIN_GEN_MEAN == 5.0
    assert PASS_MAX_ERRORS == 3
    assert "H-GENEDGE" in DECLARED_STACK
    assert "H-APPEDGE" in DECLARED_STACK
    assert "H-FASTEDGE" in DECLARED_STACK
    assert "H-CTXEDGE" in DECLARED_STACK
    assert "H-SMARTEDGE" in DECLARED_STACK
...
def test_given_lookup_true_hit_when_score_then_not_iq() -> None:
    ...
    score, err, notes = score_an6_lookup(
        mode="WRAP_LOOKUP",
        completion="gold",
        expected_gold="gold",
...
def test_given_gen_zero_wall_when_score_then_error() -> None:
    ...
    _score, err, notes = score_an6_gen(
        completion="x",
        expected_gold="gold",
        payload=payload,
    )
...
def test_given_lookup_and_gen_pass_when_decide_then_promote() -> None:
    ...
    assert decide_an6(stats) == "PROMOTE"
...
def test_given_false_hit_or_overlap_when_decide_then_kill() -> None:
    ...
    assert decide_an6(an6_stats(**base)) == "KILL"
    base["n_false_hit"] = 0
    base["n_true_hit"] = 10
