"""Contract: Wave BB5 BB-REAL-EVAL — product+ctx+speed + live battery; gen if BB4 PROMOTE."""
...
def test_given_contract_when_constants_then_match_bb5_gate() -> None:
    # GIVEN/WHEN/THEN: pesquisa §8 BB5 — product pass · gen if BB4 PROMOTE
    assert BB_REAL_EVAL_ID == "BB-REAL-EVAL"
    ...
    assert "nanogen12" in claim_rule
    assert "true_continue" in claim_rule or "true-continue" in claim_rule
    assert "rename" in claim_rule
    assert "real eval" in BB_REAL_EVAL_THESIS.lower()
    assert "INTENTGEN" in BB_REAL_EVAL_THESIS
    assert PARENT_NANOGEN12 == "DEFER"
    assert claim_is_honest(BB_REAL_EVAL_CLAIM, nanogen12_decision="DEFER")
    assert not gen_claim_allowed(BB_REAL_EVAL_CLAIM)
...
def test_given_bb_forever_fp_when_lookup_then_fail() -> None:
    row = {
        "id": "BB-ASK-07",
...
def test_given_all_pillars_when_decide_then_promote() -> None:
    ...
    out = decide_bb_real_eval(
        intentgen_decision="PROMOTE (x)",
        fasthold_decision="PROMOTE (x)",
        ctxhold_decision="PROMOTE (x)",
        nanogen12_decision="DEFER (x)",
        battery_ok=True,
        claim=BB_REAL_EVAL_CLAIM,
    )
    assert out.startswith("PROMOTE")
    assert nanogen12_outcome_ok("DEFER (x)")
...
def test_given_gen_claim_when_defer_then_kill() -> None:
    out = decide_bb_real_eval(
        ...
        nanogen12_decision="DEFER (x)",
        battery_ok=True,
        claim=BB_REAL_EVAL_CLAIM + " · mini-AGI unlocked",
    )
