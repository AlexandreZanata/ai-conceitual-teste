...
def analyze_file(path: Path) -> list[Finding]:
    ...
    suffix = path.suffix
    if suffix == ".py":
        ranges = py_function_ranges(lines)
    elif suffix in {".cpp", ".cc", ".cxx", ".hpp", ".h", ".hh"}:
        from .parse import cpp_function_ranges

        ranges = cpp_function_ranges(lines)
    else:
        ranges = c_like_function_ranges(lines)
