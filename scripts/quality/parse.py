...
def cpp_function_ranges(lines: list[str]) -> list[tuple[str, int, int]]:
    """Detect C++ function/method definitions (brace bodies only)."""
    ranges: list[tuple[str, int, int]] = []
    i = 0
    while i < len(lines):
        line = lines[i]
        ctor = CPP_CTOR_RE.search(line)
        method = None if ctor else CPP_METHOD_RE.search(line)
        if not ctor and not method:
            i += 1
            continue
        window = "\n".join(lines[i : min(i + 12, len(lines))])
        if "{" not in window or re.search(r"\)\s*;", window.split("{")[0]):
            i += 1
            continue
        if ctor:
            name = f"{ctor.group(1)}::{ctor.group(2)}"
        else:
            cls = method.group(1)
            fn = method.group(2)
